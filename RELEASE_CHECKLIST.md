...
| Desktop spike fixture/tooling change | Routine gate plus `npm run export:portable-content`, `npm run validate:portable-content`, `npm run test:save-translation-contract`, `npm run export:desktop-spike-fixture`, and `npm run validate:desktop-spike-fixture` | Add `npm run godot:all` for v0.117 Godot spike work when Godot and export templates are available. Add package generation and `npm run verify:playtest-package` only if browser playtest package metadata or package docs change. |
