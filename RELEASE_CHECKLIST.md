...
| Desktop spike fixture/tooling change | Routine gate plus `npm run export:portable-content`, `npm run validate:portable-content`, `npm run test:save-translation-contract`, `npm run export:desktop-spike-fixture`, and `npm run validate:desktop-spike-fixture` | Add package generation and `npm run verify:playtest-package` only if package metadata or package docs change. |
