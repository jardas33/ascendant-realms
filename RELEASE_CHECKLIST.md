...
| Desktop spike fixture/tooling change | Routine gate plus `npm run export:portable-content`, `npm run validate:portable-content`, `npm run test:save-translation-contract`, `npm run export:desktop-spike-fixture`, and `npm run validate:desktop-spike-fixture` | Add `npm run godot:all` for v0.117+ Godot spike work when Godot and export templates are available. Add `npm run godot:headed:smoke` and `npm run godot:capture:review` for v0.118 packaged-headed review changes. Add `npm run godot:validate:player-slice` and `npm run godot:capture:player-slice` for v0.124+ player-facing review-slice changes. Add `GODOT_VALIDATE_SALTO_VERTICAL_SLICE_WINDOWS.bat` and `GODOT_CAPTURE_SALTO_VERTICAL_SLICE_WINDOWS.bat` for v0.130 acceptance-pack changes. Add `npm run godot:validate:post-mine-flow` and `npm run godot:headed:post-mine-flow-smoke` for v0.133 post-mine player-facing flow changes. Add `GODOT_CAPTURE_STABILIZED_SALTO_REVIEW_WINDOWS.bat` for v0.139 stabilization review-pack changes. Add `npm run godot:runtime-art-comparator:validate`, `npm run godot:runtime-art-comparator:benchmark:headed`, and `npm run godot:runtime-art-comparator:capture` for v0.146 isolated comparator changes. Add `npm run godot:worker-billboard:validate`, `npm run godot:worker-billboard:fallback:reproduce`, `npm run godot:worker-billboard:benchmark:headed`, and `npm run godot:worker-billboard:capture` for v0.147 Worker billboard single-slot private comparator changes. Add `npm run godot:worker-billboard-repair:validate`, `npm run godot:worker-billboard-repair:audit`, `npm run godot:worker-billboard-repair:derivatives:reproduce`, `npm run godot:worker-billboard-repair:benchmark:headed`, and `npm run godot:worker-billboard-repair:capture` for v0.148 Worker billboard repair changes. Add package generation and `npm run verify:playtest-package` only if browser playtest package metadata or package docs change. |
...
For v0.167 three-slot presentation QA, placeholder classification, and artifact-retention changes, add `npm run godot:validate:salto-three-slot-visual-coherence`, `npm run godot:report:salto-three-slot-presentation-qa`, `npm run godot:validate:salto-experimental-artifact-retention`, `npm run godot:cleanup:salto-experimental-artifacts`, `npm run godot:cleanup:salto-experimental-artifacts -- --apply-safe-only` only when candidates are positively safe, Windows-side Computer Use review where available, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans for zero images, zero new slots, exactly three opt-in slots total, no Aster/Ashen normal-slice slots, default procedural launchers preserved, all prior opt-in launchers preserved, no browser wiring/production manifest mutation/save or stable-ID mutation/gameplay mutation/default-launcher mutation/package leakage, no required evidence deletion, and `git diff --check`.

For v0.171 five-slot visual-cohesion QA, cleanup packet, and character-integration freeze changes, add the prior opt-in validation stack through `npm run godot:validate:salto-five-slot-art-experiment`, `npm run godot:audit:salto-experimental-artifacts`, `npm run godot:validate:salto-experimental-artifact-retention`, `npm run godot:cleanup:salto-experimental-artifacts`, `npm run godot:cleanup:salto-experimental-artifacts -- --apply-safe-only` only when candidates are positively safe, Windows-side Computer Use review where available, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, `npm run validate:runtime-art-slots`, isolation scans for zero images, zero slots added, exactly five selected opt-in slots frozen, default procedural launchers preserved, all prior opt-in launchers preserved, no sixth slot/browser wiring/production manifest mutation/save or stable-ID mutation/gameplay mutation/default-launcher mutation/package leakage, no required evidence deletion, and `git diff --check`.

For v0.173 procedural environment-shell hierarchy hardening changes, add `npm run godot:validate:salto-environment-foundation`, Windows-side Computer Use review where available, `node tools/godot/saltoEnvironmentFoundationTool.mjs validation --artifact-root=artifacts/desktop-spikes/godot-salto/v0173`, `node tools/godot/saltoEnvironmentFoundationTool.mjs capture --artifact-root=artifacts/desktop-spikes/godot-salto/v0173`, `node tools/godot/saltoEnvironmentFoundationTool.mjs benchmark --artifact-root=artifacts/desktop-spikes/godot-salto/v0173`, `node tools/godot/saltoEnvironmentFoundationTool.mjs boundary --artifact-root=artifacts/desktop-spikes/godot-salto/v0173`, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, `npm run validate:runtime-art-slots`, artifact retention validation, safe-only cleanup only when candidates are positively safe, isolation scans for zero images, zero slots, no sixth character slot, no terrain-material import, no default-art enablement, default procedural launchers preserved, all prior opt-in launchers preserved, no browser/save/stable-ID/gameplay mutation, benchmark E1 FPS ratio >= 0.90 and p95 worsening <= 15%, and `git diff --check`.

For v0.174 road/river/bridge/site-marker readability hardening changes, add `npm run godot:validate:salto-environment-readability`, Windows-side Computer Use review where available, `node tools/godot/saltoEnvironmentReadabilityTool.mjs validation --artifact-root=artifacts/desktop-spikes/godot-salto/v0174`, `node tools/godot/saltoEnvironmentReadabilityTool.mjs capture --artifact-root=artifacts/desktop-spikes/godot-salto/v0174`, `node tools/godot/saltoEnvironmentReadabilityTool.mjs benchmark --artifact-root=artifacts/desktop-spikes/godot-salto/v0174`, `node tools/godot/saltoEnvironmentReadabilityTool.mjs boundary --artifact-root=artifacts/desktop-spikes/godot-salto/v0174`, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, `npm run validate:runtime-art-slots`, artifact retention validation, cleanup dry-run and safe-only cleanup only when candidates are positively safe, isolation scans for zero images, zero slots, no sixth character slot, no terrain-material import, no default-art enablement, default procedural launchers preserved, all prior opt-in launchers preserved, no browser/save/stable-ID/gameplay/pathing/navigation mutation, benchmark E2 FPS ratio >= 0.90 and p95 worsening <= 15%, and `git diff --check`.

For v0.175 private Barrosan foothold terrain-material comparator intake changes, add `npm run godot:ground-material:fallback:reproduce`, `npm run godot:ground-material:derivatives:reproduce`, `npm run godot:ground-material:validate`, `npm run godot:ground-material:benchmark:headed`, `npm run godot:ground-material:capture`, `npm run godot:ground-material:audit`, `npm run godot:cleanup:salto-experimental-artifacts -- --output-root=artifacts/desktop-spikes/godot-salto/v0175/cleanup-dry-run`, `npm run godot:validate:salto-experimental-artifact-retention`, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, `npm run validate:runtime-art-slots`, isolation scans for exactly one generated source image, zero character slots added, zero normal-slice terrain slots added, no player-slice integration, no browser wiring, no default-launcher mutation, Tier L FPS ratio >= 0.90, p95 worsening <= 15%, and `git diff --check`.

For v0.176 terrain-material opt-in readiness packet changes, add docs existence checks for `docs/V0176_TERRAIN_MATERIAL_OPT_IN_READINESS_PACKET.md`, `docs/V0176_TERRAIN_MATERIAL_RISK_AND_ROLLBACK.md`, `docs/art-prompts/V0177_01_TERRAIN_MATERIAL_OPT_IN_INTEGRATION.md`, and `docs/V0176_IMPLEMENTATION_REPORT.md`; add `npm run godot:validate:salto-experimental-artifact-retention`, `npm run godot:cleanup:salto-experimental-artifacts -- --output-root=artifacts/desktop-spikes/godot-salto/v0176/cleanup-dry-run`, isolation scans for zero images, zero slots, no runtime code changes, no player-slice integration, no browser wiring, no new launcher, no default-launcher mutation, v0.177 prepared but not started, and `git diff --check`.

For v0.177 Barrosan foothold ground-material first opt-in integration changes, add `npm run godot:validate:salto-ground-material-opt-in`, Windows-side Computer Use review where available, `node tools/godot/saltoGroundMaterialOptInTool.mjs validation --artifact-root=artifacts/desktop-spikes/godot-salto/v0177`, `node tools/godot/saltoGroundMaterialOptInTool.mjs capture --artifact-root=artifacts/desktop-spikes/godot-salto/v0177`, `node tools/godot/saltoGroundMaterialOptInTool.mjs benchmark --artifact-root=artifacts/desktop-spikes/godot-salto/v0177`, `node tools/godot/saltoGroundMaterialOptInTool.mjs boundary --artifact-root=artifacts/desktop-spikes/godot-salto/v0177`, artifact retention validation, cleanup dry-run and safe-only cleanup only when candidates are positively safe, isolation scans for zero images, exactly one environment-material opt-in slot, zero character slots added, default procedural launchers preserved, all prior opt-in launchers preserved, no browser/save/stable-ID/gameplay/pathing/objective mutation, exact selected hash `818b7743fbf192fe95dd95a0fbadb59ea92b1cb36c420dac5526c0f4d1af18a8`, benchmark E1 FPS ratio >= 0.90 and p95 worsening <= 15%, missing-art/hash-mismatch fallback, and `git diff --check`.

For v0.178 ground-material UV/noise hardening changes, add `npm run godot:validate:salto-ground-material-opt-in`, Windows-side Computer Use review where available, `node tools/godot/saltoGroundMaterialOptInTool.mjs validation --artifact-root=artifacts/desktop-spikes/godot-salto/v0178`, `node tools/godot/saltoGroundMaterialOptInTool.mjs capture --artifact-root=artifacts/desktop-spikes/godot-salto/v0178`, `node tools/godot/saltoGroundMaterialOptInTool.mjs benchmark --artifact-root=artifacts/desktop-spikes/godot-salto/v0178`, `node tools/godot/saltoGroundMaterialOptInTool.mjs boundary --artifact-root=artifacts/desktop-spikes/godot-salto/v0178`, artifact retention validation, cleanup dry-run and safe-only cleanup only when candidates are positively safe, isolation scans for zero images, zero slots added, exactly one existing environment-material opt-in slot, hardened UV scale `0.56`, noise-control alpha <= `0.52`, procedural value underlay visible only when material loads, default procedural launchers preserved, all prior opt-in launchers preserved, no browser/save/stable-ID/gameplay/pathing/objective mutation, exact selected hash `818b7743fbf192fe95dd95a0fbadb59ea92b1cb36c420dac5526c0f4d1af18a8`, benchmark E1 FPS ratio >= 0.90 and p95 worsening <= 15%, missing-art/hash-mismatch fallback, and `git diff --check`.

For v0.181 road-material opt-in player-slice integration changes, add `npm run godot:validate:salto-ground-road-material-opt-in`, Windows-side Computer Use review where available, `node tools/godot/saltoGroundRoadMaterialOptInTool.mjs validation --artifact-root=artifacts/desktop-spikes/godot-salto/v0181`, `node tools/godot/saltoGroundRoadMaterialOptInTool.mjs capture --artifact-root=artifacts/desktop-spikes/godot-salto/v0181`, `node tools/godot/saltoGroundRoadMaterialOptInTool.mjs benchmark --artifact-root=artifacts/desktop-spikes/godot-salto/v0181`, `node tools/godot/saltoGroundRoadMaterialOptInTool.mjs boundary --artifact-root=artifacts/desktop-spikes/godot-salto/v0181`, artifact retention validation, cleanup dry-run and safe-only cleanup only when candidates are positively safe, isolation scans for zero images, one new road environment-material opt-in slot, zero character slots added, exactly five character slots frozen, existing ground-material opt-in preserved, default procedural launchers preserved, all prior opt-in launchers preserved, no browser/save/stable-ID/gameplay/pathing/objective mutation, exact selected road hash `a64959ef2fd7a509fcaaa969fca3e095d590d563a4f0c578a5e96d1fb04c0e10`, road missing-art/hash-mismatch fallback, benchmark E2 FPS ratio >= 0.90 and p95 worsening <= 15%, and `git diff --check`.

For v0.182 environment-foundation visual-cohesion QA, cleanup packet, and environment freeze changes, add `npm run godot:validate:salto-ground-road-material-opt-in`, Windows-side Computer Use review of default procedural and explicit ground+road opt-in paths, `npm run godot:audit:salto-experimental-artifacts`, `npm run godot:cleanup:salto-experimental-artifacts`, `npm run godot:cleanup:salto-experimental-artifacts -- --apply-safe-only` only when candidates are positively safe, artifact retention validation after cleanup, isolation scans for zero images, zero slots added, exactly five character slots frozen, existing ground and road environment-material opt-ins preserved, default procedural launcher preserved, all prior opt-in launchers preserved, no browser/save/stable-ID/gameplay/pathing/objective mutation, benchmark ground+road FPS ratio >= 0.90 and p95 worsening <= 15%, cleanup unknown sidecars = 0, and `git diff --check`.

For v0.183 post-freeze next-phase scorecard and v0.184 preparation changes, add `npm run godot:validate:salto-experimental-artifact-retention`, cleanup dry-run only, isolation scans for zero images, zero slots, no runtime code changes, no launcher mutation, no cleanup deletion/archive move, default procedural launcher preserved, browser runtime untouched, selected art/metadata/fallbacks/latest evidence preserved, exactly one recommended v0.184 prompt created, v0.184 not started, and `git diff --check`.

For post-v0.183 Godot opt-in visual hardening review changes, add `npm run godot:validate:salto-ground-road-material-opt-in`, Windows-side live app review or Computer Use review where available, distinct screenshot/focus proof for road/bridge/pan/min/max capture actions, isolation scans for zero images, zero slots, no launcher mutation, default procedural launcher preserved, browser runtime untouched, existing ground and road environment-material opt-ins preserved, exactly five character slots frozen, no save/stable-ID/gameplay/pathing/objective/AI/balance mutation, road missing-art and hash-mismatch fallback, benchmark E2 FPS ratio >= 0.90 and p95 worsening <= 15%, v0.184 not started, and `git diff --check`.

For v0.184 opt-in environment-shell geometry convergence changes, add `npm run godot:validate:salto-ground-road-material-opt-in`, `npm run godot:validate:salto-environment-geometry-convergence`, Windows-side live app review or Computer Use review where available, cleanup dry-run, safe-only cleanup only when candidates are positively safe, artifact retention validation after cleanup, isolation scans for zero images, zero slots added, no bridge/riverbank/structure material integration, default procedural launcher preserved, all prior opt-in launchers preserved, browser runtime untouched, selected ground and road environment-material opt-ins preserved, exactly five character slots frozen, no save/stable-ID/gameplay/pathing/collision/objective/AI/balance mutation, benchmark E3 FPS ratio >= 0.90 and p95 worsening <= 15% against the E2 baseline, and `git diff --check`.

For v0.185 environment-shell live QA residual-overlay pruning changes, add `npm run godot:validate:salto-ground-road-material-opt-in`, `npm run godot:validate:salto-environment-geometry-convergence`, `npm run godot:validate:salto-environment-shell-live-qa`, Windows-side live app review or Computer Use review covering title, briefing, battle, pan, zoom, mine, Worker assignment, Barracks restoration, Militia recruitment, Ashen posture, minimap, Results, and restart, `npm run godot:headed:post-mine-flow-smoke`, `npm run godot:headed:triple-natural-playthrough`, cleanup dry-run, safe-only cleanup only when candidates are positively safe, artifact retention validation after cleanup, isolation scans for zero images, zero slots added, no bridge/riverbank/structure material integration, default procedural launcher preserved, all prior opt-in launchers preserved, browser runtime untouched, selected ground and road environment-material opt-ins preserved, exactly five character slots frozen, no save/stable-ID/gameplay/pathing/collision/objective/AI/balance mutation, benchmark E4 FPS ratio >= 0.90 and p95 worsening <= 15% against the E3 geometry-convergence baseline, and `git diff --check`.

For v0.186 procedural structure-shell hierarchy hardening changes, add `npm run godot:validate:salto-structure-shell-hardening`, Windows-side live app review or Computer Use review covering title, briefing, battle, Command Hall close/normal, mine close/normal, Barracks restoration/restored state, Worker beside Barracks, road/bridge relation, combat posture, pan/zoom, minimap, Results, cleanup dry-run, safe-only cleanup only when candidates are positively safe, artifact retention validation after cleanup, isolation scans for zero images, zero slots added, no bridge/riverbank/structure material integration, default procedural launcher preserved, all prior opt-in launchers preserved, browser runtime untouched, selected ground and road environment-material opt-ins preserved, exactly five character slots frozen, no save/stable-ID/gameplay/pathing/collision/objective/AI/balance mutation, benchmark S1 FPS ratio >= 0.90 and p95 worsening <= 15% against the E4 shell-live-QA baseline, and `git diff --check`.

For v0.187 riverbank, bridge-crossing, and approach-lane procedural visual hardening changes, add `npm run godot:validate:salto-riverbank-bridge-approach`, Windows-side live app review or Computer Use review covering title, briefing, battle, full overview, river overview, banks close/normal, bridge close/normal, road-to-bridge transition, friendly approach, hostile approach, combat crossing posture, pan/zoom, minimap, Results, `npm run godot:headed:post-mine-flow-smoke`, `npm run godot:headed:triple-natural-playthrough`, cleanup dry-run, safe-only cleanup only when candidates are positively safe, artifact retention validation after cleanup, isolation scans for zero images, zero slots added, no bridge/riverbank/structure material integration, no water shader pipeline, default procedural launcher preserved, all prior opt-in launchers preserved, browser runtime untouched, selected ground and road environment-material opt-ins preserved, exactly five character slots frozen, no save/stable-ID/gameplay/pathing/collision/objective/AI/balance mutation, benchmark R1 FPS ratio >= 0.90 and p95 worsening <= 15% against the S1 structure-shell baseline, and `git diff --check`.

For v0.188 environment-shell full cohesion QA, cleanup packet, and shell-freeze changes, add `npm run godot:validate:salto-riverbank-bridge-approach`, `npm run godot:validate:salto-ground-road-material-opt-in`, `npm run godot:validate:salto-five-slot-art-experiment`, Windows-side live app review or Computer Use review covering title, briefing, battle, tactical overview, structures, river/bridge, roads, characters, combat, pan/zoom, minimap, and contact-sheet evidence, `npm run godot:headed:post-mine-flow-smoke`, `npm run godot:headed:triple-natural-playthrough`, broad artifact inventory, cleanup dry-run, safe-only cleanup only when candidates are positively safe, artifact retention validation after cleanup, isolation scans for zero images, zero slots added, no runtime code changes, no launcher mutation, no bridge/riverbank/structure material integration, no water shader pipeline, default procedural launcher preserved, all prior opt-in launchers preserved, browser runtime untouched, selected ground and road environment-material opt-ins preserved, exactly five character slots frozen, no save/stable-ID/gameplay/pathing/collision/objective/AI/balance mutation, benchmark FPS ratio >= 0.90 and p95 worsening <= 15%, v0.189 not started, and `git diff --check`.

For v0.189 private wet-granite bridge-riverbank material comparator intake changes, add `npm run godot:bridge-riverbank-material:fallback:reproduce`, `npm run godot:bridge-riverbank-material:derivatives:reproduce`, `npm run godot:bridge-riverbank-material:validate`, `npm run godot:bridge-riverbank-material:benchmark:headed`, `npm run godot:bridge-riverbank-material:audit`, cleanup dry-run only, `npm run godot:validate:salto-experimental-artifact-retention`, isolation scans for exactly one generated source image, zero character slots added, zero normal-slice bridge-riverbank slots added, private comparator only, no player-slice integration, no broad stone replacement, no browser wiring, no default-launcher mutation, selected hash `638ce153d7a3d39db729dfa13ba05f3fb05c437c2802ab91b5cd248bd2036753`, source hash `342d058f4749e115569a82bf971bb409ccd63825f93b7428d346150ebd9d003a`, Tier L FPS ratio >= 0.90, p95 worsening <= 15%, v0.190 not started, and `git diff --check`.

For v0.190 bridge-riverbank material opt-in readiness packet changes, add docs existence checks for `docs/V0190_BRIDGE_RIVERBANK_MATERIAL_READINESS_PACKET.md`, `docs/V0190_BRIDGE_RIVERBANK_RISK_ROLLBACK.md`, `docs/art-prompts/V0191_01_BRIDGE_RIVERBANK_MATERIAL_OPT_IN.md`, and `docs/V0190_IMPLEMENTATION_REPORT.md`; add `npm run godot:validate:salto-experimental-artifact-retention`, cleanup dry-run only, `npm run validate:content`, `npm run validate:art-intake`, `npm run validate:runtime-art-slots`, isolation scans for zero images, zero slots, no runtime code changes, no player-slice integration, no browser wiring, no new launcher, no default-launcher mutation, no broad stone replacement authorization, selected hash `638ce153d7a3d39db729dfa13ba05f3fb05c437c2802ab91b5cd248bd2036753`, v0.191 prepared but not started, and `git diff --check`.

For v0.191 post-shell-freeze next-phase scorecard changes, add docs existence checks for `docs/V0191_POST_SHELL_FREEZE_SCORECARD.md`, `docs/V0191_CLEANUP_EXECUTION_DECISION.md`, `docs/art-prompts/V0192_01_RECOMMENDED_NEXT_PHASE.md`, and `docs/V0191_IMPLEMENTATION_REPORT.md`; add `npm run godot:validate:salto-experimental-artifact-retention`, cleanup dry-run only, `npm run validate:content`, `npm run validate:art-intake`, `npm run validate:runtime-art-slots`, isolation scans for zero images, zero slots, no runtime code changes, no player-slice integration, no browser wiring, no new launcher, no default-launcher mutation, no cleanup deletion/archive move, no broad stone replacement authorization, exactly one recommended v0.192 prompt created, v0.192 not started, and `git diff --check`.

For v0.192 human-review override presentation-shell v2 architecture audit changes, add docs existence checks for `docs/V0192_PRESENTATION_SHELL_V2_ARCHITECTURE_AUDIT.md`, `docs/V0192_PRESENTATION_SHELL_V2_CONTRACT_AND_ROLLBACK.md`, `docs/art-prompts/V0193_01_PRESENTATION_SHELL_V2_IMPLEMENTATION.md`, and `docs/V0192_IMPLEMENTATION_REPORT.md`; add `npm run godot:validate:salto-experimental-artifact-retention`, cleanup dry-run only, `npm run validate:content`, `npm run validate:art-intake`, `npm run validate:runtime-art-slots`, isolation scans for zero images, zero slots, no runtime implementation, no player-slice material integration, no browser wiring, no launcher mutation, no default-launcher mutation, no gameplay/pathing/collision/objective/AI/save/stable-ID mutation, selected ground and road material hashes preserved, wet-granite bridge-riverbank material comparator-only, one bounded v0.193 shell-v2 prompt created, v0.193 not started, and `git diff --check`.

For v0.193 isolated Salto presentation-shell v2 prototype changes, add `npm run godot:validate:salto-presentation-shell-v2`, `npm run godot:capture:salto-presentation-shell-v2`, Windows-side Godot review where available, `node tools/godot/saltoPresentationShellV2Tool.mjs validation --artifact-root=artifacts/desktop-spikes/godot-salto/v0193`, `node tools/godot/saltoPresentationShellV2Tool.mjs capture --artifact-root=artifacts/desktop-spikes/godot-salto/v0193`, `node tools/godot/saltoPresentationShellV2Tool.mjs benchmark --artifact-root=artifacts/desktop-spikes/godot-salto/v0193`, `node tools/godot/saltoPresentationShellV2Tool.mjs boundary --artifact-root=artifacts/desktop-spikes/godot-salto/v0193`, artifact retention validation, cleanup dry-run only unless candidates are positively safe, `npm run validate:content`, `npm run validate:art-intake`, `npm run validate:runtime-art-slots`, isolation scans for zero images, zero imported art slots, existing five character slots preserved, selected ground and road material hashes preserved, no wet-granite bridge-riverbank material integration, no default-art enablement, default procedural launcher preserved, all prior launchers preserved, legacy shell preserved as comparator/fallback, browser runtime untouched, no gameplay/pathing/collision/objective/AI/save/stable-ID mutation, benchmark v2 FPS ratio >= 0.85 against legacy L1 and p95 worsening <= 20%, v0.194 not started, and `git diff --check`.

For v0.172 cleanup execution, documentation-budget, and environment-roadmap changes, add `npm run godot:audit:salto-experimental-artifacts`, `npm run godot:validate:salto-experimental-artifact-retention`, `npm run godot:cleanup:salto-experimental-artifacts`, safe-only cleanup only when candidates are positively safe, validation for default player slice and every existing Worker/Barracks/Militia/Aster/Ashen opt-in launcher, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, `npm run validate:runtime-art-slots`, isolation scans for zero images, zero slots, no runtime visual changes, no archive moves, no default-art enablement, default procedural launchers preserved, no browser/save/stable-ID/gameplay mutation, no v0.173 work, and `git diff --check`.

For v0.166 three-slot visual coherence, review-launcher clarity, and safe-cleanup changes, add `npm run godot:validate:salto-three-slot-visual-coherence`, `npm run godot:cleanup:salto-experimental-artifacts`, `npm run godot:cleanup:salto-experimental-artifacts -- --apply-safe-only` only when candidates are positively safe, Windows-side Computer Use review where available, `node tools/godot/saltoThreeSlotVisualCoherenceTool.mjs report --artifact-root=artifacts/desktop-spikes/godot-salto/v0166`, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans for zero images, zero new slots, exactly three opt-in slots total, default procedural launchers preserved, all prior opt-in launchers preserved, no Aster/Ashen/browser wiring/production manifest mutation/save or stable-ID mutation/gameplay mutation/default-launcher mutation/package leakage, no required evidence deletion, and `git diff --check`.

For v0.165 three-slot visual hardening and artifact-hygiene changes, add `npm run godot:validate:salto-three-slot-visual-hardening`, `npm run godot:audit:salto-experimental-artifacts`, Windows-side Computer Use review where available, `node tools/godot/saltoThreeSlotVisualHardeningTool.mjs computer-use --artifact-root=artifacts/desktop-spikes/godot-salto/v0165`, `node tools/godot/saltoThreeSlotVisualHardeningTool.mjs summary --artifact-root=artifacts/desktop-spikes/godot-salto/v0165`, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans for zero images, zero new slots, exactly three opt-in slots total, all existing launchers preserved, no Aster/Ashen/browser wiring/production manifest mutation/save or stable-ID mutation/default-launcher mutation/package leakage, no required evidence deletion, and `git diff --check`.

For v0.164 Worker + Barracks + Militia opt-in player-slice changes, add `npm run godot:validate:salto-worker-barracks-militia-art-experiment`, Windows-side Computer Use review where available, `node tools/godot/saltoWorkerBarracksMilitiaArtOptInTool.mjs computer-use --artifact-root=artifacts/desktop-spikes/godot-salto/v0164`, `node tools/godot/saltoWorkerBarracksMilitiaArtOptInTool.mjs summary --artifact-root=artifacts/desktop-spikes/godot-salto/v0164`, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans for zero images, exactly three combined opt-in slots, Worker-only launcher preserved, Worker + Barracks launcher preserved, no fourth slot/browser wiring/production manifest mutation/save or stable-ID mutation/default-launcher mutation/package leakage, and `git diff --check`.

For v0.163 Worker + Barracks opt-in visual QA hardening changes, add `npm run godot:validate:salto-worker-barracks-art-opt-in-hardening`, Windows-side Computer Use review where available, `node tools/godot/saltoWorkerBarracksArtOptInHardeningTool.mjs computer-use --artifact-root=artifacts/desktop-spikes/godot-salto/v0163`, `node tools/godot/saltoWorkerBarracksArtOptInHardeningTool.mjs summary --artifact-root=artifacts/desktop-spikes/godot-salto/v0163`, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans for zero images, exactly two combined opt-in slots, Worker-only launcher preserved, combined launcher preserved, no third slot/browser wiring/production manifest mutation/save or stable-ID mutation/default-launcher mutation/package leakage, and `git diff --check`.

For v0.162 Worker + Barracks opt-in player-slice changes, add `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:validate:player-slice`, `npm run godot:validate:salto-worker-art-experiment`, `npm run godot:validate:salto-worker-barracks-art-experiment`, `npm run godot:capture:salto-worker-barracks-art-experiment`, `npm run godot:benchmark:salto-worker-barracks-art-experiment`, Windows-side Computer Use review where available, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans for zero images, exactly two combined opt-in slots, Worker-only launcher preserved, no third slot/browser wiring/production manifest mutation/save or stable-ID mutation/default-launcher mutation/package leakage, and `git diff --check`.

For v0.161 Worker-art opt-in visual QA hardening changes, add `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:validate:player-slice`, `npm run godot:validate:salto-worker-art-experiment`, `npm run godot:validate:salto-worker-art-opt-in-hardening`, Windows-side Computer Use review where available, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans for zero images, exactly one Worker opt-in slot, no second slot/browser wiring/production manifest mutation/save or stable-ID mutation/default-launcher mutation/package leakage, and `git diff --check`.

For v0.160 Worker billboard opt-in player-slice integration changes, add `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:validate:player-slice`, `npm run godot:validate:salto-worker-art-experiment`, `npm run godot:capture:salto-worker-art-experiment`, `npm run godot:benchmark:salto-worker-art-experiment`, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans for no second slot/browser wiring/production manifest mutation/save or stable-ID mutation/default-launcher mutation, and `git diff --check`.

For v0.159 first player-facing hybrid-art integration readiness packet changes, add `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:hybrid-mixed-combat:validate`, `npm run godot:hybrid-mixed-combat:audit`, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans for zero images/slots/normal-slice integration/browser wiring/default-launcher mutation, and `git diff --check`.

For v0.158 private hybrid mixed-combat readability stress comparator changes, add `npm run godot:hybrid-mixed-combat:validate`, `npm run godot:hybrid-mixed-combat:audit`, `npm run godot:hybrid-mixed-combat:benchmark:headed`, and `npm run godot:hybrid-mixed-combat:capture`.

For v0.157 private Ashen Raider visual-restraint replacement comparator changes, add `npm run godot:ashen-raider-replacement:derivatives:reproduce`, `npm run godot:ashen-raider-replacement:validate`, `npm run godot:ashen-raider-replacement:audit`, `npm run godot:ashen-raider-replacement:benchmark:headed`, and `npm run godot:ashen-raider-replacement:capture`.

For v0.156 private Ashen Raider billboard comparator changes, add `npm run godot:ashen-raider-billboard:fallback:reproduce`, `npm run godot:ashen-raider-billboard:metadata`, `npm run godot:ashen-raider-billboard:validate`, `npm run godot:ashen-raider-billboard:audit`, `npm run godot:ashen-raider-billboard:benchmark:headed`, and `npm run godot:ashen-raider-billboard:capture`.

For v0.155 private Militia billboard repair comparator changes, add `npm run godot:militia-billboard-repair:derivatives:reproduce`, `npm run godot:militia-billboard-repair:validate`, `npm run godot:militia-billboard-repair:audit`, `npm run godot:militia-billboard-repair:benchmark:headed`, and `npm run godot:militia-billboard-repair:capture`.

For v0.154 private Militia billboard comparator changes, add `npm run godot:militia-billboard:metadata`, `npm run godot:militia-billboard:fallback:reproduce`, `npm run godot:militia-billboard:validate`, `npm run godot:militia-billboard:audit`, `npm run godot:militia-billboard:benchmark:headed`, and `npm run godot:militia-billboard:capture`.

For v0.153 private hybrid three-slot composition stress comparator changes, add `npm run godot:hybrid-three-slot-composition:validate`, `npm run godot:hybrid-three-slot-composition:audit`, `npm run godot:hybrid-three-slot-composition:benchmark:headed`, and `npm run godot:hybrid-three-slot-composition:capture`.

For v0.152 private Aster billboard repair comparator changes, add `npm run godot:aster-billboard-repair:derivatives:reproduce`, `npm run godot:aster-billboard-repair:validate`, `npm run godot:aster-billboard-repair:audit`, `npm run godot:aster-billboard-repair:benchmark:headed`, and `npm run godot:aster-billboard-repair:capture`.

For v0.151 private Aster billboard comparator changes, add `npm run godot:aster-billboard:metadata`, `npm run godot:aster-billboard:fallback:reproduce`, `npm run godot:aster-billboard:validate`, `npm run godot:aster-billboard:audit`, `npm run godot:aster-billboard:benchmark:headed`, and `npm run godot:aster-billboard:capture`.

For v0.150 private Barrosan Barracks material seam-repair comparator changes, add `npm run godot:barracks-material-seam-repair:derivatives:reproduce`, `npm run godot:barracks-material-seam-repair:validate`, `npm run godot:barracks-material-seam-repair:audit`, `npm run godot:barracks-material-seam-repair:benchmark:headed`, and `npm run godot:barracks-material-seam-repair:capture`.

For v0.149 private Barrosan Barracks material comparator changes, add `npm run godot:barracks-material:validate`, `npm run godot:barracks-material:fallback:reproduce`, `npm run godot:barracks-material:derivatives:reproduce`, `npm run godot:barracks-material:audit`, `npm run godot:barracks-material:benchmark:headed`, and `npm run godot:barracks-material:capture`.

Known current realities:

- v0.166 is a Godot Salto three-slot visual coherence, review-launcher clarity, and safe-cleanup stop. It preserves the Worker slot `worker_billboard_static_v0147` / `HYBRID_WORKER_TRIMMED_1024`, Barracks slot `barrosan_barracks_material_v0149` / `HYBRID_BARRACKS_768_WRAPSAFE_OFFSET_BLEND`, and Militia slot `militia_billboard_static_v0154` / `HYBRID_MILITIA_TRIMMED_1024`, generates zero images, adds zero slots, preserves default procedural launchers and all prior opt-in launchers, adds a review-only mode label/camera framing path, records rendered pixel size measurements, and limits cleanup deletion to known Godot-generated sidecars with explicit `--apply-safe-only`. Run the exact v0.166 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run validate:runtime-art-slots`, relevant Godot default/prior opt-in validations, `npm run godot:validate:salto-three-slot-visual-coherence`, cleanup dry-run and safe-only when eligible, Windows-side Computer Use review where available, v0.166 summary/boundary scans, scaffold Vitest, and `git diff --check`.

- v0.165 is a Godot Salto three-slot visual hardening and artifact-hygiene inventory stop. It preserves the Worker slot `worker_billboard_static_v0147` / `HYBRID_WORKER_TRIMMED_1024`, Barracks slot `barrosan_barracks_material_v0149` / `HYBRID_BARRACKS_768_WRAPSAFE_OFFSET_BLEND`, and Militia slot `militia_billboard_static_v0154` / `HYBRID_MILITIA_TRIMMED_1024`, generates zero images, adds zero slots, preserves all launchers, repairs only the proven square-source-to-sub-square-quad aspect compression, adds duplicate-render and Barracks-binding evidence, performs dry-run artifact inventory only, and starts no v0.166 work. Run the exact v0.165 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:validate:player-slice`, `npm run godot:validate:salto-worker-art-experiment`, `npm run godot:validate:salto-worker-barracks-art-experiment`, `npm run godot:validate:salto-worker-barracks-militia-art-experiment`, `npm run godot:validate:salto-three-slot-visual-hardening`, `npm run godot:audit:salto-experimental-artifacts`, Windows-side Computer Use review where available, v0.165 summary/boundary scans, scaffold Vitest, and `git diff --check`.

- v0.164 is a Godot Salto Worker + Barracks + Militia opt-in player-slice integration and human review stop. It preserves the Worker slot `worker_billboard_static_v0147` / `HYBRID_WORKER_TRIMMED_1024` with SHA-256 `a628065ca92b231b0d4f6a0625d9e259dea080e80d530ee688483611d70049bc`, preserves the Barracks slot `barrosan_barracks_material_v0149` / `HYBRID_BARRACKS_768_WRAPSAFE_OFFSET_BLEND` with SHA-256 `58a60b750370df084b60a1d92077da9367c0ba8a763781e2c3a8a7d96f1c980f`, adds only the Militia slot `militia_billboard_static_v0154` / `HYBRID_MILITIA_TRIMMED_1024` with SHA-256 `c25349f00c422a0b3c9d5862027351bd70008e9314d4e3cd4001676e914321cb`, generates zero images, adds no fourth slot, keeps the default stabilized launcher procedural, preserves the Worker-only launcher and Worker + Barracks launcher, proves Militia missing-art/hash-mismatch fallback with Worker and Barracks still active, and starts no v0.165 work. Run the exact v0.164 requested stack: `npm run godot:validate:salto-worker-barracks-militia-art-experiment`, Windows-side Computer Use review where available, `node tools/godot/saltoWorkerBarracksMilitiaArtOptInTool.mjs computer-use --artifact-root=artifacts/desktop-spikes/godot-salto/v0164`, `node tools/godot/saltoWorkerBarracksMilitiaArtOptInTool.mjs summary --artifact-root=artifacts/desktop-spikes/godot-salto/v0164`, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans, and `git diff --check`.

- v0.163 is a Godot Salto Worker + Barracks opt-in visual-QA hardening and human review stop. It preserves the Worker slot `worker_billboard_static_v0147` / `HYBRID_WORKER_TRIMMED_1024` with SHA-256 `a628065ca92b231b0d4f6a0625d9e259dea080e80d530ee688483611d70049bc`, preserves the Barracks slot `barrosan_barracks_material_v0149` / `HYBRID_BARRACKS_768_WRAPSAFE_OFFSET_BLEND` with SHA-256 `58a60b750370df084b60a1d92077da9367c0ba8a763781e2c3a8a7d96f1c980f`, generates zero images, adds zero slots, keeps the default stabilized launcher procedural, preserves the Worker-only launcher and combined launcher, proves Barracks missing-art/hash-mismatch fallback with Worker still active, and starts no next-milestone work. Run the exact v0.163 requested stack: `npm run godot:validate:salto-worker-barracks-art-opt-in-hardening`, Windows-side Computer Use review where available, `node tools/godot/saltoWorkerBarracksArtOptInHardeningTool.mjs computer-use --artifact-root=artifacts/desktop-spikes/godot-salto/v0163`, `node tools/godot/saltoWorkerBarracksArtOptInHardeningTool.mjs summary --artifact-root=artifacts/desktop-spikes/godot-salto/v0163`, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans, and `git diff --check`.

- v0.162 is a two-slot Godot Salto Worker + Barracks opt-in player-slice integration and human review stop. It preserves the Worker slot `worker_billboard_static_v0147` / `HYBRID_WORKER_TRIMMED_1024` with SHA-256 `a628065ca92b231b0d4f6a0625d9e259dea080e80d530ee688483611d70049bc`, adds only the Barracks slot `barrosan_barracks_material_v0149` / `HYBRID_BARRACKS_768_WRAPSAFE_OFFSET_BLEND` with SHA-256 `58a60b750370df084b60a1d92077da9367c0ba8a763781e2c3a8a7d96f1c980f`, generates zero images, adds no third slot, keeps the default stabilized launcher procedural, preserves the Worker-only launcher, proves Barracks missing-art/hash-mismatch fallback with Worker still active, and starts no v0.163 work. Run the exact v0.162 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:validate:player-slice`, `npm run godot:validate:salto-worker-art-experiment`, `npm run godot:validate:salto-worker-barracks-art-experiment`, `npm run godot:capture:salto-worker-barracks-art-experiment`, `npm run godot:benchmark:salto-worker-barracks-art-experiment`, Windows-side Computer Use review where available, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans, and `git diff --check`.

- v0.161 is a Windows-side visual-QA and real-input hardening pass for the existing v0.160 Worker-art opt-in player-slice path. It preserves the single slot `worker_billboard_static_v0147` / `HYBRID_WORKER_TRIMMED_1024` with SHA-256 `a628065ca92b231b0d4f6a0625d9e259dea080e80d530ee688483611d70049bc`, generates zero images, adds zero slots, keeps the default stabilized launcher procedural, proves missing-art/hash-mismatch fallback, and starts no v0.162 work. Run the exact v0.161 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:validate:player-slice`, `npm run godot:validate:salto-worker-art-experiment`, `npm run godot:validate:salto-worker-art-opt-in-hardening`, Windows-side Computer Use review where available, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans, and `git diff --check`.

- v0.160 is a one-slot Godot Salto Worker billboard opt-in player-slice integration and human review stop. It integrates only `worker_billboard_static_v0147` / `HYBRID_WORKER_TRIMMED_1024` with SHA-256 `a628065ca92b231b0d4f6a0625d9e259dea080e80d530ee688483611d70049bc` behind `GODOT_LAUNCH_SALTO_WORKER_ART_EXPERIMENT_WINDOWS.bat`, preserves the default launchers as procedural, proves missing-art and hash-mismatch fallback, benchmarks against the procedural baseline, and started no v0.161 work inside that checkpoint. Run the exact v0.160 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:validate:player-slice`, `npm run godot:validate:salto-worker-art-experiment`, `npm run godot:capture:salto-worker-art-experiment`, `npm run godot:benchmark:salto-worker-art-experiment`, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans, and `git diff --check`.

- v0.159 is a documentation-only first player-facing hybrid-art integration readiness packet and future v0.160 Worker opt-in contract. It selects `worker_billboard_static_v0147` / `HYBRID_WORKER_TRIMMED_1024` with SHA-256 `a628065ca92b231b0d4f6a0625d9e259dea080e80d530ee688483611d70049bc` as the future first slot, but does not execute integration. It generates zero images, adds zero runtime-art slots, creates no opt-in launcher, preserves default launchers unchanged, uses no existing reference candidate import, wires nothing into the browser, mutates no manifests, packages, production art slots, saves, stable IDs, gameplay, balance, input, maps, objectives, campaign state, or browser behavior, and starts no v0.160 work. Run the exact v0.159 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:hybrid-mixed-combat:validate`, `npm run godot:hybrid-mixed-combat:audit`, `npx vitest run src/game/desktop-spike/GodotSaltoSpikeScaffold.test.ts`, isolation scans, and `git diff --check`.

- v0.158 is an isolated private hybrid mixed-combat readability stress gate and human review stop. It generates zero new images, adds zero runtime-art slots, uses exactly the selected Worker/Barracks/Aster/Militia/v0.157 Ashen Raider context, preserves archived v0.156 Ashen evidence, compares selected-local/fallback-only/orthographic fallback postures across 4/8/16/32 Ashen stress, and stops for Emmanuel review. It uses no existing reference candidate import, no downloaded asset, no animation, no normal Salto player-slice mutation, no browser-runtime wiring, no player-facing Godot wiring, no manifest mutation, no art-slot mutation, no production package mutation, no save change, no stable-ID change, no final runtime-art approval, no final combat-art approval, no final engine selection, no full port, and no v0.159 work. Run the exact v0.158 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:hybrid-mixed-combat:validate`, `npm run godot:hybrid-mixed-combat:audit`, `npm run godot:hybrid-mixed-combat:benchmark:headed`, `npm run godot:hybrid-mixed-combat:capture`, isolation scans, and `git diff --check`.

- v0.157 is an isolated private hybrid Ashen Raider visual-restraint replacement and human review stop. It preserves the technically valid v0.156 source/cutout as archived comparison evidence, generates exactly one restrained replacement source image, adds no sixth runtime-art slot, uses the same hostile private comparator slot, selects `HYBRID_ASHEN_RAIDER_V0157_TRIMMED_1024` with Tier L average-FPS ratio `0.9853` and p95 ratio `1.0159`, records `PASS_V0157_ASHEN_RAIDER_REPLACEMENT_SELECTION_GATE`, records `PASS_V0157_ASHEN_RAIDER_REPLACEMENT_FAIR_PATH_AUDIT`, and stops for Emmanuel review. It uses no existing reference candidate import, no downloaded asset, no animation, no normal Salto player-slice mutation, no browser-runtime wiring, no player-facing Godot wiring, no manifest mutation, no art-slot mutation, no production package mutation, no save change, no stable-ID change, no final runtime-art approval, no final Ashen Raider art approval, no final engine selection, no full port, and no v0.158 work. Run the exact v0.157 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:ashen-raider-replacement:derivatives:reproduce`, `npm run godot:ashen-raider-replacement:validate`, `npm run godot:ashen-raider-replacement:audit`, `npm run godot:ashen-raider-replacement:benchmark:headed`, `npm run godot:ashen-raider-replacement:capture`, isolation scans, and `git diff --check`.

- v0.156 is an isolated private hybrid Ashen Raider billboard intake and human review stop. It generates exactly one original ignored Ashen Raider source image, derives one deterministic alpha cutout, validates one single hostile private comparator runtime-art slot against a tracked fallback, preserves selected Worker/Barracks/Aster/Militia context, adds no sixth runtime-art slot, records Tier L average-FPS ratio `1.0051` and p95 ratio `1.0074`, records `PASS_V0156_ASHEN_RAIDER_BILLBOARD_SINGLE_SLOT_GATE`, records `PASS_V0156_ASHEN_RAIDER_BILLBOARD_FAIR_PATH_AUDIT`, and stops for Emmanuel review. It uses no existing reference candidate import, no downloaded asset, no animation, no normal Salto player-slice mutation, no browser-runtime wiring, no player-facing Godot wiring, no manifest mutation, no art-slot mutation, no production package mutation, no save change, no stable-ID change, no final runtime-art approval, no final Ashen Raider art approval, no final engine selection, no full port, and no v0.157 work. Run the exact v0.156 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:ashen-raider-billboard:fallback:reproduce`, `npm run godot:ashen-raider-billboard:metadata`, `npm run godot:ashen-raider-billboard:validate`, `npm run godot:ashen-raider-billboard:audit`, `npm run godot:ashen-raider-billboard:benchmark:headed`, `npm run godot:ashen-raider-billboard:capture`, isolation scans, and `git diff --check`.

- v0.155 is an isolated private hybrid Militia billboard repair and human review stop. It generates zero new AI images, uses only the existing v0.154 Militia source/cutout, adds no new runtime-art slot and no fifth runtime-art slot, selects `HYBRID_MILITIA_TRIMMED_1024` with Tier L average-FPS ratio `1.0702` and p95 ratio `0.9688`, records 32-Militia stress average-FPS ratio `1.0018` and p95 ratio `0.9946`, records `PASS_V0155_MILITIA_BILLBOARD_REPAIR_GATE`, records `PASS_V0155_MILITIA_BILLBOARD_REPAIR_FAIR_PATH_AUDIT`, and stops for Emmanuel review. It uses no existing reference candidate import, no downloaded asset, no animation, no normal Salto player-slice mutation, no browser-runtime wiring, no player-facing Godot wiring, no manifest mutation, no art-slot mutation, no production package mutation, no save change, no stable-ID change, no final runtime-art approval, no final Militia art approval, no final engine selection, no full port, and no v0.156 work inside that checkpoint. Run the exact v0.155 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:militia-billboard-repair:derivatives:reproduce`, `npm run godot:militia-billboard-repair:validate`, `npm run godot:militia-billboard-repair:audit`, `npm run godot:militia-billboard-repair:benchmark:headed`, `npm run godot:militia-billboard-repair:capture`, isolation scans, and `git diff --check`.

- v0.154 is an isolated private hybrid Militia static billboard single-slot intake experiment and human review stop. It generates exactly one original ignored local Militia source image, derives one deterministic alpha cutout, validates the fourth private comparator-only runtime-art intake check against a tracked diagnostic fallback with selected Aster/Worker/Barracks context, records `PASS_V0154_MILITIA_BILLBOARD_SINGLE_SLOT_GATE`, records `PASS_V0154_MILITIA_BILLBOARD_FAIR_PATH_AUDIT`, records Tier L local-vs-fallback average-FPS ratio `1.0055` plus p95 ratio `1.0199`, and captures `22` screenshots plus `21` benchmark rows. It uses no existing reference candidate import, no downloaded asset, no animation, no fifth runtime-art slot, no normal Salto player-slice mutation, no browser-runtime wiring, no player-facing Godot wiring, no manifest mutation, no art-slot mutation, no production package mutation, no save change, no stable-ID change, no final runtime-art approval, no final Militia art approval, no final engine selection, no full port, and no v0.155 work inside this checkpoint. Run the exact v0.154 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:militia-billboard:metadata`, `npm run godot:militia-billboard:fallback:reproduce`, `npm run godot:militia-billboard:validate`, `npm run godot:militia-billboard:audit`, `npm run godot:militia-billboard:benchmark:headed`, `npm run godot:militia-billboard:capture`, isolation scans, and `git diff --check`.
- v0.153 is an isolated private hybrid three-slot composition stress gate and human review stop. It generates zero new AI images, adds zero new runtime-art slots, uses only the selected private v0.148 Worker billboard, v0.150 Barracks material, and v0.152 Aster billboard paths, validates Tier S/M/L fallback-only, selected-local, and safe ortho comparison scenarios, records `PASS_V0153_HYBRID_THREE_SLOT_STRESS_GATE`, records `PASS_V0153_HYBRID_THREE_SLOT_FAIR_PATH_AUDIT`, and records selected-local Tier L average-FPS ratio `1.0071` plus p95 ratio `1.0379`. It uses no existing reference candidate import, no downloaded asset, no normal Salto player-slice mutation, no browser-runtime wiring, no player-facing Godot wiring, no manifest mutation, no art-slot mutation, no production package mutation, no save change, no stable-ID change, no final runtime-art approval, no final Worker/Barracks/Aster art approval, no final engine selection, no full port, and no v0.154 work. Run the exact v0.153 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:hybrid-three-slot-composition:validate`, `npm run godot:hybrid-three-slot-composition:audit`, `npm run godot:hybrid-three-slot-composition:benchmark:headed`, `npm run godot:hybrid-three-slot-composition:capture`, isolation scans, and `git diff --check`.
- v0.152 is an isolated private hybrid Aster billboard repair and human review stop. It generates zero new AI images, uses only the same v0.151 Aster source/cutout, adds no new runtime-art slot, selects `HYBRID_ASTER_TRIMMED_1024` with Tier L average-FPS ratio `0.9708` and p95 ratio `1.0088`, records `PASS_V0152_ASTER_BILLBOARD_REPAIR_GATE`, records `PASS_V0152_ASTER_BILLBOARD_FAIR_PATH_AUDIT`, and stops for Emmanuel review. It uses no existing reference candidate import, no downloaded asset, no normal Salto player-slice mutation, no browser-runtime wiring, no player-facing Godot wiring, no manifest mutation, no art-slot mutation, no production package mutation, no save change, no stable-ID change, no final runtime-art approval, no final Aster art approval, no final engine selection, no full port, and no v0.153 work. Run the exact v0.152 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:aster-billboard-repair:derivatives:reproduce`, `npm run godot:aster-billboard-repair:validate`, `npm run godot:aster-billboard-repair:audit`, `npm run godot:aster-billboard-repair:benchmark:headed`, `npm run godot:aster-billboard-repair:capture`, isolation scans, and `git diff --check`.
- v0.151 is an isolated private hybrid Aster static billboard single-slot intake experiment and human review stop. It generates exactly one original ignored local Aster source image, validates the local matte-to-alpha cutout against a tracked diagnostic fallback, uses the selected v0.148 Worker and v0.150 Barracks repair only as comparator context, selects `HYBRID_ASTER_LOCAL_STATIC_BILLBOARD` with Tier L average-FPS ratio `0.9273` and p95 ratio `1.1081`, records `PASS_V0151_ASTER_BILLBOARD_SINGLE_SLOT_GATE`, records `PASS_V0151_ASTER_BILLBOARD_FAIR_PATH_AUDIT`, and stops for Emmanuel review. It uses no existing reference candidate import, no generated reference image import, no downloaded asset, no fourth runtime-art slot, no normal Salto player-slice mutation, no browser-runtime wiring, no player-facing Godot wiring, no manifest mutation, no art-slot mutation, no production package mutation, no save change, no stable-ID change, no final runtime-art approval, no final Aster art approval, no final engine selection, no full port, and no v0.152 work. Run the exact v0.151 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:aster-billboard:metadata`, `npm run godot:aster-billboard:fallback:reproduce`, `npm run godot:aster-billboard:validate`, `npm run godot:aster-billboard:audit`, `npm run godot:aster-billboard:benchmark:headed`, `npm run godot:aster-billboard:capture`, isolation scans, and `git diff --check`.
- v0.150 is an isolated private Barrosan Barracks material seam-repair and lighting-review checkpoint. It generates zero new AI images, uses only the same v0.149 material source, adds no new runtime-art slot, selects `HYBRID_BARRACKS_768_WRAPSAFE_OFFSET_BLEND` with Tier L average-FPS ratio `1.0048` and p95 ratio `0.9681`, records `PASS_V0150_BARRACKS_MATERIAL_SEAM_REPAIR_GATE`, records `PASS_V0150_BARRACKS_MATERIAL_SEAM_REPAIR_FAIR_PATH_AUDIT`, and stops for Emmanuel review. It uses no reference candidate import, no downloaded asset, no normal Salto player-slice mutation, no browser-runtime wiring, no player-facing Godot wiring, no manifest mutation, no art-slot mutation, no production package mutation, no save change, no stable-ID change, no final runtime-art approval, no final Barracks material approval, no final engine selection, no full port, and no v0.151 work. Run the exact v0.150 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:barracks-material-seam-repair:derivatives:reproduce`, `npm run godot:barracks-material-seam-repair:validate`, `npm run godot:barracks-material-seam-repair:audit`, `npm run godot:barracks-material-seam-repair:benchmark:headed`, `npm run godot:barracks-material-seam-repair:capture`, isolation scans, and `git diff --check`.
- v0.149 is an isolated private hybrid Barrosan Barracks material single-slot intake experiment and human review stop. It generates exactly one original ignored local material source, validates deterministic 512/768/1024 derivatives against a tracked diagnostic fallback, uses the selected v0.148 Worker derivative only as comparator context, selects `HYBRID_BARRACKS_LOCAL_768` with Tier L average-FPS ratio `1.0499` and p95 ratio `0.9043`, records `PASS_V0149_BARRACKS_MATERIAL_ORIGINAL_GATE`, records `PASS_V0149_BARRACKS_MATERIAL_FAIR_PATH_AUDIT`, and stops for Emmanuel review. It uses no existing reference candidate import, no generated reference image import, no downloaded asset, no third runtime-art slot, no normal Salto player-slice mutation, no browser-runtime wiring, no player-facing Godot wiring, no manifest mutation, no art-slot mutation, no production package mutation, no save change, no stable-ID change, no final runtime-art approval, no final Barracks material approval, no final engine selection, no full port, and no v0.150 work. Run the exact v0.149 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:barracks-material:validate`, `npm run godot:barracks-material:fallback:reproduce`, `npm run godot:barracks-material:derivatives:reproduce`, `npm run godot:barracks-material:audit`, `npm run godot:barracks-material:benchmark:headed`, `npm run godot:barracks-material:capture`, and `git diff --check`.
- v0.148 is an isolated private hybrid Worker billboard single-slot repair and human review stop. It generates zero new AI images, uses only the existing ignored v0.147 Worker source/cutout, adds no second runtime-art slot, preserves the original acceptance gate, selects `HYBRID_WORKER_TRIMMED_1024` with Tier L average-FPS ratio `0.9915` and p95 ratio `1.0053`, records `PASS_V0148_WORKER_BILLBOARD_ORIGINAL_GATE`, records `PASS_V0148_WORKER_BILLBOARD_FAIR_PATH_AUDIT`, and stops for Emmanuel review. It uses no existing reference candidate import, no generated reference image import, no downloaded asset, no normal Salto player-slice mutation, no browser-runtime wiring, no player-facing Godot wiring, no manifest mutation, no art-slot mutation, no production package mutation, no save change, no stable-ID change, no final runtime-art approval, no final Worker design approval, no final engine selection, no full port, and no v0.149 work. Run the exact v0.148 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:worker-billboard-repair:validate`, `npm run godot:worker-billboard-repair:audit`, `npm run godot:worker-billboard-repair:derivatives:reproduce`, `npm run godot:worker-billboard-repair:benchmark:headed`, `npm run godot:worker-billboard-repair:capture`, and `git diff --check`.
- v0.147 is an isolated private hybrid Worker billboard single-slot intake experiment and human review stop. It generates exactly one original ignored local Worker cutout, validates it against deterministic tracked diagnostic fallback evidence, captures Tier S/M/L private comparator proof for the local Worker slot and fallback comparators, records `FAIL_V0147_WORKER_BILLBOARD_TIER_L_THRESHOLD` because the local Worker Tier L average-FPS ratio was `0.8464` against the `0.90` target, and stops for Emmanuel review with a bounded repair-pass recommendation. It uses no existing reference candidate import, no downloaded asset, no normal Salto player-slice mutation, no browser-runtime wiring, no player-facing Godot wiring, no manifest mutation, no art-slot mutation, no production package mutation, no save change, no stable-ID change, no final runtime-art approval, no final Worker design approval, no final engine selection, no full port, and no v0.148 work. Run the exact v0.147 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:worker-billboard:validate`, `npm run godot:worker-billboard:fallback:reproduce`, `npm run godot:worker-billboard:benchmark:headed`, `npm run godot:worker-billboard:capture`, and `git diff --check`.
- v0.146 is an isolated private Godot runtime-art pipeline comparator and human decision stop. It validates the v0.145 reference-art boundary, encodes H3 as primary HUD reference direction with H2 material companion and H1 limited tactical companion, runs procedural-only Tier S/M/L evidence for `ORTHO_3D_MESH`, `BILLBOARD_2D_ATLAS`, and `HYBRID_3D_WORLD_BILLBOARD_UNITS`, recommends `HYBRID_3D_WORLD_BILLBOARD_UNITS` as the next single-slot experiment, and stops for Emmanuel review. It uses zero image generation, no generated reference image import, no downloaded asset, no normal Salto player-slice mutation, no browser-runtime wiring, no player-facing Godot wiring, no manifest mutation, no art-slot mutation, no production package mutation, no save change, no stable-ID change, no final runtime-art approval, no final engine selection, no full port, and no v0.147 work. Run the exact v0.146 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, `npm run godot:runtime-art-comparator:validate`, `npm run godot:runtime-art-comparator:benchmark:headed`, `npm run godot:runtime-art-comparator:capture`, and `git diff --check`.
- v0.145 is a controlled Codex image-generation Salto HUD reference-style human-review stop. It generates exactly three ignored local HUD frames plus matching metadata, contact sheet, review pack, and tracked docs only: no production UI asset lock, runtime-art integration, character portrait, character generation, environment generation, sprite, texture, model, icon-as-asset, UI kit, atlas, animation sheet, additional frame, Godot wiring, browser wiring, runtime import, manifest mutation, art-slot mutation, package inclusion, save change, stable-ID change, final runtime-art choice, protected-IP approval, or v0.146 work. Run the exact v0.145 requested stack: `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run art:reference:init`, `npm run art:reference:validate`, `npm run art:reference:contact-sheet`, `npm run art:reference:review-pack`, and `git diff --check`.
