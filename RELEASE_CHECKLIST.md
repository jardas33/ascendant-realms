...
For post-v0.183 Godot opt-in visual hardening review changes, add `npm run godot:validate:salto-ground-road-material-opt-in`, Windows-side Computer Use review where available, isolation scans for zero images, zero slots, no launcher mutation, default procedural launcher preserved, browser runtime untouched, existing ground and road environment-material opt-ins preserved, exactly five character slots frozen, no save/stable-ID/gameplay/pathing/objective/AI/balance mutation, road missing-art and hash-mismatch fallback, benchmark E2 FPS ratio >= 0.90 and p95 worsening <= 15%, v0.184 not started, and `git diff --check`.
