...
| Private benchmark/profile change | Routine gate plus `npm run benchmark:battle:smoke`, `npm run benchmark:battle:representative`, `npm run benchmark:battle:report`, `npm run test:e2e:smoke:fast`, and `npm run visual:qa` | Add `npm run benchmark:battle:stress`, `npm run visual:review-pack`, `npm run package:playtest`, and `npm run verify:playtest-package` when package metadata/docs change. |
