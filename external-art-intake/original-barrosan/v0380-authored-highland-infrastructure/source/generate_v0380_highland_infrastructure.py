...
EXPORT_PATH = ROOT / "exports" / "barrosan_highland_infrastructure_v0380.glb"
...
def river_center(z: np.ndarray | float) -> np.ndarray:
    z = np.asarray(z)
    # Broad, continuous meander with no abrupt centreline changes.
    return 2.65 * np.sin(z / 13.5) + 0.055 * z


def river_half_width(z: np.ndarray | float) -> np.ndarray:
    z = np.asarray(z)
    return 2.05 + 0.24 * np.sin(z / 9.5 + 0.5) + 0.10 * np.sin(z / 4.8)


def road_center(x: np.ndarray | float) -> np.ndarray:
    x = np.asarray(x)
    return 1.15 * np.sin(x / 13.0) - 0.012 * x


def road_half_width(x: np.ndarray | float) -> np.ndarray:
    x = np.asarray(x)
    return 1.20 + 0.15 * np.sin(x / 8.5 + 0.4)
...
def make_terrain(nx: int = 101, nz: int = 77) -> trimesh.Trimesh:
    ...
    inner_t = 1.0 - smoothstep(0.15, rw, dr)
    bed_t = 1.0 - smoothstep(rw * 0.42, rw * 0.96, dr)
    # A continuous carved channel: banks slope down, the river bed stays below water.
    Y = base - 1.28 * valley_t - 0.50 * inner_t - 0.34 * bed_t
    ...
    road_feather = 1.0 - smoothstep(road_w, road_w + 0.88, d_road)
    road_grade = 0.50 + 0.08 * np.sin(X / 17.0) + 0.025 * np.cos(X / 6.0)
    # Never raise the river bed into a land plug. Road shaping fades out before
    # the wet bank and resumes on the opposite landing.
    river_protect = smoothstep(rw + 0.28, rw + 1.45, dr)
    road_effect = road_feather * river_protect
    Y = Y * (1.0 - 0.68 * road_effect) + road_grade * (0.68 * road_effect)
    Y -= 0.055 * road_core * river_protect

    # Stabilized bridge landings while retaining bank depth.
    near_crossing = np.exp(-((X / 7.0) ** 4 + ((Z - road_center(X)) / 2.8) ** 4))
    landing_target = 0.50 + 0.03 * np.sin(X / 5.0)
    bank_side = smoothstep(rw + 0.12, rw + 1.35, dr)
    bank_limit = 1.0 - smoothstep(rw + 2.6, rw + 4.0, dr)
    landing_blend = near_crossing * bank_side * bank_limit
    Y = Y * (1.0 - landing_blend) + landing_target * landing_blend

    # Add gentle embedded wheel ruts in road core.
    for offset in (-0.62, 0.62):
        rut = np.exp(-((Z - (rdc + offset)) ** 2) / 0.045) * road_core * river_protect
        Y -= 0.018 * rut
    ...
    grass_low = np.array([70, 92, 57], dtype=float)
    grass_high = np.array([112, 128, 76], dtype=float)
    grass_dry = np.array([132, 125, 76], dtype=float)
    wet = np.array([48, 68, 56], dtype=float)
    mud = np.array([74, 61, 45], dtype=float)
    dirt = np.array([105, 70, 43], dtype=float)
    dirt_light = np.array([137, 94, 53], dtype=float)
    rock = np.array([105, 103, 92], dtype=float)
    ...
    wet_factor = np.clip((outer - dr) / 4.2, 0, 1) * (1.0 - inner_t * 0.55)
    ...
    road_color_t = np.clip(road_effect * 0.88 + road_core * river_protect * 0.20, 0, 1)
    ...
    shade = np.clip(0.98 + 0.055 * micro, 0.93, 1.04)
    colors *= shade[..., None]
...
def make_water(samples: int = 180) -> trimesh.Trimesh:
    ...
    for i, (z, c, w) in enumerate(zip(zs, centers, widths)):
        y = -0.67 + 0.018 * math.sin(z / 4.2)
        for side in (-1.0, 1.0):
            verts.append([c + side * w, y, z])
            blue = np.array([39, 89, 94], dtype=float)
...
def create_bridge() -> list[trimesh.Trimesh]:
    ...
    post_xs = np.linspace(-4.30, 4.30, 6)
    for side in (-1, 1):
        ...
        for h, suffix in ((0.54, "Low"), (1.02, "High")):
...
def create_scene() -> trimesh.Scene:
    ...
    scene.metadata["asset"] = {
        "name": "Ascendant Realms v0.380 Corrected Highland Infrastructure",
        "units": "metres",
        "up_axis": "Y",
        "seed": SEED,
...
def write_previews(scene: trimesh.Scene) -> None:
    ...
    ax.set_aspect("equal")
    ax.set_axis_off()
    ax.set_title("v0.380 corrected authored layout — continuous terrain, embedded road, recessed river and bridge", fontsize=13, pad=12)
    fig.tight_layout()
    ...
    fig.suptitle("v0.380 bridge assembly schematic", fontsize=14)
    fig.tight_layout()
...
def main() -> None:
    ...
    EXPORT_PATH.write_bytes(export_glb(scene, include_normals=True))
    manifest = {
        "name": "barrosan_highland_infrastructure_v0380",
        "seed": SEED,
