...
const SLOT_ID := "barrosan_wet_granite_bridge_riverbank_material_v0189"
const APPROACH_FALLBACK := "BRIDGE_RIVERBANK_MATERIAL_DIAGNOSTIC_FALLBACK"
const APPROACH_LOCAL_512 := "BRIDGE_RIVERBANK_MATERIAL_LOCAL_512"
const APPROACH_LOCAL_768 := "BRIDGE_RIVERBANK_MATERIAL_LOCAL_768"
const APPROACH_LOCAL_1024 := "BRIDGE_RIVERBANK_MATERIAL_LOCAL_1024"
const APPROACH_WRAPSAFE_1024 := "BRIDGE_RIVERBANK_MATERIAL_1024_WRAPSAFE_OFFSET_BLEND"
const APPROACHES := [
	APPROACH_FALLBACK,
	APPROACH_LOCAL_512,
	APPROACH_LOCAL_768,
...
var total_bridge_riverbank_tile_rebuild_count := 0
...
func start() -> void:
	...
	local_slot_root = ProjectSettings.globalize_path("res://../../artifacts/desktop-spikes/godot-salto/v0189/local-bridge-riverbank-material-slot")
	material_sources = _load_material_sources()
	if args.has("--bridge-riverbank-material-validate-only"):
		var validation := _validation_report()
		_write_absolute_json(_path_join(artifact_root, "bridge-riverbank-material-validation-runtime.json"), validation)
		get_tree().quit(0 if validation.get("status", "FAIL") == "PASS_V0189_BRIDGE_RIVERBANK_MATERIAL_RUNTIME_VALIDATION" else 1)
		return
	var run_kind := "bridge-riverbank-material-headed-benchmark-and-capture"
	if args.has("--bridge-riverbank-material-capture-only"):
		run_kind = "bridge-riverbank-material-capture-refresh"
	var report := await _run_sequence(run_kind)
	_write_absolute_json(_path_join(artifact_root, "bridge-riverbank-material-runtime.json"), report)
	get_tree().quit(0 if report.get("status", "FAIL") == "PASS_V0189_BRIDGE_RIVERBANK_MATERIAL_RUNTIME_EVIDENCE" else 1)
...
func _validation_report() -> Dictionary:
	...
	for key in material_sources.keys():
		...
		if source.get("status", "FAIL") != "PASS":
			errors.append_array(source.get("errors", ["Unknown Bridge-riverbank material source validation failure."]))
	return {
		"schemaVersion": 1,
		"checkpoint": CHECKPOINT,
		"slotId": SLOT_ID,
		"status": "PASS_V0189_BRIDGE_RIVERBANK_MATERIAL_RUNTIME_VALIDATION" if errors.is_empty() else "FAIL_V0189_BRIDGE_RIVERBANK_MATERIAL_RUNTIME_VALIDATION",
		...
		"bridgeRiverbankMaterialImportedToPlayerSlice": false,
		"roadMaterialImportedToPlayerSlice": false,
		"structureMaterialImportedToPlayerSlice": false,
		"errors": errors
	}
...
func _run_sequence(run_kind: String) -> Dictionary:
		...
		for trial_index in range(trials):
			...
			for approach in ordered_approaches:
				...
				if benchmark.get("status", "FAIL") != "PASS":
					errors.append("Bridge-riverbank material benchmark failed for %s %s trial %d." % [approach, tier, trial_index + 1])
				if trial_index == 0:
	...
	return {
		...
		"slotId": SLOT_ID,
		"runKind": run_kind,
		"status": "PASS_V0189_BRIDGE_RIVERBANK_MATERIAL_RUNTIME_EVIDENCE" if errors.is_empty() else "FAIL_V0189_BRIDGE_RIVERBANK_MATERIAL_RUNTIME_EVIDENCE",
...
func _benchmark_current_view(config: Dictionary) -> Dictionary:
	...
	return {
		...
		"navigationParity": "not-applicable-static-bridge-riverbank-material-comparator",
		"pressureParity": "not-applicable-static-bridge-riverbank-material-comparator",
		"stuckUnitCount": 0,
...
func _build_scene(config: Dictionary) -> void:
	...
	active_scene.name = "V0189BridgeRiverbankMaterialComparatorScene"
	add_child(active_scene)
	_setup_camera_and_lighting(config)
	var source: Dictionary = config.get("assetSource", {})
	var material := _bridge_riverbank_material_for_source(source)
	...
	_add_context_bridge_riverbank_markers(material)
...
func _setup_camera_and_lighting(config: Dictionary) -> void:
	...
	active_camera.name = "V0189BridgeRiverbankMaterialComparatorCamera"
...
func _add_base_slab(material: StandardMaterial3D) -> void:
	...
	node.name = "bridge_riverbank_material_base_repeat_slab"
	node.mesh = mesh
	node.position = Vector3(0, 0.0, 0)
	node.material_override = material
	active_scene.add_child(node)
	total_bridge_riverbank_tile_rebuild_count += 1
...
func _add_tiled_planes(count: int, area: Vector2, material: StandardMaterial3D, view: String) -> void:
	...
	for index in range(count):
		...
		mesh.size = area
		var node := MeshInstance3D.new()
		node.name = "bridge_riverbank_material_repeat_patch_%03d" % index
		node.mesh = mesh
...
func _add_context_bridge_riverbank_markers(material: StandardMaterial3D) -> void:
	_add_box("procedural_road_readability_context", Vector3(0, 0.052, -4.9), Vector3(22.0, 0.060, 0.44), _flat_material(Color(0.20, 0.17, 0.11, 0.94), true))
	_add_box("procedural_road_north_shoulder_context", Vector3(0, 0.070, -4.62), Vector3(22.0, 0.034, 0.055), _flat_material(Color(0.08, 0.075, 0.060, 0.82), true))
	_add_box("procedural_road_south_shoulder_context", Vector3(0, 0.070, -5.18), Vector3(22.0, 0.034, 0.055), _flat_material(Color(0.08, 0.075, 0.060, 0.82), true))
	_add_box("river_water_readability_context", Vector3(-6.35, 0.056, 1.15), Vector3(1.62, 0.060, 10.8), _flat_material(Color(0.075, 0.27, 0.31, 0.76), true, true, 0.04))
	_add_box("river_water_inner_shadow_context", Vector3(-6.35, 0.066, 1.15), Vector3(1.08, 0.030, 10.2), _flat_material(Color(0.040, 0.13, 0.16, 0.70), true, true, 0.02))
	_add_box("west_retaining_edge_material_context", Vector3(-7.33, 0.108, 1.15), Vector3(0.34, 0.115, 10.7), material)
	_add_box("east_retaining_edge_material_context", Vector3(-5.37, 0.108, 1.15), Vector3(0.34, 0.115, 10.7), material)
	_add_box("bridge_deck_readability_context", Vector3(-6.35, 0.130, -0.80), Vector3(2.78, 0.125, 0.82), _flat_material(Color(0.26, 0.245, 0.20, 0.93), true))
	_add_box("north_bridge_abutment_material_context", Vector3(-6.35, 0.160, -1.42), Vector3(2.26, 0.150, 0.34), material)
	_add_box("south_bridge_abutment_material_context", Vector3(-6.35, 0.160, -0.18), Vector3(2.26, 0.150, 0.34), material)
	_add_box("bank_transition_ground_comparison", Vector3(-3.75, 0.067, 2.95), Vector3(2.1, 0.055, 1.3), _flat_material(Color(0.19, 0.17, 0.115, 0.76), true))
	for i in range(4):
...
func _texture_for_source(source: Dictionary) -> Texture2D:
	...
	if load_error != OK:
		push_error("Could not load Bridge-riverbank material comparator texture %s" % path)
...
func _bridge_riverbank_material_for_source(source: Dictionary) -> StandardMaterial3D:
	var path := str(source.get("absolutePath", source.get("path", "")))
	var key := "%s|bridge-riverbank-material|%s" % [path, str(source.get("uvScale", 1.0))]
...
func _load_material_sources() -> Dictionary:
	return {
		"fallback": _validated_source(
			...
			"tracked-bridge-riverbank-material-diagnostic-fallback"
		),
		"local_512": _validated_source(
			...
			"local-bridge-riverbank-material-512"
		),
		"local_768": _validated_source(
			...
			"local-bridge-riverbank-material-768"
		),
		"local_1024": _validated_source(
			...
			"local-bridge-riverbank-material-1024"
		),
		"wrapsafe_1024": _validated_source(
			...
			"local-bridge-riverbank-material-1024-wrapsafe-offset-blend"
		)
	}
...
func _fair_path_audit() -> Dictionary:
	return {
		"schemaVersion": 1,
		"checkpoint": CHECKPOINT,
		"localAndFallbackShareBridgeRiverbankPlaneRenderPath": true,
		...
		"bridgeRiverbankTileNodeRebuildCount": total_bridge_riverbank_tile_rebuild_count,
...
func _artifact_root_from_args() -> String:
	...
	return ProjectSettings.globalize_path("res://../../artifacts/desktop-spikes/godot-salto/v0189/evidence")
...
func _is_script_arg(arg: String) -> bool:
	return arg == "--barrosan-bridge-riverbank-material-single-slot" or arg == "--bridge-riverbank-material-validate-only" or arg == "--bridge-riverbank-material-benchmark-sequence" or arg == "--bridge-riverbank-material-capture-only" or arg.begins_with("--artifact-root=") or arg.begins_with("--viewport=") or arg.begins_with("--resolution=")
