...
const APPROACH_LOCAL := "HYBRID_ASHEN_RAIDER_LOCAL_STATIC_BILLBOARD"
const APPROACH_ORTHO := "ORTHO_ASHEN_RAIDER_PROCEDURAL_FALLBACK"
...
func start() -> void:
	...
	local_slot_root = ProjectSettings.globalize_path("res://../../artifacts/desktop-spikes/godot-salto/v0156/local-ashen-raider-slot")
	...
	if args.has("--ashen-raider-billboard-single-slot-validate-only"):
		var validation := _validation_report()
		_write_absolute_json(_path_join(artifact_root, "ashen-raider-billboard-single-slot-validation-runtime.json"), validation)
		get_tree().quit(0 if validation.get("status", "FAIL") == "PASS_V0156_ASHEN_RAIDER_BILLBOARD_RUNTIME_VALIDATION" else 1)
		return
	var run_kind := "ashen-raider-billboard-single-slot-headed-benchmark-and-capture"
	if args.has("--ashen-raider-billboard-single-slot-capture-only"):
		run_kind = "ashen-raider-billboard-single-slot-capture-refresh"
	var report := await _run_sequence(run_kind)
	_write_absolute_json(_path_join(artifact_root, "ashen-raider-billboard-single-slot-runtime.json"), report)
	get_tree().quit(0 if report.get("status", "FAIL") == "PASS_V0156_ASHEN_RAIDER_BILLBOARD_RUNTIME_EVIDENCE" else 1)

func _validation_report() -> Dictionary:
	var errors: Array[String] = []
	for source in [fallback_source, local_source, aster_context_source, worker_context_source, barracks_context_source, militia_context_source]:
	...
	return {
		...
		"slotId": SLOT_ID,
		"status": "PASS_V0156_ASHEN_RAIDER_BILLBOARD_RUNTIME_VALIDATION" if errors.is_empty() else "FAIL_V0156_ASHEN_RAIDER_BILLBOARD_RUNTIME_VALIDATION",
...
func _run_sequence(run_kind: String) -> Dictionary:
	var errors: Array[String] = []
	var validation := _validation_report()
	if validation.get("status", "FAIL") != "PASS_V0156_ASHEN_RAIDER_BILLBOARD_RUNTIME_VALIDATION":
		errors.append_array(validation.get("errors", ["Ashen Raider billboard validation failed."]))
	...
	return {
		...
		"slotId": SLOT_ID,
		"runKind": run_kind,
		"status": "PASS_V0156_ASHEN_RAIDER_BILLBOARD_RUNTIME_EVIDENCE" if errors.is_empty() else "FAIL_V0156_ASHEN_RAIDER_BILLBOARD_RUNTIME_EVIDENCE",
		...
		"limitations": [
			"v0.156 only tests a private Ashen Raider static billboard source.",
...
func _source_for_approach(approach: String) -> Dictionary:
	...
	if approach == APPROACH_ORTHO:
		return {"status": "PASS", "sourceKind": "orthographic-procedural-ashen-raider-fallback", "sha256": "not-applicable", "absolutePath": "procedural-ashen-raider"}
...
func _build_scene(config: Dictionary) -> void:
	...
	if approach == APPROACH_ORTHO:
		_add_procedural_ashenRaider_units(int(config.get("ashenRaiderCount", 0)), float(config.get("scaleMultiplier", 1.0)), view)
	else:
...
func _setup_camera_and_lighting(config: Dictionary) -> void:
	...
	active_camera.name = "V0156Ashen RaiderComparatorCamera"
...
func _capture_review_views(start_index: int) -> Array[Dictionary]:
	var captures: Array[Dictionary] = []
	var views := [
		{"id": "one_ashenRaider_source", "tier": "S", "scale": 1.0, "approach": APPROACH_LOCAL},
		...
		{"id": "aster_ashenRaider_hierarchy", "tier": "S", "scale": 1.0, "approach": APPROACH_LOCAL},
		{"id": "worker_ashenRaider_distinction", "tier": "M", "scale": 1.0, "approach": APPROACH_LOCAL},
		{"id": "militia_ashen_raider_friendly_hostile_distinction", "tier": "M", "scale": 1.0, "approach": APPROACH_LOCAL},
		{"id": "four_ashen_raider_wave", "tier": "M", "scale": 1.0, "approach": APPROACH_LOCAL},
...
func _load_local_source() -> Dictionary:
	return _validated_source(
		...
		"local-ashen-raider-static-billboard",
		SLOT_ID,
		""
	)
...
func _artifact_root_from_args() -> String:
	...
	return ProjectSettings.globalize_path("res://../../artifacts/desktop-spikes/godot-salto/v0156/evidence")
...
func _is_script_arg(arg: String) -> bool:
	return arg == "--ashen-raider-billboard-single-slot" or arg == "--ashen-raider-billboard-single-slot-validate-only" or arg == "--ashen-raider-billboard-single-slot-benchmark-sequence" or arg == "--ashen-raider-billboard-single-slot-capture-only" or arg.begins_with("--artifact-root=") or arg.begins_with("--viewport=") or arg.begins_with("--resolution=")
