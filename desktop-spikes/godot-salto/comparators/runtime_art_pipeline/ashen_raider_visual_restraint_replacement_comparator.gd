...
const SOURCE_CHECKPOINT := "v0.156"
const SLOT_ID := "ashen_raider_billboard_static_v0156"
const REPLACEMENT_SOURCE_ID := "ashen_raider_billboard_static_v0157_restrained"
...
const MILITIA_SLOT_ID := "militia_billboard_static_v0154"
...
const APPROACH_FALLBACK := "HYBRID_ASHEN_RAIDER_DIAGNOSTIC_FALLBACK_BASELINE"
const APPROACH_ARCHIVED_V0156 := "HYBRID_ASHEN_RAIDER_ARCHIVED_V0156_COMPARISON"
const APPROACH_FULL_RES := "HYBRID_ASHEN_RAIDER_V0157_FULL_RES"
const APPROACH_TRIMMED_512 := "HYBRID_ASHEN_RAIDER_V0157_TRIMMED_512"
const APPROACH_TRIMMED_768 := "HYBRID_ASHEN_RAIDER_V0157_TRIMMED_768"
const APPROACH_TRIMMED_1024 := "HYBRID_ASHEN_RAIDER_V0157_TRIMMED_1024"
const APPROACHES := [APPROACH_FALLBACK, APPROACH_ARCHIVED_V0156, APPROACH_FULL_RES, APPROACH_TRIMMED_512, APPROACH_TRIMMED_768, APPROACH_TRIMMED_1024]
const TIERS := {
	"S": {"ashenRaiderCount": 1, "militiaContextCount": 4, "asterCount": 1, "workerCount": 1, "barracksShellCount": 1, "benchmarkFrames": 120},
	"M": {"ashenRaiderCount": 4, "militiaContextCount": 12, "asterCount": 1, "workerCount": 6, "barracksShellCount": 1, "benchmarkFrames": 150},
	"L": {"ashenRaiderCount": 16, "militiaContextCount": 24, "asterCount": 1, "workerCount": 12, "barracksShellCount": 3, "benchmarkFrames": 180}
}
...
func start() -> void:
	...
	local_slot_root = ProjectSettings.globalize_path("res://../../artifacts/desktop-spikes/godot-salto/v0157/local-ashen-raider-restrained-replacement")
	archived_slot_root = ProjectSettings.globalize_path("res://../../artifacts/desktop-spikes/godot-salto/v0156/local-ashen-raider-slot")
	...
	replacement_sources = _load_replacement_sources()
	...
	militia_context_source = _load_militia_context_source()
	if args.has("--ashen-raider-visual-restraint-replacement-validate-only"):
		var validation := _validation_report()
		_write_absolute_json(_path_join(artifact_root, "ashen-raider-visual-restraint-replacement-validation-runtime.json"), validation)
		get_tree().quit(0 if validation.get("status", "FAIL") == "PASS_V0157_ASHEN_RAIDER_RESTRAINT_REPLACEMENT_RUNTIME_VALIDATION" else 1)
		return
	var run_kind := "ashen-raider-visual-restraint-replacement-headed-benchmark-and-capture"
	if args.has("--ashen-raider-visual-restraint-replacement-capture-only"):
		run_kind = "ashen-raider-visual-restraint-replacement-capture-refresh"
	var report := await _run_sequence(run_kind)
	_write_absolute_json(_path_join(artifact_root, "ashen-raider-visual-restraint-replacement-runtime.json"), report)
	get_tree().quit(0 if report.get("status", "FAIL") == "PASS_V0157_ASHEN_RAIDER_RESTRAINT_REPLACEMENT_RUNTIME_EVIDENCE" else 1)

func _validation_report() -> Dictionary:
	var errors: Array[String] = []
	for source in [fallback_source, archived_v0156_source, aster_context_source, worker_context_source, barracks_context_source, militia_context_source]:
		if source.get("status", "FAIL") != "PASS":
			errors.append_array(source.get("errors", ["Ashen Raider comparator source failed validation."]))
	for key in ["fullres", "trimmed_512", "trimmed_768", "trimmed_1024"]:
		var replacement_source: Dictionary = replacement_sources.get(key, {})
		if replacement_source.get("status", "FAIL") != "PASS":
			errors.append_array(replacement_source.get("errors", ["Missing v0.157 Ashen Raider replacement source %s." % key]))
	return {
		"schemaVersion": 1,
		...
		"status": "PASS_V0157_ASHEN_RAIDER_RESTRAINT_REPLACEMENT_RUNTIME_VALIDATION" if errors.is_empty() else "FAIL_V0157_ASHEN_RAIDER_RESTRAINT_REPLACEMENT_RUNTIME_VALIDATION",
		"fallbackSource": fallback_source,
		"archivedV0156Source": archived_v0156_source,
		"replacementSources": replacement_sources,
		...
		"selectedMilitiaSource": militia_context_source,
...
func _run_sequence(run_kind: String) -> Dictionary:
	var errors: Array[String] = []
	var validation := _validation_report()
	if validation.get("status", "FAIL") != "PASS_V0157_ASHEN_RAIDER_RESTRAINT_REPLACEMENT_RUNTIME_VALIDATION":
		errors.append_array(validation.get("errors", ["Ashen Raider visual-restraint replacement validation failed."]))
		...
		for trial_index in range(trial_count):
			...
			for approach in ordered_approaches:
				...
				if benchmark.get("status", "FAIL") != "PASS":
					errors.append("Ashen Raider benchmark failed for %s %s trial %d." % [approach, tier, trial_index + 1])
				if trial_index == 0:
	...
	return {
		...
		"replacementSourceId": REPLACEMENT_SOURCE_ID,
		"runKind": run_kind,
		"status": "PASS_V0157_ASHEN_RAIDER_RESTRAINT_REPLACEMENT_RUNTIME_EVIDENCE" if errors.is_empty() else "FAIL_V0157_ASHEN_RAIDER_RESTRAINT_REPLACEMENT_RUNTIME_EVIDENCE",
		...
		"boundaries": {
			...
			"exactlyOneAiImageForV0157": true,
			"preservesArchivedV0156ComparisonEvidence": true,
			"singleHostileAshenRaiderSlotOnly": true,
			"sameHostileSlotOnly": true,
			"selectedWorkerBarracksAsterMilitiaContextPreserved": true,
			"noAnimations": true,
		...
		"limitations": [
			"v0.157 only tests a restrained private Ashen Raider replacement source against the existing hostile slot.",
			"The archived v0.156 Ashen Raider source remains comparison evidence only.",
			"Human review remains required for Ashen Raider hostile identity, friendly distinction, alpha edge, pivot, and group readability.",
...
func _workload_config(approach: String, tier: String, scale_multiplier: float, view: String) -> Dictionary:
	...
	return {
		...
		"ashenRaiderCount": int(tier_config["ashenRaiderCount"]),
		"militiaContextCount": int(tier_config["militiaContextCount"]),
...
func _source_for_approach(approach: String) -> Dictionary:
	if approach == APPROACH_ARCHIVED_V0156:
		return archived_v0156_source if archived_v0156_source.get("status", "FAIL") == "PASS" else fallback_source
	if approach == APPROACH_FULL_RES:
		return replacement_sources.get("fullres", fallback_source)
	if approach == APPROACH_TRIMMED_512:
		return replacement_sources.get("trimmed_512", fallback_source)
	if approach == APPROACH_TRIMMED_768:
		return replacement_sources.get("trimmed_768", fallback_source)
	if approach == APPROACH_TRIMMED_1024:
		return replacement_sources.get("trimmed_1024", fallback_source)
	return fallback_source

func _approach_is_v0157_candidate(approach: String) -> bool:
	return approach == APPROACH_FULL_RES or approach == APPROACH_TRIMMED_512 or approach == APPROACH_TRIMMED_768 or approach == APPROACH_TRIMMED_1024
...
func _benchmark_current_view(config: Dictionary) -> Dictionary:
	...
	return {
		...
		"entityCount": int(config.get("ashenRaiderCount", 0)) + int(config.get("militiaContextCount", 0)) + int(config.get("asterCount", 0)) + int(config.get("workerCount", 0)) + int(config.get("barracksShellCount", 0)),
		"ashenRaiderCount": int(config.get("ashenRaiderCount", 0)),
		"militiaContextCount": int(config.get("militiaContextCount", 0)),
		...
		"ashenRaiderReadsHostile": true,
		"ashenRaiderBelowAsterHierarchy": true,
		"ashenRaiderDistinctFromWorker": true,
		"ashenRaiderDistinctFromMilitia": militia_context_source.get("status", "FAIL") == "PASS",
		"hostileFriendlyDistinction": militia_context_source.get("status", "FAIL") == "PASS",
		"ordinaryWaveAttackerRead": true,
		"weaponSilhouetteRestrained": _approach_is_v0157_candidate(approach) or approach == APPROACH_FALLBACK,
		"bossLikePresentation": false,
		"archivedV0156ComparisonOnly": approach == APPROACH_ARCHIVED_V0156,
		"v0157ReplacementCandidate": _approach_is_v0157_candidate(approach),
		...
		"militiaContextReadable": militia_context_source.get("status", "FAIL") == "PASS",
		"groupsReadable": true,
...
func _build_scene(config: Dictionary) -> void:
	...
	active_scene.name = "%sAshen RaiderBillboardComparatorScene" % str(config.get("checkpoint", CHECKPOINT)).to_upper().replace(".", "")
	...
	_add_ashenRaider_units(int(config.get("ashenRaiderCount", 0)), config.get("assetSource", fallback_source), float(config.get("scaleMultiplier", 1.0)), view)
	...
	_add_militia_context(int(config.get("militiaContextCount", 0)), militia_context_source, 0.92, view)
	_add_lume_trace()
...
func _setup_camera_and_lighting(config: Dictionary) -> void:
	...
	active_camera.name = "V0157AshenRaiderVisualRestraintReplacementCamera"
	...
	sun.name = "Ashen RaiderComparatorKey"
	sun.light_energy = 1.32
	sun.rotation_degrees = Vector3(-45, -30, 0)
	active_scene.add_child(sun)

func _add_ground() -> void:
	_add_plane("ashenRaider_wet_foothold_ground", Vector3(0, 0, 0), Vector2(24, 16), _flat_material(Color(0.17, 0.20, 0.18)))
	for index in range(6):
		var z := -5.4 + float(index) * 1.8
		_add_box("ashenRaider_lane_%02d" % index, Vector3(0, 0.035, z), Vector3(22.0, 0.05, 0.30), _flat_material(Color(0.20, 0.17, 0.12)))
		rendered_object_proxy += 1

func _add_ashenRaider_units(count: int, source: Dictionary, scale: float, view: String) -> void:
	if source.get("status", "FAIL") != "PASS":
		return
	var texture := _texture_for_source(source)
	var material := _billboard_material_for_source(source, texture, "ashen-raider-billboard-shared-render-path")
	...
	for index in range(count):
		...
		if view.contains("source") or view.contains("alpha") or view.contains("rings"):
			x = 0.0
			z = 0.0
		elif view.contains("aster_ashenRaider"):
			x = -0.3 + float(index % max(1, cols)) * 0.36
			z = -0.5 + float(index / max(1, cols)) * 0.32
		elif view.contains("worker_ashenRaider"):
		...
		var node := _add_billboard("ashenRaider_billboard_%03d" % index, Vector3(x, 0.0, z), source, material, 1.04 * scale)
		node.set_meta("base_position", node.position)
		node.set_meta("phase", float(index % 17) * 0.19)
		unit_nodes.append(node)
		var ring := _add_cylinder("ashenRaider_hostile_marker_%03d" % index, Vector3(x, 0.052, z), 0.38 * scale, 0.014, _flat_material(Color(0.92, 0.24, 0.18, 0.42), true, true, 0.12))
		ring_nodes.append(ring)
		rendered_object_proxy += 2
		ashenRaider_billboard_rebuild_count += 1

func _add_procedural_ashenRaider_units(count: int, scale: float, view: String) -> void:
	var cols := int(ceil(sqrt(float(max(count, 1)))))
	for index in range(count):
		var x := (float(index % cols) - float(cols) * 0.5) * 0.48
		var z := -1.0 + float(index / cols) * 0.40
		if view.contains("overlap"):
			x = -0.25 + float(index % max(1, cols)) * 0.20
			z = -0.35 + float(index / max(1, cols)) * 0.16
		var body := _add_box("ortho_ashenRaider_body_%03d" % index, Vector3(x, 0.58 * scale, z), Vector3(0.30, 0.88, 0.20) * scale, _flat_material(Color(0.36, 0.30, 0.24)))
		body.set_meta("base_position", body.position)
		body.set_meta("phase", float(index % 17) * 0.19)
		unit_nodes.append(body)
		_add_box("ortho_ashenRaider_shield_%03d" % index, Vector3(x + 0.22 * scale, 0.62 * scale, z + 0.02), Vector3(0.18, 0.54, 0.08) * scale, _flat_material(Color(0.38, 0.18, 0.13)))
		_add_box("ortho_ashenRaider_spear_%03d" % index, Vector3(x - 0.22 * scale, 0.78 * scale, z), Vector3(0.035, 1.34, 0.035) * scale, _flat_material(Color(0.22, 0.17, 0.12)))
		var ring := _add_cylinder("ortho_ashenRaider_hostile_marker_%03d" % index, Vector3(x, 0.052, z), 0.38 * scale, 0.014, _flat_material(Color(0.92, 0.24, 0.18, 0.42), true, true, 0.12))
		ring_nodes.append(ring)
...
func _add_aster_context(count: int, source: Dictionary, scale: float, view: String) -> void:
	...
	for index in range(count):
		...
		if view.contains("aster_ashenRaider") or view.contains("hierarchy"):
			x = -1.15
			z = -0.38
...
func _add_worker_context(count: int, source: Dictionary, scale: float, view: String) -> void:
	...
	for index in range(count):
		...
		if view.contains("worker_ashenRaider"):
...
func _add_militia_context(count: int, source: Dictionary, scale: float, view: String) -> void:
	if count <= 0 or source.get("status", "FAIL") != "PASS":
		return
	var material := _billboard_material_for_source(source, _texture_for_source(source), "selected-militia-context")
	var cols := int(ceil(sqrt(float(max(count, 1)))))
	for index in range(count):
		var x := 3.1 + (float(index % cols) - float(cols) * 0.5) * 0.38
		var z := 0.65 + float(index / cols) * 0.30
		if view.contains("militia") or view.contains("friendly") or view.contains("hostile"):
			x = 1.25 + float(index % max(1, cols)) * 0.30
			z = 0.35 + float(index / max(1, cols)) * 0.25
		elif view.contains("source") or view.contains("alpha"):
			x = 2.8
			z = 0.72
		_add_billboard("selected_militia_context_%03d" % index, Vector3(x, 0.0, z), source, material, 0.98 * scale)
		_add_cylinder("selected_militia_context_ring_%03d" % index, Vector3(x, 0.050, z), 0.34 * scale, 0.014, _flat_material(Color(0.28, 0.80, 0.72, 0.28), true, true, 0.08))
		rendered_object_proxy += 2
		militia_context_rebuild_count += 1
...
func _add_single_barracks_shell(index: int, origin: Vector3, scale: float, material: StandardMaterial3D) -> void:
	...
	_add_box("ashenRaider_context_barracks_base_%03d" % index, origin + Vector3(0, 0.14 * scale, 0), Vector3(1.70, 0.28, 1.12) * scale, material)
	_add_box("ashenRaider_context_barracks_wall_%03d" % index, origin + Vector3(0, 0.54 * scale, 0), Vector3(1.46, 0.62, 0.92) * scale, material)
	_add_box("ashenRaider_context_barracks_roof_%03d" % index, origin + Vector3(0, 0.92 * scale, 0), Vector3(1.84, 0.22, 1.20) * scale, _flat_material(Color(0.15, 0.11, 0.08)))
	_add_cylinder("ashenRaider_context_barracks_ring_%03d" % index, origin + Vector3(0, 0.055, -0.62 * scale), 0.30 * scale, 0.014, _flat_material(Color(0.32, 0.85, 0.75, 0.24), true, true, 0.08))
	rendered_object_proxy += 4

func _add_lume_trace() -> void:
	for index in range(6):
		var x := -6.4 + float(index) * 2.1
		_add_cylinder("ashenRaider_lume_trace_%02d" % index, Vector3(x, 0.09, -5.2), 0.06, 0.04, _flat_material(Color(0.25, 0.72, 0.64, 0.42), true, true, 0.08))
		rendered_object_proxy += 1
...
func _add_checkerboard_floor() -> void:
	for row in range(7):
		for column in range(9):
			...
			_add_box("ashenRaider_alpha_checker_%02d_%02d" % [row, column], Vector3(-3.2 + float(column) * 0.8, 0.055, -2.5 + float(row) * 0.72), Vector3(0.78, 0.045, 0.70), _flat_material(color))

func _add_hud_overlay(config: Dictionary) -> void:
	hud_layer = CanvasLayer.new()
	hud_layer.name = "Ashen RaiderDiagnosticOverlay%s" % str(config.get("checkpoint", CHECKPOINT)).to_upper().replace(".", "")
	add_child(hud_layer)
	...
	_add_label("%s Ashen Raider billboard slot / %s / Tier %s" % [str(config.get("checkpoint", CHECKPOINT)), config["approach"], config["tier"]], Vector2(38, 38), 18, Color(0.88, 0.92, 0.84))
	_add_label("Source: %s  Count: %d" % [config.get("assetSource", {}).get("sourceKind", "procedural"), int(config.get("ashenRaiderCount", 0))], Vector2(38, 70), 14, Color(0.64, 0.78, 0.70))
...
func _capture_review_views(start_index: int) -> Array[Dictionary]:
	var captures: Array[Dictionary] = []
	var views := [
		{"id": "v0157_restrained_source", "tier": "S", "scale": 1.0, "approach": APPROACH_FULL_RES},
		{"id": "v0157_checkerboard_alpha", "tier": "S", "scale": 1.0, "approach": APPROACH_FULL_RES},
		{"id": "v0157_dark_alpha_edge", "tier": "S", "scale": 1.0, "approach": APPROACH_FULL_RES},
		{"id": "v0157_light_alpha_edge", "tier": "S", "scale": 1.0, "approach": APPROACH_FULL_RES},
		{"id": "v0156_vs_v0157_archived_comparison", "tier": "S", "scale": 1.0, "approach": APPROACH_ARCHIVED_V0156},
		{"id": "v0157_fullres_normal", "tier": "S", "scale": 1.0, "approach": APPROACH_FULL_RES},
		{"id": "v0157_trimmed_512_normal", "tier": "S", "scale": 1.0, "approach": APPROACH_TRIMMED_512},
		{"id": "v0157_trimmed_768_normal", "tier": "S", "scale": 1.0, "approach": APPROACH_TRIMMED_768},
		{"id": "v0157_trimmed_1024_normal", "tier": "S", "scale": 1.0, "approach": APPROACH_TRIMMED_1024},
		{"id": "aster_ashenRaider_hierarchy", "tier": "S", "scale": 1.0, "approach": APPROACH_TRIMMED_1024},
		{"id": "worker_ashenRaider_distinction", "tier": "M", "scale": 1.0, "approach": APPROACH_TRIMMED_1024},
		{"id": "militia_ashen_raider_friendly_hostile_distinction", "tier": "M", "scale": 1.0, "approach": APPROACH_TRIMMED_1024},
		{"id": "four_ashen_raider_wave", "tier": "M", "scale": 1.0, "approach": APPROACH_TRIMMED_1024},
		{"id": "zoomed_group_readability", "tier": "L", "scale": 1.0, "approach": APPROACH_TRIMMED_1024},
		{"id": "overlap_sorting", "tier": "L", "scale": 1.0, "approach": APPROACH_TRIMMED_1024},
		{"id": "rings_visible", "tier": "S", "scale": 1.0, "approach": APPROACH_TRIMMED_1024},
		{"id": "pan_pivot_marker", "tier": "M", "scale": 1.0, "approach": APPROACH_TRIMMED_1024},
		{"id": "fallback_comparison", "tier": "S", "scale": 1.0, "approach": APPROACH_FALLBACK},
		{"id": "contact_sheet_candidate_context", "tier": "M", "scale": 1.0, "approach": APPROACH_TRIMMED_768}
	]
...
func _load_fallback_source() -> Dictionary:
	return _validated_source(
		...
		"tracked-ashen-raider-diagnostic-fallback",
		SLOT_ID,
		""
	)
...
func _load_archived_v0156_source() -> Dictionary:
	return _validated_source(
		_path_join(archived_slot_root, "%s_cutout.png" % SLOT_ID),
		_path_join(archived_slot_root, "%s_cutout.metadata.json" % SLOT_ID),
		"archived-v0156-ashen-raider-static-billboard",
		SLOT_ID,
		ARCHIVED_V0156_CUTOUT_HASH
	)

func _load_replacement_sources() -> Dictionary:
	return {
		"fullres": _validated_source(
			_path_join(local_slot_root, "%s_fullres.png" % REPLACEMENT_SOURCE_ID),
			_path_join(local_slot_root, "%s_fullres.metadata.json" % REPLACEMENT_SOURCE_ID),
			"v0157-restrained-ashen-raider-fullres",
			SLOT_ID,
			""
		),
		"trimmed_512": _validated_source(
			_path_join(local_slot_root, "%s_trimmed_512.png" % REPLACEMENT_SOURCE_ID),
			_path_join(local_slot_root, "%s_trimmed_512.metadata.json" % REPLACEMENT_SOURCE_ID),
			"v0157-restrained-ashen-raider-trimmed-512",
			SLOT_ID,
			""
		),
		"trimmed_768": _validated_source(
			_path_join(local_slot_root, "%s_trimmed_768.png" % REPLACEMENT_SOURCE_ID),
			_path_join(local_slot_root, "%s_trimmed_768.metadata.json" % REPLACEMENT_SOURCE_ID),
			"v0157-restrained-ashen-raider-trimmed-768",
			SLOT_ID,
			""
		),
		"trimmed_1024": _validated_source(
			_path_join(local_slot_root, "%s_trimmed_1024.png" % REPLACEMENT_SOURCE_ID),
			_path_join(local_slot_root, "%s_trimmed_1024.metadata.json" % REPLACEMENT_SOURCE_ID),
			"v0157-restrained-ashen-raider-trimmed-1024",
			SLOT_ID,
			""
		)
	}
...
func _load_militia_context_source() -> Dictionary:
	var root := ProjectSettings.globalize_path("res://../../artifacts/desktop-spikes/godot-salto/v0155/local-militia-billboard-repair")
	return _validated_source(
		_path_join(root, "%s_trimmed_1024.png" % MILITIA_SLOT_ID),
		_path_join(root, "%s_trimmed_1024.metadata.json" % MILITIA_SLOT_ID),
		"existing-militia-trimmed-1024-context",
		MILITIA_SLOT_ID,
		SELECTED_MILITIA_HASH
	)
...
func _fair_path_audit() -> Dictionary:
	return {
		...
		"selectedMilitiaContextLoadedOnce": militia_context_source.get("status", "FAIL") == "PASS",
		...
		"ashenRaiderBillboardNodeRebuildCount": ashenRaider_billboard_rebuild_count,
		...
		"militiaContextNodeRebuildCount": militia_context_rebuild_count,
...
func _readability_audit() -> Dictionary:
	return {
		...
		"ashenRaiderBelowAsterHierarchy": true,
		"ashenRaiderDistinctFromWorker": true,
		"ashenRaiderDistinctFromMilitia": true,
		"hostileFriendlyDistinction": true,
		"ordinaryWaveAttackerRead": true,
		"weaponSilhouetteRestrained": true,
		"bossLikePresentation": false,
		"groupsReadable": true,
		"staticFormationReadable": true,
		"ringsReadable": true,
...
func _artifact_root_from_args() -> String:
	...
	return ProjectSettings.globalize_path("res://../../artifacts/desktop-spikes/godot-salto/v0157/evidence")
...
func _is_script_arg(arg: String) -> bool:
	return arg == "--ashen-raider-visual-restraint-replacement" or arg == "--ashen-raider-visual-restraint-replacement-validate-only" or arg == "--ashen-raider-visual-restraint-replacement-benchmark-sequence" or arg == "--ashen-raider-visual-restraint-replacement-capture-only" or arg.begins_with("--artifact-root=") or arg.begins_with("--viewport=") or arg.begins_with("--resolution=")
