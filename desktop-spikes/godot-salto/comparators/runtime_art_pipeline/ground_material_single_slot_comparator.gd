...
const SLOT_ID := "barrosan_foothold_ground_material_v0175"
const APPROACH_FALLBACK := "GROUND_MATERIAL_DIAGNOSTIC_FALLBACK"
const APPROACH_LOCAL_512 := "GROUND_MATERIAL_LOCAL_512"
const APPROACH_LOCAL_768 := "GROUND_MATERIAL_LOCAL_768"
const APPROACH_LOCAL_1024 := "GROUND_MATERIAL_LOCAL_1024"
const APPROACH_WRAPSAFE_1024 := "GROUND_MATERIAL_1024_WRAPSAFE_OFFSET_BLEND"
const APPROACHES := [
	APPROACH_FALLBACK,
	APPROACH_LOCAL_512,
	APPROACH_LOCAL_768,
...
var total_ground_tile_rebuild_count := 0
...
func start() -> void:
	...
	local_slot_root = ProjectSettings.globalize_path("res://../../artifacts/desktop-spikes/godot-salto/v0175/local-ground-material-slot")
	material_sources = _load_material_sources()
	if args.has("--ground-material-validate-only"):
		var validation := _validation_report()
		_write_absolute_json(_path_join(artifact_root, "ground-material-validation-runtime.json"), validation)
		get_tree().quit(0 if validation.get("status", "FAIL") == "PASS_V0175_GROUND_MATERIAL_RUNTIME_VALIDATION" else 1)
		return
	var run_kind := "ground-material-headed-benchmark-and-capture"
	if args.has("--ground-material-capture-only"):
		run_kind = "ground-material-capture-refresh"
	var report := await _run_sequence(run_kind)
	_write_absolute_json(_path_join(artifact_root, "ground-material-runtime.json"), report)
	get_tree().quit(0 if report.get("status", "FAIL") == "PASS_V0175_GROUND_MATERIAL_RUNTIME_EVIDENCE" else 1)
...
func _validation_report() -> Dictionary:
	...
	for key in material_sources.keys():
		...
		if source.get("status", "FAIL") != "PASS":
			errors.append_array(source.get("errors", ["Unknown ground material source validation failure."]))
	return {
		"schemaVersion": 1,
		"checkpoint": CHECKPOINT,
		"slotId": SLOT_ID,
		"status": "PASS_V0175_GROUND_MATERIAL_RUNTIME_VALIDATION" if errors.is_empty() else "FAIL_V0175_GROUND_MATERIAL_RUNTIME_VALIDATION",
...
func _run_sequence(run_kind: String) -> Dictionary:
		...
		for trial_index in range(trials):
			...
			for approach in ordered_approaches:
				...
				if benchmark.get("status", "FAIL") != "PASS":
					errors.append("Ground material benchmark failed for %s %s trial %d." % [approach, tier, trial_index + 1])
				if trial_index == 0:
	...
	return {
		...
		"slotId": SLOT_ID,
		"runKind": run_kind,
		"status": "PASS_V0175_GROUND_MATERIAL_RUNTIME_EVIDENCE" if errors.is_empty() else "FAIL_V0175_GROUND_MATERIAL_RUNTIME_EVIDENCE",
...
func _benchmark_current_view(config: Dictionary) -> Dictionary:
	...
	return {
		...
		"navigationParity": "not-applicable-static-ground-material-comparator",
		"pressureParity": "not-applicable-static-ground-material-comparator",
		"stuckUnitCount": 0,
...
func _build_scene(config: Dictionary) -> void:
	...
	active_scene.name = "V0175GroundMaterialComparatorScene"
	add_child(active_scene)
	_setup_camera_and_lighting(config)
	var source: Dictionary = config.get("assetSource", {})
	var material := _ground_material_for_source(source)
...
func _setup_camera_and_lighting(config: Dictionary) -> void:
	...
	active_camera.name = "V0175GroundMaterialComparatorCamera"
...
func _add_base_slab(material: StandardMaterial3D) -> void:
	...
	node.name = "ground_material_base_repeat_slab"
	node.mesh = mesh
	node.position = Vector3(0, 0.0, 0)
	node.material_override = material
	active_scene.add_child(node)
	total_ground_tile_rebuild_count += 1
...
func _add_tiled_planes(count: int, area: Vector2, material: StandardMaterial3D, view: String) -> void:
	...
	for index in range(count):
		...
		mesh.size = area
		var node := MeshInstance3D.new()
		node.name = "ground_material_repeat_patch_%03d" % index
		node.mesh = mesh
...
func _add_context_roads_and_markers() -> void:
	_add_box("procedural_road_readability_context", Vector3(0, 0.045, -4.7), Vector3(22.0, 0.055, 0.32), _flat_material(Color(0.17, 0.15, 0.11, 0.86), true))
	_add_box("river_edge_readability_context", Vector3(-6.6, 0.05, 2.8), Vector3(0.34, 0.06, 9.4), _flat_material(Color(0.12, 0.24, 0.25, 0.55), true, true, 0.04))
	_add_box("bridge_deck_readability_context", Vector3(-6.6, 0.08, -0.5), Vector3(1.42, 0.10, 0.70), _flat_material(Color(0.25, 0.24, 0.20, 0.78), true))
	for i in range(4):
		_add_cylinder("site_claim_marker_context_%02d" % i, Vector3(4.8 - float(i % 2) * 1.8, 0.07, 2.4 - float(i / 2) * 1.7), 0.28, 0.026, _flat_material(Color(0.26, 0.72, 0.64, 0.40), true, true, 0.08))
...
func _texture_for_source(source: Dictionary) -> Texture2D:
	...
	if load_error != OK:
		push_error("Could not load ground material comparator texture %s" % path)
...
func _ground_material_for_source(source: Dictionary) -> StandardMaterial3D:
	var path := str(source.get("absolutePath", source.get("path", "")))
	var key := "%s|ground-material|%s" % [path, str(source.get("uvScale", 1.0))]
...
func _load_material_sources() -> Dictionary:
	return {
		"fallback": _validated_source(
			...
			"tracked-ground-material-diagnostic-fallback"
		),
		...
		"wrapsafe_1024": _validated_source(
			...
			"local-ground-material-1024-wrapsafe-offset-blend"
		)
	}
...
func _fair_path_audit() -> Dictionary:
	return {
		"schemaVersion": 1,
		"checkpoint": CHECKPOINT,
		"localAndFallbackShareGroundPlaneRenderPath": true,
		...
		"groundTileNodeRebuildCount": total_ground_tile_rebuild_count,
...
func _artifact_root_from_args() -> String:
	...
	return ProjectSettings.globalize_path("res://../../artifacts/desktop-spikes/godot-salto/v0175/evidence")
...
func _is_script_arg(arg: String) -> bool:
	return arg == "--barrosan-ground-material-single-slot" or arg == "--ground-material-validate-only" or arg == "--ground-material-benchmark-sequence" or arg == "--ground-material-capture-only" or arg.begins_with("--artifact-root=") or arg.begins_with("--viewport=") or arg.begins_with("--resolution=")
