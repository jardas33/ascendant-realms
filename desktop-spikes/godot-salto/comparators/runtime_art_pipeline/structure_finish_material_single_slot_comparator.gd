...
const SLOT_ID := "barrosan_structure_finish_material_v0202"
const APPROACH_FALLBACK := "STRUCTURE_FINISH_MATERIAL_DIAGNOSTIC_FALLBACK"
const APPROACH_LOCAL_512 := "STRUCTURE_FINISH_MATERIAL_LOCAL_512"
const APPROACH_LOCAL_768 := "STRUCTURE_FINISH_MATERIAL_LOCAL_768"
const APPROACH_LOCAL_1024 := "STRUCTURE_FINISH_MATERIAL_LOCAL_1024"
const APPROACH_WRAPSAFE_1024 := "STRUCTURE_FINISH_MATERIAL_1024_WRAPSAFE_OFFSET_BLEND"
const APPROACHES := [
	APPROACH_FALLBACK,
	APPROACH_LOCAL_512,
	APPROACH_LOCAL_768,
...
var total_structure_finish_tile_rebuild_count := 0
...
func start() -> void:
	...
	local_slot_root = ProjectSettings.globalize_path("res://../../artifacts/desktop-spikes/godot-salto/v0202/local-structure-finish-material-slot")
	material_sources = _load_material_sources()
	if args.has("--structure-finish-material-validate-only"):
		var validation := _validation_report()
		_write_absolute_json(_path_join(artifact_root, "structure-finish-material-validation-runtime.json"), validation)
		get_tree().quit(0 if validation.get("status", "FAIL") == "PASS_V0202_STRUCTURE_FINISH_MATERIAL_RUNTIME_VALIDATION" else 1)
		return
	var run_kind := "structure-finish-material-headed-benchmark-and-capture"
	if args.has("--structure-finish-material-capture-only"):
		run_kind = "structure-finish-material-capture-refresh"
	var report := await _run_sequence(run_kind)
	_write_absolute_json(_path_join(artifact_root, "structure-finish-material-runtime.json"), report)
	get_tree().quit(0 if report.get("status", "FAIL") == "PASS_V0202_STRUCTURE_FINISH_MATERIAL_RUNTIME_EVIDENCE" else 1)
...
func _validation_report() -> Dictionary:
	...
	for key in material_sources.keys():
		...
		if source.get("status", "FAIL") != "PASS":
			errors.append_array(source.get("errors", ["Unknown Structure-finish material source validation failure."]))
	return {
		"schemaVersion": 1,
		"checkpoint": CHECKPOINT,
		"slotId": SLOT_ID,
		"status": "PASS_V0202_STRUCTURE_FINISH_MATERIAL_RUNTIME_VALIDATION" if errors.is_empty() else "FAIL_V0202_STRUCTURE_FINISH_MATERIAL_RUNTIME_VALIDATION",
		...
		"structureFinishMaterialImportedToPlayerSlice": false,
...
func _run_sequence(run_kind: String) -> Dictionary:
		...
		for trial_index in range(trials):
			...
			for approach in ordered_approaches:
				...
				if benchmark.get("status", "FAIL") != "PASS":
					errors.append("Structure-finish material benchmark failed for %s %s trial %d." % [approach, tier, trial_index + 1])
				if trial_index == 0:
	...
	return {
		...
		"slotId": SLOT_ID,
		"runKind": run_kind,
		"status": "PASS_V0202_STRUCTURE_FINISH_MATERIAL_RUNTIME_EVIDENCE" if errors.is_empty() else "FAIL_V0202_STRUCTURE_FINISH_MATERIAL_RUNTIME_EVIDENCE",
...
func _benchmark_current_view(config: Dictionary) -> Dictionary:
	...
	return {
		...
		"navigationParity": "not-applicable-static-structure-finish-material-comparator",
		"pressureParity": "not-applicable-static-structure-finish-material-comparator",
		"stuckUnitCount": 0,
...
func _build_scene(config: Dictionary) -> void:
	...
	active_scene.name = "V0202StructureFinishMaterialComparatorScene"
	add_child(active_scene)
	_setup_camera_and_lighting(config)
	var source: Dictionary = config.get("assetSource", {})
	var material := _structure_finish_material_for_source(source)
	...
	_add_context_structure_finish_markers(material)
...
func _setup_camera_and_lighting(config: Dictionary) -> void:
	...
	active_camera.name = "V0202StructureFinishMaterialComparatorCamera"
...
func _add_base_slab(material: StandardMaterial3D) -> void:
	...
	node.name = "structure_finish_material_base_repeat_slab"
	node.mesh = mesh
	node.position = Vector3(0, 0.0, 0)
	node.material_override = material
	active_scene.add_child(node)
	total_structure_finish_tile_rebuild_count += 1
...
func _add_tiled_planes(count: int, area: Vector2, material: StandardMaterial3D, view: String) -> void:
	...
	for index in range(count):
		...
		mesh.size = area
		var node := MeshInstance3D.new()
		node.name = "structure_finish_material_repeat_patch_%03d" % index
		node.mesh = mesh
...
func _add_context_structure_finish_markers(material: StandardMaterial3D) -> void:
	...
	_add_box("structure_wall_west_finish_context", Vector3(-6.6, 0.88, 0.55), Vector3(0.46, 1.62, 4.5), material)
	_add_box("structure_wall_north_finish_context", Vector3(-5.05, 0.74, -1.95), Vector3(3.1, 1.34, 0.50), material)
	_add_box("structure_foundation_finish_context", Vector3(-4.75, 0.18, 1.78), Vector3(3.7, 0.30, 1.38), material)
	_add_box("structure_scaffold_upright_context_a", Vector3(-7.25, 0.92, -1.18), Vector3(0.20, 1.78, 0.20), material)
	_add_box("structure_scaffold_upright_context_b", Vector3(-3.52, 0.92, -1.18), Vector3(0.20, 1.78, 0.20), material)
	_add_box("structure_scaffold_crossbeam_context", Vector3(-5.38, 1.55, -1.18), Vector3(3.84, 0.22, 0.18), material)
	_add_box("ground_comparison_plain_dirt", Vector3(-3.75, 0.067, 3.0), Vector3(2.1, 0.055, 1.3), _flat_material(Color(0.19, 0.17, 0.115, 0.76), true))
	_add_box("road_comparison_plain_gravel", Vector3(-1.15, 0.078, 3.0), Vector3(2.1, 0.055, 1.3), _flat_material(Color(0.23, 0.21, 0.16, 0.82), true))
	_add_box("riverbank_comparison_plain_stone", Vector3(1.45, 0.087, 3.0), Vector3(2.1, 0.055, 1.3), _flat_material(Color(0.22, 0.23, 0.22, 0.82), true))
	for i in range(4):
...
func _texture_for_source(source: Dictionary) -> Texture2D:
	...
	if load_error != OK:
		push_error("Could not load Structure-finish material comparator texture %s" % path)
...
func _structure_finish_material_for_source(source: Dictionary) -> StandardMaterial3D:
	var path := str(source.get("absolutePath", source.get("path", "")))
	var key := "%s|structure-finish-material|%s" % [path, str(source.get("uvScale", 1.0))]
...
func _load_material_sources() -> Dictionary:
	return {
		"fallback": _validated_source(
			...
			"tracked-structure-finish-material-diagnostic-fallback"
		),
		"local_512": _validated_source(
			...
			"local-structure-finish-material-512"
		),
		"local_768": _validated_source(
			...
			"local-structure-finish-material-768"
		),
		"local_1024": _validated_source(
			...
			"local-structure-finish-material-1024"
		),
		"wrapsafe_1024": _validated_source(
			...
			"local-structure-finish-material-1024-wrapsafe-offset-blend"
		)
	}
...
func _fair_path_audit() -> Dictionary:
	return {
		"schemaVersion": 1,
		"checkpoint": CHECKPOINT,
		"localAndFallbackShareStructureFinishPlaneRenderPath": true,
		...
		"structureFinishTileNodeRebuildCount": total_structure_finish_tile_rebuild_count,
...
func _artifact_root_from_args() -> String:
	...
	return ProjectSettings.globalize_path("res://../../artifacts/desktop-spikes/godot-salto/v0202/evidence")
...
func _is_script_arg(arg: String) -> bool:
	return arg == "--barrosan-structure-finish-material-single-slot" or arg == "--structure-finish-material-validate-only" or arg == "--structure-finish-material-benchmark-sequence" or arg == "--structure-finish-material-capture-only" or arg.begins_with("--artifact-root=") or arg.begins_with("--viewport=") or arg.begins_with("--resolution=")
