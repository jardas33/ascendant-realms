...
func _live_ui_shell_state() -> Dictionary:
	...
	if action.begins_with("v0280_"):
		...
		var v0280_cleared := action == "v0280_clear_guard_removes_commit_label"
		var v0280_reguard := action == "v0280_reguard_availability_clean"
		var v0280_no_commit_entity := action in ["v0280_watchpost_no_engage_commit_action", "v0280_barracks_no_engage_commit_action", "v0280_default_runtime_unchanged_probe"]
		active_tab = "TRAIN"
		selected_title = "Militia Defender"
		selected_subtitle = "Engage armed | East bridge | Pressure contained 90/100"
		command_hint = "Commit Engage  Clear Guard  Hold"
		status_pip = "ARMED"
		...
		if v0280_committed:
			selected_subtitle = "Engagement committed | Bridge held | Pressure checked 80/100"
			command_hint = "Commit locked  Clear Guard  Hold"
			...
			selected_subtitle = "Guard cleared | Commit label removed | Pressure remains 80/100"
			command_hint = "Guard cleared  Reguard available"
			status_pip = "CLEAR"
			tooltip = "Clear Guard removes the commit label and does not restore pressure."
			objective = "Guard cleared"
			objective_detail = "Commit label removed.\nPressure remains 80/100."
			...
			selected_subtitle = "Reguard clean | Commit result locked | Pressure remains 80/100"
			command_hint = "Reguard clean  Commit locked"
			status_pip = "LOCKED"
			...
			selected_title = "No Commit Target"
			selected_subtitle = "No Engage/Commit action | Default-safe probe"
			command_hint = "No Commit"
			status_pip = "SAFE"
			tooltip = "Watchpost, Barracks and default probes do not receive Engage or Commit actions."
			objective = "No Commit action"
			objective_detail = "Entity is not Militia.\nDefault runtime remains unchanged."
			objective_next_action = "Only Militia at east bridge can commit."
			alert_text = "No commit action"
			alert_severity = "info"
			events = ["No Engage action", "No Commit action", "Default safe"]
		...
		selection_panel_data["commands"] = [
			_v0210_command_spec("Commit locked" if (v0280_committed or v0280_reguard) else ("No Commit" if v0280_no_commit_entity else "Commit Engage"), "C", tooltip, "disabled" if (v0280_committed or v0280_reguard or v0280_no_commit_entity) else "ready", "_on_live_ui_shell_attack_pressed"),
...
func _apply_player_slice_action(action: String) -> Dictionary:
	...
	if action == "production_build":
		...
		player_ui_shell_production_tab_override = "TRAIN"
		player_ui_shell_production_focus_hint = "train_militia"
	if action.begins_with("v0271_") or action.begins_with("v0272_") or action.begins_with("v0273_") or action.begins_with("v0274_") or action.begins_with("v0275_") or action.begins_with("v0276_") or action.begins_with("v0277_") or action.begins_with("v0278_") or action.begins_with("v0279_") or action.begins_with("v0280_"):
