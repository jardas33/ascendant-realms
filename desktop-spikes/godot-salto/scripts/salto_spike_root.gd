...
func _ready() -> void:
	...
	if args.has("--salto-barrosan-material-richness"):
		...
		return
	if args.has("--salto-barrosan-building-roster"):
		var roster_scene := load("res://scenes/salto_barrosan_building_roster.tscn") as PackedScene
		if roster_scene == null:
			get_tree().quit(1)
			return
		var roster_slice := roster_scene.instantiate()
		add_child(roster_slice)
		roster_slice.call_deferred("start")
		return
	if args.has("--salto-barrosan-roster-silhouette-beauty"):
		var silhouette_scene := load("res://scenes/salto_barrosan_roster_silhouette_beauty.tscn") as PackedScene
		if silhouette_scene == null:
			get_tree().quit(1)
			return
...
func load_mode(mode: String) -> void:
	...
	if mode == MODE_25D:
		scene_path = "res://scenes/salto_barrosan_playable_runtime_skin.tscn" if _player_capture_checkpoint() in ["v0.254", "v0.255", "v0.256", "v0.257", "v0.258", "v0.259", "v0.261"] or _barrosan_runtime_scene_requested() else "res://scenes/salto_2_5d_orthographic_placeholder.tscn"
	if scene_path == "res://scenes/salto_barrosan_playable_runtime_skin.tscn":
		...
		active_scene.name = "SaltoBarrosanPlayableRuntimeSkin"
	else:
...
func _ensure_player_battle_scene() -> void:
	if _player_capture_checkpoint() in ["v0.254", "v0.255", "v0.256", "v0.257", "v0.258", "v0.259", "v0.261"] and (
		active_scene == null
	...
	if active_scene == null or not is_instance_valid(active_scene) or active_mode != MODE_25D:
		show_player_battle()
	elif (_player_capture_checkpoint() in ["v0.254", "v0.255", "v0.256", "v0.257", "v0.258", "v0.259", "v0.261"] or _barrosan_runtime_scene_requested()) and not active_scene.has_method("configure_barrosan_playable_runtime_skin"):
		load_mode(MODE_25D)
		_call_scene("set_player_facing_mode", [true])
	else:
		_call_scene("set_player_facing_mode", [true])
...
func _load_v0254_capture_scene() -> void:
	...
	if home_screen:
		home_screen.visible = false
	_update_review_overlay("Launched %s Barrosan runtime capture" % _player_capture_checkpoint())
...
func _live_ui_shell_state() -> Dictionary:
	...
	if action.begins_with("v0280_") or action.begins_with("v0281_") or action.begins_with("v0283_") or action.begins_with("v0284_") or action.begins_with("v0285_"):
		var v0281 := action.begins_with("v0281_") or action.begins_with("v0283_") or action.begins_with("v0284_") or action.begins_with("v0285_")
		...
		var v0285_held := action in ["v0285_hold_line_clicked", "v0285_line_held_exactly_once", "v0285_ashen_contained_exactly_once", "v0285_combined_line_held_ashen_contained_readable_hud", "v0285_repeat_hold_line_no_duplicate_no_stack", "v0285_no_projectile_damage_death_despawn"]
		var v0280_cleared := action in ["v0280_clear_guard_removes_commit_label", "v0281_clear_guard_removes_commit_label_real_hud_clean", "v0283_clear_guard_settles_ashen_response", "v0284_clear_guard_settles_ashen_response", "v0285_clear_guard_settles_hold_line"]
		var v0280_reguard := action in ["v0280_reguard_availability_clean", "v0281_reguard_availability_clean_real_hud", "v0283_reguard_availability_clean", "v0284_reguard_availability_clean", "v0285_reguard_availability_clean_after_hold_line"]
		var v0280_no_commit_entity := action in ["v0280_watchpost_no_engage_commit_action", "v0280_barracks_no_engage_commit_action", "v0280_default_runtime_unchanged_probe", "v0281_watchpost_no_engage_commit_action_real_hud", "v0281_barracks_no_engage_commit_action_real_hud", "v0281_default_runtime_unchanged_probe", "v0283_watchpost_no_engage_commit_ashen_braced", "v0283_barracks_no_engage_commit_ashen_braced", "v0284_watchpost_no_engage_commit_ashen_braced", "v0284_barracks_no_engage_commit_ashen_braced", "v0285_watchpost_no_hold_line_engage_commit_ashen", "v0285_barracks_no_hold_line_engage_commit_ashen"]
		active_tab = "TRAIN"
		...
		if v0285_held:
			...
			selected_subtitle = "Engagement committed | Bridge held | Pressure checked 80/100 | Ashen braced | Commit locked" if (action.begins_with("v0283_") or action.begins_with("v0284_") or action.begins_with("v0285_")) else "Engagement committed | Bridge held | Pressure checked 80/100 | Commit locked"
			command_hint = "Hold Line ready  No projectile  No unit damage" if action == "v0285_hold_line_available_after_commit_locked" else ("No projectile  No unit damage  No enemy death/despawn  No repeat pressure effect" if v0281 else "Commit locked  Clear Guard  Hold")
			...
			objective_next_action = "Use Hold Line to contain Ashen pressure." if action == "v0285_hold_line_available_after_commit_locked" else "Commit locked; keep holding the bridge."
			alert_text = "Pressure checked 80/100"
			alert_severity = "info"
			events = ["Engagement committed", "Pressure checked 80/100", "Ashen braced", "Hold Line ready"] if action == "v0285_hold_line_available_after_commit_locked" else (["Engagement committed", "Pressure checked 80/100", "Ashen braced", "Commit locked"] if (action.begins_with("v0283_") or action.begins_with("v0284_") or action.begins_with("v0285_")) else ["Engagement committed", "Bridge held", "Pressure checked 80/100", "Commit locked"])
		elif v0280_cleared:
			selected_title = "Militia Defender | East bridge" if v0281 else selected_title
			...
			var is_watchpost_no_commit := action in ["v0281_watchpost_no_engage_commit_action_real_hud", "v0283_watchpost_no_engage_commit_ashen_braced", "v0284_watchpost_no_engage_commit_ashen_braced", "v0285_watchpost_no_hold_line_engage_commit_ashen"]
		...
		selection_panel_data["commands"] = [
			_v0210_command_spec("Held" if v0285_held else ("Hold Line" if action == "v0285_hold_line_available_after_commit_locked" else ("Commit locked" if (v0280_committed or v0280_reguard) else (("Observe" if action in ["v0281_watchpost_no_engage_commit_action_real_hud", "v0283_watchpost_no_engage_commit_ashen_braced", "v0284_watchpost_no_engage_commit_ashen_braced", "v0285_watchpost_no_hold_line_engage_commit_ashen"] else ("Train" if action in ["v0281_barracks_no_engage_commit_action_real_hud", "v0283_barracks_no_engage_commit_ashen_braced", "v0284_barracks_no_engage_commit_ashen_braced", "v0285_barracks_no_hold_line_engage_commit_ashen"] else ("Ready" if action == "v0281_default_runtime_unchanged_probe" else "No Commit"))) if v0280_no_commit_entity else "Commit Engage"))), "C", tooltip, "disabled" if (v0285_held or (v0280_committed and action != "v0285_hold_line_available_after_commit_locked") or v0280_reguard or v0280_no_commit_entity) else "ready", "_on_live_ui_shell_attack_pressed"),
...
func _apply_player_slice_action(action: String) -> Dictionary:
	...
	match action:
		...
		"v0245_overview":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_overview"])
			_render_player_screen("battle")
		"v0245_starting_resources":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_starting_resources"])
			_render_player_screen("battle")
		"v0245_select_builder":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_select_builder"])
			_render_player_screen("battle")
		"v0245_valid_preview_cancel":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_valid_preview_cancel"])
			_render_player_screen("battle")
		"v0245_cancel_preview":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_cancel_preview"])
			_render_player_screen("battle")
		"v0245_blocked_preview":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_blocked_preview"])
			_render_player_screen("battle")
		"v0245_blocked_attempt":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_blocked_attempt"])
			_render_player_screen("battle")
		"v0245_valid_preview_confirm":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_valid_preview_confirm"])
			_render_player_screen("battle")
		"v0245_confirm_placement":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_confirm_placement"])
			_render_player_screen("battle")
		"v0245_resource_delta":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_resource_delta"])
			_render_player_screen("battle")
		"v0245_select_constructed":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_select_constructed"])
			_render_player_screen("battle")
		"v0245_minimap":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_minimap"])
			_render_player_screen("battle")
		"v0245_preserve_keep":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_preserve_keep"])
			_render_player_screen("battle")
		"v0245_preserve_barracks":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_preserve_barracks"])
			_render_player_screen("battle")
		"v0245_preserve_mine":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_preserve_mine"])
			_render_player_screen("battle")
		"v0245_preserve_shells":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_preserve_shells"])
			_render_player_screen("battle")
		"v0245_pathing":
			_ensure_player_battle_scene()
			_call_scene("set_barrosan_runtime_review_mode", ["v0245_pathing"])
			_render_player_screen("battle")
		"v0245_clean":
			_ensure_player_battle_scene()
...
func _player_capture_checkpoint() -> String:
	...
	if normalized_root.contains("/v0267"):
		return "v0.267"
	if normalized_root.contains("/v0266"):
		return "v0.266"
	if normalized_root.contains("/v0265"):
		return "v0.265"
	if normalized_root.contains("/v0264"):
		return "v0.264"
	if normalized_root.contains("/v0263"):
		return "v0.263"
...
func _player_capture_steps() -> Array[Dictionary]:
	...
	if _player_capture_checkpoint() == "v0.271":
		return [
			...
			{"id":"v0271_watchpost_complete_no_intel_no_contact_visible","label":"v0.271 no intel no contact", "action":"v0271_watchpost_complete_no_intel_no_contact"},
			{"id":"v0271_barracks_train_militia_visible","label":"v0.271 Barracks trains Militia", "action":"v0271_barracks_train_militia"},
			{"id":"v0271_militia_training_guard_unavailable_visible","label":"v0.271 guard unavailable while training", "action":"v0271_militia_training_guard_unavailable"},
			{"id":"v0271_militia_ready_guard_available_visible","label":"v0.271 guard available", "action":"v0271_militia_ready_guard_available"},
			{"id":"v0271_guard_button_visible_on_militia_not_watchpost_visible","label":"v0.271 guard button Militia only", "action":"v0271_guard_button_visible_on_militia_not_watchpost"},
			{"id":"v0271_guard_order_pending_militia_away_visible","label":"v0.271 guard pending away", "action":"v0271_guard_order_pending_militia_away"},
			{"id":"v0271_no_auto_move_after_guard_order_visible","label":"v0.271 no auto move", "action":"v0271_no_auto_move_after_guard_order"},
			{"id":"v0271_watchpost_advises_move_to_bridge_visible","label":"v0.271 Watchpost advises move", "action":"v0271_watchpost_advises_move_to_bridge"},
			{"id":"v0271_militia_near_bridge_guard_confirmed_visible","label":"v0.271 guard confirmed", "action":"v0271_militia_near_bridge_guard_confirmed"},
			{"id":"v0271_guard_marker_east_bridge_visible","label":"v0.271 guard marker", "action":"v0271_guard_marker_east_bridge"},
			{"id":"v0271_defender_position_holding_from_guard_order_visible","label":"v0.271 holding from guard", "action":"v0271_defender_position_holding_from_guard_order"},
			{"id":"v0271_intercept_preview_requires_guard_order_visible","label":"v0.271 intercept requires guard", "action":"v0271_intercept_preview_requires_guard_order"},
			{"id":"v0271_current_detection_no_guard_no_contact_visible","label":"v0.271 no guard no contact", "action":"v0271_current_detection_no_guard_no_contact"},
			{"id":"v0271_current_detection_guard_pending_no_contact_visible","label":"v0.271 pending no contact", "action":"v0271_current_detection_guard_pending_no_contact"},
			{"id":"v0271_current_detection_guard_holding_contact_armed_visible","label":"v0.271 holding armed", "action":"v0271_current_detection_guard_holding_contact_armed"},
			{"id":"v0271_intercept_ready_integrity_100_visible","label":"v0.271 intercept ready 100", "action":"v0271_intercept_ready_integrity_100"},
			{"id":"v0271_first_contact_feedback_pulse_visible","label":"v0.271 first contact pulse", "action":"v0271_first_contact_feedback_pulse"},
			{"id":"v0271_first_contact_integrity_90_visible","label":"v0.271 first contact 90", "action":"v0271_first_contact_integrity_90"},
			{"id":"v0271_contact_resolved_cooldown_locked_visible","label":"v0.271 resolved cooldown", "action":"v0271_contact_resolved_cooldown_locked"},
			{"id":"v0271_no_repeated_damage_below_90_visible","label":"v0.271 no repeat damage", "action":"v0271_no_repeated_damage_below_90"},
			{"id":"v0271_overlap_continues_integrity_still_90_visible","label":"v0.271 overlap still 90", "action":"v0271_overlap_continues_integrity_still_90"},
			{"id":"v0271_threat_leaves_memory_integrity_90_visible","label":"v0.271 memory after contact", "action":"v0271_threat_leaves_memory_integrity_90"},
			{"id":"v0271_memory_only_no_new_contact_damage_visible","label":"v0.271 memory no new contact", "action":"v0271_memory_only_no_new_contact_damage"},
			{"id":"v0271_no_enemy_death_or_despawn_visible","label":"v0.271 no death/despawn", "action":"v0271_no_enemy_death_or_despawn"},
			{"id":"v0271_no_enemy_slow_stop_redirect_visible","label":"v0.271 no slow/stop/redirect", "action":"v0271_no_enemy_slow_stop_redirect"},
			{"id":"v0271_no_militia_hp_loss_visible","label":"v0.271 no Militia HP loss", "action":"v0271_no_militia_hp_loss"},
			{"id":"v0271_no_watchpost_hp_loss_visible","label":"v0.271 no Watchpost HP loss", "action":"v0271_no_watchpost_hp_loss"},
			{"id":"v0271_no_watchpost_attack_projectile_tower_visible","label":"v0.271 no Watchpost attack", "action":"v0271_no_watchpost_attack_projectile_tower"},
			{"id":"v0271_watchpost_hud_advisory_only_no_train_no_guard_action_visible","label":"v0.271 Watchpost advisory only", "action":"v0271_watchpost_hud_advisory_only_no_train_no_guard_action"},
			{"id":"v0271_barracks_hud_train_militia_no_full_relay_visible","label":"v0.271 Barracks separated", "action":"v0271_barracks_hud_train_militia_no_full_relay"},
			{"id":"v0271_militia_hud_guard_order_contact_resolved_visible","label":"v0.271 Militia HUD resolved", "action":"v0271_militia_hud_guard_order_contact_resolved"},
			{"id":"v0271_minimap_guard_pending_indicator_visible","label":"v0.271 minimap guard pending", "action":"v0271_minimap_guard_pending_indicator"},
			{"id":"v0271_minimap_guard_holding_indicator_visible","label":"v0.271 minimap guard holding", "action":"v0271_minimap_guard_holding_indicator"},
			{"id":"v0271_minimap_contact_ping_current_only_visible","label":"v0.271 contact ping current only", "action":"v0271_minimap_contact_ping_current_only"},
			{"id":"v0271_minimap_memory_no_contact_ping_visible","label":"v0.271 memory no contact ping", "action":"v0271_minimap_memory_no_contact_ping"},
			{"id":"v0271_label_declutter_guard_pending_visible","label":"v0.271 declutter pending", "action":"v0271_label_declutter_guard_pending"},
			{"id":"v0271_label_declutter_guard_holding_visible","label":"v0.271 declutter holding", "action":"v0271_label_declutter_guard_holding"},
			{"id":"v0271_label_declutter_first_contact_visible","label":"v0.271 declutter contact", "action":"v0271_label_declutter_first_contact"},
			{"id":"v0271_existing_barracks_rebuild_path_still_valid_visible","label":"v0.271 Barracks rebuild retained", "action":"v0271_existing_barracks_rebuild_path_still_valid"},
			{"id":"v0271_existing_barracks_still_trains_militia_visible","label":"v0.271 Barracks trains retained", "action":"v0271_existing_barracks_still_trains_militia"},
		]
	if _player_capture_checkpoint() == "v0.270":
		return [
			{"id":"v0270_watchpost_build_path_visible","label":"v0.270 Watchpost build path", "action":"v0270_watchpost_build_path"},
			{"id":"v0270_watchpost_complete_no_intel_no_contact_visible","label":"v0.270 no intel no contact", "action":"v0270_watchpost_complete_no_intel_no_contact"},
			{"id":"v0270_ashen_outside_zone_no_contact_visible","label":"v0.270 outside no contact", "action":"v0270_ashen_outside_zone_no_contact"},
			{"id":"v0270_current_detection_no_militia_contact_unavailable_visible","label":"v0.270 no Militia unavailable", "action":"v0270_current_detection_no_militia_contact_unavailable"},
			{"id":"v0270_militia_training_contact_pending_visible","label":"v0.270 training pending", "action":"v0270_militia_training_contact_pending"},
			{"id":"v0270_militia_ready_away_contact_unavailable_visible","label":"v0.270 ready away unavailable", "action":"v0270_militia_ready_away_contact_unavailable"},
			{"id":"v0270_militia_holding_guarding_lane_integrity_100_visible","label":"v0.270 guarding lane integrity 100", "action":"v0270_militia_holding_guarding_lane_integrity_100"},
			{"id":"v0270_intercept_ready_integrity_100_visible","label":"v0.270 intercept ready integrity 100", "action":"v0270_intercept_ready_integrity_100"},
			{"id":"v0270_contact_armed_no_damage_yet_visible","label":"v0.270 armed no damage", "action":"v0270_contact_armed_no_damage_yet"},
			{"id":"v0270_first_contact_feedback_pulse_visible","label":"v0.270 first contact pulse", "action":"v0270_first_contact_feedback_pulse"},
			{"id":"v0270_first_contact_integrity_90_visible","label":"v0.270 first contact integrity 90", "action":"v0270_first_contact_integrity_90"},
			{"id":"v0270_contact_resolved_cooldown_locked_visible","label":"v0.270 resolved cooldown locked", "action":"v0270_contact_resolved_cooldown_locked"},
			{"id":"v0270_no_repeated_damage_below_90_visible","label":"v0.270 no repeated damage", "action":"v0270_no_repeated_damage_below_90"},
			{"id":"v0270_overlap_continues_integrity_still_90_visible","label":"v0.270 overlap still 90", "action":"v0270_overlap_continues_integrity_still_90"},
			{"id":"v0270_no_enemy_death_or_despawn_visible","label":"v0.270 no death/despawn", "action":"v0270_no_enemy_death_or_despawn"},
			{"id":"v0270_no_enemy_slow_stop_redirect_visible","label":"v0.270 no slow stop redirect", "action":"v0270_no_enemy_slow_stop_redirect"},
			{"id":"v0270_no_militia_hp_loss_visible","label":"v0.270 no Militia HP loss", "action":"v0270_no_militia_hp_loss"},
			{"id":"v0270_no_watchpost_hp_loss_visible","label":"v0.270 no Watchpost HP loss", "action":"v0270_no_watchpost_hp_loss"},
			{"id":"v0270_no_watchpost_attack_damage_projectile_visible","label":"v0.270 no Watchpost attack projectile", "action":"v0270_no_watchpost_attack_damage_projectile"},
			{"id":"v0270_no_tower_behavior_visible","label":"v0.270 no tower behavior", "action":"v0270_no_tower_behavior"},
			{"id":"v0270_threat_leaves_contact_ended_memory_visible","label":"v0.270 contact ended memory", "action":"v0270_threat_leaves_contact_ended_memory"},
			{"id":"v0270_memory_only_no_new_contact_damage_visible","label":"v0.270 memory only no damage", "action":"v0270_memory_only_no_new_contact_damage"},
			{"id":"v0270_last_seen_integrity_90_visible","label":"v0.270 last seen integrity 90", "action":"v0270_last_seen_integrity_90"},
			{"id":"v0270_minimap_contact_ping_current_only_visible","label":"v0.270 minimap contact current only", "action":"v0270_minimap_contact_ping_current_only"},
			{"id":"v0270_minimap_memory_no_contact_ping_visible","label":"v0.270 minimap memory no contact", "action":"v0270_minimap_memory_no_contact_ping"},
			{"id":"v0270_watchpost_hud_contact_cooldown_no_train_militia_visible","label":"v0.270 Watchpost HUD cooldown", "action":"v0270_watchpost_hud_contact_cooldown_no_train_militia"},
			{"id":"v0270_barracks_hud_train_militia_no_full_relay_visible","label":"v0.270 Barracks separated", "action":"v0270_barracks_hud_train_militia_no_full_relay"},
			{"id":"v0270_militia_selected_contact_resolved_visible","label":"v0.270 Militia selected resolved", "action":"v0270_militia_selected_contact_resolved"},
			{"id":"v0270_label_declutter_pre_contact_visible","label":"v0.270 label declutter pre-contact", "action":"v0270_label_declutter_pre_contact"},
			{"id":"v0270_label_declutter_first_contact_visible","label":"v0.270 label declutter first contact", "action":"v0270_label_declutter_first_contact"},
			{"id":"v0270_label_declutter_contact_resolved_visible","label":"v0.270 label declutter resolved", "action":"v0270_label_declutter_contact_resolved"},
			{"id":"v0270_existing_barracks_rebuild_path_still_valid_visible","label":"v0.270 Barracks rebuild retained", "action":"v0270_existing_barracks_rebuild_path_still_valid"},
			{"id":"v0270_existing_barracks_still_trains_militia_visible","label":"v0.270 Barracks trains Militia", "action":"v0270_existing_barracks_still_trains_militia"},
		]
	...
	if _player_capture_checkpoint() == "v0.268":
		return [
			...
			{"id":"v0268_watchpost_complete_no_intel_no_intercept_preview_visible","label":"v0.268 no intel no intercept", "action":"v0268_watchpost_complete_no_intel_no_intercept_preview"},
			{"id":"v0268_ashen_outside_zone_no_false_intercept_visible","label":"v0.268 outside no false intercept", "action":"v0268_ashen_outside_zone_no_false_intercept"},
			{"id":"v0268_current_detection_no_militia_intercept_unavailable_visible","label":"v0.268 no Militia unavailable", "action":"v0268_current_detection_no_militia_intercept_unavailable"},
			{"id":"v0268_current_detection_relay_intercept_unavailable_visible","label":"v0.268 relay unavailable", "action":"v0268_current_detection_relay_intercept_unavailable"},
			{"id":"v0268_barracks_train_militia_advisory_still_visible","label":"v0.268 Barracks advisory retained", "action":"v0268_barracks_train_militia_advisory_still"},
			{"id":"v0268_militia_training_intercept_pending_visible","label":"v0.268 training pending", "action":"v0268_militia_training_intercept_pending"},
			{"id":"v0268_current_detection_relay_intercept_pending_visible","label":"v0.268 relay pending", "action":"v0268_current_detection_relay_intercept_pending"},
			{"id":"v0268_militia_ready_away_intercept_cannot_visible","label":"v0.268 ready away cannot", "action":"v0268_militia_ready_away_intercept_cannot"},
			{"id":"v0268_current_detection_relay_cannot_intercept_visible","label":"v0.268 relay cannot", "action":"v0268_current_detection_relay_cannot_intercept"},
			{"id":"v0268_objective_move_defender_to_east_bridge_visible","label":"v0.268 move defender objective", "action":"v0268_objective_move_defender_to_east_bridge"},
			{"id":"v0268_militia_holding_bridge_guarding_lane_visible","label":"v0.268 holding guarding lane", "action":"v0268_militia_holding_bridge_guarding_lane"},
			{"id":"v0268_current_detection_relay_guarding_lane_visible","label":"v0.268 relay guarding lane", "action":"v0268_current_detection_relay_guarding_lane"},
			{"id":"v0268_ashen_inside_intercept_envelope_visible","label":"v0.268 Ashen inside envelope", "action":"v0268_ashen_inside_intercept_envelope"},
			{"id":"v0268_current_detection_relay_intercept_ready_visible","label":"v0.268 relay intercept ready", "action":"v0268_current_detection_relay_intercept_ready"},
			{"id":"v0268_objective_intercept_ready_hold_bridge_visible","label":"v0.268 objective hold bridge", "action":"v0268_objective_intercept_ready_hold_bridge"},
			{"id":"v0268_no_damage_after_intercept_preview_visible","label":"v0.268 no damage preview", "action":"v0268_no_damage_after_intercept_preview"},
			{"id":"v0268_no_enemy_slow_or_redirect_after_intercept_preview_visible","label":"v0.268 no slow redirect", "action":"v0268_no_enemy_slow_or_redirect_after_intercept_preview"},
			{"id":"v0268_threat_leaves_memory_cannot_intercept_visible","label":"v0.268 memory cannot", "action":"v0268_threat_leaves_memory_cannot_intercept"},
			{"id":"v0268_threat_leaves_memory_guarding_last_seen_lane_visible","label":"v0.268 memory guarding", "action":"v0268_threat_leaves_memory_guarding_last_seen_lane"},
			{"id":"v0268_last_seen_memory_marker_still_distinct_visible","label":"v0.268 memory marker distinct", "action":"v0268_last_seen_memory_marker_still_distinct"},
			{"id":"v0268_current_vs_memory_vs_position_vs_intercept_not_confused_visible","label":"v0.268 state layers distinct", "action":"v0268_current_vs_memory_vs_position_vs_intercept_not_confused"},
			{"id":"v0268_watchpost_hud_no_train_militia_visible","label":"v0.268 Watchpost no Train Militia", "action":"v0268_watchpost_hud_no_train_militia"},
			{"id":"v0268_barracks_hud_no_watchpost_relay_card_visible","label":"v0.268 Barracks no relay card", "action":"v0268_barracks_hud_no_watchpost_relay_card"},
			{"id":"v0268_existing_barracks_rebuild_path_still_valid_visible","label":"v0.268 Barracks rebuild retained", "action":"v0268_existing_barracks_rebuild_path_still_valid"},
			{"id":"v0268_existing_barracks_still_trains_militia_visible","label":"v0.268 Barracks trains Militia", "action":"v0268_existing_barracks_still_trains_militia"},
			{"id":"v0268_no_intercept_before_watchpost_complete_visible","label":"v0.268 no precomplete intercept", "action":"v0268_no_intercept_before_watchpost_complete"},
			{"id":"v0268_no_intercept_ready_before_militia_holding_bridge_visible","label":"v0.268 no false ready", "action":"v0268_no_intercept_ready_before_militia_holding_bridge"},
			{"id":"v0268_world_label_clutter_not_regressed_visible","label":"v0.268 world labels retained", "action":"v0268_world_label_clutter_not_regressed"},
		]
	if _player_capture_checkpoint() == "v0.267":
		return [
			{"id":"v0267_watchpost_build_path_visible","label":"v0.267 Watchpost build path", "action":"v0267_watchpost_build_path"},
			{"id":"v0267_watchpost_complete_no_intel_no_position_alarm_visible","label":"v0.267 complete no position alarm", "action":"v0267_watchpost_complete_no_intel_no_position_alarm"},
			{"id":"v0267_ashen_outside_zone_no_false_positive_no_position_alarm_visible","label":"v0.267 outside no false position", "action":"v0267_ashen_outside_zone_no_false_positive_no_position_alarm"},
			{"id":"v0267_current_detection_no_militia_position_none_visible","label":"v0.267 current no Militia position none", "action":"v0267_current_detection_no_militia_position_none"},
			{"id":"v0267_current_detection_relay_position_none_visible","label":"v0.267 relay position none", "action":"v0267_current_detection_relay_position_none"},
			{"id":"v0267_barracks_train_militia_advisory_still_visible","label":"v0.267 Barracks advisory retained", "action":"v0267_barracks_train_militia_advisory_still"},
			{"id":"v0267_militia_training_position_pending_visible","label":"v0.267 training position pending", "action":"v0267_militia_training_position_pending"},
			{"id":"v0267_current_detection_relay_position_pending_visible","label":"v0.267 relay position pending", "action":"v0267_current_detection_relay_position_pending"},
			{"id":"v0267_militia_ready_away_from_bridge_visible","label":"v0.267 Militia ready away", "action":"v0267_militia_ready_away_from_bridge"},
			{"id":"v0267_current_detection_relay_ready_not_in_position_visible","label":"v0.267 relay not in position", "action":"v0267_current_detection_relay_ready_not_in_position"},
			{"id":"v0267_objective_move_defender_to_east_bridge_visible","label":"v0.267 move defender objective", "action":"v0267_objective_move_defender_to_east_bridge"},
			{"id":"v0267_militia_moved_near_bridge_visible","label":"v0.267 Militia near bridge", "action":"v0267_militia_moved_near_bridge"},
			{"id":"v0267_current_detection_relay_holding_east_bridge_visible","label":"v0.267 relay holding east bridge", "action":"v0267_current_detection_relay_holding_east_bridge"},
			{"id":"v0267_objective_defender_in_position_visible","label":"v0.267 defender in position objective", "action":"v0267_objective_defender_in_position"},
			{"id":"v0267_threat_leaves_memory_position_not_in_position_visible","label":"v0.267 memory not in position", "action":"v0267_threat_leaves_memory_position_not_in_position"},
			{"id":"v0267_threat_leaves_memory_position_holding_bridge_visible","label":"v0.267 memory holding bridge", "action":"v0267_threat_leaves_memory_position_holding_bridge"},
			{"id":"v0267_last_seen_memory_marker_still_distinct_visible","label":"v0.267 memory marker distinct", "action":"v0267_last_seen_memory_marker_still_distinct"},
			{"id":"v0267_current_vs_memory_vs_position_not_confused_visible","label":"v0.267 current memory position distinct", "action":"v0267_current_vs_memory_vs_position_not_confused"},
			{"id":"v0267_watchpost_hud_no_train_militia_visible","label":"v0.267 Watchpost no Train Militia", "action":"v0267_watchpost_hud_no_train_militia"},
			{"id":"v0267_barracks_hud_no_watchpost_relay_card_visible","label":"v0.267 Barracks no relay card", "action":"v0267_barracks_hud_no_watchpost_relay_card"},
			{"id":"v0267_existing_barracks_rebuild_path_still_valid_visible","label":"v0.267 Barracks rebuild retained", "action":"v0267_existing_barracks_rebuild_path_still_valid"},
			{"id":"v0267_existing_barracks_still_trains_militia_visible","label":"v0.267 Barracks still trains Militia", "action":"v0267_existing_barracks_still_trains_militia"},
			{"id":"v0267_no_position_before_watchpost_complete_visible","label":"v0.267 no precomplete position", "action":"v0267_no_position_before_watchpost_complete"},
			{"id":"v0267_no_position_holding_before_militia_near_bridge_visible","label":"v0.267 no false holding before near", "action":"v0267_no_position_holding_before_militia_near_bridge"},
			{"id":"v0267_world_label_clutter_not_regressed_visible","label":"v0.267 world labels retained", "action":"v0267_world_label_clutter_not_regressed"},
		]
	if _player_capture_checkpoint() == "v0.266":
		return [
			{"id":"v0266_watchpost_build_path_visible","label":"v0.266 Watchpost build path", "action":"v0266_watchpost_build_path"},
			{"id":"v0266_complete_no_intel_no_readiness_alarm_visible","label":"v0.266 complete no intel calm", "action":"v0266_complete_no_intel_no_readiness_alarm"},
			{"id":"v0266_outside_no_false_positive_no_readiness_alarm_visible","label":"v0.266 outside no false readiness", "action":"v0266_outside_no_false_positive_no_readiness_alarm"},
			{"id":"v0266_current_detection_no_defender_readiness_visible","label":"v0.266 current no defender", "action":"v0266_current_detection_no_defender_readiness"},
			{"id":"v0266_current_detection_train_militia_objective_visible","label":"v0.266 train Militia objective", "action":"v0266_current_detection_train_militia_objective"},
			{"id":"v0266_current_detection_relay_readiness_none_visible","label":"v0.266 relay readiness none", "action":"v0266_current_detection_relay_readiness_none"},
			{"id":"v0266_barracks_selected_current_detection_train_militia_advisory_visible","label":"v0.266 Barracks advisory", "action":"v0266_barracks_selected_current_detection_train_militia_advisory"},
			{"id":"v0266_militia_training_started_from_barracks_visible","label":"v0.266 training from Barracks", "action":"v0266_militia_training_started_from_barracks"},
			{"id":"v0266_current_detection_relay_readiness_training_visible","label":"v0.266 relay readiness training", "action":"v0266_current_detection_relay_readiness_training"},
			{"id":"v0266_objective_militia_training_underway_visible","label":"v0.266 objective training underway", "action":"v0266_objective_militia_training_underway"},
			{"id":"v0266_militia_ready_after_existing_training_visible","label":"v0.266 Militia ready after existing training", "action":"v0266_militia_ready_after_existing_training"},
			{"id":"v0266_current_detection_relay_readiness_ready_visible","label":"v0.266 relay readiness ready", "action":"v0266_current_detection_relay_readiness_ready"},
			{"id":"v0266_threat_leaves_memory_readiness_ready_visible","label":"v0.266 memory readiness ready", "action":"v0266_threat_leaves_memory_readiness_ready"},
			{"id":"v0266_last_seen_memory_readiness_none_path_visible","label":"v0.266 memory readiness none", "action":"v0266_last_seen_memory_readiness_none_path"},
			{"id":"v0266_last_seen_memory_readiness_training_path_visible","label":"v0.266 memory readiness training", "action":"v0266_last_seen_memory_readiness_training_path"},
			{"id":"v0266_last_seen_memory_marker_distinct_visible","label":"v0.266 memory marker distinct", "action":"v0266_last_seen_memory_marker_distinct"},
			{"id":"v0266_current_vs_memory_readiness_not_confused_visible","label":"v0.266 current vs memory distinct", "action":"v0266_current_vs_memory_readiness_not_confused"},
			{"id":"v0266_watchpost_hud_no_train_militia_visible","label":"v0.266 Watchpost no Train Militia", "action":"v0266_watchpost_hud_no_train_militia"},
			{"id":"v0266_barracks_hud_no_watchpost_relay_card_visible","label":"v0.266 Barracks no relay card", "action":"v0266_barracks_hud_no_watchpost_relay_card"},
			{"id":"v0266_existing_barracks_rebuild_path_valid_visible","label":"v0.266 Barracks rebuild retained", "action":"v0266_existing_barracks_rebuild_path_valid"},
			{"id":"v0266_existing_barracks_still_trains_militia_visible","label":"v0.266 Barracks still trains Militia", "action":"v0266_existing_barracks_still_trains_militia"},
			{"id":"v0266_no_detection_readiness_before_watchpost_complete_visible","label":"v0.266 no precomplete readiness", "action":"v0266_no_detection_readiness_before_watchpost_complete"},
			{"id":"v0266_world_label_clutter_not_regressed_visible","label":"v0.266 world labels retained", "action":"v0266_world_label_clutter_not_regressed"},
		]
	if _player_capture_checkpoint() == "v0.265":
		return [
			{"id":"v0265_watchpost_build_path_visible","label":"v0.265 Watchpost build path", "action":"v0265_watchpost_build_path"},
			{"id":"v0265_watchpost_online_monitoring_objective_visible","label":"v0.265 online monitoring objective", "action":"v0265_watchpost_online_monitoring_objective"},
			{"id":"v0265_no_prior_intel_relay_visible","label":"v0.265 no prior intel relay", "action":"v0265_no_prior_intel_relay"},
			{"id":"v0265_ashen_outside_zone_monitoring_not_alarm_visible","label":"v0.265 outside monitoring not alarm", "action":"v0265_ashen_outside_zone_monitoring_not_alarm"},
			{"id":"v0265_no_false_positive_outside_zone_visible","label":"v0.265 no false positive outside", "action":"v0265_no_false_positive_outside_zone"},
			{"id":"v0265_current_detection_objective_prepare_defenders_visible","label":"v0.265 current detection objective", "action":"v0265_current_detection_objective_prepare_defenders"},
			{"id":"v0265_current_detection_intel_relay_visible","label":"v0.265 current detection relay", "action":"v0265_current_detection_intel_relay"},
			{"id":"v0265_current_detection_world_label_clean_visible","label":"v0.265 current world label clean", "action":"v0265_current_detection_world_label_clean"},
			{"id":"v0265_current_detection_minimap_ping_visible","label":"v0.265 current minimap ping", "action":"v0265_current_detection_minimap_ping"},
			{"id":"v0265_barracks_selected_current_detection_militia_advisory_visible","label":"v0.265 Barracks current advisory", "action":"v0265_barracks_selected_current_detection_militia_advisory"},
			{"id":"v0265_barracks_train_militia_after_watchpost_advisory_visible","label":"v0.265 train after advisory", "action":"v0265_barracks_train_militia_after_watchpost_advisory"},
			{"id":"v0265_threat_leaves_memory_objective_visible","label":"v0.265 memory objective", "action":"v0265_threat_leaves_memory_objective"},
			{"id":"v0265_last_seen_memory_intel_relay_visible","label":"v0.265 last seen relay", "action":"v0265_last_seen_memory_intel_relay"},
			{"id":"v0265_last_seen_memory_marker_distinct_visible","label":"v0.265 memory marker distinct", "action":"v0265_last_seen_memory_marker_distinct"},
			{"id":"v0265_last_seen_memory_minimap_ping_distinct_visible","label":"v0.265 memory minimap distinct", "action":"v0265_last_seen_memory_minimap_ping_distinct"},
			{"id":"v0265_memory_not_current_detection_visible","label":"v0.265 memory not current", "action":"v0265_memory_not_current_detection"},
			{"id":"v0265_watchpost_hud_no_barracks_actions_visible","label":"v0.265 Watchpost no Barracks actions", "action":"v0265_watchpost_hud_no_barracks_actions"},
			{"id":"v0265_barracks_hud_no_watchpost_relay_card_visible","label":"v0.265 Barracks no relay card", "action":"v0265_barracks_hud_no_watchpost_relay_card"},
			{"id":"v0265_existing_barracks_rebuild_path_still_valid_visible","label":"v0.265 old Barracks lifecycle preserved", "action":"v0265_existing_barracks_rebuild_path_still_valid"},
			{"id":"v0265_no_detection_or_advisory_before_watchpost_complete_visible","label":"v0.265 no precomplete advisory", "action":"v0265_no_detection_or_advisory_before_watchpost_complete"},
			{"id":"v0265_world_label_clutter_not_regressed_visible","label":"v0.265 world labels retained", "action":"v0265_world_label_clutter_not_regressed"},
		]
	if _player_capture_checkpoint() == "v0.264":
		return [
			{"id":"v0264_watchpost_build_path_visible","label":"v0.264 Watchpost build path", "action":"v0264_watchpost_build_path"},
			{"id":"v0264_watchpost_complete_no_threat_no_history_intel_relay_visible","label":"v0.264 no threat relay", "action":"v0264_watchpost_complete_no_threat_no_history_intel_relay"},
			{"id":"v0264_watch_zone_clean_labeling_visible","label":"v0.264 clean WATCH ZONE labels", "action":"v0264_watch_zone_clean_labeling"},
			{"id":"v0264_ashen_outside_zone_no_false_positive_visible","label":"v0.264 outside no false positive", "action":"v0264_ashen_outside_zone_no_false_positive"},
			{"id":"v0264_ashen_touching_zone_current_scouted_visible","label":"v0.264 touching current scouted", "action":"v0264_ashen_touching_zone_current_scouted"},
			{"id":"v0264_ashen_inside_zone_current_scouted_visible","label":"v0.264 inside current scouted", "action":"v0264_ashen_inside_zone_current_scouted"},
			{"id":"v0264_current_scouted_intel_relay_visible","label":"v0.264 current relay", "action":"v0264_current_scouted_intel_relay"},
			{"id":"v0264_current_scouted_world_label_not_overlapping_watchpost_visible","label":"v0.264 current label separated", "action":"v0264_current_scouted_world_label_not_overlapping_watchpost"},
			{"id":"v0264_current_scouted_minimap_ping_visible","label":"v0.264 current minimap ping", "action":"v0264_current_scouted_minimap_ping"},
			{"id":"v0264_threat_leaves_zone_last_seen_memory_visible","label":"v0.264 threat leaves last seen", "action":"v0264_threat_leaves_zone_last_seen_memory"},
			{"id":"v0264_last_seen_memory_intel_relay_visible","label":"v0.264 memory relay", "action":"v0264_last_seen_memory_intel_relay"},
			{"id":"v0264_last_seen_memory_world_marker_distinct_visible","label":"v0.264 memory world marker distinct", "action":"v0264_last_seen_memory_world_marker_distinct"},
			{"id":"v0264_last_seen_memory_minimap_ping_distinct_visible","label":"v0.264 memory minimap distinct", "action":"v0264_last_seen_memory_minimap_ping_distinct"},
			{"id":"v0264_memory_clearly_not_current_detection_visible","label":"v0.264 memory not current", "action":"v0264_memory_clearly_not_current_detection"},
			{"id":"v0264_watchpost_hud_no_barracks_text_visible","label":"v0.264 Watchpost no Barracks text", "action":"v0264_watchpost_hud_no_barracks_text"},
			{"id":"v0264_barracks_hud_no_watchpost_text_visible","label":"v0.264 Barracks no Watchpost text", "action":"v0264_barracks_hud_no_watchpost_text"},
			{"id":"v0264_barracks_still_trains_militia_visible","label":"v0.264 Barracks still trains", "action":"v0264_barracks_still_trains_militia"},
			{"id":"v0264_existing_barracks_rebuild_path_still_valid_visible","label":"v0.264 old Barracks lifecycle preserved", "action":"v0264_existing_barracks_rebuild_path_still_valid"},
			{"id":"v0264_no_detection_or_memory_before_watchpost_complete_visible","label":"v0.264 no precomplete intel", "action":"v0264_no_detection_or_memory_before_watchpost_complete"},
			{"id":"v0264_world_label_clutter_reduced_visible","label":"v0.264 world label clutter reduced", "action":"v0264_world_label_clutter_reduced"},
		]
	if _player_capture_checkpoint() == "v0.263":
		return [
			{"id":"v0263_watchpost_build_path_visible","label":"v0.263 Watchpost build path", "action":"v0263_watchpost_build_path"},
			{"id":"v0263_watchpost_complete_no_threat_no_history_visible","label":"v0.263 no threat no history", "action":"v0263_watchpost_complete_no_threat_no_history"},
			{"id":"v0263_watch_zone_clean_labeling_visible","label":"v0.263 clean WATCH ZONE labels", "action":"v0263_watch_zone_clean_labeling"},
			{"id":"v0263_ashen_outside_zone_no_false_positive_visible","label":"v0.263 outside no false positive", "action":"v0263_ashen_outside_zone_no_false_positive"},
			{"id":"v0263_ashen_touching_zone_current_scouted_visible","label":"v0.263 touching current scouted", "action":"v0263_ashen_touching_zone_current_scouted"},
			{"id":"v0263_ashen_inside_zone_current_scouted_visible","label":"v0.263 inside current scouted", "action":"v0263_ashen_inside_zone_current_scouted"},
			{"id":"v0263_current_scouted_minimap_ping_visible","label":"v0.263 current minimap ping", "action":"v0263_current_scouted_minimap_ping"},
			{"id":"v0263_current_scouted_hud_intel_only_visible","label":"v0.263 current HUD intel only", "action":"v0263_current_scouted_hud_intel_only"},
			{"id":"v0263_threat_leaves_zone_last_seen_memory_visible","label":"v0.263 threat leaves last seen", "action":"v0263_threat_leaves_zone_last_seen_memory"},
			{"id":"v0263_last_seen_memory_minimap_ping_visible","label":"v0.263 memory minimap ping", "action":"v0263_last_seen_memory_minimap_ping"},
			{"id":"v0263_last_seen_memory_world_marker_visible","label":"v0.263 memory world marker", "action":"v0263_last_seen_memory_world_marker"},
			{"id":"v0263_memory_clearly_not_current_detection_visible","label":"v0.263 memory not current", "action":"v0263_memory_clearly_not_current_detection"},
			{"id":"v0263_watchpost_hud_no_barracks_text_visible","label":"v0.263 Watchpost no Barracks text", "action":"v0263_watchpost_hud_no_barracks_text"},
			{"id":"v0263_barracks_hud_no_watchpost_text_visible","label":"v0.263 Barracks no Watchpost text", "action":"v0263_barracks_hud_no_watchpost_text"},
			{"id":"v0263_barracks_still_trains_militia_visible","label":"v0.263 Barracks still trains", "action":"v0263_barracks_still_trains_militia"},
			{"id":"v0263_existing_barracks_rebuild_path_still_valid_visible","label":"v0.263 old Barracks lifecycle preserved", "action":"v0263_existing_barracks_rebuild_path_still_valid"},
			{"id":"v0263_no_detection_or_memory_before_watchpost_complete_visible","label":"v0.263 no precomplete intel", "action":"v0263_no_detection_or_memory_before_watchpost_complete"},
		]
	...
	if _player_capture_checkpoint() == "v0.261":
		return [
			...
			{"id":"v0261_after_aster_select_worker_visible","label":"v0.261 after Aster", "action":"v0261_after_aster"},
			{"id":"v0261_place_field_barracks_visible","label":"v0.261 place Field Barracks", "action":"v0261_place_field_barracks"},
			{"id":"v0261_field_barracks_built_visible","label":"v0.261 Field Barracks built", "action":"v0261_field_barracks_built"},
			{"id":"v0261_new_objective_build_watchpost_visible","label":"v0.261 Watchpost objective", "action":"v0261_new_objective_build_watchpost"},
			{"id":"v0261_worker_watchpost_button_visible","label":"v0.261 Watchpost button", "action":"v0261_worker_watchpost_button"},
			{"id":"v0261_watchpost_placement_cost_visible","label":"v0.261 Watchpost cost", "action":"v0261_watchpost_placement_cost"},
			{"id":"v0261_watchpost_valid_site_visible","label":"v0.261 Watchpost valid", "action":"v0261_watchpost_valid_site"},
			{"id":"v0261_watchpost_built_resource_delta_visible","label":"v0.261 Watchpost built", "action":"v0261_watchpost_built_resource_delta"},
			{"id":"v0261_watchpost_selected_hud_visible","label":"v0.261 Watchpost selected", "action":"v0261_watchpost_selected_hud"},
			{"id":"v0261_watch_zone_overlay_visible","label":"v0.261 Watch Zone", "action":"v0261_watch_zone_overlay"},
			{"id":"v0261_watchpost_minimap_marker_visible","label":"v0.261 Watchpost minimap", "action":"v0261_watchpost_minimap_marker"},
			{"id":"v0261_barracks_still_trains_militia_visible","label":"v0.261 Barracks still trains", "action":"v0261_barracks_still_trains_militia"},
			{"id":"v0261_militia_training_after_watchpost_visible","label":"v0.261 Militia training", "action":"v0261_militia_training_after_watchpost"},
			{"id":"v0261_no_barracks_text_on_watchpost_visible","label":"v0.261 no Barracks text on Watchpost", "action":"v0261_no_barracks_text_on_watchpost"},
			{"id":"v0261_no_watchpost_text_on_barracks_visible","label":"v0.261 no Watchpost text on Barracks", "action":"v0261_no_watchpost_text_on_barracks"},
			{"id":"v0261_existing_barracks_rebuild_path_still_valid_visible","label":"v0.261 old Barracks lifecycle preserved", "action":"v0261_existing_barracks_rebuild_path_still_valid"},
		]
	if _player_capture_checkpoint() == "v0.259":
		return [
	...
	if _player_capture_checkpoint() == "v0.258":
		return [
			...
			{"id":"after_aster_select_worker_instruction","label":"v0.258 next Worker","action":"v0258_after_aster_select_worker"},
			{"id":"worker_place_barracks_instruction","label":"v0.258 place Barracks","action":"v0258_worker_place_barracks"},
			{"id":"valid_placement_instruction","label":"v0.258 valid placement","action":"v0258_valid_placement"},
			{"id":"barracks_built_instruction_and_visual_full","label":"v0.258 full","action":"v0258_barracks_built"},
			{"id":"hp_125_damaged_functional_instruction_and_visual","label":"v0.258 damaged","action":"v0258_hp_125"},
			{"id":"hp_25_critical_functional_instruction_and_visual","label":"v0.258 critical","action":"v0258_hp_25"},
			{"id":"hp_0_destroyed_instruction_and_visual","label":"v0.258 destroyed","action":"v0258_hp_0"},
			{"id":"destroyed_no_select_aster_stale_text","label":"v0.258 no stale Aster","action":"v0258_destroyed_no_stale"},
			{"id":"worker_rebuild_instruction","label":"v0.258 rebuild instruction","action":"v0258_worker_rebuild_instruction"},
			{"id":"worker_rebuild_hud_and_button","label":"v0.258 rebuild HUD","action":"v0258_worker_rebuild_hud"},
			{"id":"rebuild_resource_delta","label":"v0.258 rebuild spend","action":"v0258_rebuild_delta"},
			{"id":"rebuild_progress_25_visual_text","label":"v0.258 rebuild 25","action":"v0258_rebuild_25"},
			{"id":"rebuild_progress_50_visual_text","label":"v0.258 rebuild 50","action":"v0258_rebuild_50"},
			{"id":"rebuild_progress_75_visual_text","label":"v0.258 rebuild 75","action":"v0258_rebuild_75"},
			{"id":"rebuild_complete_100_visual_text","label":"v0.258 rebuild 100","action":"v0258_rebuild_100"},
			{"id":"train_available_after_rebuild","label":"v0.258 train available","action":"v0258_train_available"},
			{"id":"train_resource_delta_after_rebuild","label":"v0.258 train spend","action":"v0258_train_delta"},
			{"id":"militia_ready_defend_instruction","label":"v0.258 Militia ready","action":"v0258_militia_ready"},
			{"id":"repair_rebuild_separation_worker_text","label":"v0.258 separation","action":"v0258_separation"},
			{"id":"defended_branch_preserved","label":"v0.258 defended","action":"v0258_defended"},
			{"id":"minimap_preserved","label":"v0.258 minimap","action":"v0258_minimap"},
			{"id":"existing_structures_preserved","label":"v0.258 structures","action":"v0258_structures"},
			{"id":"no_stale_rebuild_not_implemented_text","label":"v0.258 no stale Rebuild","action":"v0258_no_stale_rebuild"},
			{"id":"no_stale_select_aster_after_initial_phase","label":"v0.258 no stale Aster","action":"v0258_no_stale_aster"},
		]
	if _player_capture_checkpoint() == "v0.257":
		return [
			{"id":"opt_in_overview_before_build","label":"v0.257 overview","action":"v0257_overview"},
			{"id":"starting_resources","label":"v0.257 resources","action":"v0257_starting_resources"},
			{"id":"barracks_built_hp_200","label":"v0.257 HP 200","action":"v0257_barracks_built"},
			{"id":"first_pressure_hp_125_damaged_functional_text","label":"v0.257 HP 125 text","action":"v0257_first_pressure_125"},
			{"id":"hp_25_still_functional_text","label":"v0.257 HP 25 text","action":"v0257_hp_25"},
			{"id":"hp_0_destroyed_text_corrected","label":"v0.257 destroyed text","action":"v0257_second_damage_0"},
			{"id":"destroyed_train_unavailable_text","label":"v0.257 train unavailable","action":"v0257_destroyed_train_unavailable"},
			{"id":"worker_selected_rebuild_available_text","label":"v0.257 Worker Rebuild","action":"v0257_worker_rebuild_available"},
			{"id":"worker_repair_unavailable_target_destroyed_text","label":"v0.257 Repair unavailable","action":"v0257_repair_unavailable_zero"},
			{"id":"rebuild_ordered","label":"v0.257 rebuild ordered","action":"v0257_rebuild_ordered"},
			{"id":"rebuild_resource_delta","label":"v0.257 rebuild spend","action":"v0257_rebuild_delta"},
			{"id":"rebuild_progress_hp_25_text","label":"v0.257 rebuild 25","action":"v0257_rebuild_25"},
			{"id":"rebuild_progress_hp_50_text","label":"v0.257 rebuild 50","action":"v0257_rebuild_50"},
			{"id":"rebuild_progress_hp_75_text","label":"v0.257 rebuild 75","action":"v0257_rebuild_75"},
			{"id":"rebuild_complete_hp_100_text","label":"v0.257 rebuild 100","action":"v0257_rebuild_100"},
			{"id":"rebuilt_train_available_text","label":"v0.257 production restored","action":"v0257_rebuilt_train_available"},
			{"id":"train_from_rebuilt_resource_delta","label":"v0.257 train spend","action":"v0257_train_rebuilt_ordered"},
			{"id":"militia_from_rebuilt_ready","label":"v0.257 Militia ready","action":"v0257_militia_ready_rebuilt"},
			{"id":"worker_rebuild_unavailable_after_rebuild","label":"v0.257 no rebuild target","action":"v0257_worker_after_rebuild"},
			{"id":"worker_repair_available_or_insufficient_resources_after_rebuild","label":"v0.257 repair state","action":"v0257_rebuild_unavailable_no_target"},
			{"id":"defended_branch_combat_preserved","label":"v0.257 defended combat","action":"v0257_defended_combat"},
			{"id":"barracks_unharmed_after_defense","label":"v0.257 defended Barracks","action":"v0257_defended_barracks"},
			{"id":"aster_worker_unharmed","label":"v0.257 units preserved","action":"v0257_units_unharmed"},
			{"id":"minimap_preserved","label":"v0.257 minimap","action":"v0257_minimap"},
			{"id":"existing_structures_preserved","label":"v0.257 structures","action":"v0257_preserve_structures"},
			{"id":"no_stale_rebuild_not_implemented_text","label":"v0.257 stale text absent","action":"v0257_no_stale_text"},
		]
	...
	if _player_capture_checkpoint() == "v0.255":
		return [
			...
			{"id":"starting_resources","label":"v0.255 resources","action":"v0255_starting_resources"},
			{"id":"select_worker_construction_available","label":"v0.255 Worker","action":"v0255_select_worker"},
			{"id":"valid_barracks_preview","label":"v0.255 preview","action":"v0255_valid_preview"},
			{"id":"confirm_authoritative_barracks_placement","label":"v0.255 placement","action":"v0255_confirm_placement"},
			{"id":"construction_resource_delta","label":"v0.255 construction delta","action":"v0255_construction_delta"},
			{"id":"field_barracks_hp_200","label":"v0.255 HP 200","action":"v0255_barracks_hp_200"},
			{"id":"first_raider_spawned_once","label":"v0.255 first Raider","action":"v0255_first_raider"},
			{"id":"first_warning_started","label":"v0.255 first warning","action":"v0255_first_warning_started"},
			{"id":"first_warning_midpoint_no_damage","label":"v0.255 first warning midpoint","action":"v0255_first_warning_midpoint"},
			{"id":"first_warning_expired","label":"v0.255 first warning expired","action":"v0255_first_warning_expired"},
			{"id":"first_damage_hp_175","label":"v0.255 first damage 175","action":"v0255_first_damage_175"},
			{"id":"first_damage_hp_150","label":"v0.255 first damage 150","action":"v0255_first_damage_150"},
			{"id":"first_damage_hp_125","label":"v0.255 first damage 125","action":"v0255_first_damage_125"},
			{"id":"first_pressure_stops_at_125","label":"v0.255 first pressure stopped","action":"v0255_first_pressure_stopped"},
			{"id":"no_passive_collapse_after_wait","label":"v0.255 no passive collapse","action":"v0255_no_passive_collapse"},
			{"id":"damaged_barracks_selectable","label":"v0.255 damaged selectable","action":"v0255_damaged_selectable"},
			{"id":"damaged_barracks_train_available","label":"v0.255 damaged production","action":"v0255_damaged_train_available"},
			{"id":"train_from_damaged_ordered","label":"v0.255 train damaged","action":"v0255_train_damaged_ordered"},
			{"id":"train_from_damaged_resource_delta","label":"v0.255 train delta","action":"v0255_train_damaged_delta"},
			{"id":"militia_ready_from_damaged_barracks","label":"v0.255 damaged Militia ready","action":"v0255_militia_ready_damaged"},
			{"id":"damaged_barracks_still_hp_125_after_training","label":"v0.255 damaged HP retained","action":"v0255_damaged_hp_after_training"},
			{"id":"second_pressure_triggered_explicitly","label":"v0.255 second pressure","action":"v0255_second_pressure_triggered"},
			{"id":"second_warning_started","label":"v0.255 second warning","action":"v0255_second_warning_started"},
			{"id":"second_warning_midpoint_no_damage","label":"v0.255 second warning midpoint","action":"v0255_second_warning_midpoint"},
			{"id":"second_warning_expired_no_intercept","label":"v0.255 second warning expired","action":"v0255_second_warning_expired"},
			{"id":"second_damage_hp_100","label":"v0.255 HP 100","action":"v0255_second_damage_100"},
			{"id":"second_damage_hp_75","label":"v0.255 HP 75","action":"v0255_second_damage_75"},
			{"id":"second_damage_hp_50","label":"v0.255 HP 50","action":"v0255_second_damage_50"},
			{"id":"second_damage_hp_25_still_functional","label":"v0.255 HP 25 functional","action":"v0255_second_damage_25"},
			{"id":"second_damage_hp_0_destroyed","label":"v0.255 HP 0 destroyed","action":"v0255_second_damage_0"},
			{"id":"destroyed_barracks_selected","label":"v0.255 destroyed selected","action":"v0255_destroyed_selected"},
			{"id":"destroyed_barracks_train_unavailable","label":"v0.255 train unavailable","action":"v0255_destroyed_train_unavailable"},
			{"id":"destroyed_barracks_repair_unavailable","label":"v0.255 repair unavailable","action":"v0255_destroyed_repair_unavailable"},
			{"id":"no_refund_after_destruction","label":"v0.255 no refund","action":"v0255_no_refund_destroyed"},
			{"id":"train_from_damaged_intercepts_second_pressure_start","label":"v0.255 second intercept start","action":"v0255_second_intercept_start"},
			{"id":"second_pressure_intercept_order","label":"v0.255 second intercept order","action":"v0255_second_intercept_order"},
			{"id":"second_pressure_intercept_combat_90_40","label":"v0.255 second combat 90/40","action":"v0255_second_intercept_tick_1"},
			{"id":"second_pressure_intercept_combat_80_20","label":"v0.255 second combat 80/20","action":"v0255_second_intercept_tick_2"},
			{"id":"second_pressure_intercept_combat_70_0","label":"v0.255 second combat 70/0","action":"v0255_second_intercept_tick_3"},
			{"id":"second_pressure_intercept_barracks_survives_125","label":"v0.255 second pressure stopped","action":"v0255_second_intercept_survives"},
			{"id":"repair_branch_available_at_125","label":"v0.255 repair available","action":"v0255_repair_available"},
			{"id":"repair_resource_delta","label":"v0.255 repair delta","action":"v0255_repair_delta"},
			{"id":"repair_hp_150","label":"v0.255 repair HP 150","action":"v0255_repair_150"},
			{"id":"repair_hp_175","label":"v0.255 repair HP 175","action":"v0255_repair_175"},
			{"id":"repair_hp_200","label":"v0.255 repair HP 200","action":"v0255_repair_200"},
			{"id":"repair_unavailable_full_hp","label":"v0.255 repair unavailable full","action":"v0255_repair_unavailable_full"},
			{"id":"defended_first_branch_start","label":"v0.255 defended first start","action":"v0255_defended_first_start"},
			{"id":"defended_first_combat_90_40","label":"v0.255 defended 90/40","action":"v0255_defended_first_tick_1"},
			{"id":"defended_first_combat_80_20","label":"v0.255 defended 80/20","action":"v0255_defended_first_tick_2"},
			{"id":"defended_first_combat_70_0","label":"v0.255 defended 70/0","action":"v0255_defended_first_tick_3"},
			{"id":"defended_barracks_unharmed_200","label":"v0.255 defended Barracks","action":"v0255_defended_barracks"},
			{"id":"aster_worker_unharmed_proof","label":"v0.255 units preserved","action":"v0255_units_unharmed"},
			{"id":"minimap_preserved","label":"v0.255 minimap","action":"v0255_minimap"},
			{"id":"existing_barracks_preserved","label":"v0.255 existing Barracks","action":"v0255_preserve_barracks"},
			{"id":"command_keep_lume_mine_preserved","label":"v0.255 Keep and Mine","action":"v0255_preserve_keep_mine"},
			{"id":"shells_remain_non_producing","label":"v0.255 shells","action":"v0255_preserve_shells"},
			{"id":"default_runtime_clean_after_opt_in_work","label":"v0.255 default clean","action":"v0255_default_clean"},
			{"id":"unselected_clean_view","label":"v0.255 clean","action":"v0255_clean"},
		]
	...
	if _player_capture_checkpoint() == "v0.252":
		return [
			...
			{"id":"starting_resources","label":"v0.252 resources","action":"v0252_starting_resources"},
			{"id":"select_builder_unit","label":"v0.252 Worker","action":"v0252_select_builder"},
			{"id":"valid_barracks_preview","label":"v0.252 preview","action":"v0252_valid_preview"},
			{"id":"confirm_authoritative_barracks_placement","label":"v0.252 placement","action":"v0252_confirm_placement"},
			{"id":"construction_resource_delta","label":"v0.252 construction delta","action":"v0252_construction_delta"},
			{"id":"field_barracks_hp_200","label":"v0.252 Barracks HP","action":"v0252_barracks_hp_200"},
			{"id":"train_militia_command_available","label":"v0.252 train command","action":"v0252_train_command"},
			{"id":"train_militia_resource_delta","label":"v0.252 train spend","action":"v0252_train_militia"},
			{"id":"militia_training_progress","label":"v0.252 training","action":"v0252_training_progress"},
			{"id":"militia_ready_spawned","label":"v0.252 Militia ready","action":"v0252_militia_ready"},
			{"id":"raider_spawned_once_hp_full","label":"v0.252 Raider","action":"v0252_raider_spawned"},
			{"id":"raider_minimap_presence","label":"v0.252 minimap","action":"v0252_raider_minimap"},
			{"id":"raider_entering_threat_range","label":"v0.252 threat range","action":"v0252_raider_entering_threat_range"},
			{"id":"warning_window_started","label":"v0.252 warning start","action":"v0252_warning_started"},
			{"id":"warning_window_midpoint_no_damage","label":"v0.252 warning midpoint","action":"v0252_warning_midpoint"},
			{"id":"field_barracks_still_hp_200_during_warning","label":"v0.252 warning HP","action":"v0252_warning_barracks_hp_200"},
			{"id":"select_militia_during_warning","label":"v0.252 select Militia","action":"v0252_select_militia_warning"},
			{"id":"attack_command_available_during_warning","label":"v0.252 Attack available","action":"v0252_attack_available_warning"},
			{"id":"attack_order_accepted_during_warning","label":"v0.252 Attack accepted","action":"v0252_attack_accepted_warning"},
			{"id":"militia_closing_to_raider","label":"v0.252 closing","action":"v0252_militia_closing"},
			{"id":"combat_tick_1_hp_90_40","label":"v0.252 tick 1","action":"v0252_combat_tick_1"},
			{"id":"combat_tick_2_hp_80_20","label":"v0.252 tick 2","action":"v0252_combat_tick_2"},
			{"id":"combat_tick_3_raider_hp_zero_militia_70","label":"v0.252 tick 3","action":"v0252_combat_tick_3"},
			{"id":"raider_defeated_or_removed","label":"v0.252 Raider defeated","action":"v0252_raider_defeated"},
			{"id":"pressure_contained_before_impact","label":"v0.252 contained","action":"v0252_pressure_contained_before_impact"},
			{"id":"field_barracks_unharmed_hp_200","label":"v0.252 Barracks unharmed","action":"v0252_barracks_unharmed"},
			{"id":"no_resource_mutation_after_defended_combat","label":"v0.252 defended resources","action":"v0252_defended_no_resource_mutation"},
			{"id":"missed_window_branch_start","label":"v0.252 missed branch","action":"v0252_missed_window_start"},
			{"id":"missed_window_raider_spawned_once","label":"v0.252 missed Raider","action":"v0252_missed_raider_spawned"},
			{"id":"missed_window_warning_started","label":"v0.252 missed warning","action":"v0252_missed_warning_started"},
			{"id":"missed_window_expired_no_intercept","label":"v0.252 warning expired","action":"v0252_missed_warning_expired"},
			{"id":"damage_begins_after_warning_expiry","label":"v0.252 damage begins","action":"v0252_damage_begins_after_expiry"},
			{"id":"barracks_damage_tick_1_hp_175","label":"v0.252 damage 1","action":"v0252_building_tick_1"},
			{"id":"barracks_damage_tick_2_hp_150","label":"v0.252 damage 2","action":"v0252_building_tick_2"},
			{"id":"barracks_damage_tick_3_hp_125","label":"v0.252 damage 3","action":"v0252_building_tick_3"},
			{"id":"damage_sequence_stopped","label":"v0.252 damage stopped","action":"v0252_damage_stopped"},
			{"id":"barracks_damaged_but_standing","label":"v0.252 damaged","action":"v0252_barracks_damaged"},
			{"id":"no_resource_mutation_after_undefended_pressure","label":"v0.252 missed resources","action":"v0252_missed_no_resource_mutation"},
			{"id":"aster_worker_unharmed_proof","label":"v0.252 units preserved","action":"v0252_aster_worker_unharmed"},
			{"id":"raider_bounded_stop_after_pressure","label":"v0.252 Raider bounded","action":"v0252_raider_bounded_stop"},
			{"id":"existing_barracks_train_flow_preserved","label":"v0.252 Barracks preserved","action":"v0252_preserve_barracks"},
			{"id":"command_keep_preserved","label":"v0.252 Keep preserved","action":"v0252_preserve_keep"},
			{"id":"lume_mine_preserved","label":"v0.252 Mine preserved","action":"v0252_preserve_mine"},
			{"id":"shells_remain_non_producing","label":"v0.252 shells","action":"v0252_preserve_shells"},
			{"id":"default_runtime_clean_after_opt_in_work","label":"v0.252 default clean","action":"v0252_default_clean"},
			{"id":"unselected_clean_view","label":"v0.252 clean","action":"v0252_clean"},
		]
	if _player_capture_checkpoint() == "v0.251":
		return [
			{"id":"opt_in_overview_before_build","label":"v0.251 overview","action":"v0251_overview"},
			{"id":"starting_resources","label":"v0.251 resources","action":"v0251_starting_resources"},
			{"id":"select_builder_unit","label":"v0.251 Worker","action":"v0251_select_builder"},
			{"id":"valid_barracks_preview","label":"v0.251 preview","action":"v0251_valid_preview"},
			{"id":"confirm_authoritative_barracks_placement","label":"v0.251 placement","action":"v0251_confirm_placement"},
			{"id":"construction_resource_delta","label":"v0.251 construction delta","action":"v0251_construction_delta"},
			{"id":"field_barracks_hp_200","label":"v0.251 Barracks HP","action":"v0251_barracks_hp_200"},
			{"id":"select_new_field_barracks_hud","label":"v0.251 Field Barracks","action":"v0251_select_field_barracks"},
			{"id":"train_militia_command_available","label":"v0.251 train command","action":"v0251_train_command"},
			{"id":"train_militia_resource_delta","label":"v0.251 train spend","action":"v0251_train_militia"},
			{"id":"militia_training_progress","label":"v0.251 training","action":"v0251_training_progress"},
			{"id":"militia_ready_spawned","label":"v0.251 Militia","action":"v0251_militia_ready"},
			{"id":"select_spawned_militia_hp_100_attack_available","label":"v0.251 Militia selected","action":"v0251_select_militia"},
			{"id":"pressure_telegraph_marker_preserved","label":"v0.251 telegraph","action":"v0251_pressure_telegraph"},
			{"id":"intercept_zone_marker_preserved","label":"v0.251 intercept","action":"v0251_intercept_marker"},
			{"id":"ashen_raider_spawned_once_hp_full","label":"v0.251 Raider","action":"v0251_raider_spawned"},
			{"id":"raider_minimap_presence","label":"v0.251 minimap","action":"v0251_raider_minimap"},
			{"id":"attack_command_available","label":"v0.251 Attack","action":"v0251_attack_available"},
			{"id":"attack_order_accepted","label":"v0.251 accepted","action":"v0251_attack_accepted"},
			{"id":"militia_closing_to_raider","label":"v0.251 closing","action":"v0251_militia_closing"},
			{"id":"combat_tick_1_hp_90_40","label":"v0.251 tick 1","action":"v0251_combat_tick_1"},
			{"id":"combat_tick_2_hp_80_20","label":"v0.251 tick 2","action":"v0251_combat_tick_2"},
			{"id":"combat_tick_3_raider_hp_zero_militia_70","label":"v0.251 tick 3","action":"v0251_combat_tick_3"},
			{"id":"raider_defeated_or_removed","label":"v0.251 defeated","action":"v0251_raider_defeated"},
			{"id":"pressure_contained_by_attack_order","label":"v0.251 defended","action":"v0251_pressure_contained"},
			{"id":"field_barracks_unharmed_defended_branch_hp_200","label":"v0.251 Barracks unharmed","action":"v0251_barracks_unharmed"},
			{"id":"no_resource_mutation_after_defended_combat","label":"v0.251 defended resources","action":"v0251_defended_no_resource_mutation"},
			{"id":"undefended_branch_start","label":"v0.251 undefended","action":"v0251_undefended_start"},
			{"id":"undefended_raider_spawned_once","label":"v0.251 undefended Raider","action":"v0251_undefended_raider_spawned"},
			{"id":"undefended_raider_advancing_to_barracks","label":"v0.251 advancing","action":"v0251_undefended_advancing"},
			{"id":"undefended_barracks_pressure_contact","label":"v0.251 contact","action":"v0251_undefended_contact"},
			{"id":"barracks_damage_tick_1_hp_175","label":"v0.251 damage 1","action":"v0251_building_tick_1"},
			{"id":"barracks_damage_tick_2_hp_150","label":"v0.251 damage 2","action":"v0251_building_tick_2"},
			{"id":"barracks_damage_tick_3_hp_125","label":"v0.251 damage 3","action":"v0251_building_tick_3"},
			{"id":"barracks_damaged_but_standing","label":"v0.251 damaged","action":"v0251_barracks_damaged"},
			{"id":"undefended_pressure_damage_contained","label":"v0.251 bounded stop","action":"v0251_undefended_contained"},
			{"id":"no_resource_mutation_after_building_damage","label":"v0.251 resources","action":"v0251_undefended_no_resource_mutation"},
			{"id":"aster_worker_unharmed_proof","label":"v0.251 units preserved","action":"v0251_aster_worker_unharmed"},
			{"id":"existing_barracks_train_flow_preserved","label":"v0.251 Barracks preserved","action":"v0251_preserve_barracks"},
			{"id":"command_keep_preserved","label":"v0.251 Keep preserved","action":"v0251_preserve_keep"},
			{"id":"lume_mine_preserved","label":"v0.251 Mine preserved","action":"v0251_preserve_mine"},
			{"id":"shells_remain_non_producing","label":"v0.251 shells","action":"v0251_preserve_shells"},
			{"id":"default_runtime_clean_after_opt_in_work","label":"v0.251 default clean","action":"v0251_default_clean"},
			{"id":"unselected_clean_view","label":"v0.251 clean","action":"v0251_clean"},
		]
	...
	if _player_capture_checkpoint() == "v0.249":
		return [
			...
			{"id": "starting_resources", "label": "v0.249 starting resources", "action": "v0249_starting_resources"},
			{"id": "select_builder_unit", "label": "v0.249 select Worker", "action": "v0249_select_builder"},
			{"id": "valid_barracks_preview", "label": "v0.249 Barracks preview", "action": "v0249_valid_preview"},
			{"id": "confirm_authoritative_barracks_placement", "label": "v0.249 Barracks placement", "action": "v0249_confirm_placement"},
			{"id": "construction_resource_delta", "label": "v0.249 construction delta", "action": "v0249_construction_delta"},
			{"id": "select_new_field_barracks_hud", "label": "v0.249 Field Barracks HUD", "action": "v0249_select_field_barracks"},
			{"id": "train_militia_command_available", "label": "v0.249 Train Militia", "action": "v0249_train_command"},
			{"id": "train_militia_resource_delta", "label": "v0.249 training delta", "action": "v0249_train_militia"},
			{"id": "militia_training_progress", "label": "v0.249 training progress", "action": "v0249_training_progress"},
			{"id": "militia_ready_spawned", "label": "v0.249 Militia ready", "action": "v0249_militia_ready"},
			{"id": "select_spawned_militia_hp_100", "label": "v0.249 Militia HP 100", "action": "v0249_select_militia"},
			{"id": "pressure_telegraph_marker_preserved", "label": "v0.249 telegraph", "action": "v0249_pressure_telegraph"},
			{"id": "intercept_zone_marker_preserved", "label": "v0.249 intercept marker", "action": "v0249_intercept_marker"},
			{"id": "ashen_pressure_incoming_objective", "label": "v0.249 pressure incoming", "action": "v0249_pressure_incoming"},
			{"id": "ashen_raider_spawned_once_hp_full", "label": "v0.249 Raider HP 60", "action": "v0249_raider_spawned"},
			{"id": "ashen_raider_minimap_presence", "label": "v0.249 Raider minimap", "action": "v0249_raider_minimap"},
			{"id": "ashen_raider_advancing_readable_timing", "label": "v0.249 Raider advancing", "action": "v0249_raider_advancing"},
			{"id": "militia_moves_to_intercept", "label": "v0.249 Militia intercept", "action": "v0249_militia_move"},
			{"id": "combat_contact_begins", "label": "v0.249 combat contact", "action": "v0249_combat_contact"},
			{"id": "combat_tick_1_hp_delta", "label": "v0.249 combat tick 1", "action": "v0249_combat_tick_1"},
			{"id": "combat_tick_2_hp_delta", "label": "v0.249 combat tick 2", "action": "v0249_combat_tick_2"},
			{"id": "raider_defeated_hp_zero", "label": "v0.249 Raider defeated", "action": "v0249_raider_defeated"},
			{"id": "raider_removed_or_defeated_state", "label": "v0.249 Raider removed", "action": "v0249_raider_defeated_state"},
			{"id": "raider_minimap_removed_or_defeated", "label": "v0.249 Raider minimap removed", "action": "v0249_raider_minimap_removed"},
			{"id": "militia_survives_after_combat", "label": "v0.249 Militia survives", "action": "v0249_militia_survives"},
			{"id": "pressure_contained_by_combat_status", "label": "v0.249 contained by combat", "action": "v0249_pressure_contained"},
			{"id": "no_resource_mutation_after_combat", "label": "v0.249 no resource mutation", "action": "v0249_no_resource_mutation"},
			{"id": "no_building_damage_proof", "label": "v0.249 no building damage", "action": "v0249_no_building_damage"},
			{"id": "aster_worker_unharmed_proof", "label": "v0.249 Aster Worker unharmed", "action": "v0249_aster_worker_unharmed"},
			{"id": "existing_barracks_train_flow_preserved", "label": "v0.249 Restored Barracks", "action": "v0249_preserve_barracks"},
			{"id": "command_keep_preserved", "label": "v0.249 Command Keep", "action": "v0249_preserve_keep"},
			{"id": "lume_mine_preserved", "label": "v0.249 Lume Mine", "action": "v0249_preserve_mine"},
			{"id": "shells_remain_non_producing", "label": "v0.249 shells", "action": "v0249_preserve_shells"},
			{"id": "unselected_clean_view", "label": "v0.249 clean view", "action": "v0249_clean"},
		]
	...
	if _player_capture_checkpoint() == "v0.245":
		return [
			...
			{"id": "starting_resources", "label": "v0.245 starting resources", "action": "v0245_starting_resources"},
			{"id": "select_builder_unit", "label": "v0.245 select Worker builder", "action": "v0245_select_builder"},
			{"id": "valid_preview_before_cancel", "label": "v0.245 valid preview before cancel", "action": "v0245_valid_preview_cancel"},
			{"id": "cancel_preview_resources_unchanged", "label": "v0.245 cancel preview", "action": "v0245_cancel_preview"},
			{"id": "blocked_preview_real_reason", "label": "v0.245 blocked preview", "action": "v0245_blocked_preview"},
			{"id": "blocked_attempt_no_resource_mutation", "label": "v0.245 blocked placement attempt", "action": "v0245_blocked_attempt"},
			{"id": "valid_preview_before_confirm", "label": "v0.245 valid preview before confirm", "action": "v0245_valid_preview_confirm"},
			{"id": "confirm_real_placement", "label": "v0.245 confirm real placement", "action": "v0245_confirm_placement"},
			{"id": "resource_delta_after_real_placement", "label": "v0.245 exact resource delta", "action": "v0245_resource_delta"},
			{"id": "new_structure_registered_selected", "label": "v0.245 new structure selected", "action": "v0245_select_constructed"},
			{"id": "new_structure_minimap_presence", "label": "v0.245 minimap registration", "action": "v0245_minimap"},
			{"id": "command_keep_live_hud_preserved", "label": "v0.245 Command Keep preserved", "action": "v0245_preserve_keep"},
			{"id": "barracks_live_hud_and_train_preserved", "label": "v0.245 Restored Barracks preserved", "action": "v0245_preserve_barracks"},
			{"id": "lume_mine_live_hud_preserved", "label": "v0.245 Lume Mine preserved", "action": "v0245_preserve_mine"},
			{"id": "shell_forge_market_watchtower_preserved", "label": "v0.245 shells preserved", "action": "v0245_preserve_shells"},
			{"id": "unit_pathing_near_new_structure", "label": "v0.245 new structure pathing probe", "action": "v0245_pathing"},
			{"id": "unselected_clean_view", "label": "v0.245 clean view", "action": "v0245_clean"},
		]
...
func _barrosan_runtime_scene_requested() -> bool:
	return _barrosan_runtime_skin_requested() or _player_capture_checkpoint() in ["v0.254", "v0.255", "v0.256", "v0.257", "v0.258", "v0.259", "v0.261", "v0.262", "v0.263", "v0.264", "v0.265", "v0.266", "v0.267", "v0.268", "v0.269", "v0.270", "v0.271", "v0.272", "v0.273", "v0.274", "v0.275", "v0.276", "v0.277", "v0.278", "v0.279", "v0.280"]
