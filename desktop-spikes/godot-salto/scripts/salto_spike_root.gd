...
func _live_ui_shell_state() -> Dictionary:
	...
	if action.begins_with("v0280_") or action.begins_with("v0281_"):
		var v0281 := action.begins_with("v0281_")
		...
		var v0280_cleared := action in ["v0280_clear_guard_removes_commit_label", "v0281_clear_guard_removes_commit_label_real_hud_clean"]
		var v0280_reguard := action in ["v0280_reguard_availability_clean", "v0281_reguard_availability_clean_real_hud"]
		var v0280_no_commit_entity := action in ["v0280_watchpost_no_engage_commit_action", "v0280_barracks_no_engage_commit_action", "v0280_default_runtime_unchanged_probe", "v0281_watchpost_no_engage_commit_action_real_hud", "v0281_barracks_no_engage_commit_action_real_hud", "v0281_default_runtime_unchanged_probe"]
		active_tab = "TRAIN"
		...
		if v0280_committed:
			...
			selected_subtitle = "Engagement committed | Bridge held | Pressure checked 80/100 | Commit locked"
			...
			alert_severity = "info"
			events = ["Engagement committed", "Bridge held", "Pressure checked 80/100", "Commit locked"]
		elif v0280_cleared:
			selected_title = "Militia Defender | East bridge" if v0281 else selected_title
			selected_subtitle = "Guard cleared | Engagement stance ended | Pressure checked 80/100"
			...
			alert_severity = "info"
			events = ["Guard cleared", "Commit label removed", "Pressure remains 80/100"]
		elif v0280_reguard:
			selected_title = "Militia Defender | East bridge" if v0281 else selected_title
			selected_subtitle = "Reguard available | Pressure checked 80/100 | Commit locked"
			...
			selected_title = ("Watchpost | Passive awareness" if action == "v0281_watchpost_no_engage_commit_action_real_hud" else ("Field Barracks | Production" if action == "v0281_barracks_no_engage_commit_action_real_hud" else "Default runtime probe")) if v0281 else "No Commit Target"
			selected_subtitle = ("Detection and advisory only" if action == "v0281_watchpost_no_engage_commit_action_real_hud" else ("Train Militia source only" if action == "v0281_barracks_no_engage_commit_action_real_hud" else "Opt-in Barrosan skin only")) if v0281 else "No Engage/Commit action | Default-safe probe"
			command_hint = ("Observe" if action == "v0281_watchpost_no_engage_commit_action_real_hud" else ("Train Militia" if action == "v0281_barracks_no_engage_commit_action_real_hud" else "Default safe")) if v0281 else "No Commit"
			status_pip = "SAFE"
			tooltip = "Watchpost, Barracks and default probes do not receive Engage or Commit actions."
			objective = ("Passive role" if action == "v0281_watchpost_no_engage_commit_action_real_hud" else ("Production role" if action == "v0281_barracks_no_engage_commit_action_real_hud" else "Default runtime unchanged")) if v0281 else "No Commit action"
		...
		selection_panel_data["commands"] = [
			_v0210_command_spec("Commit locked" if (v0280_committed or v0280_reguard) else (("Observe" if action == "v0281_watchpost_no_engage_commit_action_real_hud" else ("Train" if action == "v0281_barracks_no_engage_commit_action_real_hud" else ("Ready" if action == "v0281_default_runtime_unchanged_probe" else "No Commit"))) if v0280_no_commit_entity else "Commit Engage"), "C", tooltip, "disabled" if (v0280_committed or v0280_reguard or v0280_no_commit_entity) else "ready", "_on_live_ui_shell_attack_pressed"),
...
func _apply_player_slice_action(action: String) -> Dictionary:
	...
	if action == "production_build":
		...
		player_ui_shell_production_tab_override = "TRAIN"
		player_ui_shell_production_focus_hint = "train_militia"
	if action.begins_with("v0271_") or action.begins_with("v0272_") or action.begins_with("v0273_") or action.begins_with("v0274_") or action.begins_with("v0275_") or action.begins_with("v0276_") or action.begins_with("v0277_") or action.begins_with("v0278_") or action.begins_with("v0279_") or action.begins_with("v0280_") or action.begins_with("v0281_"):
