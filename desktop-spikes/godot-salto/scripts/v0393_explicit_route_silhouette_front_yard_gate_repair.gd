...
## v0.393 is an isolated, opt-in explicit-route and front-yard-gate readability repair. It preserves the accepted
## crossing infrastructure and all gameplay/default-runtime semantics.

const V0388_CHECKPOINT := "v0.393"
const V0388_CAPTURE_ROOT := "artifacts/runtime/v0393"
...
func _ready() -> void:
	_read_v0388_args()
	v0388_capture_mode = v0388_capture_mode or bool(get_meta("v0393_capture_mode", false))
	v0388_smoke_mode = v0388_smoke_mode or bool(get_meta("v0393_smoke_mode", false))
	_build_v0383_environment()
	var world_ok := _build_v0383_world()
	if not world_ok:
		get_tree().quit(1)
		return
	_build_v0383_camera()
...
func _read_v0388_args() -> void:
	for raw in OS.get_cmdline_args() + OS.get_cmdline_user_args():
		var arg := str(raw)
		if arg == "--v0393-explicit-route-capture": v0388_capture_mode = true
		if arg == "--v0393-explicit-route-smoke": v0388_smoke_mode = true
		if arg.begins_with("--artifact-root="): v0388_capture_root = arg.trim_prefix("--artifact-root=")
		if arg.begins_with("--v0393-iteration="): v0388_iteration = clampi(int(arg.trim_prefix("--v0393-iteration=")), 1, 4)
	if v0388_capture_mode: v0388_smoke_mode = true

func _build_v0388_cluster() -> void:
	v0388_cluster = Node3D.new()
	v0388_cluster.name = "V0393_Explicit_Route_Silhouette_Front_Yard_Gate_Repair"
	v0388_cluster.position = Vector3(-2.2, 0.0, 1.10)
	v0383_world.add_child(v0388_cluster)
	v0388_yard = Node3D.new()
	v0388_yard.name = "V0393_Five_Functional_Yard_Groups"
	v0388_cluster.add_child(v0388_yard)
	v0388_props = Node3D.new()
	v0388_props.name = "V0393_Direct_Grounded_Yard_Functions"
...
func _build_v0388_primary() -> void:
	var primary := _instantiate_asset(V0388_PRIMARY_ASSET, v0388_cluster, "V0393_One_Primary_Barrosan_Homestead", Vector3(-5.85, 0.66, 1.15), Vector3.ONE * 0.40, 0.0)
	if primary:
...
func _build_v0388_subordinate() -> void:
	var subordinate := _instantiate_asset(V0388_SUBORDINATE_ASSET, v0388_cluster, "V0393_One_Subordinate_Agricultural_Barn_Separate", Vector3(-8.90, 0.42, 4.50), Vector3.ONE * 0.22, deg_to_rad(-42.0))
	if subordinate:
...
func _subdue_v0391_bridge_value(root: Node) -> void:
	for child in root.get_children():
		...
		if child is MeshInstance3D and child_name.contains("bridge"):
			var bridge_material := _ground_material("V0393_Bridge_Desaturated_Value", Color("#655b50"))
			bridge_material.roughness = 0.98
			(child as MeshInstance3D).material_override = bridge_material
			child.set_meta("v0393_bridge_value_reduced", true)
...
func _build_v0388_ground_connection() -> void:
	var yard_patch := PackedVector3Array([
		Vector3(-11.4, 0.0, -0.20), Vector3(-4.2, 0.0, -0.12), Vector3(-2.8, 0.0, 2.20),
		Vector3(-3.7, 0.0, 5.65), Vector3(-9.6, 0.0, 6.75), Vector3(-12.0, 0.0, 3.05)
	])
	_ground_patch(v0388_yard, "V0393_Flush_Irregular_Contrasting_Worn_Earth_Yard", yard_patch, Color("#765038"), 0.80)
	var path := Node3D.new()
	path.name = "V0393_Road_Yard_Entrance_Connection"
	...
	_ground_ribbon(path, "V0393_One_Flush_Contrasting_Bridge_Yard_Door_Route", centerline, 2.72, Color("#a8663d"), 1.02)
	var barn_branch := PackedVector3Array([
		Vector3(-3.90, 0.0, 2.60), Vector3(-5.05, 0.0, 3.32), Vector3(-6.38, 0.0, 3.78), Vector3(-8.28, 0.0, 4.18)
	])
	_ground_ribbon(path, "V0393_Short_Flush_Contrasting_Barn_Entrance_Branch", barn_branch, 1.42, Color("#a8663d"), 1.025)
	var segment_names := [
		"V0393_Connection_Segment_A",
		"V0393_Connection_Segment_B",
		"V0393_Connection_Segment_C",
		"V0393_Connection_Segment_D",
		"V0393_Connection_Segment_E_Door_Approach"
	]
	...
	_box_prop(path, "V0393_Door_Threshold_Stone", Vector3(-5.98, 1.04, 1.08), Vector3(1.35, 0.12, 0.42), Color("#5d4b38"), deg_to_rad(-18.0))

func _build_v0388_yard_groups() -> void:
	var cart := Node3D.new(); cart.name = "V0393_Yard_Group_01_Cart_Direct_Ground"; v0388_props.add_child(cart)
	...
	var lumber := Node3D.new(); lumber.name = "V0393_Yard_Group_02_Stacked_Firewood_Direct_Ground"; v0388_props.add_child(lumber)
	for i in range(3):
	...
	var storage := Node3D.new(); storage.name = "V0393_Yard_Group_03_Barrel_Crate_Storage_Direct_Ground"; v0388_props.add_child(storage)
	...
	var work := Node3D.new(); work.name = "V0393_Yard_Group_04_Trough_Workbench_Direct_Ground"; v0388_props.add_child(work)
	...
	var boundary := Node3D.new(); boundary.name = "V0393_Yard_Group_05_Coherent_Fence_With_Open_Gate"; v0388_props.add_child(boundary)
	_box_prop(boundary, "V0393_Front_Fence_Run_Left", Vector3(-10.05, 1.00, 5.18), Vector3(2.25, 0.78, 0.16), Color("#5a3d2b"), deg_to_rad(0.0))
	_box_prop(boundary, "V0393_Front_Fence_Run_Right", Vector3(-4.85, 1.00, 5.18), Vector3(1.85, 0.78, 0.16), Color("#5a3d2b"), deg_to_rad(0.0))
	_box_prop(boundary, "V0393_Open_Gate_Post_Left", Vector3(-8.55, 1.05, 5.18), Vector3(0.18, 1.18, 0.18), Color("#3f2d22"))
	_box_prop(boundary, "V0393_Open_Gate_Post_Right", Vector3(-6.85, 1.05, 5.18), Vector3(0.18, 1.18, 0.18), Color("#3f2d22"))
...
func _capture_v0388_sequence() -> void:
	...
	var primary := await _capture_v0388_view("01_PRIMARY_RTS_VIEW.png", Vector3(24.0, 31.0, 24.0), Vector3(-5.3, 0.90, 3.05), 19.0, root)
	...
	_write_v0388_json("v0393-explicit-route-silhouette-front-yard-gate-repair.json", {
...
func _smoke_v0388_exit() -> void:
	...
	_write_v0388_json("v0393-explicit-route-silhouette-front-yard-gate-repair-smoke.json", {"checkpoint": V0388_CHECKPOINT, "status": "PASS", "sceneLoaded": true, "acceptedInfrastructureUnchanged": true, "gameplay": false, "defaultRuntime": "unchanged", "route": "opt-in-only"})
	get_tree().quit(0)
