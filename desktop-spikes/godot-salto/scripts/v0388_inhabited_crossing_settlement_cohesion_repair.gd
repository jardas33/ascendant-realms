...
## v0.388 is an isolated, opt-in route-hierarchy repair. It preserves the accepted
## crossing infrastructure and all gameplay/default-runtime semantics.

const V0388_CHECKPOINT := "v0.388"
const V0388_CAPTURE_ROOT := "artifacts/runtime/v0388"
...
func _read_v0388_args() -> void:
	for raw in OS.get_cmdline_args() + OS.get_cmdline_user_args():
		var arg := str(raw)
		if arg == "--v0388-inhabited-crossing-capture": v0388_capture_mode = true
		if arg == "--v0388-inhabited-crossing-smoke": v0388_smoke_mode = true
		if arg.begins_with("--artifact-root="): v0388_capture_root = arg.trim_prefix("--artifact-root=")
		if arg.begins_with("--v0388-iteration="): v0388_iteration = clampi(int(arg.trim_prefix("--v0388-iteration=")), 1, 4)
	if v0388_capture_mode: v0388_smoke_mode = true

func _build_v0388_cluster() -> void:
	v0388_cluster = Node3D.new()
	v0388_cluster.name = "V0388_Inhabited_Crossing_Settlement_Cohesion_Repair"
	v0388_cluster.position = Vector3(-2.2, 0.0, 1.10)
	v0383_world.add_child(v0388_cluster)
	v0388_yard = Node3D.new()
	v0388_yard.name = "V0388_Five_Functional_Yard_Groups"
	v0388_cluster.add_child(v0388_yard)
	v0388_props = Node3D.new()
	v0388_props.name = "V0388_Visible_Curated_Yard_Props"
...
func _build_v0388_primary() -> void:
	var primary := _instantiate_asset(V0388_PRIMARY_ASSET, v0388_cluster, "V0388_One_Primary_Barrosan_Homestead", Vector3(-5.85, 0.66, 1.15), Vector3.ONE * 0.40, 0.0)
	if primary:
...
func _build_v0388_subordinate() -> void:
	var subordinate := _instantiate_asset(V0388_SUBORDINATE_ASSET, v0388_cluster, "V0388_One_Subordinate_Agricultural_Barn", Vector3(-9.55, 0.32, 4.65), Vector3.ONE * 0.24, deg_to_rad(12.0))
	if subordinate:
...
func _build_v0388_ground_connection() -> void:
	...
	_ground_patch(v0388_yard, "V0388_Working_Yard_Ground", yard_patch, Color("#927b5b"), 0.74)
	var path := Node3D.new()
	...
	_ground_ribbon(path, "V0388_Continuous_Bridge_Yard_Door_Route", centerline, 1.05, path_color, 0.80)
	var segment_names := [
		"V0388_Connection_Segment_A",
		"V0388_Connection_Segment_B",
		"V0388_Connection_Segment_C",
		"V0388_Connection_Segment_D",
		"V0388_Connection_Segment_E_Door_Approach"
	]
...
func _build_v0388_yard_groups() -> void:
	var cart := Node3D.new(); cart.name = "V0388_Yard_Group_01_Cart"; v0388_props.add_child(cart)
	_instantiate_asset(V0388_QUATERNIUS_ROOT + "village/Prop_Wagon.gltf", cart, "Cart_Visible", Vector3(-3.05, 0.84, 4.10), Vector3.ONE * 0.60, deg_to_rad(-8.0))
	var lumber := Node3D.new(); lumber.name = "V0388_Yard_Group_02_Stacked_Firewood"; v0388_props.add_child(lumber)
	for i in range(3):
		_instantiate_asset(V0388_QUATERNIUS_ROOT + "props/Crate_Wooden.gltf", lumber, "Lumber_Crate_%d" % i, Vector3(-9.12 + i * 0.58, 0.84 + (i % 2) * 0.18, 4.42), Vector3.ONE * 0.62, deg_to_rad(4.0 * i))
	var storage := Node3D.new(); storage.name = "V0388_Yard_Group_03_Barrel_Crate_Storage"; v0388_props.add_child(storage)
	_instantiate_asset(V0388_QUATERNIUS_ROOT + "props/Barrel_Apples.gltf", storage, "Storage_Barrel", Vector3(-1.72, 1.00, 5.05), Vector3.ONE * 0.66, 0.18)
	_instantiate_asset(V0388_QUATERNIUS_ROOT + "props/Crate_Wooden.gltf", storage, "Storage_Crate", Vector3(-0.88, 0.88, 5.02), Vector3.ONE * 0.58, -0.10)
	var work := Node3D.new(); work.name = "V0388_Yard_Group_04_Trough_Workbench"; v0388_props.add_child(work)
	_instantiate_asset(V0388_QUATERNIUS_ROOT + "props/Whetstone.gltf", work, "Workstone", Vector3(-5.05, 0.86, 3.78), Vector3.ONE * 0.78, 0.0)
	_instantiate_asset(V0388_QUATERNIUS_ROOT + "village/Prop_Crate.gltf", work, "Workbench_Block", Vector3(-4.38, 0.88, 3.82), Vector3.ONE * 0.58, 0.16)
	var boundary := Node3D.new(); boundary.name = "V0388_Yard_Group_05_Fence_Gate"; v0388_props.add_child(boundary)
	_instantiate_asset(V0388_QUATERNIUS_ROOT + "village/Prop_WoodenFence_Extension1.gltf", boundary, "Fence_Left", Vector3(-8.92, 0.85, 2.62), Vector3(0.86, 0.68, 0.68), deg_to_rad(-8.0))
	_instantiate_asset(V0388_QUATERNIUS_ROOT + "village/Prop_WoodenFence_Single.gltf", boundary, "Fence_Right", Vector3(-7.62, 0.85, 2.68), Vector3(0.86, 0.68, 0.68), deg_to_rad(-8.0))
...
func _build_v0388_characters() -> void:
	...
	var resident := _instantiate_asset(V0388_QUATERNIUS_ROOT + "men/Farmer.gltf", v0388_humans, "V0388_Resident_Worker", Vector3(-5.35, 0.78, 3.45), Vector3.ONE * 1.10, deg_to_rad(20.0))
	var guard := _instantiate_asset(V0388_QUATERNIUS_ROOT + "men/Adventurer.gltf", v0388_humans, "V0388_Crossing_Guard", Vector3(-0.95, 0.78, 1.60), Vector3.ONE * 1.06, deg_to_rad(-22.0))
	var traveller := _instantiate_asset(V0388_QUATERNIUS_ROOT + "men/Worker.gltf", v0388_humans, "V0388_Traveller_Porter", Vector3(-3.95, 0.90, 3.42), Vector3.ONE * 1.08, deg_to_rad(155.0))
...
func _capture_v0388_sequence() -> void:
	...
	var primary := await _capture_v0388_view("01_PRIMARY_RTS_VIEW.png", Vector3(24.0, 31.0, 24.0), Vector3(-5.1, 0.80, 2.85), 23.0, root)
	...
	_write_v0388_json("v0388-inhabited-crossing-settlement-cohesion-repair.json", {
...
func _smoke_v0388_exit() -> void:
	...
	_write_v0388_json("v0388-inhabited-crossing-settlement-cohesion-repair-smoke.json", {"checkpoint": V0388_CHECKPOINT, "status": "PASS", "sceneLoaded": true, "acceptedInfrastructureUnchanged": true, "gameplay": false, "defaultRuntime": "unchanged", "route": "opt-in-only"})
	get_tree().quit(0)
