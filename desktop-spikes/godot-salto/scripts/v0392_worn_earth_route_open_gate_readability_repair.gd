...
## v0.392 is an isolated, opt-in worn-earth-route and open-gate readability repair. It preserves the accepted
## crossing infrastructure and all gameplay/default-runtime semantics.

const V0388_CHECKPOINT := "v0.392"
const V0388_CAPTURE_ROOT := "artifacts/runtime/v0392"
...
func _ready() -> void:
	...
	inhabited_fill.name = "V0392_Local_Settlement_Contrast_Fill"
	...
	homestead_fill.name = "V0392_Warm_Homestead_Fill"
	...
	bridge_control.name = "V0392_Bridge_Value_Control"
...
func _read_v0388_args() -> void:
	for raw in OS.get_cmdline_args() + OS.get_cmdline_user_args():
		var arg := str(raw)
		if arg == "--v0392-worn-earth-capture": v0388_capture_mode = true
		if arg == "--v0392-worn-earth-smoke": v0388_smoke_mode = true
		if arg.begins_with("--artifact-root="): v0388_capture_root = arg.trim_prefix("--artifact-root=")
		if arg.begins_with("--v0392-iteration="): v0388_iteration = clampi(int(arg.trim_prefix("--v0392-iteration=")), 1, 4)
	if v0388_capture_mode: v0388_smoke_mode = true

func _build_v0388_cluster() -> void:
	v0388_cluster = Node3D.new()
	v0388_cluster.name = "V0392_Worn_Earth_Route_Open_Gate_Readability_Repair"
	v0388_cluster.position = Vector3(-2.2, 0.0, 1.10)
	v0383_world.add_child(v0388_cluster)
	v0388_yard = Node3D.new()
	v0388_yard.name = "V0392_Five_Functional_Yard_Groups"
	v0388_cluster.add_child(v0388_yard)
	v0388_props = Node3D.new()
	v0388_props.name = "V0392_Direct_Grounded_Yard_Functions"
...
func _build_v0388_primary() -> void:
	var primary := _instantiate_asset(V0388_PRIMARY_ASSET, v0388_cluster, "V0392_One_Primary_Barrosan_Homestead", Vector3(-5.85, 0.66, 1.15), Vector3.ONE * 0.40, 0.0)
	if primary:
...
func _build_v0388_subordinate() -> void:
	var subordinate := _instantiate_asset(V0388_SUBORDINATE_ASSET, v0388_cluster, "V0392_One_Subordinate_Agricultural_Barn_Separate", Vector3(-8.90, 0.42, 4.50), Vector3.ONE * 0.22, deg_to_rad(-42.0))
	if subordinate:
...
func _subdue_v0391_bridge_value(root: Node) -> void:
	for child in root.get_children():
		...
		if child is MeshInstance3D and child_name.contains("bridge"):
			var bridge_material := _ground_material("V0392_Bridge_Desaturated_Value", Color("#655b50"))
			bridge_material.roughness = 0.98
			(child as MeshInstance3D).material_override = bridge_material
			child.set_meta("v0392_bridge_value_reduced", true)
...
func _build_v0388_ground_connection() -> void:
	...
	_ground_patch(v0388_yard, "V0392_Flush_Irregular_Contrasting_Worn_Earth_Yard", yard_patch, Color("#806044"), 0.78)
	var path := Node3D.new()
	path.name = "V0392_Road_Yard_Entrance_Connection"
	...
	_ground_ribbon(path, "V0392_One_Flush_Contrasting_Bridge_Yard_Door_Route", centerline, 1.92, Color("#4d3323"), 0.84)
	...
	_ground_ribbon(path, "V0392_Short_Flush_Contrasting_Barn_Entrance_Branch", barn_branch, 1.02, Color("#5b3c28"), 0.845)
	var segment_names := [
		"V0392_Connection_Segment_A",
		"V0392_Connection_Segment_B",
		"V0392_Connection_Segment_C",
		"V0392_Connection_Segment_D",
		"V0392_Connection_Segment_E_Door_Approach"
	]
	...
	_box_prop(path, "V0392_Door_Threshold_Stone", Vector3(-5.92, 0.86, 1.12), Vector3(1.35, 0.12, 0.42), Color("#5d4b38"), deg_to_rad(-18.0))

func _build_v0388_yard_groups() -> void:
	var cart := Node3D.new(); cart.name = "V0392_Yard_Group_01_Cart_Direct_Ground"; v0388_props.add_child(cart)
	...
	var lumber := Node3D.new(); lumber.name = "V0392_Yard_Group_02_Stacked_Firewood_Direct_Ground"; v0388_props.add_child(lumber)
	for i in range(3):
	...
	var storage := Node3D.new(); storage.name = "V0392_Yard_Group_03_Barrel_Crate_Storage_Direct_Ground"; v0388_props.add_child(storage)
	...
	var work := Node3D.new(); work.name = "V0392_Yard_Group_04_Trough_Workbench_Direct_Ground"; v0388_props.add_child(work)
	...
	var boundary := Node3D.new(); boundary.name = "V0392_Yard_Group_05_Coherent_Fence_With_Open_Gate"; v0388_props.add_child(boundary)
	_instantiate_asset(V0388_QUATERNIUS_ROOT + "village/Prop_WoodenFence_Extension1.gltf", boundary, "Fence_Left_Of_Open_Gate", Vector3(-10.10, 0.82, 5.55), Vector3(0.78, 0.64, 0.64), deg_to_rad(82.0))
	_instantiate_asset(V0388_QUATERNIUS_ROOT + "village/Prop_WoodenFence_Extension1.gltf", boundary, "Fence_Right_Of_Open_Gate", Vector3(-7.80, 0.82, 5.55), Vector3(0.78, 0.64, 0.64), deg_to_rad(82.0))
	_instantiate_asset(V0388_QUATERNIUS_ROOT + "village/Prop_WoodenFence_Single.gltf", boundary, "Open_Gate_Post_Left", Vector3(-8.86, 0.82, 5.55), Vector3(0.55, 0.72, 0.55), deg_to_rad(82.0))
...
func _capture_v0388_sequence() -> void:
	...
	_write_v0388_json("v0392-worn-earth-route-open-gate-readability-repair.json", {
...
func _smoke_v0388_exit() -> void:
	...
	_write_v0388_json("v0392-worn-earth-route-open-gate-readability-repair-smoke.json", {"checkpoint": V0388_CHECKPOINT, "status": "PASS", "sceneLoaded": true, "acceptedInfrastructureUnchanged": true, "gameplay": false, "defaultRuntime": "unchanged", "route": "opt-in-only"})
	get_tree().quit(0)
