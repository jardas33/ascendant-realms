...
## v0.394 is an isolated, opt-in route-only visibility repair. It rolls back the v0.393 fence regression and preserves the accepted
## crossing infrastructure and all gameplay/default-runtime semantics.

const V0388_CHECKPOINT := "v0.394"
const V0388_CAPTURE_ROOT := "artifacts/runtime/v0394"
...
func _ready() -> void:
	_read_v0388_args()
	v0388_capture_mode = v0388_capture_mode or bool(get_meta("v0394_capture_mode", false))
	v0388_smoke_mode = v0388_smoke_mode or bool(get_meta("v0394_smoke_mode", false))
	...
	inhabited_fill.name = "V0394_Local_Settlement_Contrast_Fill"
	...
	homestead_fill.name = "V0394_Warm_Homestead_Fill"
	...
	bridge_control.name = "V0394_Bridge_Value_Control"
...
func _read_v0388_args() -> void:
	for raw in OS.get_cmdline_args() + OS.get_cmdline_user_args():
		var arg := str(raw)
		if arg == "--v0394-route-only-capture": v0388_capture_mode = true
		if arg == "--v0394-route-only-smoke": v0388_smoke_mode = true
		if arg.begins_with("--artifact-root="): v0388_capture_root = arg.trim_prefix("--artifact-root=")
		if arg.begins_with("--v0394-iteration="): v0388_iteration = clampi(int(arg.trim_prefix("--v0394-iteration=")), 1, 4)
	if v0388_capture_mode: v0388_smoke_mode = true

func _build_v0388_cluster() -> void:
	v0388_cluster = Node3D.new()
	v0388_cluster.name = "V0394_Route_Only_Visibility_Repair_Fence_Regression_Rollback"
	v0388_cluster.position = Vector3(-2.2, 0.0, 1.10)
	v0383_world.add_child(v0388_cluster)
	v0388_yard = Node3D.new()
	v0388_yard.name = "V0394_Five_Functional_Yard_Groups"
	v0388_cluster.add_child(v0388_yard)
	v0388_props = Node3D.new()
	v0388_props.name = "V0394_Direct_Grounded_Yard_Functions"
...
func _build_v0388_primary() -> void:
	var primary := _instantiate_asset(V0388_PRIMARY_ASSET, v0388_cluster, "V0394_One_Primary_Barrosan_Homestead", Vector3(-5.85, 0.66, 1.15), Vector3.ONE * 0.40, 0.0)
	if primary:
...
func _build_v0388_subordinate() -> void:
	var subordinate := _instantiate_asset(V0388_SUBORDINATE_ASSET, v0388_cluster, "V0394_One_Subordinate_Agricultural_Barn_Separate", Vector3(-8.90, 0.42, 4.50), Vector3.ONE * 0.22, deg_to_rad(-42.0))
	if subordinate:
...
func _ground_material(name: String, color: Color, unshaded := false) -> StandardMaterial3D:
...
func _ground_ribbon(parent: Node3D, name: String, centerline: PackedVector3Array, width: float, color: Color, y := 0.80) -> MeshInstance3D:
	...
	instance.material_override = _ground_material(name + "_Material", color, true)
	instance.position.y = y
...
func _subdue_v0391_bridge_value(root: Node) -> void:
	for child in root.get_children():
		...
		if child is MeshInstance3D and child_name.contains("bridge"):
			var bridge_material := _ground_material("V0394_Bridge_Desaturated_Value", Color("#655b50"))
			bridge_material.roughness = 0.98
			(child as MeshInstance3D).material_override = bridge_material
			child.set_meta("v0394_bridge_value_reduced", true)
...
func _build_v0388_ground_connection() -> void:
	var path := Node3D.new()
	path.name = "V0394_One_Flush_Continuous_Worn_Earth_Route_Only"
	...
	_ground_ribbon(path, "V0394_One_Flush_Contrasting_Bridge_Yard_Door_Route", centerline, 2.15, Color("#9a613b"), 1.12)
	...
	_ground_ribbon(path, "V0394_Short_Flush_Contrasting_Barn_Entrance_Branch", barn_branch, 1.15, Color("#9a613b"), 1.125)
	var segment_names := [
		"V0394_Connection_Segment_A",
		"V0394_Connection_Segment_B",
		"V0394_Connection_Segment_C",
		"V0394_Connection_Segment_D",
		"V0394_Connection_Segment_E_Door_Approach"
	]
	...
	_box_prop(path, "V0394_Door_Threshold_Stone", Vector3(-5.98, 1.14, 1.08), Vector3(1.35, 0.12, 0.42), Color("#5d4b38"), deg_to_rad(-18.0))

func _build_v0388_yard_groups() -> void:
	var cart := Node3D.new(); cart.name = "V0394_Yard_Group_01_Cart_Direct_Ground"; v0388_props.add_child(cart)
	...
	var lumber := Node3D.new(); lumber.name = "V0394_Yard_Group_02_Stacked_Firewood_Direct_Ground"; v0388_props.add_child(lumber)
	for i in range(3):
	...
	var storage := Node3D.new(); storage.name = "V0394_Yard_Group_03_Barrel_Crate_Storage_Direct_Ground"; v0388_props.add_child(storage)
	...
	var work := Node3D.new(); work.name = "V0394_Yard_Group_04_Trough_Workbench_Direct_Ground"; v0388_props.add_child(work)
	...
	var boundary := Node3D.new(); boundary.name = "V0394_Yard_Group_05_Fence_Regression_Rolled_Back"; v0388_props.add_child(boundary)
	boundary.set_meta("v0393_fence_geometry_removed", true)
...
func _capture_v0388_sequence() -> void:
	...
	_write_v0388_json("v0394-route-only-visibility-repair-fence-regression-rollback.json", {
...
func _smoke_v0388_exit() -> void:
	...
	_write_v0388_json("v0394-route-only-visibility-repair-fence-regression-rollback-smoke.json", {"checkpoint": V0388_CHECKPOINT, "status": "PASS", "sceneLoaded": true, "acceptedInfrastructureUnchanged": true, "gameplay": false, "defaultRuntime": "unchanged", "route": "opt-in-only"})
	get_tree().quit(0)
