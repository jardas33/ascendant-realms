...
const CHECKER_TEXTURE := "res://assets/v0331/numbered_square_checker.png"
...
func start() -> void:
	...
	_build_sector()
	_load_house()
	if OS.get_environment("V0331_BENCHMARK_ONLY") == "1":
...
func _build_environment() -> void:
	...
	world.name = "V0331House02DocumentaryWorld"
	world.environment = environment
	add_child(world)
	var sun := DirectionalLight3D.new()
	sun.name = "V0331ConsistentWarmKey"
...
func _apply_house_material_language() -> void:
	for node in house.find_children("*", "MeshInstance3D", true, false):
		# v0.331 keeps the imported GLB PBR maps visible. The old broad tint
		# override was the source of the beige/clay read rejected by review.
...
func _shader_checker() -> ShaderMaterial:
	...
	material.set_shader_parameter("checker_tex", load(CHECKER_TEXTURE))
	return material
...
func _capture_views() -> void:
	...
	await _capture("roof_eave_front.png", Vector3(-5.0, 2.8, 8.0), Vector3(-2.6, 4.0, -2.8), 5.0, "front_eave_underside")
	await _capture("roof_eave_rear.png", Vector3(0.0, 2.8, -10.0), Vector3(-2.6, 4.0, 0.0), 5.0, "rear_eave_underside")
	...
	await _capture("collision_overview.png", Vector3(-16.0, 12.0, 18.0), Vector3(0.5, 2.0, 0.0), 17.0, "collision")
	_set_fragment_visible("COLLISION", false)
	get_viewport().debug_draw = Viewport.DEBUG_DRAW_WIREFRAME
	await _capture("wireframe_lod0.png", Vector3(-10.0, 8.0, 12.0), Vector3(-2.6, 2.8, -1.5), 10.0, "wireframe")
	get_viewport().debug_draw = Viewport.DEBUG_DRAW_DISABLED


func _capture(file_name: String, camera_position: Vector3, target: Vector3, ortho_size: float, camera_mode: String = "oblique_rts") -> void:
	...
	if image.save_png(path) != OK:
		...
		captures.append({"fileName": file_name, "path": path, "width": image.get_width(), "height": image.get_height(), "cameraMode": camera_mode, "projection": "orthographic"})
...
func _capture_turntable() -> void:
	for index in range(CAPTURE_FRAMES):
		...
		var angle := phase * TAU
		var near_section := index < 72
		camera.position = Vector3(cos(angle) * (10.0 if near_section else 16.0), 7.0 if near_section else 9.0, sin(angle) * (10.0 if near_section else 16.0))
		camera.size = 9.0 if near_section else 17.0
		camera.look_at(Vector3(0.5, 2.0, 0.0), Vector3.UP)
...
func _write_manifest_v331() -> void:
	var file := FileAccess.open(capture_root.path_join("v0331-house02-review-runtime.json"), FileAccess.WRITE)
	if file != null:
		var manifest := {"schemaVersion": 2, "checkpoint": "v0.331", "status": "PASS_V0331_HOUSE02_DOCUMENTARY_ROOF_MATERIAL_CLOSURE" if errors.is_empty() else "FAIL_V0331_HOUSE02_DOCUMENTARY_ROOF_MATERIAL_CLOSURE", "outcome": "READY FOR HUMAN BARROSAN HOUSE 02 MATERIAL-AND-ROOF REVIEW" if errors.is_empty() else "REJECTED INTERNALLY - DOCUMENTARY, ROOF, MATERIAL OR EVIDENCE GATE FAILED", "prototypeOptIn": true, "sourceBlend": "art-source/blender/v0330/barrosan_house_gold_02.blend", "sourceGLB": GLB, "scenePath": "res://scenes/review/V0331BarrosanHouse02Review.tscn", "documentaryPath": "Path A - exterior-stair archetype", "documentaryRegister": "art-source/references/v0331/documentary/README.md", "moodTargetSeparated": true, "noGameplay": true, "noMovement": true, "noPathfinding": true, "noCombat": true, "noEconomy": true, "noResources": true, "house01Imported": false, "checkerUsesUV": true, "checkerScreenSpaceOverlay": false, "numberedSquareChecker": true, "mediaSegments": {"near": [0, 71], "rts": [72, 287]}, "captures": captures, "errors": errors}
...
func _artifact_root_from_args() -> String:
	...
	return OS.get_environment("V0331_ARTIFACT_ROOT")
