...
## v0.389 is an isolated, opt-in farmyard-readability repair. It preserves the accepted
## crossing infrastructure and all gameplay/default-runtime semantics.

const V0388_CHECKPOINT := "v0.389"
const V0388_CAPTURE_ROOT := "artifacts/runtime/v0389"
...
func _ready() -> void:
	...
	inhabited_fill.light_energy = 0.64
	...
	homestead_fill.light_energy = 5.8
...
func _read_v0388_args() -> void:
	for raw in OS.get_cmdline_args() + OS.get_cmdline_user_args():
		var arg := str(raw)
		if arg == "--v0389-inhabited-crossing-capture": v0388_capture_mode = true
		if arg == "--v0389-inhabited-crossing-smoke": v0388_smoke_mode = true
		if arg.begins_with("--artifact-root="): v0388_capture_root = arg.trim_prefix("--artifact-root=")
		if arg.begins_with("--v0389-iteration="): v0388_iteration = clampi(int(arg.trim_prefix("--v0389-iteration=")), 1, 4)
	if v0388_capture_mode: v0388_smoke_mode = true

func _build_v0388_cluster() -> void:
	v0388_cluster = Node3D.new()
	v0388_cluster.name = "V0389_Inhabited_Crossing_Farmyard_Readability_Repair"
	v0388_cluster.position = Vector3(-2.2, 0.0, 1.10)
	v0383_world.add_child(v0388_cluster)
	v0388_yard = Node3D.new()
	v0388_yard.name = "V0389_Five_Functional_Yard_Groups"
...
func _build_v0388_primary() -> void:
	var primary := _instantiate_asset(V0388_PRIMARY_ASSET, v0388_cluster, "V0389_One_Primary_Barrosan_Homestead", Vector3(-5.85, 0.66, 1.15), Vector3.ONE * 0.40, 0.0)
	if primary:
...
func _build_v0388_subordinate() -> void:
	var subordinate := _instantiate_asset(V0388_SUBORDINATE_ASSET, v0388_cluster, "V0389_One_Subordinate_Agricultural_Barn", Vector3(-8.55, 0.32, 3.35), Vector3.ONE * 0.23, deg_to_rad(12.0))
	if subordinate:
...
func _build_v0388_ground_connection() -> void:
	...
	_ground_ribbon(path, "V0389_Continuous_Bridge_Yard_Door_Route", centerline, 1.40, Color("#5f4632"), 1.03)
	_ground_ribbon(path, "V0389_Worn_Earth_Route_Inlay", centerline, 0.86, Color("#a27b51"), 1.065)
	var segment_names := [
		"V0389_Connection_Segment_A",
		"V0389_Connection_Segment_B",
		"V0389_Connection_Segment_C",
		"V0389_Connection_Segment_D",
		"V0389_Connection_Segment_E_Door_Approach"
	]
	...
	_box_prop(path, "V0389_Door_Threshold_Stone", Vector3(-5.92, 1.10, 1.12), Vector3(1.35, 0.12, 0.42), Color("#6d6252"), deg_to_rad(-18.0))

func _build_v0388_yard_groups() -> void:
	var cart := Node3D.new(); cart.name = "V0389_Yard_Group_01_Cart"; v0388_props.add_child(cart)
	...
	_box_prop(cart, "Cart_Function_Platform", Vector3(-3.00, 0.78, 4.35), Vector3(1.9, 0.08, 1.25), Color("#796044"), deg_to_rad(-8.0))
	var lumber := Node3D.new(); lumber.name = "V0389_Yard_Group_02_Stacked_Firewood"; v0388_props.add_child(lumber)
	for i in range(3):
	...
	_box_prop(lumber, "Firewood_Stack_Base", Vector3(-7.60, 0.78, 3.90), Vector3(2.25, 0.08, 0.95), Color("#765439"), deg_to_rad(5.0))
	var storage := Node3D.new(); storage.name = "V0389_Yard_Group_03_Barrel_Crate_Storage"; v0388_props.add_child(storage)
	...
	_box_prop(storage, "Storage_Stone_Base", Vector3(-1.38, 0.78, 4.90), Vector3(1.65, 0.08, 1.05), Color("#73614b"), deg_to_rad(-4.0))
	var work := Node3D.new(); work.name = "V0389_Yard_Group_04_Trough_Workbench"; v0388_props.add_child(work)
	...
	var boundary := Node3D.new(); boundary.name = "V0389_Yard_Group_05_Fence_Gate"; v0388_props.add_child(boundary)
	...
	_box_prop(boundary, "Gate_Threshold", Vector3(-7.98, 0.78, 2.62), Vector3(1.55, 0.08, 0.70), Color("#5e4b39"), deg_to_rad(-8.0))
...
func _capture_v0388_sequence() -> void:
	...
	_write_v0388_json("v0389-inhabited-crossing-farmyard-readability-repair.json", {
...
func _smoke_v0388_exit() -> void:
	...
	_write_v0388_json("v0389-inhabited-crossing-farmyard-readability-repair-smoke.json", {"checkpoint": V0388_CHECKPOINT, "status": "PASS", "sceneLoaded": true, "acceptedInfrastructureUnchanged": true, "gameplay": false, "defaultRuntime": "unchanged", "route": "opt-in-only"})
	get_tree().quit(0)
