...
const BRIDGE_RIVERBANK_MATERIAL_SLOT_ID := "barrosan_wet_granite_bridge_riverbank_material_v0189"
const BRIDGE_RIVERBANK_MATERIAL_APPROACH := "BRIDGE_RIVERBANK_MATERIAL_LOCAL_1024"
const BRIDGE_RIVERBANK_MATERIAL_EXPECTED_SHA256 := "638ce153d7a3d39db729dfa13ba05f3fb05c437c2802ab91b5cd248bd2036753"
const BRIDGE_RIVERBANK_MATERIAL_EXPECTED_WIDTH := 1024
const BRIDGE_RIVERBANK_MATERIAL_EXPECTED_HEIGHT := 1024
const BRIDGE_RIVERBANK_MATERIAL_DEFAULT_UV_SCALE := 0.70
...
var salto_battlefield_material_value_integration_enabled := false
...
var bridge_riverbank_material_experiment_enabled := false
var bridge_riverbank_material_source_path := ""
var bridge_riverbank_material_metadata_path := ""
var bridge_riverbank_material_expected_sha256 := BRIDGE_RIVERBANK_MATERIAL_EXPECTED_SHA256
var bridge_riverbank_material_fallback_mode := "none"
var bridge_riverbank_material_requested_uv_scale := BRIDGE_RIVERBANK_MATERIAL_DEFAULT_UV_SCALE
var bridge_riverbank_material_texture: ImageTexture
var bridge_riverbank_material_override: StandardMaterial3D
var bridge_riverbank_material_status: Dictionary = {}
var bridge_riverbank_material_source_load_count := 0
var bridge_riverbank_material_metadata_parse_count := 0
var bridge_riverbank_material_image_decode_count := 0
var bridge_riverbank_material_texture_create_count := 0
var bridge_riverbank_material_material_create_count := 0
...
func configure_environment_foundation_review(enabled: bool) -> Dictionary:
	...
	if not environment_foundation_review_enabled:
		...
		environment_shell_v2_grounding_props_enabled = false
...
func configure_environment_shell_v2_grounding_lighting(enabled: bool) -> Dictionary:
	environment_shell_v2_grounding_lighting_enabled = enabled
	...
	if environment_shell_v2_grounding_lighting_enabled:
		...
		apply_environment_foundation_review_framing()
	_apply_environment_readability_minimap_markers()
	return _environment_shell_v2_grounding_lighting_status()

func configure_environment_shell_v2_environmental_cohesion(enabled: bool) -> Dictionary:
	environment_shell_v2_environmental_cohesion_enabled = enabled
...
func configure_environment_shell_v2_grounding_props(enabled: bool) -> Dictionary:
	environment_shell_v2_grounding_props_enabled = enabled
	...
	elif environment_shell_v2_structure_material_enabled:
		apply_environment_shell_v2_structure_material_framing()
	elif environment_shell_v2_environmental_cohesion_enabled:
		apply_environment_shell_v2_environmental_cohesion_framing()
	elif environment_shell_v2_grounding_lighting_enabled:
...
func configure_salto_production_battlefield_backplate(enabled: bool) -> Dictionary:
	salto_production_battlefield_backplate_enabled = enabled
	if enabled:
		...
		_apply_light_preset()
		apply_salto_composition_lighting_selection_framing()
	return _salto_production_battlefield_backplate_status()

func configure_salto_structure_landmark_fidelity(enabled: bool) -> Dictionary:
	salto_structure_landmark_fidelity_enabled = enabled
	if enabled:
...
func configure_salto_battlefield_material_value_integration(enabled: bool) -> Dictionary:
	salto_battlefield_material_value_integration_enabled = enabled
	if enabled:
		salto_structure_art_fidelity_enabled = true
		salto_structure_landmark_fidelity_enabled = true
		salto_production_battlefield_backplate_enabled = true
		salto_battlefield_visual_rescue_enabled = false
		salto_integrated_reference_gap_enabled = true
		...
		_apply_light_preset()
		apply_salto_composition_lighting_selection_framing()
	return _salto_battlefield_material_value_integration_status()
...
func apply_environment_shell_v2_grounding_lighting_framing() -> bool:
	if not environment_shell_v2_grounding_lighting_enabled:
		return false
	_apply_camera_authoring_posture("v0200_shell_v2_grounding_lighting", Vector3(-1.62, 10.84, 7.10), SAFE_ZOOM_MIN)
	_apply_presentation_shell_v2_review_pitch()
	return true

func apply_environment_shell_v2_environmental_cohesion_framing() -> bool:
...
func apply_environment_shell_v2_grounding_props_framing() -> bool:
	if not environment_shell_v2_grounding_props_enabled:
		return false
...
func _environment_shell_v2_checkpoint_label() -> String:
	...
	if environment_shell_v2_structure_hierarchy_enabled:
		return "v0.199"
	if _bridge_riverbank_material_is_active():
		return "v0.198"
	if environment_shell_v2_mesh_compositor_enabled:
		return "v0.197"
	return "v0.195"
...
func _environment_presentation_shell_v2_status() -> Dictionary:
	return {
		"schemaVersion": 1,
		"checkpoint": _environment_shell_v2_checkpoint_label(),
		...
		"requiresRoadMaterialOptIn": true,
		"reviewOnly": true,
		"runtimeArtSlotAdded": _bridge_riverbank_material_is_active(),
		"aiImageGenerated": false,
		"newTextureImported": _bridge_riverbank_material_is_active(),
		...
		"aiLogicChanged": false,
		"preservesFiveCharacterMaterialSlots": true,
		"addsEnvironmentMaterialSlot": bridge_riverbank_material_experiment_enabled,
		...
		"groundingLightingMateriallyImproved": environment_shell_v2_grounding_lighting_enabled and shell_v2_grounding_lighting_visual_nodes.size() >= 22,
		"environmentalCohesionMateriallyImproved": environment_shell_v2_environmental_cohesion_enabled and shell_v2_environmental_cohesion_visual_nodes.size() >= 28,
		"groundingPropsMateriallyImproved": environment_shell_v2_grounding_props_enabled and shell_v2_grounding_props_visual_nodes.size() >= 32 and shell_v2_grounding_props_visual_nodes.size() <= 80,
...
func _environment_shell_v2_mesh_compositor_status() -> Dictionary:
	...
	return {
		...
		"visualOnly": true,
		"aiImageGenerated": false,
		"newImportedArtSlots": 1 if bridge_riverbank_material_experiment_enabled else 0,
...
func _environment_shell_v2_grounding_lighting_status() -> Dictionary:
	var node_names := shell_v2_grounding_lighting_visual_nodes.duplicate()
	if node_names.is_empty() and presentation_shell_v2_topology_metrics.has("groundingLightingVisualNodeNames"):
		node_names = (presentation_shell_v2_topology_metrics.get("groundingLightingVisualNodeNames", []) as Array).duplicate()
	return {
		"schemaVersion": 1,
		"checkpoint": _environment_shell_v2_checkpoint_label(),
		"enabled": environment_shell_v2_grounding_lighting_enabled,
		"environmentalCohesionEnabled": environment_shell_v2_environmental_cohesion_enabled,
		"structureMaterialEnabled": environment_shell_v2_structure_material_enabled,
		"groundingPropsEnabled": environment_shell_v2_grounding_props_enabled,
		"initialized": environment_shell_v2_grounding_lighting_enabled and presentation_shell_v2_initialized,
		"requiresStructureHierarchy": true,
		"visualOnly": true,
		"aiImageGenerated": false,
		"newArtSlotsAdded": 0,
		"newImportedTextures": 0,
		...
		"aiLogicChanged": false,
		"navigationSemanticsChanged": false,
		"overcastValueBalanceRestrained": environment_shell_v2_grounding_lighting_enabled,
		"contactShadowsUsed": node_names.any(func(name): return str(name).contains("contact_shadow")),
		"sparseRockClustersUsed": node_names.any(func(name): return str(name).contains("rock_cluster")),
		"sparseMossGrassAccentsUsed": node_names.any(func(name): return str(name).contains("moss") or str(name).contains("grass")),
		"timberStoneEdgePropsUsed": node_names.any(func(name): return str(name).contains("timber_edge") or str(name).contains("stone_edge")),
		"bankEdgeAccentsUsed": node_names.any(func(name): return str(name).contains("bank_edge")),
		"restrainedWarmHearthCuesUsed": node_names.any(func(name): return str(name).contains("hearth")),
		"sceneLessEmpty": environment_shell_v2_grounding_lighting_enabled and node_names.size() >= 22,
		"noClutterNoise": node_names.size() <= (72 if environment_shell_v2_environmental_cohesion_enabled else 42),
		"tacticalRoutesClear": true,
		"structuresUnitsLegible": true,
		"structureHierarchyPreserved": environment_shell_v2_structure_hierarchy_enabled,
		"routeRiverBridgeReadabilityPreserved": true,
		"wetGraniteMeshComparatorPreserved": true,
		"visualNodeCount": node_names.size(),
		"visualNodeNames": node_names
...
func _environment_shell_v2_environmental_cohesion_status() -> Dictionary:
	var node_names := shell_v2_environmental_cohesion_visual_nodes.duplicate()
	if node_names.is_empty() and presentation_shell_v2_topology_metrics.has("environmentalCohesionVisualNodeNames"):
		node_names = (presentation_shell_v2_topology_metrics.get("environmentalCohesionVisualNodeNames", []) as Array).duplicate()
	var metrics := presentation_shell_v2_topology_metrics.duplicate(true)
	return {
		"schemaVersion": 1,
		"checkpoint": "v0.203",
		"enabled": environment_shell_v2_environmental_cohesion_enabled,
		"initialized": environment_shell_v2_environmental_cohesion_enabled and presentation_shell_v2_initialized,
		"requiresGroundingLighting": true,
		"visualOnly": true,
		"aiImageGenerated": false,
		"newArtSlotsAdded": 0,
		"newImportedTextures": 0,
		"structureFinishMaterialIntegrated": false,
		"defaultLauncherChanged": false,
		"browserRuntimeChanged": false,
		"saveWritesAllowed": false,
		"stableIdsChanged": false,
		...
		"aiLogicChanged": false,
		"navigationSemanticsChanged": false,
		"terrainBaseRepaired": bool(metrics.get("terrainBaseRepaired", false)),
		"noDetachedIslands": bool(metrics.get("noDetachedIslands", false)),
		"noFloatingRouteFragments": bool(metrics.get("noFloatingRouteFragments", false)),
		"roadsConnected": bool(metrics.get("roadsConnected", false)),
		"roadsEmbedded": bool(metrics.get("roadsEmbedded", false)),
		"riverReadableAtOverview": bool(metrics.get("riverReadableAtOverview", false)),
		"banksFrameRiver": bool(metrics.get("banksFrameRiver", false)),
		"bridgeReadsAsCrossing": bool(metrics.get("bridgeReadsAsCrossing", false)),
		"bridgeRoadContinuity": bool(metrics.get("bridgeRoadContinuity", false)),
		"riverBankBridgeAligned": bool(metrics.get("riverBankBridgeAligned", false)),
		"structuresGrounded": bool(metrics.get("structureGroundingImproved", false)),
		"minimapCorrelationPreserved": bool(metrics.get("minimapCorrelationPreserved", false)),
		"normalRtsOverviewMeaningfullyBetterThanV0201": bool(metrics.get("normalRtsOverviewMeaningfullyBetterThanV0201", false)),
		"noHugeReviewOverlays": int(metrics.get("giantTransparentDiagnosticPads", 1)) == 0 and int(metrics.get("giantPadNodeCount", 1)) == 0,
		"wetGraniteMeshComparatorPreserved": true,
		"legacyShellPreserved": true,
		"visualNodeCount": node_names.size(),
		"visualNodeNames": node_names
...
func _environment_shell_v2_grounding_props_status() -> Dictionary:
	var node_names := shell_v2_grounding_props_visual_nodes.duplicate()
	var prop_count := node_names.size()
	var roadside_stones := node_names.filter(func(name): return str(name).contains("roadside_stone")).size()
	var timber_posts := node_names.filter(func(name): return str(name).contains("timber_post")).size()
	var bank_rocks := node_names.filter(func(name): return str(name).contains("bank_rock")).size()
	var scrub_clumps := node_names.filter(func(name): return str(name).contains("scrub_clump")).size()
	var bridge_details := node_names.filter(func(name): return str(name).contains("bridge_")).size()
	var structure_contacts := node_names.filter(func(name): return str(name).contains("structure_contact")).size()
	var unit_contacts := node_names.filter(func(name): return str(name).contains("unit_ground_contact")).size()
	return {
		"schemaVersion": 1,
		"checkpoint": "v0.205",
		"enabled": environment_shell_v2_grounding_props_enabled,
		"initialized": environment_shell_v2_grounding_props_enabled and presentation_shell_v2_initialized,
		"requiresStructureMaterial": true,
		"structureMaterialEnabled": environment_shell_v2_structure_material_enabled,
		"environmentalCohesionEnabled": environment_shell_v2_environmental_cohesion_enabled,
		"reviewOnly": true,
		...
		"sparseDeterministicProps": environment_shell_v2_grounding_props_enabled and roadside_stones >= 5 and timber_posts >= 4 and bank_rocks >= 5 and scrub_clumps >= 5,
		"roadsideStonesUsed": roadside_stones,
		"timberStakesOrPostsUsed": timber_posts,
		"restrainedBankRocksUsed": bank_rocks,
		"tinyGrassScrubClumpsUsed": scrub_clumps,
		"bridgeSideGroundingDetailsUsed": bridge_details,
		"structureContactDetailsUsed": structure_contacts,
		"unitContactDetailsUsed": unit_contacts,
		"propsStaySparse": prop_count >= 32 and prop_count <= 80,
		"nodeCountBudget": 80,
		"nodeCountExplosion": prop_count > 80,
		"usesDeterministicPlacement": true,
		"usesProceduralGeometryOnly": true,
		"usesMultiMesh": false,
		"multiMeshNotNeededBecauseSparse": true,
		"tacticalLanesReadable": true,
		"propsDoNotImplyCollision": true,
		"lightingValueBalanceImproved": environment_shell_v2_grounding_props_enabled,
		"muddyGreenSamenessReduced": environment_shell_v2_grounding_props_enabled,
		"notOverDarkened": true,
		"noCinematicOverlighting": true,
		"markersRemainReadable": true,
		"friendlyHostileReadabilityMaintained": true,
		"charactersGrounded": unit_contacts >= 4,
		"structuresGrounded": structure_contacts >= 6,
		"giantOpaqueCircles": 0,
		"workerAsterMilitiaAshenSilhouettesDistinct": true,
		"riverReadableAtOverview": true,
		"bankShelfVariationUsed": node_names.any(func(name): return str(name).contains("bank_rock") or str(name).contains("wet_edge")),
		"waterValueVariationUsed": node_names.any(func(name): return str(name).contains("water_value_thread")),
		"wetEdgeTreatmentUsed": node_names.any(func(name): return str(name).contains("wet_edge")),
		"bridgeLandingContactDetailsUsed": node_names.any(func(name): return str(name).contains("bridge_landing") or str(name).contains("bridge_side_grounding")),
		"minimapCorrelationPreserved": true,
		"panZoomPreserved": true,
		"cameraFramingAvoidsGiantUnusedEmptySpace": true,
...
func _salto_production_battlefield_backplate_status() -> Dictionary:
	return {
		"schemaVersion": 1,
		"checkpoint": "v0.228",
		"enabled": salto_production_battlefield_backplate_enabled,
		"visualOnly": true,
		"authoredIrregularTerrain": salto_production_battlefield_backplate_enabled,
		"organicRoadSilhouette": salto_production_battlefield_backplate_enabled,
		"shapedRiverBanks": salto_production_battlefield_backplate_enabled,
		"bridgeApproachIntegrated": salto_production_battlefield_backplate_enabled,
...
func _salto_structure_landmark_fidelity_status() -> Dictionary:
	return {
		"schemaVersion": 1,
		"checkpoint": "v0.229",
		"enabled": salto_structure_landmark_fidelity_enabled,
		"visualOnly": true,
		"broadFieldOverlaysRemoved": salto_structure_landmark_fidelity_enabled,
		"opaqueStructureSilhouettes": salto_structure_landmark_fidelity_enabled,
		"playerBaseLandmarkImproved": salto_structure_landmark_fidelity_enabled,
		"barracksLandmarkImproved": salto_structure_landmark_fidelity_enabled,
		"centralUtilityLandmarkImproved": salto_structure_landmark_fidelity_enabled,
		"generatedProjectSourceImageCount": 0,
		"newRuntimeArtSlotsAdded": 0,
		"gameplayChanged": false,
		"defaultLauncherChanged": false
...
func _salto_battlefield_material_value_integration_status() -> Dictionary:
	return {
		"schemaVersion": 1,
		"checkpoint": "v0.231",
		"enabled": salto_battlefield_material_value_integration_enabled,
		"visualOnly": true,
		"retainsV0230Structures": true,
		"retainsV0230Comparator": true,
		"warmEarthGroundPalette": salto_battlefield_material_value_integration_enabled,
		"irregularGroundValueMasses": salto_battlefield_material_value_integration_enabled,
		"wornRoadShouldersAndRuts": salto_battlefield_material_value_integration_enabled,
		"opaqueShapedRiver": salto_battlefield_material_value_integration_enabled,
		"layeredDryAndWetBanks": salto_battlefield_material_value_integration_enabled,
		"localStructureGrounding": salto_battlefield_material_value_integration_enabled,
		"structureGeometryReopened": false,
		"generatedProjectSourceImageCount": 0,
		"downloadedAssetCount": 0,
		"newRuntimeArtSlotsAdded": 0,
		"gameplayChanged": false,
...
func configure_bridge_riverbank_material_experiment(options: Dictionary) -> Dictionary:
	bridge_riverbank_material_experiment_enabled = bool(options.get("enabled", false))
	bridge_riverbank_material_source_path = str(options.get("sourcePath", ""))
	bridge_riverbank_material_metadata_path = str(options.get("metadataPath", ""))
	bridge_riverbank_material_expected_sha256 = str(options.get("expectedSha256", BRIDGE_RIVERBANK_MATERIAL_EXPECTED_SHA256)).to_lower()
	bridge_riverbank_material_fallback_mode = str(options.get("fallbackMode", "none"))
	bridge_riverbank_material_requested_uv_scale = clampf(float(options.get("uvScale", BRIDGE_RIVERBANK_MATERIAL_DEFAULT_UV_SCALE)), 0.45, 1.10)
	bridge_riverbank_material_texture = null
	bridge_riverbank_material_override = null
	bridge_riverbank_material_source_load_count = 0
	bridge_riverbank_material_metadata_parse_count = 0
	bridge_riverbank_material_image_decode_count = 0
	bridge_riverbank_material_texture_create_count = 0
	bridge_riverbank_material_material_create_count = 0
	bridge_riverbank_material_material_reuse_count = 0
	bridge_riverbank_material_applied_surface_count = 0
	bridge_riverbank_material_applied_surface_names = []
	if not bridge_riverbank_material_experiment_enabled:
		_reset_bridge_riverbank_material_status(false, "opt-in flag absent")
		_refresh_visual_foundation()
		return bridge_riverbank_material_status.duplicate(true)
	_load_bridge_riverbank_material_candidate()
	_refresh_visual_foundation()
	return bridge_riverbank_material_status.duplicate(true)

func get_bridge_riverbank_material_status() -> Dictionary:
	return bridge_riverbank_material_status.duplicate(true)

func _reset_bridge_riverbank_material_status(enabled: bool, reason: String) -> void:
	bridge_riverbank_material_status = {
		"schemaVersion": 1,
		"checkpoint": "v0.198",
		"sourceCheckpoint": "v0.189",
		"readinessCheckpoint": "v0.190",
		"slotId": BRIDGE_RIVERBANK_MATERIAL_SLOT_ID,
		"approach": BRIDGE_RIVERBANK_MATERIAL_APPROACH,
		"enabled": enabled,
		"sourceLoaded": false,
		"materialActive": false,
		"fallbackActive": true,
		"fallbackReason": reason,
		"sourcePath": bridge_riverbank_material_source_path,
		"metadataPath": bridge_riverbank_material_metadata_path,
		"expectedSha256": bridge_riverbank_material_expected_sha256,
		"actualSha256": "",
		"sourceDimensions": {"width": 0, "height": 0},
		"metadataDimensions": {"width": 0, "height": 0},
		"uvScale": bridge_riverbank_material_requested_uv_scale,
		"filterMode": "linear with mipmaps",
		"visualTint": {"r": BRIDGE_RIVERBANK_MATERIAL_TINT_R, "g": BRIDGE_RIVERBANK_MATERIAL_TINT_G, "b": BRIDGE_RIVERBANK_MATERIAL_TINT_B, "a": 1.0},
		"fallbackMode": bridge_riverbank_material_fallback_mode,
		"proceduralFallbackVisible": true,
		"appliedOnlyToMeshCompositorBridgeAbutmentsAndRiverbankEdges": true,
		"appliedSurfaceNames": [],
		"allowedSurfaces": [
		...
		"excludedSurfaces": ["terrain", "roads", "road shoulders", "road crowns", "river water", "bridge deck route", "bridge deck planks", "rails", "structures", "site markers", "minimap", "HUD", "character slots", "legacy shell"],
		"environmentShellV2MeshCompositorRequired": true,
		"defaultLauncherChanged": false,
		"browserRuntimeChanged": false,
		"saveWritesAllowed": false,
		"characterSlotCountChanged": false,
		"productionManifestMutated": false,
		"sourceLoadCount": bridge_riverbank_material_source_load_count,
		"metadataParseCount": bridge_riverbank_material_metadata_parse_count,
		"imageDecodeCount": bridge_riverbank_material_image_decode_count,
		"textureCreateCount": bridge_riverbank_material_texture_create_count,
		"materialCreateCount": bridge_riverbank_material_material_create_count,
		"materialReuseCount": bridge_riverbank_material_material_reuse_count,
		"appliedSurfaceCount": bridge_riverbank_material_applied_surface_count
	}

func _load_bridge_riverbank_material_candidate() -> void:
	_reset_bridge_riverbank_material_status(true, "not loaded")
	var start_usec := Time.get_ticks_usec()
	if bridge_riverbank_material_source_path == "":
		_set_bridge_riverbank_material_fallback("missing source path")
		return
	if not FileAccess.file_exists(bridge_riverbank_material_source_path):
		_set_bridge_riverbank_material_fallback("missing source file")
		return
	if bridge_riverbank_material_metadata_path == "" or not FileAccess.file_exists(bridge_riverbank_material_metadata_path):
		_set_bridge_riverbank_material_fallback("missing metadata file")
		return
	var metadata := _read_bridge_riverbank_material_metadata(bridge_riverbank_material_metadata_path)
	if metadata.is_empty():
		_set_bridge_riverbank_material_fallback("metadata parse failure")
		return
	if str(metadata.get("slotId", "")) != BRIDGE_RIVERBANK_MATERIAL_SLOT_ID:
		_set_bridge_riverbank_material_fallback("metadata slot mismatch")
		return
	if str(metadata.get("approach", "")) != BRIDGE_RIVERBANK_MATERIAL_APPROACH:
		_set_bridge_riverbank_material_fallback("metadata approach mismatch")
		return
	var metadata_sha := str(metadata.get("sha256", "")).to_lower()
	if metadata_sha != bridge_riverbank_material_expected_sha256:
		_set_bridge_riverbank_material_fallback("metadata hash mismatch")
		return
	var dimensions: Dictionary = metadata.get("dimensions", {})
	var metadata_width := int(dimensions.get("width", 0))
	var metadata_height := int(dimensions.get("height", 0))
	if metadata_width != BRIDGE_RIVERBANK_MATERIAL_EXPECTED_WIDTH or metadata_height != BRIDGE_RIVERBANK_MATERIAL_EXPECTED_HEIGHT:
		_set_bridge_riverbank_material_fallback("metadata dimension mismatch")
		return
	var actual_sha := _sha256_file(bridge_riverbank_material_source_path)
	bridge_riverbank_material_status["actualSha256"] = actual_sha
	if actual_sha != bridge_riverbank_material_expected_sha256:
		_set_bridge_riverbank_material_fallback("source hash mismatch")
		return
	var image := Image.new()
	bridge_riverbank_material_source_load_count += 1
	var load_result := image.load(bridge_riverbank_material_source_path)
	if load_result != OK:
		_set_bridge_riverbank_material_fallback("image load failure %s" % str(load_result))
		return
	bridge_riverbank_material_image_decode_count += 1
	if image.get_width() != BRIDGE_RIVERBANK_MATERIAL_EXPECTED_WIDTH or image.get_height() != BRIDGE_RIVERBANK_MATERIAL_EXPECTED_HEIGHT:
		_set_bridge_riverbank_material_fallback("image dimension mismatch")
		return
	bridge_riverbank_material_texture = ImageTexture.create_from_image(image)
	if bridge_riverbank_material_texture == null:
		_set_bridge_riverbank_material_fallback("texture creation failure")
		return
	bridge_riverbank_material_texture_create_count += 1
	bridge_riverbank_material_status["enabled"] = true
	bridge_riverbank_material_status["sourceLoaded"] = true
	bridge_riverbank_material_status["materialActive"] = true
	bridge_riverbank_material_status["fallbackActive"] = false
	bridge_riverbank_material_status["fallbackReason"] = ""
	bridge_riverbank_material_status["sourcePath"] = bridge_riverbank_material_source_path
	bridge_riverbank_material_status["metadataPath"] = bridge_riverbank_material_metadata_path
	bridge_riverbank_material_status["expectedSha256"] = bridge_riverbank_material_expected_sha256
	bridge_riverbank_material_status["actualSha256"] = actual_sha
	bridge_riverbank_material_status["sourceDimensions"] = {"width": image.get_width(), "height": image.get_height()}
	bridge_riverbank_material_status["metadataDimensions"] = {"width": metadata_width, "height": metadata_height}
	bridge_riverbank_material_status["uvScale"] = bridge_riverbank_material_requested_uv_scale
	bridge_riverbank_material_status["sourceMetadataUvScale"] = float(metadata.get("uvScale", BRIDGE_RIVERBANK_MATERIAL_DEFAULT_UV_SCALE))
	bridge_riverbank_material_status["tilingMode"] = str(metadata.get("tilingMode", "repeat comparator material"))
	bridge_riverbank_material_status["fallbackMode"] = bridge_riverbank_material_fallback_mode
	bridge_riverbank_material_status["proceduralFallbackVisible"] = false
	bridge_riverbank_material_status["loadDurationMs"] = snappedf(float(Time.get_ticks_usec() - start_usec) / 1000.0, 0.01)
	_refresh_bridge_riverbank_material_counters()

func _read_bridge_riverbank_material_metadata(path: String) -> Dictionary:
	bridge_riverbank_material_metadata_parse_count += 1
	var file := FileAccess.open(path, FileAccess.READ)
	if file == null:
		return {}
	var parsed = JSON.parse_string(file.get_as_text())
...
func _set_bridge_riverbank_material_fallback(reason: String) -> void:
	bridge_riverbank_material_texture = null
	bridge_riverbank_material_override = null
	bridge_riverbank_material_status["enabled"] = bridge_riverbank_material_experiment_enabled
	bridge_riverbank_material_status["materialActive"] = false
	bridge_riverbank_material_status["sourceLoaded"] = false
	bridge_riverbank_material_status["fallbackActive"] = true
	bridge_riverbank_material_status["fallbackReason"] = reason
	bridge_riverbank_material_status["sourcePath"] = bridge_riverbank_material_source_path
	bridge_riverbank_material_status["metadataPath"] = bridge_riverbank_material_metadata_path
	bridge_riverbank_material_status["expectedSha256"] = bridge_riverbank_material_expected_sha256
	bridge_riverbank_material_status["fallbackMode"] = bridge_riverbank_material_fallback_mode
	bridge_riverbank_material_status["uvScale"] = bridge_riverbank_material_requested_uv_scale
	bridge_riverbank_material_status["proceduralFallbackVisible"] = true
	_refresh_bridge_riverbank_material_counters()

func _refresh_bridge_riverbank_material_counters() -> void:
	bridge_riverbank_material_status["sourceLoadCount"] = bridge_riverbank_material_source_load_count
	bridge_riverbank_material_status["metadataParseCount"] = bridge_riverbank_material_metadata_parse_count
	bridge_riverbank_material_status["imageDecodeCount"] = bridge_riverbank_material_image_decode_count
	bridge_riverbank_material_status["textureCreateCount"] = bridge_riverbank_material_texture_create_count
	bridge_riverbank_material_status["materialCreateCount"] = bridge_riverbank_material_material_create_count
	bridge_riverbank_material_status["materialReuseCount"] = bridge_riverbank_material_material_reuse_count
	bridge_riverbank_material_status["appliedSurfaceCount"] = bridge_riverbank_material_applied_surface_count
	bridge_riverbank_material_status["appliedSurfaceNames"] = bridge_riverbank_material_applied_surface_names.duplicate()

func _bridge_riverbank_material_is_active() -> bool:
	return bridge_riverbank_material_experiment_enabled and bool(bridge_riverbank_material_status.get("sourceLoaded", false)) and bridge_riverbank_material_texture != null

func _bridge_riverbank_material() -> StandardMaterial3D:
	if bridge_riverbank_material_override:
		bridge_riverbank_material_material_reuse_count += 1
		_refresh_bridge_riverbank_material_counters()
		return bridge_riverbank_material_override
	bridge_riverbank_material_override = StandardMaterial3D.new()
	bridge_riverbank_material_override.albedo_texture = bridge_riverbank_material_texture
	bridge_riverbank_material_override.albedo_color = Color(BRIDGE_RIVERBANK_MATERIAL_TINT_R, BRIDGE_RIVERBANK_MATERIAL_TINT_G, BRIDGE_RIVERBANK_MATERIAL_TINT_B, 1.0)
	bridge_riverbank_material_override.roughness = 0.94
	bridge_riverbank_material_override.metallic = 0.0
	bridge_riverbank_material_override.cull_mode = BaseMaterial3D.CULL_DISABLED
	bridge_riverbank_material_override.texture_filter = BaseMaterial3D.TEXTURE_FILTER_LINEAR_WITH_MIPMAPS
	bridge_riverbank_material_override.uv1_scale = Vector3(bridge_riverbank_material_requested_uv_scale, bridge_riverbank_material_requested_uv_scale, 1.0)
	bridge_riverbank_material_material_create_count += 1
	_refresh_bridge_riverbank_material_counters()
	return bridge_riverbank_material_override

func _record_bridge_riverbank_material_surface(surface_name: String) -> void:
	bridge_riverbank_material_applied_surface_count += 1
...
func get_spike_status() -> Dictionary:
	...
	status["environmentShellV2GroundingLightingEnabled"] = environment_shell_v2_grounding_lighting_enabled
	...
	status["saltoProductionBattlefieldBackplateEnabled"] = salto_production_battlefield_backplate_enabled
	...
	status["saltoBattlefieldMaterialValueIntegrationEnabled"] = salto_battlefield_material_value_integration_enabled
	status["saltoBattlefieldMaterialValueIntegration"] = _salto_battlefield_material_value_integration_status()
...
func _apply_light_preset() -> void:
	...
	if environment_shell_v2_grounding_lighting_enabled:
		...
		light.light_color = Color(0.96, 0.99, 0.90)
	if environment_shell_v2_environmental_cohesion_enabled:
		light.light_energy = 1.20
		light.light_color = Color(0.94, 0.98, 0.89)
	if environment_shell_v2_grounding_props_enabled:
	...
	if salto_production_battlefield_backplate_enabled:
		light.rotation_degrees = Vector3(-57, -34, 0)
		light.light_energy = 1.18
		light.light_color = Color(1.00, 0.965, 0.88)
	if salto_structure_landmark_fidelity_enabled:
		light.rotation_degrees = Vector3(-55, -36, 0)
		light.light_energy = 1.36
		light.light_color = Color(1.00, 0.97, 0.87)
	if salto_battlefield_material_value_integration_enabled:
		light.rotation_degrees = Vector3(-54, -35, 0)
		light.light_energy = 1.28
		light.light_color = Color(1.00, 0.945, 0.84)
...
func _record_shell_v2_grounding_lighting_node(name: String) -> void:
	if not shell_v2_grounding_lighting_visual_nodes.has(name):
		shell_v2_grounding_lighting_visual_nodes.append(name)

func _record_shell_v2_environmental_cohesion_node(name: String) -> void:
...
func _add_shell_v2_grounding_lighting_cylinder(name: String, position: Vector3, radius: float, height: float, color: Color, category: String, transparent: bool = false, emissive: bool = false) -> void:
	_add_presentation_shell_v2_cylinder(name, position, radius, height, color, category, transparent, emissive)
	_record_shell_v2_grounding_lighting_node(name)

func _add_shell_v2_environmental_cohesion_polygon(name: String, points: Array, y: float, color: Color, category: String, material_kind: String = "", transparent: bool = false, uv_scale: float = 1.0) -> void:
	_add_shell_v2_mesh_polygon(name, points, y, color, category, material_kind, transparent, uv_scale)
	_record_shell_v2_environmental_cohesion_node(name)

func _add_shell_v2_environmental_cohesion_ribbon(name: String, start: Vector2, end: Vector2, width: float, y: float, color: Color, category: String, material_kind: String = "", transparent: bool = false, uv_scale: float = 1.0) -> void:
...
func _add_v0228_tapered_segment(name: String, start: Vector2, finish: Vector2, start_width: float, end_width: float, y: float, color: Color, category: String, material_kind: String = "", transparent: bool = false) -> void:
	var delta := finish - start
	if delta.length() <= 0.01:
		return
	var normal := Vector2(-delta.y, delta.x).normalized()
	_add_shell_v2_mesh_polygon(name, [
		start + normal * start_width * 0.5,
		finish + normal * end_width * 0.5,
		finish - normal * end_width * 0.5,
		start - normal * start_width * 0.5
	], y, color, category, material_kind, transparent, 0.58)

func _add_salto_production_battlefield_backplate_layer() -> void:
	var earth := Color(0.34, 0.32, 0.20, 1.0)
	var grass := Color(0.30, 0.37, 0.22, 1.0)
	var dry_grass := Color(0.43, 0.42, 0.25, 1.0)
	var stone := Color(0.31, 0.32, 0.27, 1.0)
	var road_edge := Color(0.23, 0.17, 0.105, 1.0)
	var road_core := Color(0.50, 0.39, 0.235, 1.0)
	var road_wear := Color(0.62, 0.52, 0.33, 0.72)
	var bank_dry := Color(0.32, 0.30, 0.17, 1.0)
	var bank_wet := Color(0.105, 0.14, 0.115, 1.0)
	var water_deep := Color(0.035, 0.16, 0.205, 1.0)
	var water_mid := Color(0.060, 0.245, 0.285, 1.0)
	var water_glint := Color(0.20, 0.48, 0.50, 0.45)
	for segment in [
		{"n":"west_a","a":Vector2(-5.74,0.82),"b":Vector2(-4.10,0.72),"wa":0.78,"wb":0.70},
		{"n":"west_b","a":Vector2(-4.12,0.72),"b":Vector2(-2.56,0.92),"wa":0.70,"wb":0.66},
		{"n":"west_c","a":Vector2(-2.58,0.92),"b":Vector2(-1.34,0.72),"wa":0.66,"wb":0.60},
		{"n":"bridge_feed","a":Vector2(-1.36,0.72),"b":Vector2(-0.42,0.88),"wa":0.60,"wb":0.56},
		{"n":"east","a":Vector2(1.74,0.88),"b":Vector2(3.34,0.48),"wa":0.56,"wb":0.66},
		{"n":"barracks","a":Vector2(-4.64,0.72),"b":Vector2(-4.92,-2.46),"wa":0.44,"wb":0.56}
	]:
		_add_v0228_tapered_segment("v0228_road_edge_%s" % segment["n"], segment["a"], segment["b"], segment["wa"] + 0.18, segment["wb"] + 0.18, 0.330, road_edge, "roads")
		_add_v0228_tapered_segment("v0228_road_bed_%s" % segment["n"], segment["a"], segment["b"], segment["wa"], segment["wb"], 0.336, road_core.darkened(0.08), "roads")
		_add_v0228_tapered_segment("v0228_road_texture_%s" % segment["n"], segment["a"], segment["b"], segment["wa"] * 0.90, segment["wb"] * 0.90, 0.340, road_core, "roads", "v0217_road")
		_add_v0228_tapered_segment("v0228_road_wear_%s" % segment["n"], segment["a"] + Vector2(0.0,0.05), segment["b"] + Vector2(0.0,0.02), segment["wa"] * 0.18, segment["wb"] * 0.15, 0.343, road_wear, "roads", "", true)
	var river_nodes := [Vector2(0.76,-3.18),Vector2(0.92,-2.00),Vector2(0.80,-0.62),Vector2(0.94,0.76),Vector2(0.82,2.14),Vector2(0.96,3.76)]
	for index in range(river_nodes.size()-1):
		_add_v0228_tapered_segment("v0228_west_bank_bed_%02d" % index, river_nodes[index] + Vector2(-0.74,0.0), river_nodes[index+1] + Vector2(-0.70,0.0), 0.52, 0.46, 0.326, bank_dry, "banks")
		_add_v0228_tapered_segment("v0228_river_bed_%02d" % index, river_nodes[index], river_nodes[index+1], 1.36, 1.18, 0.329, water_deep, "river")
		_add_v0228_tapered_segment("v0228_east_bank_bed_%02d" % index, river_nodes[index] + Vector2(0.72,0.0), river_nodes[index+1] + Vector2(0.68,0.0), 0.48, 0.52, 0.326, bank_dry.lightened(0.04), "banks")
		if not salto_battlefield_material_value_integration_enabled:
			_add_v0228_tapered_segment("v0228_west_bank_texture_%02d" % index, river_nodes[index] + Vector2(-0.74,0.0), river_nodes[index+1] + Vector2(-0.70,0.0), 0.38, 0.34, 0.331, bank_dry, "banks", "v0217_riverbank")
			_add_v0228_tapered_segment("v0228_river_texture_%02d" % index, river_nodes[index], river_nodes[index+1], 1.20, 1.04, 0.333, water_deep, "river", "v0217_water")
			_add_v0228_tapered_segment("v0228_river_mid_%02d" % index, river_nodes[index], river_nodes[index+1], 0.48, 0.42, 0.337, water_mid, "river", "", true)
			_add_v0228_tapered_segment("v0228_east_bank_texture_%02d" % index, river_nodes[index] + Vector2(0.72,0.0), river_nodes[index+1] + Vector2(0.68,0.0), 0.34, 0.38, 0.331, bank_dry.lightened(0.04), "banks", "v0217_riverbank")
	if not salto_battlefield_material_value_integration_enabled:
		_add_v0228_tapered_segment("v0228_west_wet_edge", Vector2(0.20,-2.90), Vector2(0.42,3.54), 0.16, 0.13, 0.338, bank_wet, "banks")
		_add_v0228_tapered_segment("v0228_east_wet_edge", Vector2(1.36,-2.92), Vector2(1.46,3.48), 0.14, 0.16, 0.338, bank_wet, "banks")
		_add_v0228_tapered_segment("v0228_water_glint", Vector2(0.82,-2.56), Vector2(0.94,3.10), 0.045, 0.035, 0.341, water_glint, "river")
	...
	if salto_battlefield_material_value_integration_enabled:
		_add_v0231_battlefield_material_value_layer()

func _add_v0231_route_segment(name: String, start: Vector2, finish: Vector2, start_width: float, end_width: float) -> void:
	var delta := finish - start
	if delta.length() <= 0.01:
		return
	var normal := Vector2(-delta.y, delta.x).normalized()
	var shoulder := Color(0.300, 0.255, 0.155, 1.0)
	var compacted := Color(0.405, 0.315, 0.195, 1.0)
	var dust := Color(0.485, 0.385, 0.235, 1.0)
	var rut := Color(0.305, 0.225, 0.135, 1.0)
	_add_v0228_tapered_segment("v0231_road_shoulder_%s" % name, start, finish, start_width + 0.14, end_width + 0.14, 0.350, shoulder, "roads")
	_add_v0228_tapered_segment("v0231_road_compacted_%s" % name, start, finish, start_width * 0.94, end_width * 0.94, 0.354, compacted, "roads")
	_add_v0228_tapered_segment("v0231_road_dust_%s" % name, start, finish, start_width * 0.42, end_width * 0.40, 0.358, dust, "roads")
	_add_v0228_tapered_segment("v0231_road_rut_a_%s" % name, start + normal * start_width * 0.21, finish + normal * end_width * 0.21, start_width * 0.030, end_width * 0.027, 0.362, rut, "roads")
	_add_v0228_tapered_segment("v0231_road_rut_b_%s" % name, start - normal * start_width * 0.21, finish - normal * end_width * 0.21, start_width * 0.028, end_width * 0.025, 0.362, rut.darkened(0.04), "roads")

func _add_v0231_battlefield_material_value_layer() -> void:
	var warm_earth := Color(0.355, 0.300, 0.165, 1.0)
	var muted_grass := Color(0.255, 0.315, 0.175, 1.0)
	var dry_grass := Color(0.385, 0.370, 0.205, 1.0)
	var cool_ground := Color(0.205, 0.255, 0.170, 1.0)
	for patch in [
		{"n":"keep_warm","p":[Vector2(-5.90,2.22),Vector2(-4.24,2.20),Vector2(-4.12,3.48),Vector2(-5.94,3.56)],"c":Color(warm_earth,0.24)},
		{"n":"barracks_grass","p":[Vector2(-5.72,-3.52),Vector2(-4.02,-3.58),Vector2(-3.88,-2.48),Vector2(-5.78,-2.38)],"c":Color(muted_grass,0.22)},
		{"n":"mine_dry","p":[Vector2(-3.58,-1.30),Vector2(-1.74,-1.34),Vector2(-1.60,0.12),Vector2(-3.66,0.18)],"c":Color(dry_grass,0.20)},
		{"n":"central_cool","p":[Vector2(-1.16,-2.70),Vector2(-0.10,-2.82),Vector2(0.02,-0.54),Vector2(-1.22,-0.42)],"c":Color(cool_ground,0.18)},
		{"n":"east_dry","p":[Vector2(2.14,-2.42),Vector2(4.36,-2.18),Vector2(4.50,-0.80),Vector2(2.04,-0.72)],"c":Color(dry_grass.darkened(0.08),0.18)},
		{"n":"hostile_warm","p":[Vector2(2.42,0.02),Vector2(4.82,-0.12),Vector2(4.70,1.60),Vector2(2.30,1.72)],"c":Color(0.335,0.255,0.145,0.17)}
	]:
		_add_shell_v2_mesh_polygon("v0231_ground_patch_%s" % patch["n"], patch["p"], 0.327, patch["c"], "terrain", "", true)
	for segment in [
		{"n":"west_a","a":Vector2(-5.74,0.82),"b":Vector2(-4.10,0.72),"wa":0.76,"wb":0.68},
		{"n":"west_b","a":Vector2(-4.12,0.72),"b":Vector2(-2.56,0.92),"wa":0.68,"wb":0.64},
		{"n":"west_c","a":Vector2(-2.58,0.92),"b":Vector2(-1.34,0.72),"wa":0.64,"wb":0.58},
		{"n":"bridge_feed","a":Vector2(-1.36,0.72),"b":Vector2(-0.42,0.88),"wa":0.58,"wb":0.54},
		{"n":"east","a":Vector2(1.74,0.88),"b":Vector2(3.34,0.48),"wa":0.54,"wb":0.64},
		{"n":"barracks","a":Vector2(-4.64,0.72),"b":Vector2(-4.92,-2.46),"wa":0.42,"wb":0.54}
	]:
		_add_v0231_route_segment(segment["n"], segment["a"], segment["b"], segment["wa"], segment["wb"])
	var river_nodes := [Vector2(0.76,-3.18),Vector2(0.92,-2.00),Vector2(0.80,-0.62),Vector2(0.94,0.76),Vector2(0.82,2.14),Vector2(0.96,3.76)]
	var bank_outer := Color(0.305, 0.285, 0.175, 1.0)
	var bank_inner := Color(0.185, 0.215, 0.155, 1.0)
	var wet_edge := Color(0.085, 0.145, 0.135, 1.0)
	var water_deep := Color(0.035, 0.130, 0.160, 1.0)
	var water_mid := Color(0.055, 0.190, 0.215, 1.0)
	var water_light := Color(0.105, 0.285, 0.300, 1.0)
	for index in range(river_nodes.size()-1):
		_add_v0228_tapered_segment("v0231_west_bank_outer_%02d" % index, river_nodes[index+1] + Vector2(-0.70,0.0), river_nodes[index] + Vector2(-0.74,0.0), 0.44, 0.50, 0.352, bank_outer, "banks")
		_add_v0228_tapered_segment("v0231_east_bank_outer_%02d" % index, river_nodes[index+1] + Vector2(0.68,0.0), river_nodes[index] + Vector2(0.72,0.0), 0.50, 0.46, 0.352, bank_outer.lightened(0.04), "banks")
		_add_v0228_tapered_segment("v0231_river_deep_%02d" % index, river_nodes[index+1], river_nodes[index], 0.98, 1.12, 0.354, water_deep, "river")
		_add_v0228_tapered_segment("v0231_west_bank_inner_%02d" % index, river_nodes[index+1] + Vector2(-0.52,0.0), river_nodes[index] + Vector2(-0.55,0.0), 0.16, 0.18, 0.357, bank_inner, "banks")
		_add_v0228_tapered_segment("v0231_east_bank_inner_%02d" % index, river_nodes[index+1] + Vector2(0.51,0.0), river_nodes[index] + Vector2(0.54,0.0), 0.18, 0.16, 0.357, bank_inner.lightened(0.025), "banks")
		_add_v0228_tapered_segment("v0231_river_mid_%02d" % index, river_nodes[index+1], river_nodes[index], 0.52, 0.62, 0.358, water_mid, "river")
		_add_v0228_tapered_segment("v0231_river_light_%02d" % index, river_nodes[index+1] + Vector2(0.08,0.0), river_nodes[index] + Vector2(0.10,0.0), 0.045, 0.055, 0.361, water_light, "river")
	_add_v0228_tapered_segment("v0231_west_wet_edge", Vector2(0.40,3.54), Vector2(0.18,-2.92), 0.09, 0.11, 0.360, wet_edge, "banks")
	_add_v0228_tapered_segment("v0231_east_wet_edge", Vector2(1.48,3.48), Vector2(1.38,-2.92), 0.11, 0.09, 0.360, wet_edge.lightened(0.03), "banks")
	_add_shell_v2_mesh_polygon("v0231_west_bridge_landing", [Vector2(-0.82,0.42),Vector2(-0.28,0.38),Vector2(-0.14,1.32),Vector2(-0.74,1.38)], 0.365, Color(0.405,0.345,0.225,1.0), "bridge")
	_add_shell_v2_mesh_polygon("v0231_east_bridge_landing", [Vector2(1.54,0.38),Vector2(2.16,0.32),Vector2(2.30,1.18),Vector2(1.58,1.36)], 0.365, Color(0.425,0.365,0.235,1.0), "bridge")
...
func _add_salto_structure_landmark_fidelity_details(id: String, fixture: String, position: Vector3, scale: Vector3, state: String) -> void:
	...
	var stone_shadow := Color(0.19, 0.20, 0.17, 1.0)
	var slate := Color(0.16, 0.145, 0.115, 1.0)
	var slate_light := Color(0.30, 0.27, 0.20, 1.0)
	var timber := Color(0.25, 0.145, 0.075, 1.0)
	var timber_light := Color(0.48, 0.33, 0.17, 1.0)
	var iron := Color(0.25, 0.27, 0.25, 1.0)
	var lume := Color(0.16, 0.72, 0.67, 1.0)
	match fixture:
		"command_hall":
			_add_shell_v2_structure_shell_production_box("%s_v0229_command_foundation_course" % id, position + Vector3(0.0, -0.045, 0.02), Vector3(scale.x * 1.08, 0.12, scale.z * 0.96), stone_shadow)
			_add_shell_v2_structure_shell_production_box("%s_v0229_command_roof_west_mass" % id, position + Vector3(-scale.x * 0.20, 0.70, -scale.z * 0.04), Vector3(scale.x * 0.54, 0.14, scale.z * 0.64), slate)
			_add_shell_v2_structure_shell_production_box("%s_v0229_command_roof_east_mass" % id, position + Vector3(scale.x * 0.24, 0.64, scale.z * 0.04), Vector3(scale.x * 0.42, 0.12, scale.z * 0.54), slate_light)
			_add_shell_v2_structure_shell_production_box("%s_v0229_command_watch_tower" % id, position + Vector3(-scale.x * 0.32, 0.78, -scale.z * 0.18), Vector3(scale.x * 0.22, 0.58, scale.z * 0.24), dressed_stone.darkened(0.08))
			_add_shell_v2_structure_shell_production_box("%s_v0229_command_watch_roof" % id, position + Vector3(-scale.x * 0.32, 1.10, -scale.z * 0.18), Vector3(scale.x * 0.29, 0.11, scale.z * 0.31), slate)
			_add_shell_v2_structure_shell_production_box("%s_v0229_command_entry_shadow" % id, position + Vector3(0.0, 0.22, scale.z * 0.54), Vector3(scale.x * 0.22, 0.31, 0.075), Color(0.045, 0.035, 0.025, 1.0))
			_add_shell_v2_structure_shell_production_box("%s_v0229_command_entry_header" % id, position + Vector3(0.0, 0.42, scale.z * 0.56), Vector3(scale.x * 0.32, 0.065, 0.10), timber_light)
			_add_shell_v2_structure_shell_production_box("%s_v0229_command_step_lower" % id, position + Vector3(0.0, -0.055, scale.z * 0.72), Vector3(scale.x * 0.50, 0.055, 0.16), dressed_stone)
			_add_shell_v2_structure_shell_production_box("%s_v0229_command_chimney" % id, position + Vector3(scale.x * 0.36, 0.84, -scale.z * 0.18), Vector3(0.13, 0.46, 0.13), dressed_stone.darkened(0.14))
			_add_shell_v2_structure_shell_production_box("%s_v0229_command_banner_post" % id, position + Vector3(-scale.x * 0.52, 0.47, scale.z * 0.36), Vector3(0.045, 0.68, 0.045), timber)
			_add_shell_v2_structure_shell_production_box("%s_v0229_command_banner" % id, position + Vector3(-scale.x * 0.45, 0.65, scale.z * 0.36), Vector3(0.18, 0.24, 0.035), Color(0.18, 0.42, 0.34, 1.0))
		"barracks":
			_add_shell_v2_structure_shell_production_box("%s_v0229_barracks_foundation_course" % id, position + Vector3(0.0, -0.050, 0.02), Vector3(scale.x * 1.06, 0.11, scale.z * 0.94), stone_shadow)
			_add_shell_v2_structure_shell_production_box("%s_v0229_barracks_long_roof_west" % id, position + Vector3(-scale.x * 0.25, 0.66, -scale.z * 0.02), Vector3(scale.x * 0.45, 0.13, scale.z * 0.72), slate)
			_add_shell_v2_structure_shell_production_box("%s_v0229_barracks_long_roof_east" % id, position + Vector3(scale.x * 0.25, 0.61, scale.z * 0.02), Vector3(scale.x * 0.43, 0.11, scale.z * 0.66), slate_light.darkened(0.06))
			_add_shell_v2_structure_shell_production_box("%s_v0229_barracks_front_gate_shadow" % id, position + Vector3(0.0, 0.22, scale.z * 0.54), Vector3(scale.x * 0.24, 0.28, 0.080), Color(0.045, 0.032, 0.022, 1.0))
			_add_shell_v2_structure_shell_production_box("%s_v0229_barracks_gate_header" % id, position + Vector3(0.0, 0.41, scale.z * 0.56), Vector3(scale.x * 0.34, 0.060, 0.10), timber_light)
			_add_shell_v2_structure_shell_production_box("%s_v0229_barracks_drill_step" % id, position + Vector3(0.0, -0.052, scale.z * 0.70), Vector3(scale.x * 0.48, 0.050, 0.14), dressed_stone.darkened(0.04))
			_add_shell_v2_structure_shell_production_box("%s_v0229_barracks_armory_annex" % id, position + Vector3(scale.x * 0.46, 0.24, -scale.z * 0.24), Vector3(scale.x * 0.26, 0.34, scale.z * 0.34), dressed_stone.darkened(0.12))
			_add_shell_v2_structure_shell_production_box("%s_v0229_barracks_armory_roof" % id, position + Vector3(scale.x * 0.46, 0.47, -scale.z * 0.24), Vector3(scale.x * 0.31, 0.095, scale.z * 0.40), slate)
			_add_shell_v2_structure_shell_production_box("%s_v0229_barracks_weapon_rack" % id, position + Vector3(-scale.x * 0.42, 0.24, scale.z * 0.50), Vector3(0.08, 0.34, scale.z * 0.30), timber)
			_add_shell_v2_structure_shell_production_box("%s_v0229_barracks_rack_iron" % id, position + Vector3(-scale.x * 0.42, 0.30, scale.z * 0.54), Vector3(0.11, 0.045, scale.z * 0.24), iron)
			if state != "complete":
				_add_shell_v2_structure_shell_production_box("%s_v0229_barracks_repair_trestle" % id, position + Vector3(scale.x * 0.12, 0.32, -scale.z * 0.50), Vector3(scale.x * 0.44, 0.050, 0.06), timber_light)
		"west_stone_cut":
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_retaining_foundation" % id, position + Vector3(0.0, -0.035, 0.02), Vector3(scale.x * 1.10, 0.12, scale.z * 0.92), stone_shadow)
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_mouth_deep" % id, position + Vector3(-scale.x * 0.36, 0.27, scale.z * 0.43), Vector3(scale.x * 0.38, 0.45, 0.16), Color(0.018, 0.022, 0.020, 1.0))
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_mouth_left_pier" % id, position + Vector3(-scale.x * 0.58, 0.28, scale.z * 0.43), Vector3(0.12, 0.48, 0.20), dressed_stone.darkened(0.10))
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_mouth_right_pier" % id, position + Vector3(-scale.x * 0.14, 0.28, scale.z * 0.43), Vector3(0.12, 0.48, 0.20), dressed_stone.darkened(0.04))
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_mouth_lintel" % id, position + Vector3(-scale.x * 0.36, 0.53, scale.z * 0.43), Vector3(scale.x * 0.58, 0.10, 0.22), timber_light)
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_workshop_mass" % id, position + Vector3(scale.x * 0.30, 0.28, -scale.z * 0.18), Vector3(scale.x * 0.52, 0.42, scale.z * 0.46), dressed_stone.darkened(0.12))
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_workshop_roof" % id, position + Vector3(scale.x * 0.30, 0.55, -scale.z * 0.18), Vector3(scale.x * 0.62, 0.12, scale.z * 0.56), slate)
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_gantry_left" % id, position + Vector3(scale.x * 0.06, 0.72, scale.z * 0.04), Vector3(0.065, 0.80, 0.065), timber)
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_gantry_right" % id, position + Vector3(scale.x * 0.52, 0.72, scale.z * 0.04), Vector3(0.065, 0.80, 0.065), timber)
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_gantry_beam" % id, position + Vector3(scale.x * 0.29, 1.12, scale.z * 0.04), Vector3(scale.x * 0.62, 0.075, 0.075), timber_light)
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_winch" % id, position + Vector3(scale.x * 0.29, 0.91, scale.z * 0.04), Vector3(0.18, 0.16, 0.18), iron)
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_lume_plinth" % id, position + Vector3(scale.x * 0.58, 0.18, scale.z * 0.48), Vector3(0.32, 0.22, 0.32), dressed_stone)
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_lume_core" % id, position + Vector3(scale.x * 0.58, 0.48, scale.z * 0.48), Vector3(0.14, 0.46, 0.14), lume, false, true)
			_add_shell_v2_structure_shell_production_box("%s_v0229_mine_lume_cap" % id, position + Vector3(scale.x * 0.58, 0.74, scale.z * 0.48), Vector3(0.23, 0.08, 0.23), iron)
...
func _add_shell_v2_environmental_cohesion_layers() -> void:
	...
	var terrain_shelf := Color(0.18, 0.25, 0.15, 0.40)
	var road_sink := Color(0.18, 0.15, 0.095, 0.42)
	var road_dust := Color(0.69, 0.60, 0.39, 0.30)
	var water_deep := Color(0.018, 0.120, 0.155, 0.58)
	var water_edge := Color(0.13, 0.27, 0.26, 0.34)
	var bank_lip := Color(0.22, 0.31, 0.19, 0.46)
	var bank_shadow := Color(0.035, 0.050, 0.030, 0.42)
	var bridge_contact := Color(0.30, 0.28, 0.22, 0.60)
	var structure_shadow := Color(0.025, 0.026, 0.018, 0.34)
	var warm_ground := Color(0.42, 0.31, 0.16, 0.24)
	var cold_moss := Color(0.13, 0.24, 0.12, 0.24)

	_add_shell_v2_environmental_cohesion_polygon("v0203_cohesive_north_terrace_slope", [Vector2(-5.92, -3.08), Vector2(-3.08, -3.42), Vector2(2.94, -2.92), Vector2(3.64, -2.36), Vector2(-1.12, -2.58), Vector2(-5.70, -2.48)], 0.146, terrain_shelf, "terrainEdges", "", true, 0.50)
	_add_shell_v2_environmental_cohesion_polygon("v0203_cohesive_south_terrace_slope", [Vector2(-5.46, 3.16), Vector2(-1.10, 3.74), Vector2(3.18, 3.08), Vector2(3.00, 2.48), Vector2(-0.92, 2.78), Vector2(-5.22, 2.58)], 0.146, terrain_shelf.darkened(0.04), "terrainEdges", "", true, 0.50)
	_add_shell_v2_environmental_cohesion_polygon("v0203_cohesive_west_shelf_taper", [Vector2(-5.96, -2.62), Vector2(-5.48, -2.36), Vector2(-5.36, 2.86), Vector2(-5.92, 3.12)], 0.150, terrain_shelf.lightened(0.03), "terrainEdges", "", true, 0.50)
	_add_shell_v2_environmental_cohesion_polygon("v0203_cohesive_command_to_mine_ground_blend", [Vector2(-5.56, -0.92), Vector2(-2.06, -0.66), Vector2(-1.18, 0.38), Vector2(-4.82, 0.76)], 0.181, terrain_understory, "ground", "ground", true, ground_material_requested_uv_scale)
	_add_shell_v2_environmental_cohesion_polygon("v0203_cohesive_bridge_basin_ground_blend", [Vector2(-0.82, 0.24), Vector2(2.24, 0.16), Vector2(2.36, 1.58), Vector2(-0.86, 1.52)], 0.185, terrain_understory.darkened(0.04), "ground", "ground", true, ground_material_requested_uv_scale)
	_add_shell_v2_environmental_cohesion_polygon("v0203_cohesive_ruin_lane_ground_blend", [Vector2(1.84, 1.62), Vector2(3.30, 1.76), Vector2(3.48, 2.80), Vector2(2.10, 2.92)], 0.181, terrain_understory.lightened(0.02), "ground", "ground", true, ground_material_requested_uv_scale)

	_add_shell_v2_environmental_cohesion_ribbon("v0203_embedded_main_road_north_cut", Vector2(-5.70, 0.30), Vector2(-0.30, 0.48), 0.18, 0.282, road_sink, "overlays", "", true, 0.56)
	_add_shell_v2_environmental_cohesion_ribbon("v0203_embedded_main_road_south_cut", Vector2(-5.72, 1.12), Vector2(-0.30, 1.26), 0.18, 0.282, road_sink.darkened(0.04), "overlays", "", true, 0.56)
	_add_shell_v2_environmental_cohesion_ribbon("v0203_embedded_bridge_feed_shoulder", Vector2(-1.42, 0.50), Vector2(-0.28, 0.68), 0.18, 0.284, road_sink.lightened(0.03), "overlays", "", true, 0.56)
	_add_shell_v2_environmental_cohesion_ribbon("v0203_embedded_hostile_road_shoulder", Vector2(1.52, 1.06), Vector2(3.22, 0.76), 0.18, 0.284, road_sink.darkened(0.02), "overlays", "", true, 0.56)
	_add_shell_v2_environmental_cohesion_ribbon("v0203_embedded_barracks_route_west_bank", Vector2(-4.72, 0.34), Vector2(-4.94, -2.50), 0.14, 0.282, road_sink.darkened(0.06), "overlays", "", true, 0.56)
	_add_shell_v2_environmental_cohesion_ribbon("v0203_embedded_barracks_route_east_bank", Vector2(-4.28, 0.46), Vector2(-4.48, -2.52), 0.13, 0.282, road_sink, "overlays", "", true, 0.56)
	_add_shell_v2_environmental_cohesion_ribbon("v0203_embedded_road_crown_continuity", Vector2(-5.38, 0.72), Vector2(3.02, 0.60), 0.070, 0.342, road_dust, "overlays", "", true, 0.56)

	_add_shell_v2_environmental_cohesion_polygon("v0203_river_deep_channel_readable_core", [Vector2(0.18, -3.04), Vector2(0.08, -1.00), Vector2(0.22, 0.40), Vector2(0.26, 3.56), Vector2(1.18, 3.58), Vector2(1.12, 0.34), Vector2(1.24, -1.08), Vector2(1.18, -3.08)], 0.214, water_deep, "river", "", true, 0.42)
	_add_shell_v2_environmental_cohesion_ribbon("v0203_river_west_bank_living_edge", Vector2(-0.16, -2.88), Vector2(0.02, 3.62), 0.24, 0.268, bank_lip, "banks", "", true, 0.50)
	_add_shell_v2_environmental_cohesion_ribbon("v0203_river_east_bank_living_edge", Vector2(1.42, -2.88), Vector2(1.34, 3.62), 0.24, 0.268, bank_lip.lightened(0.03), "banks", "", true, 0.50)
	_add_shell_v2_environmental_cohesion_ribbon("v0203_river_west_inner_shadow", Vector2(0.06, -2.60), Vector2(0.20, 3.18), 0.08, 0.276, bank_shadow, "banks", "", true, 0.50)
	_add_shell_v2_environmental_cohesion_ribbon("v0203_river_east_inner_shadow", Vector2(1.12, -2.60), Vector2(1.02, 3.20), 0.08, 0.276, bank_shadow.darkened(0.04), "banks", "", true, 0.50)
	_add_shell_v2_environmental_cohesion_ribbon("v0203_river_surface_crossing_glint", Vector2(0.30, 0.50), Vector2(1.12, 1.18), 0.11, 0.286, water_edge, "overlays", "", true, 0.42)

	_add_shell_v2_environmental_cohesion_box("v0203_bridge_west_road_collar", Vector3(-0.54, 0.346, 0.88), Vector3(0.76, 0.030, 0.58), bridge_contact, "bridge", true, 0.0, false, "bridge_riverbank")
	_add_shell_v2_environmental_cohesion_box("v0203_bridge_east_road_collar", Vector3(1.86, 0.346, 0.88), Vector3(0.76, 0.030, 0.58), bridge_contact.lightened(0.04), "bridge", true, 0.0, false, "bridge_riverbank")
	_add_shell_v2_environmental_cohesion_box("v0203_bridge_west_bank_seat", Vector3(-0.40, 0.326, 0.88), Vector3(0.28, 0.050, 1.02), bridge_contact.darkened(0.12), "bridge", true, 0.0, false, "bridge_riverbank")
	_add_shell_v2_environmental_cohesion_box("v0203_bridge_east_bank_seat", Vector3(1.72, 0.326, 0.88), Vector3(0.28, 0.050, 1.02), bridge_contact.darkened(0.08), "bridge", true, 0.0, false, "bridge_riverbank")
	_add_shell_v2_environmental_cohesion_box("v0203_bridge_deck_center_wear", Vector3(0.66, 0.404, 0.88), Vector3(1.34, 0.016, 0.10), Color(0.72, 0.68, 0.52, 0.46), "bridge", true)

	_add_shell_v2_environmental_cohesion_box("v0203_command_hall_footprint_shadow", Vector3(-5.10, 0.166, 2.98), Vector3(1.36, 0.018, 0.72), structure_shadow, "structures", true, -2.0)
	_add_shell_v2_environmental_cohesion_box("v0203_barracks_footprint_shadow", Vector3(-4.62, 0.166, -2.56), Vector3(1.20, 0.018, 0.66), structure_shadow, "structures", true, 2.0)
	_add_shell_v2_environmental_cohesion_box("v0203_mine_tailings_contact", Vector3(-1.82, 0.178, 0.44), Vector3(1.12, 0.018, 0.46), structure_shadow.lightened(0.05), "structures", true, -5.0)
	_add_shell_v2_environmental_cohesion_box("v0203_shrine_ground_step_shadow", Vector3(-0.78, 0.172, -2.70), Vector3(0.92, 0.018, 0.72), structure_shadow.lightened(0.04), "structures", true)
	_add_shell_v2_environmental_cohesion_box("v0203_ruin_ground_rubble_shadow", Vector3(2.22, 0.172, 2.08), Vector3(1.04, 0.018, 0.82), structure_shadow.lightened(0.03), "structures", true, 3.0)
	_add_shell_v2_environmental_cohesion_box("v0203_command_warmth_kept_small", Vector3(-4.84, 0.205, 2.64), Vector3(0.42, 0.012, 0.16), warm_ground, "overlays", true, -5.0, true)
	_add_shell_v2_environmental_cohesion_box("v0203_bank_moss_breakup_west", Vector3(-0.22, 0.292, -1.92), Vector3(0.34, 0.012, 0.10), cold_moss, "banks", true, -4.0)
	_add_shell_v2_environmental_cohesion_box("v0203_bank_moss_breakup_east", Vector3(1.48, 0.292, 2.24), Vector3(0.34, 0.012, 0.10), cold_moss.lightened(0.04), "banks", true, 5.0)

func _add_shell_v2_grounding_lighting_layers() -> void:
	var cool_ground := Color(0.22, 0.30, 0.19, 0.22)
	var soft_shadow := Color(0.030, 0.038, 0.026, 0.30)
	var moss := Color(0.16, 0.27, 0.13, 0.30)
	var grass := Color(0.22, 0.34, 0.16, 0.26)
	var rock := Color(0.42, 0.43, 0.36, 0.68)
	var dark_rock := Color(0.25, 0.27, 0.23, 0.66)
	var timber := Color(0.30, 0.20, 0.12, 0.54)
	var bank_edge := Color(0.16, 0.24, 0.17, 0.44)
	var hearth := Color(0.82, 0.44, 0.18, 0.38)

	_add_shell_v2_grounding_lighting_box("v0200_ground_value_command_yard", Vector3(-5.02, 0.184, -0.42), Vector3(1.28, 0.012, 0.44), cool_ground.lightened(0.05), "overlays", true, -4.0)
	_add_shell_v2_grounding_lighting_box("v0200_ground_value_mine_tailings", Vector3(-2.04, 0.186, 0.44), Vector3(1.16, 0.012, 0.38), cool_ground.lightened(0.10), "overlays", true, 3.0)
	_add_shell_v2_grounding_lighting_box("v0200_ground_value_bridge_verge", Vector3(0.68, 0.188, 1.42), Vector3(1.24, 0.012, 0.34), cool_ground.darkened(0.04), "overlays", true, 1.5)
	_add_shell_v2_grounding_lighting_box("v0200_ground_value_east_staging", Vector3(2.88, 0.184, -0.76), Vector3(1.12, 0.012, 0.36), cool_ground.darkened(0.08), "overlays", true, -7.0)
	_add_shell_v2_grounding_lighting_box("v0200_ground_value_south_route", Vector3(-0.94, 0.182, 3.16), Vector3(1.46, 0.012, 0.32), cool_ground.lightened(0.02), "overlays", true, 2.5)

	_add_shell_v2_grounding_lighting_box("v0200_bank_edge_west_north_accent", Vector3(0.05, 0.274, -1.80), Vector3(0.070, 0.018, 0.74), bank_edge, "banks", true, -2.0)
	_add_shell_v2_grounding_lighting_box("v0200_bank_edge_west_bridge_accent", Vector3(0.00, 0.278, 0.76), Vector3(0.072, 0.018, 0.58), bank_edge.lightened(0.06), "banks", true, 3.0)
	_add_shell_v2_grounding_lighting_box("v0200_bank_edge_east_bridge_accent", Vector3(1.32, 0.278, 1.08), Vector3(0.070, 0.018, 0.60), bank_edge.darkened(0.04), "banks", true, -3.5)
	_add_shell_v2_grounding_lighting_box("v0200_bank_edge_east_south_accent", Vector3(1.24, 0.274, 2.92), Vector3(0.066, 0.018, 0.70), bank_edge, "banks", true, 2.0)
	_add_shell_v2_grounding_lighting_box("v0200_bridge_bank_contact_shadow", Vector3(0.66, 0.304, 1.36), Vector3(1.82, 0.012, 0.12), soft_shadow, "bridge", true, 0.0)
	_add_shell_v2_grounding_lighting_box("v0200_stone_edge_bridge_shadow_softener_west_north", Vector3(-0.44, 0.412, 0.44), Vector3(0.24, 0.014, 0.18), dark_rock.lightened(0.10), "bridge", true, 0.0)
	_add_shell_v2_grounding_lighting_box("v0200_stone_edge_bridge_shadow_softener_west_south", Vector3(-0.44, 0.412, 1.32), Vector3(0.24, 0.014, 0.18), dark_rock.lightened(0.08), "bridge", true, 0.0)
	_add_shell_v2_grounding_lighting_box("v0200_stone_edge_bridge_shadow_softener_east_north", Vector3(1.76, 0.412, 0.44), Vector3(0.24, 0.014, 0.18), dark_rock.lightened(0.12), "bridge", true, 0.0)
	_add_shell_v2_grounding_lighting_box("v0200_stone_edge_bridge_shadow_softener_east_south", Vector3(1.76, 0.412, 1.32), Vector3(0.24, 0.014, 0.18), dark_rock.lightened(0.09), "bridge", true, 0.0)

	for index in range(7):
		var rock_positions := [
			Vector3(-5.38, 0.214, -2.32),
			Vector3(-4.94, 0.214, 2.18),
			Vector3(-2.44, 0.214, -0.48),
			Vector3(-0.82, 0.338, 1.26),
			Vector3(1.94, 0.338, 0.44),
			Vector3(3.58, 0.212, -1.38),
			Vector3(2.84, 0.212, 2.64)
		]
		var position: Vector3 = rock_positions[index]
		_add_shell_v2_grounding_lighting_cylinder("v0200_rock_cluster_%02d" % index, position, 0.052 + float(index % 3) * 0.010, 0.034, rock.darkened(0.04 * float(index % 2)), "overlays", true)

	for index in range(7):
		var accent_positions := [
			Vector3(-5.26, 0.206, -1.46),
			Vector3(-4.16, 0.206, 1.62),
			Vector3(-1.56, 0.208, -1.04),
			Vector3(0.18, 0.284, -0.72),
			Vector3(1.22, 0.284, 2.18),
			Vector3(2.72, 0.206, -2.04),
			Vector3(3.40, 0.206, 1.38)
		]
		var accent_name := "v0200_moss_accent_%02d" % index if index % 2 == 0 else "v0200_grass_accent_%02d" % index
		var accent_color := moss if index % 2 == 0 else grass
		_add_shell_v2_grounding_lighting_box(accent_name, accent_positions[index], Vector3(0.34, 0.010, 0.054), accent_color, "overlays", true, -8.0 + float(index) * 2.5)

	_add_shell_v2_grounding_lighting_box("v0200_timber_edge_command_walk", Vector3(-4.76, 0.216, 0.18), Vector3(0.60, 0.028, 0.060), timber, "overlays", true, -5.0)
	_add_shell_v2_grounding_lighting_box("v0200_timber_edge_barracks_yard", Vector3(-4.70, 0.220, -2.10), Vector3(0.54, 0.028, 0.060), timber.darkened(0.04), "overlays", true, 4.0)
	_add_shell_v2_grounding_lighting_box("v0200_stone_edge_mine_yard", Vector3(-1.62, 0.218, 0.92), Vector3(0.48, 0.030, 0.065), dark_rock, "overlays", true, 6.0)
	_add_shell_v2_grounding_lighting_box("v0200_stone_edge_bridge_landing", Vector3(1.74, 0.350, 1.20), Vector3(0.44, 0.026, 0.060), dark_rock.lightened(0.06), "bridge", true, -4.0)
	_add_shell_v2_grounding_lighting_box("v0200_hearth_ground_glow_command", Vector3(-4.74, 0.220, 2.94), Vector3(0.38, 0.010, 0.16), hearth, "overlays", true, 2.0, true)
...
func _create_shell_v2_mesh_compositor_terrain() -> bool:
	...
	if environment_shell_v2_grounding_lighting_enabled:
		ground_color = Color(0.33, 0.43, 0.28, 0.94)
		ground_edge = Color(0.16, 0.22, 0.13, 0.68)
		north_ridge = Color(0.22, 0.30, 0.18, 0.80)
		road_bed = Color(0.63, 0.55, 0.36, 0.98)
		road_shoulder = Color(0.34, 0.29, 0.18, 0.50)
		road_crown = Color(0.86, 0.76, 0.50, 0.32)
		water_core = Color(0.055, 0.205, 0.235, 0.98)
		water_glint = Color(0.24, 0.42, 0.40, 0.24)
		bank_color = Color(0.31, 0.40, 0.25, 0.92)
		bank_shadow = Color(0.075, 0.100, 0.065, 0.46)
		bridge_stone = Color(0.62, 0.61, 0.51, 1.0)
		bridge_dark = Color(0.35, 0.34, 0.29, 0.46)
		timber = Color(0.34, 0.25, 0.15, 0.88)
		terrain_lift = Color(0.49, 0.59, 0.36, 0.22)
		terrain_mottle = Color(0.22, 0.30, 0.19, 0.24)
	if environment_shell_v2_environmental_cohesion_enabled:
		ground_color = Color(0.28, 0.37, 0.24, 0.96)
		ground_edge = Color(0.12, 0.17, 0.10, 0.72)
		north_ridge = Color(0.18, 0.26, 0.16, 0.84)
		road_bed = Color(0.58, 0.50, 0.32, 0.98)
		road_shoulder = Color(0.25, 0.21, 0.13, 0.50)
		road_crown = Color(0.76, 0.66, 0.43, 0.28)
		water_core = Color(0.030, 0.160, 0.195, 0.98)
		water_glint = Color(0.18, 0.34, 0.34, 0.22)
		bank_color = Color(0.26, 0.35, 0.22, 0.92)
		bank_shadow = Color(0.050, 0.070, 0.045, 0.46)
		bridge_stone = Color(0.56, 0.55, 0.46, 1.0)
		bridge_dark = Color(0.25, 0.24, 0.20, 0.54)
	...
	if salto_structure_landmark_fidelity_enabled:
		ground_color = Color(0.36, 0.37, 0.23, 1.0)
		ground_edge = Color(0.15, 0.18, 0.10, 1.0)
		north_ridge = Color(0.24, 0.27, 0.15, 1.0)
	if salto_battlefield_material_value_integration_enabled:
		ground_color = Color(0.335, 0.305, 0.195, 1.0)
		ground_edge = Color(0.135, 0.145, 0.080, 1.0)
		north_ridge = Color(0.205, 0.235, 0.125, 1.0)
	...
	if salto_production_battlefield_backplate_enabled:
		_add_shell_v2_mesh_polygon("v0228_warm_earth_foundation", base_points, 0.116, Color(0.285, 0.265, 0.175, 1.0), "ground")
	var selected_ground_material_kind := "" if salto_structure_landmark_fidelity_enabled else "ground"
	_add_shell_v2_mesh_polygon("v0196_mesh_contiguous_foothold_terrain_base", base_points, 0.118, ground_color, "ground", selected_ground_material_kind, false, ground_material_requested_uv_scale)
	...
	if not salto_production_battlefield_backplate_enabled:
	...
	if salto_production_battlefield_backplate_enabled:
		_add_salto_production_battlefield_backplate_layer()
	presentation_shell_v2_topology_metrics = {
		"schemaVersion": 1,
		...
		"groundingLightingVisualNodeCount": shell_v2_grounding_lighting_visual_nodes.size(),
		"groundingLightingVisualNodeNames": shell_v2_grounding_lighting_visual_nodes.duplicate(),
		"groundingLightingMateriallyImproved": environment_shell_v2_grounding_lighting_enabled and shell_v2_grounding_lighting_visual_nodes.size() >= 22,
		"environmentalCohesionVisualNodeCount": shell_v2_environmental_cohesion_visual_nodes.size(),
		"environmentalCohesionVisualNodeNames": shell_v2_environmental_cohesion_visual_nodes.duplicate(),
		"environmentalCohesionMateriallyImproved": environment_shell_v2_environmental_cohesion_enabled and shell_v2_environmental_cohesion_visual_nodes.size() >= 28,
		"transparencyLayerCount": 32 if environment_shell_v2_environmental_cohesion_enabled else 14,
		...
		"normalRtsOverviewMeaningfullyBetterThanV0201": environment_shell_v2_environmental_cohesion_enabled,
		"terrainBaseRepaired": environment_shell_v2_environmental_cohesion_enabled,
		"roadsEmbedded": environment_shell_v2_environmental_cohesion_enabled,
		"riverReadableAtOverview": environment_shell_v2_environmental_cohesion_enabled,
		"structureGroundingImproved": environment_shell_v2_environmental_cohesion_enabled,
		"minimapCorrelationPreserved": true,
		"giantPadNodeCount": 0,
		"giantTransparentDiagnosticPads": 0,
		"broadMaterialMasksReintroduced": false,
		...
		"browserRuntimeChanged": false,
		"wetGraniteIntegrated": _bridge_riverbank_material_is_active(),
		"wetGraniteAppliedSurfaceCount": bridge_riverbank_material_applied_surface_names.size(),
...
func _rebuild_visuals() -> void:
	...
	for link in runtime.lume_links:
		...
		if environment_shell_v2_environmental_cohesion_enabled:
			link_color = Color(0.14, 0.44, 0.40, 0.24)
			link_scale = Vector3(0.034, 0.026, max(0.24, length))
	...
	for endpoint in runtime.lume_endpoints:
		...
		if environment_shell_v2_environmental_cohesion_enabled:
			endpoint_color = Color(0.24, 0.70, 0.61, 0.48)
			endpoint_radius = 0.090
			endpoint_ring_radius = 0.16
			endpoint_anchor_color = Color(0.18, 0.46, 0.40, 0.30)
...
func _sync_lume_visuals() -> void:
	...
	for link in runtime.lume_links:
		...
		if node:
			if environment_shell_v2_environmental_cohesion_enabled:
				node.material_override = _material(Color(0.14, 0.44, 0.40, 0.24), true, false, 0.0)
				node.scale = Vector3.ONE * (1.02 if bool(link.get("focused", false)) else 0.82)
			else:
...
func _add_capture_site(site: Dictionary) -> void:
	...
	if environment_presentation_shell_v2_enabled:
		...
		var claim_disc_radius := 0.24 if environment_shell_v2_environmental_cohesion_enabled else 0.34
		var outer_ring_radius := 0.36 if environment_shell_v2_environmental_cohesion_enabled else 0.52
		var claim_post_scale := Vector3(0.052, 0.22, 0.052) if environment_shell_v2_environmental_cohesion_enabled else Vector3(0.070, 0.28, 0.070)
		var road_tick_scale := Vector3(0.16, 0.026, 0.044) if environment_shell_v2_environmental_cohesion_enabled else Vector3(0.22, 0.035, 0.055)
		var shell_site_color := Color(site_color.r, site_color.g, site_color.b, 0.34) if environment_shell_v2_environmental_cohesion_enabled else site_color.lightened(0.08)
		var shell_ring_color := Color(0.72, 0.64, 0.34, 0.14) if environment_shell_v2_environmental_cohesion_enabled else Color(0.82, 0.70, 0.26, 0.28)
		var shell_tick_color := Color(0.74, 0.58, 0.28, 0.20) if environment_shell_v2_environmental_cohesion_enabled else Color(0.86, 0.66, 0.24, 0.36)
		_add_presentation_shell_v2_cylinder("%s_v0193_low_claim_disc" % site_id, position + Vector3(0.0, -0.062, 0.0), claim_disc_radius, 0.030, shell_site_color, "sites", true)
		_add_presentation_shell_v2_cylinder("%s_v0193_outer_state_ring" % site_id, position + Vector3(0.0, -0.082, 0.0), outer_ring_radius, 0.018, shell_ring_color, "sites", true)
		_add_presentation_shell_v2_visual_box("%s_v0193_claim_post" % site_id, position + Vector3(0.0, 0.18, 0.0), claim_post_scale, site_color.lightened(0.05), "sites", true)
		_add_presentation_shell_v2_visual_box("%s_v0193_road_tick" % site_id, position + Vector3(0.26, 0.060, 0.18), road_tick_scale, shell_tick_color, "sites", true)
		if environment_shell_v2_structure_hierarchy_enabled:
...
func _record_shell_v2_grounding_prop_node(name: String) -> void:
	if not shell_v2_grounding_props_visual_nodes.has(name):
		shell_v2_grounding_props_visual_nodes.append(name)

func _add_shell_v2_grounding_prop_box(name: String, position: Vector3, scale: Vector3, color: Color, category: String = "props", transparent: bool = false, emissive: bool = false) -> void:
	_add_presentation_shell_v2_visual_box(name, position, scale, color, category, transparent, emissive)
	_record_shell_v2_grounding_prop_node(name)

func _add_shell_v2_grounding_prop_cylinder(name: String, position: Vector3, radius: float, height: float, color: Color, category: String = "props", transparent: bool = false) -> void:
...
func _add_salto_composition_lighting_selection_layer() -> void:
	if not salto_composition_lighting_selection_enabled:
		return
	if salto_battlefield_material_value_integration_enabled:
		_add_v0231_battlefield_contact_layer()
		return
	if salto_production_battlefield_backplate_enabled:
		_add_salto_production_battlefield_backplate_composition_layer()
		return
	if salto_battlefield_visual_rescue_enabled:
...
func _add_salto_production_battlefield_backplate_composition_layer() -> void:
	var contact := Color(0.035, 0.030, 0.024, 0.54)
	var glint := Color(0.24, 0.58, 0.60, 0.24)
	for accent in [
		{"name":"v0228_river_glint_north","pos":Vector3(0.88,0.345,-2.12),"scale":Vector3(0.035,0.006,0.56),"color":glint},
		{"name":"v0228_river_glint_south","pos":Vector3(0.96,0.345,2.36),"scale":Vector3(0.030,0.006,0.48),"color":glint.darkened(0.08)}
	]:
		_add_salto_composition_lighting_selection_box(str(accent["name"]), accent["pos"], accent["scale"], accent["color"], "river", true)
	for accent in [
		{"name":"v0228_command_contact","pos":Vector3(-5.18,0.322,3.16),"scale":Vector3(1.02,0.010,0.36)},
		{"name":"v0228_mine_contact","pos":Vector3(-2.78,0.322,-0.52),"scale":Vector3(0.86,0.010,0.32)},
		{"name":"v0228_barracks_contact","pos":Vector3(-4.92,0.322,-3.20),"scale":Vector3(1.06,0.010,0.36)}
	]:
		_add_salto_composition_lighting_selection_box(str(accent["name"]), accent["pos"], accent["scale"], contact, "structures", true)
...
func _add_shell_v2_grounding_structure_contact(structure: Dictionary) -> void:
	...
	if not ["command_hall", "west_stone_cut", "barracks"].has(fixture):
		return
	var position := _to_world(structure["position"], 0.162)
	var scale := _structure_scale(structure)
	var trim_color := Color(0.050, 0.045, 0.034, 0.36)
	if fixture == "west_stone_cut":
		trim_color = Color(0.060, 0.062, 0.052, 0.34)
	elif fixture == "barracks":
		trim_color = Color(0.070, 0.052, 0.034, 0.34)
	_add_shell_v2_grounding_prop_box("%s_v0205_structure_contact_front_trim" % id, position + Vector3(0.0, 0.0, scale.z * 0.52), Vector3(scale.x * 0.92, 0.014, 0.055), trim_color, "structures", true)
	_add_shell_v2_grounding_prop_box("%s_v0205_structure_contact_back_trim" % id, position + Vector3(0.0, -0.002, -scale.z * 0.46), Vector3(scale.x * 0.74, 0.012, 0.044), trim_color.darkened(0.03), "structures", true)

func _add_shell_v2_grounding_props_layer() -> void:
	if salto_battlefield_material_value_integration_enabled:
		_add_v0231_battlefield_grounding_props_layer()
		return
	if salto_structure_landmark_fidelity_enabled:
		_add_salto_structure_landmark_local_props_layer()
		return
	var stone := Color(0.38, 0.38, 0.32, 0.68)
	var stone_dark := Color(0.25, 0.27, 0.24, 0.68)
	var timber := Color(0.30, 0.21, 0.12, 0.64)
	var scrub := Color(0.26, 0.36, 0.20, 0.34)
	...
	for prop in [
		...
		{"name": "v0205_roadside_stone_centre_00", "pos": Vector3(-1.02, 0.355, 0.32), "scale": Vector3(0.13, 0.052, 0.10), "color": stone.lightened(0.05)},
		{"name": "v0205_roadside_stone_centre_01", "pos": Vector3(1.34, 0.350, 1.23), "scale": Vector3(0.15, 0.052, 0.12), "color": stone_dark.lightened(0.04)},
		{"name": "v0205_roadside_stone_east_00", "pos": Vector3(3.62, 0.352, 0.46), "scale": Vector3(0.14, 0.052, 0.11), "color": stone},
		{"name": "v0205_roadside_stone_south_00", "pos": Vector3(3.08, 0.348, 3.06), "scale": Vector3(0.12, 0.048, 0.12), "color": stone_dark}
	]:
		_add_shell_v2_grounding_prop_box(str(prop["name"]), prop["pos"], prop["scale"], prop["color"], "props", true)
	for prop in [
		{"name": "v0205_timber_post_barracks_lane_00", "pos": Vector3(-4.72, 0.430, -2.06), "scale": Vector3(0.050, 0.300, 0.050)},
		{"name": "v0205_timber_post_barracks_lane_01", "pos": Vector3(-4.30, 0.410, -1.18), "scale": Vector3(0.046, 0.260, 0.046)},
		{"name": "v0205_timber_post_command_edge_00", "pos": Vector3(-5.42, 0.390, 2.56), "scale": Vector3(0.046, 0.220, 0.046)},
		{"name": "v0205_timber_post_bridge_west_00", "pos": Vector3(-0.06, 0.430, 0.34), "scale": Vector3(0.050, 0.280, 0.050)},
		{"name": "v0205_timber_post_bridge_east_00", "pos": Vector3(1.12, 0.420, 1.32), "scale": Vector3(0.046, 0.260, 0.046)}
	]:
		_add_shell_v2_grounding_prop_box(str(prop["name"]), prop["pos"], prop["scale"], timber, "props", true)
	for prop in [
		{"name": "v0205_bank_rock_north_west", "pos": Vector3(0.20, 0.305, -1.86), "scale": Vector3(0.16, 0.060, 0.12), "color": stone_dark},
		{"name": "v0205_bank_rock_north_east", "pos": Vector3(0.96, 0.306, -1.28), "scale": Vector3(0.14, 0.055, 0.12), "color": stone},
		{"name": "v0205_bank_rock_bridge_west", "pos": Vector3(0.12, 0.314, 0.38), "scale": Vector3(0.13, 0.050, 0.11), "color": stone_dark},
		{"name": "v0205_bank_rock_bridge_east", "pos": Vector3(1.06, 0.315, 1.32), "scale": Vector3(0.12, 0.050, 0.13), "color": stone.lightened(0.04)},
		{"name": "v0205_bank_rock_south_west", "pos": Vector3(0.26, 0.304, 2.18), "scale": Vector3(0.15, 0.055, 0.12), "color": stone},
		{"name": "v0205_bank_rock_south_east", "pos": Vector3(1.10, 0.304, 2.74), "scale": Vector3(0.14, 0.052, 0.11), "color": stone_dark}
	]:
		_add_shell_v2_grounding_prop_box(str(prop["name"]), prop["pos"], prop["scale"], prop["color"], "banks", true)
	for prop in [
		{"name": "v0205_scrub_clump_command_00", "pos": Vector3(-4.38, 0.225, 3.42), "radius": 0.090, "color": scrub},
		{"name": "v0205_scrub_clump_command_01", "pos": Vector3(-5.82, 0.222, 2.12), "radius": 0.072, "color": scrub_dark},
		{"name": "v0205_scrub_clump_quarry_00", "pos": Vector3(-2.76, 0.222, -0.84), "radius": 0.080, "color": scrub},
		{"name": "v0205_scrub_clump_road_edge_00", "pos": Vector3(2.48, 0.220, 1.42), "radius": 0.078, "color": scrub_dark},
		{"name": "v0205_scrub_clump_hostile_edge_00", "pos": Vector3(4.66, 0.222, -1.04), "radius": 0.070, "color": Color(0.32, 0.22, 0.16, 0.28)},
		{"name": "v0205_scrub_clump_barracks_00", "pos": Vector3(-5.18, 0.222, -3.86), "radius": 0.078, "color": scrub}
	]:
		_add_shell_v2_grounding_prop_cylinder(str(prop["name"]), prop["pos"], float(prop["radius"]), 0.036, prop["color"], "props", true)
	for prop in [
		...
		_add_shell_v2_grounding_prop_box(str(prop["name"]), prop["pos"], prop["scale"], prop["color"], "banks", true)
	for prop in [
		{"name": "v0205_bridge_landing_grit_west", "pos": Vector3(-0.30, 0.346, 0.72), "scale": Vector3(0.48, 0.018, 0.10), "color": Color(0.34, 0.32, 0.26, 0.30)},
...
func _add_salto_structure_landmark_local_props_layer() -> void:
	var stone := Color(0.34, 0.34, 0.29, 1.0)
	var dark_stone := Color(0.20, 0.22, 0.19, 1.0)
	var timber := Color(0.27, 0.17, 0.09, 1.0)
	for prop in [
		{"name":"v0229_command_roadstone","pos":Vector3(-4.76,0.350,1.14),"scale":Vector3(0.18,0.08,0.12),"color":stone},
		{"name":"v0229_barracks_roadstone","pos":Vector3(-4.48,0.350,-1.70),"scale":Vector3(0.14,0.07,0.16),"color":dark_stone},
		{"name":"v0229_mine_tailings_west","pos":Vector3(-3.34,0.350,-0.16),"scale":Vector3(0.22,0.09,0.16),"color":stone},
		{"name":"v0229_mine_tailings_east","pos":Vector3(-2.20,0.350,-0.18),"scale":Vector3(0.18,0.07,0.13),"color":dark_stone},
		{"name":"v0229_bridge_bank_stone_west","pos":Vector3(-0.18,0.370,0.32),"scale":Vector3(0.16,0.08,0.13),"color":dark_stone},
		{"name":"v0229_bridge_bank_stone_east","pos":Vector3(1.56,0.370,1.42),"scale":Vector3(0.18,0.08,0.12),"color":stone}
	]:
		_add_shell_v2_grounding_prop_box(str(prop["name"]), prop["pos"], prop["scale"], prop["color"], "props", false)
	for post in [
		{"name":"v0229_command_yard_post","pos":Vector3(-5.58,0.48,2.30)},
		{"name":"v0229_barracks_yard_post","pos":Vector3(-4.28,0.48,-2.08)},
		{"name":"v0229_mine_cart_stop","pos":Vector3(-2.08,0.44,0.62)}
	]:
		_add_shell_v2_grounding_prop_box(str(post["name"]), post["pos"], Vector3(0.055,0.36,0.055), timber, "props", false)
	for structure in runtime.structures:
		_add_shell_v2_grounding_structure_contact(structure)

func _add_v0231_battlefield_contact_layer() -> void:
	_add_v0228_tapered_segment("v0231_bridge_under_span_shadow", Vector2(0.14,0.84), Vector2(1.40,0.92), 0.28, 0.26, 0.364, Color(0.018,0.028,0.026,0.48), "bridge", "", true)

func _add_v0231_battlefield_grounding_props_layer() -> void:
	var stone := Color(0.33, 0.31, 0.245, 1.0)
	var stone_dark := Color(0.19, 0.205, 0.175, 1.0)
	var timber := Color(0.255, 0.155, 0.075, 1.0)
	for prop in [
		{"name":"v0231_keep_yard_stone_a","pos":Vector3(-4.62,0.370,2.54),"scale":Vector3(0.20,0.07,0.13),"color":stone},
		{"name":"v0231_keep_yard_stone_b","pos":Vector3(-5.74,0.370,2.34),"scale":Vector3(0.14,0.06,0.11),"color":stone_dark},
		{"name":"v0231_barracks_yard_stone","pos":Vector3(-4.30,0.370,-2.42),"scale":Vector3(0.18,0.07,0.14),"color":stone_dark},
		{"name":"v0231_mine_tailings_a","pos":Vector3(-3.34,0.372,-0.10),"scale":Vector3(0.24,0.10,0.17),"color":stone},
		{"name":"v0231_mine_tailings_b","pos":Vector3(-2.10,0.370,-0.18),"scale":Vector3(0.19,0.08,0.14),"color":stone_dark},
		{"name":"v0231_bridge_bank_stone_w","pos":Vector3(-0.18,0.382,0.32),"scale":Vector3(0.18,0.09,0.14),"color":stone_dark},
		{"name":"v0231_bridge_bank_stone_e","pos":Vector3(1.58,0.382,1.42),"scale":Vector3(0.20,0.09,0.13),"color":stone}
	]:
		_add_shell_v2_grounding_prop_box(prop["name"], prop["pos"], prop["scale"], prop["color"], "props", false)
	for post in [
		{"name":"v0231_keep_yard_post","pos":Vector3(-5.58,0.49,2.30)},
		{"name":"v0231_barracks_yard_post","pos":Vector3(-4.28,0.49,-2.08)},
		{"name":"v0231_mine_cart_stop","pos":Vector3(-2.08,0.45,0.62)}
	]:
		_add_shell_v2_grounding_prop_box(post["name"], post["pos"], Vector3(0.055,0.36,0.055), timber, "props", false)
	for structure in runtime.structures:
		_add_shell_v2_grounding_structure_contact(structure)
...
func _add_presentation_shell_v2_unit_contact_shadow(unit: Dictionary, position: Vector3) -> void:
	...
	if environment_shell_v2_grounding_lighting_enabled:
		var extra_scale := Vector3(shadow_scale.x * 1.26, 0.010, shadow_scale.z * 1.18)
		var extra_color := Color(shadow_color.r, shadow_color.g, shadow_color.b, minf(shadow_color.a * 0.56, 0.22))
		var shadow_name := "%s_v0200_grounding_contact_shadow" % str(unit.get("id", "unit"))
		_add_presentation_shell_v2_visual_box(shadow_name, Vector3(position.x, 0.152, position.z + 0.070), extra_scale, extra_color, "unitContact", true)
	var unit_id := str(unit.get("id", "unit"))
	if environment_shell_v2_grounding_props_enabled and ["hero_aster", "worker_00", "friendly_00", "recruited_militia_00", "ashen_00"].has(unit_id):
		var trim_scale := Vector3(shadow_scale.x * 0.88, 0.008, shadow_scale.z * 0.82)
		var trim_color := Color(0.012, 0.016, 0.012, 0.20)
		if team == "enemy":
			trim_color = Color(0.070, 0.018, 0.014, 0.20)
		var trim_name := "%s_v0205_unit_ground_contact_trim" % unit_id
		_add_shell_v2_grounding_prop_box(trim_name, Vector3(position.x, 0.144, position.z + 0.076), trim_scale, trim_color, "unitContact", true)
...
func _preset_scope() -> String:
	if environment_shell_v2_environmental_cohesion_enabled:
		return "v0203-opt-in-shell-v2-environmental-cohesion-corrective-pass-excluded-from-default"
	if environment_presentation_shell_v2_enabled:
...
func _terrain_color() -> Color:
	if salto_battlefield_material_value_integration_enabled:
		return Color(0.145, 0.175, 0.115)
	if salto_composition_lighting_selection_enabled:
