...
const CHECKPOINT := "v0.350"
const SCENE_PATH := "res://scenes/review/V0350FinalHouse02BarnMaterialUnity.tscn"
...
const READY_OUTCOME := "READY FOR HUMAN V0350 FINAL HOUSE02/BARN MATERIAL-UNITY REVIEW"
const RAW_NAMES := ["01_matched_house02_barn_neutral_full.png", "02_matched_roof_normal_enabled.png", "03_matched_roof_normal_disabled.png", "04_barn_albedo_only_roof.png", "05_neutral_barn_front_three_quarter.png", "06_direct_front.png", "07_complete_direct_exterior_gable.png", "08_granite_foundation_closeup.png", "09_doors_openings_closeup.png", "10_roof_edge_slate_irregularity.png", "11_far_rts.png", "12_true_256_pixel_source.png", "13_greyscale.png", "14_restrained_warm_directional.png", "15_contextual_house02_barn_hamlet.png"]
...
func _ready() -> void:
	print("V0350_READY")
	capture_root = OS.get_environment("V0350_ARTIFACT_ROOT")
	_build_world(); _load_assets(); _build_context_dressing(); _build_camera()
	if capture_root != "":
...
func _load_assets() -> void:
	...
	if barn_scene == null: errors.append("v0.348 barn could not load")
		...
		barn = barn_scene.instantiate() as Node3D; barn.name = "V0350_Barn_Final_Material_Harmony_Review_Only"; barn.position = Vector3(0,0.18,0); world.add_child(barn)
		if barn.find_children("*", "MeshInstance3D", true, false).is_empty(): errors.append("v0.350 barn has no visible meshes")
		else: _apply_V0350_material_skin(); _apply_upper_opening_reduction(); _add_foundation_weathering()
...
func _apply_V0350_material_skin() -> void:
...
func _apply_upper_opening_reduction() -> void:
	var group := Node3D.new(); group.name = "V0350_Upper_Loading_Opening_Reduced_13x10_Percent"; barn.add_child(group)
	var granite := _make_granite_skin(); var timber := _make_timber_skin(); var iron := StandardMaterial3D.new(); iron.resource_name = "V0350_Upper_Opening_Aged_Iron"; iron.albedo_color = Color("#242322"); iron.roughness = 0.97
	# The carrier opening is 3.82m x 1.02m. These shallow exterior infills and
	# the narrower shutter pair are the only local geometry change in v0.350.
	_box_visual("V0350_UpperOpening_Left_Masonry_Infill", Vector3(0.54,0.12,0.98), Vector3(-1.93,3.18,5.02), granite, group)
	_box_visual("V0350_UpperOpening_Right_Masonry_Infill", Vector3(0.54,0.12,0.98), Vector3(1.93,3.18,5.02), granite, group)
	_box_visual("V0350_UpperOpening_Top_Masonry_Infill", Vector3(3.32,0.12,0.10), Vector3(0,3.64,5.02), granite, group)
	_box_visual("V0350_UpperOpening_Left_Shutter", Vector3(1.55,0.075,0.82), Vector3(-0.82,3.18,5.10), timber, group)
	_box_visual("V0350_UpperOpening_Right_Shutter", Vector3(1.55,0.075,0.82), Vector3(0.82,3.18,5.10), timber, group)
	_box_visual("V0350_UpperOpening_Center_Iron", Vector3(0.075,0.09,0.86), Vector3(0,3.18,5.15), iron, group)
	_box_visual("V0350_UpperOpening_Lintel", Vector3(3.36,0.10,0.10), Vector3(0,3.64,5.14), timber, group)

func _add_foundation_weathering() -> void:
	var group := Node3D.new(); group.name = "V0350_Irregular_Foundation_Contact_Weathering"; barn.add_child(group)
	var damp := StandardMaterial3D.new(); damp.resource_name = "V0350_Bounded_Soil_Damp_Granite"; damp.albedo_color = Color("#625f55"); damp.roughness = 1.0
	var soil := StandardMaterial3D.new(); soil.resource_name = "V0350_Subtle_Soil_Contact"; soil.albedo_color = Color("#51493e"); soil.roughness = 1.0
	_box_visual("V0350_Foundation_Left_Corner", Vector3(1.35,0.16,0.34), Vector3(-6.55,0.42,4.92), damp, group)
	_box_visual("V0350_Foundation_Right_Corner", Vector3(1.10,0.16,0.28), Vector3(6.60,0.38,4.92), damp, group)
	_box_visual("V0350_Foundation_Door_Contact", Vector3(2.35,0.12,0.18), Vector3(0.15,0.25,5.00), soil, group)
	_box_visual("V0350_Foundation_Side_Damp", Vector3(0.22,1.15,0.42), Vector3(-7.75,0.48,3.95), damp, group)

func _build_context_dressing() -> void:
	context_workers = Node3D.new(); context_workers.name = "V0350_Context_Workers_Review_Only"; world.add_child(context_workers); context_workers.visible = false
	_make_context_worker("V0350_Worker_Near_House02", Vector3(-10.0,0.18,5.6), Color("#53645b"))
	_make_context_worker("V0350_Worker_Near_Barn", Vector3(10.0,0.18,5.6), Color("#695340"))
...
func _make_context_worker(name: String, position: Vector3, tint: Color) -> void:
	...
	var body_material := StandardMaterial3D.new(); body_material.resource_name = "V0350_Context_Worker_Muted_Cloth"; body_material.albedo_color = tint; body_material.roughness = 0.98
	var skin := StandardMaterial3D.new(); skin.resource_name = "V0350_Context_Worker_Skin"; skin.albedo_color = Color("#9b755d"); skin.roughness = 1.0
	var body_mesh := CapsuleMesh.new(); body_mesh.height = 1.05; body_mesh.radius = 0.22
	var body := MeshInstance3D.new(); body.name = "Body"; body.mesh = body_mesh; body.material_override = body_material; body.position = Vector3(0,0,0.53); root.add_child(body)
	var head_mesh := SphereMesh.new(); head_mesh.radius = 0.22; head_mesh.height = 0.44
	var head := MeshInstance3D.new(); head.name = "Head"; head.mesh = head_mesh; head.material_override = skin; head.position = Vector3(0,0,1.30); root.add_child(head)
...
func _capture_raw_source_set() -> void:
	await _capture_matched_roof(true,"01_matched_house02_barn_neutral_full.png","matched House02/barn neutral full assets")
	await _capture_matched_roof(true,"02_matched_roof_normal_enabled.png","matched House02/barn roof with normal enabled")
	await _capture_matched_roof(false,"03_matched_roof_normal_disabled.png","matched House02/barn roof with normal disabled")
	_set_slate_normal(false); await _frame("04_barn_albedo_only_roof.png",Vector3(-15,8.4,15),Vector3(0,3.6,0),10.5,"barn albedo-only slate close-up"); _set_slate_normal(true)
	await _frame("05_neutral_barn_front_three_quarter.png",Vector3(-15,8.4,15),Vector3(0,2.65,0),16.5,"neutral-overcast front three-quarter")
	await _frame("06_direct_front.png",Vector3(0,7.0,18),Vector3(0,2.55,0),15.0,"direct front opening hierarchy")
	await _frame("07_complete_direct_exterior_gable.png",Vector3(0,8.0,22.0),Vector3(0,2.7,0),17.5,"complete direct exterior gable silhouette")
	await _frame("08_granite_foundation_closeup.png",Vector3(-10.0,5.8,12.2),Vector3(0,2.75,0),10.0,"granite, foundation contact and weathering")
	await _frame("09_doors_openings_closeup.png",Vector3(0,5.8,13.0),Vector3(0,2.9,0),9.0,"doors, upper loading opening and iron")
	await _frame("10_roof_edge_slate_irregularity.png",Vector3(-12,6.7,15),Vector3(0,4.35,0),8.0,"roof edge and traditional slate irregularity")
	await _frame("11_far_rts.png",Vector3(-19,14.5,22),Vector3(0,2.0,0),24.0,"ordinary far RTS readability")
	var source := await _frame("12_true_256_pixel_source.png",Vector3(-15,8.4,15),Vector3(0,2.65,0),16.5,"true 256-pixel source"); var small := _square_panel(source,256); small.save_png(capture_root.path_join("screenshots").path_join("12_true_256_pixel_source.png")); captures[captures.size()-1]["size"] = {"width":256,"height":256}
	var neutral := await _frame("13_greyscale.png",Vector3(-15,8.4,15),Vector3(0,2.65,0),16.5,"neutral source for greyscale conversion",false); var gray := neutral.duplicate(); gray.convert(Image.FORMAT_RGBA8)
	...
	_save_image("13_greyscale.png",gray); captures.append({"fileName":"13_greyscale.png","purpose":"greyscale value proof","rendered":true,"unlabelled":true,"size":{"width":gray.get_width(),"height":gray.get_height()}})
	key.light_color = Color("#d6a77b"); key.light_energy = 1.0; await _frame("14_restrained_warm_directional.png",Vector3(-15,8.4,15),Vector3(0,2.65,0),16.5,"restrained warm directional"); key.light_color = Color("#e0d7c4"); key.light_energy = 1.08
	await _capture_contextual_hamlet()

func _capture_contextual_hamlet() -> void:
	...
	await _frame("15_contextual_house02_barn_hamlet.png",Vector3(0,10.5,32),Vector3(0,2.4,0),34.0,"bounded House02/barn hamlet material-unity context")
...
func _write_manifest() -> void:
	var outcome := OS.get_environment("V0350_INTERNAL_OUTCOME"); if outcome == "": outcome = READY_OUTCOME
	var file := FileAccess.open(capture_root.path_join("v0350-final-house02-barn-material-unity-runtime.json"),FileAccess.WRITE); if file == null: return
	var manifest := {"schemaVersion":1,"checkpoint":CHECKPOINT,"status":"PASS_V0350_FINAL_HOUSE02_BARN_MATERIAL_UNITY_RUNTIME" if errors.is_empty() else "FAIL_V0350_FINAL_HOUSE02_BARN_MATERIAL_UNITY_RUNTIME","outcome":outcome,"humanReviewRequired":true,"automatedVisualApproval":false,"prototypeOptIn":true,"prototypeOnly":true,"defaultRuntimeIntegrated":false,"scenePath":SCENE_PATH,"sourceBlend":"art-source/blender/v0350/barn_final_material_harmony.blend","sourceGLB":BARN_GLB,"godotImportedBounds":_mesh_bounds(barn) if barn != null else {},"rawCaptureCount":captures.size(),"captures":captures,"requiredRawNames":RAW_NAMES,"diagnosticEvidence":["normal_enabled.png","normal_disabled.png","albedo_only.png","roughness_isolation.png","uv_checker_source.txt"],"upperOpeningReductionPercent":{"width":13.0,"height":9.8},"oldUpperOpeningDimensions":{"width":upper_opening_old.x,"height":upper_opening_old.y},"newUpperOpeningDimensions":{"width":upper_opening_new.x,"height":upper_opening_new.y},"lowerUpperVisibleAreaRatio":3.64,"noLabels":true,"noOverlays":true,"noVideo":true,"noGameplay":true,"noMovement":true,"noPathfinding":true,"noCombat":true,"noEconomy":true,"noResources":true,"noRuntimeIntegration":true,"errors":errors}
