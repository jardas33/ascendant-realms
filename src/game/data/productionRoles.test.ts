...
describe("production role architecture", () => {
  it("keeps Command Hall as the Worker-only base hub", () => {
    ...
    expect(commandHall.upgradeOptions).toEqual(["camp_foundations_1"]);
    ...
    expect(UPGRADE_BY_ID.camp_foundations_1.ownerBuildingId).toBe("command_hall");
    expect(UPGRADE_BY_ID.camp_foundations_1.category).toBe("core");
  ...
  it("keeps Barracks as basic army production and research owner", () => {
    ...
    expect(UPGRADE_BY_ID.infantry_weapons_1.ownerBuildingId).toBe("barracks");
    expect(UPGRADE_BY_ID.reinforced_armor_1.ownerBuildingId).toBe("barracks");
    expect(UPGRADE_BY_ID.ranger_training_1.ownerBuildingId).toBe("barracks");
  ...
  it("keeps Mystic Lodge and Watchtower on their completed-building roles", () => {
    ...
    expect(UPGRADE_BY_ID.aether_study_1.ownerBuildingId).toBe("mystic_lodge");
    ...
    expect(UPGRADE_BY_ID.sentry_bracing_1.ownerBuildingId).toBe("watchtower");
    expect(UPGRADE_BY_ID.sentry_bracing_1.prerequisites?.buildingIds).toEqual(["watchtower"]);
    expect(UPGRADE_BY_ID.sentry_bracing_1.prerequisites?.upgradeIds).toEqual(["camp_foundations_1"]);
