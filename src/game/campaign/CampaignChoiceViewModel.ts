...
export interface CampaignChoiceViewModel {
...
interface CampaignChoiceViewModelInput {
...
export function createCampaignChoiceViewModels(options: Omit<CampaignChoiceViewModelInput, "choice">): CampaignChoiceViewModel[] {
  return (options.node.choices ?? []).map((choice) => createCampaignChoiceViewModel({ ...options, choice }));
}

export function createCampaignChoiceViewModel(options: CampaignChoiceViewModelInput): CampaignChoiceViewModel {
  const { node, status, campaignSave, heroSave, choice } = options;
  ...
  return {
    ...
    locked,
    reason,
    costSummary: formatChoiceCostSummary({ node, choice, heroSave }),
    rewardSummary: formatChoiceRewardSummary(choice, heroSave, node),
    reputationSummary: formatChoiceReputationSummary(choice, heroSave, node),
    modifierSummary: formatChoiceModifierSummary(choice, heroSave, node),
    outcomeSummary: choice.completesNode === false ? "Keeps this node open." : isTown ? repeatabilityLabel(choice) : "Completes this node.",
    availabilityLabel: locked ? reason || "Locked" : isTown ? repeatabilityLabel(choice) : "Available",
    ctaLabel: isTown ? townServiceActionLabel(Boolean(stock)) : "Choose",
    stock: stock
      ? {
          name: stock.name,
          rarityLabel: titleCase(stock.rarity),
          slotLabel: titleCase(stock.slot)
        }
      : undefined
  };
}
