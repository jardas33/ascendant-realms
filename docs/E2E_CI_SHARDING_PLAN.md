...
| --- | --- | --- | --- |
| Smoke/default | `npm run test:e2e:smoke` | `tests/e2e/smoke.spec.ts` | Fastest frequent browser iteration lane. Covers boot, Settings, campaign launch, Cinderfen route smoke, skirmish, difficulty, and inventory. |
...
| Release gate | `npm run test:e2e:release` or `npm run test:e2e` | All e2e specs | Full 59-test release/checkpoint gate. |
...
- Both shards together preserve the full 59-test release gate.
