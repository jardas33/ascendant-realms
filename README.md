...
Current v0.18 work adds the first Worker construction foundation after the clean v0.17.5 baseline: Command Hall trains Workers, a Worker can build one Barracks construction site, assigned Worker proximity gates construction progress, incomplete Barracks cannot produce, and completed Barracks keeps existing production. It preserves save format, runtime art/assets, maps, factions, Tutorial requirements, Command Hall direct placement fallback, enemy construction AI scope, Patrol, formations, and broad economy/balance architecture.

The package flow still packages the browser prototype for private human playtesting: playtest-safe production build output, ignored package folders under `artifacts/playtest/`, tester-facing README and feedback files, current retest materials, build metadata, local server launchers, and package verification.
