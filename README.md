...
- v0.11.5 GitHub Actions Fast Confidence Lane Split: automatic push/PR CI now runs `npm run test:e2e:smoke:fast`, a six-test `@ci-fast` subset, while full `npm run test:e2e:smoke` and the release/manual lanes keep all campaign/skirmish smoke coverage. No smoke tests were deleted, skipped, or weakened.
- v0.11.9 Hosted Release Matrix Split: manual GitHub release matrix now uses six hosted shards with a 45-minute per-shard timeout, while local full release and local 3-way shard scripts remain available. No release tests were deleted, skipped, or weakened.
...
The browser suite uses Playwright and starts the Vite dev server automatically. The automatic GitHub fast-confidence lane runs `npm run test:e2e:smoke:fast`, which selects the six `@ci-fast` smoke checks: main menu boot, Tutorial / Proving Grounds no-reward entry, Tutorial exit without saving, Settings persistence, New Campaign into Campaign Map, locked-node blocking, and inventory reachability.

The full smoke lane remains `npm run test:e2e:smoke`. It runs all 12 tests in `tests/e2e/smoke.spec.ts`, including the `@ci-fast` checks plus the extended campaign/skirmish coverage: Border Village battle launch, Cinderfen Overlook/Waystation/Crossing reward persistence, Cinderfen Watch and Aftermath completion, Malrec trophy event behavior, skirmish launch, and difficulty/fog setup.
...
v0.11.9 adds additive hosted-only 6-shard release scripts for the manual GitHub Actions release matrix:
...
All six hosted scripts together equal the same full release suite. They use Playwright test-level sharding with `--fully-parallel --workers=1`, which keeps each hosted shard single-worker while avoiding file-shaped shard imbalance. They are CI ergonomics for GitHub-hosted runners after the 3-way hosted matrix hit the 35-minute job cap; local full release, local 2-way shards, and local 3-way shards remain unchanged.
...
v0.11.1 adds `.github/workflows/ci.yml` as a conservative GitHub Actions dry run. v0.11.5 keeps that workflow but changes automatic fast confidence on pull requests and pushes to `main` to run `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run test:e2e:smoke:fast`, and `npm run smoke:preview` after `npm ci` and Playwright Chromium installation. v0.11.9 keeps heavy lanes manual and changes the release-matrix workflow input to run the hosted 6-way release matrix plus simulator; optional visual QA and the full one-command release lane remain manual inputs. Heavy lanes remain manual so the local final gate stays authoritative and CI does not duplicate every expensive run on every push. v0.11.2 could not fetch remote Actions evidence from this environment because authenticated access was unavailable; use `docs/V112_MANUAL_GITHUB_ACTIONS_CHECKLIST.md` to inspect the workflow in GitHub UI and report hosted-run timing/artifacts before making CI-only tuning changes.
...
This runs the deterministic campaign battle simulator and regenerates `PLAYTEST_TELEMETRY.md` and `PLAYTEST_TELEMETRY.json`. Latest v0.10 report-gate status, 2026-05-11: passed with 255 simulated runs across 85 campaign battle node/profile summaries. The latest pressure telemetry remains no too-easy nodes, no structural too-hard nodes, Ashen Outpost beatable, no Stronghold warnings, no enemy-pressure warnings, 75 pressure-enabled Cinderfen runs, 63 triggered pressure runs, 12 quiet/untriggered pressure runs, 149 pressure warnings, 147 losses after pressure, and 0 simulated reinforcement applications. v0.10 applies no balance tuning and keeps `reinforce_next_wave`, `contest_capture_site`, and `defensive_hold` warning/telemetry-only.
