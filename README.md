...
- v0.11.10 Hosted Release Matrix Determinism: manual GitHub release matrix now uses explicit hosted groups for deep-flow, layout, pressure, and smoke coverage instead of `--fully-parallel` native shards. Seeded-save tests now seed before app boot, and no release tests were deleted, skipped, or weakened.
...
v0.11.10 supersedes the v0.11.9 hosted native shards with explicit hosted release groups for the manual GitHub Actions release matrix:
...
All six hosted group scripts together equal the same 67-test full release suite. They avoid `--fully-parallel` test-level sharding, keep the hosted matrix manual-only, and group related deep-flow, layout, pressure, and smoke coverage for GitHub-hosted runners. Local full release, local 2-way shards, and local 3-way shards remain unchanged.
...
v0.11.1 adds `.github/workflows/ci.yml` as a conservative GitHub Actions dry run. v0.11.5 keeps that workflow but changes automatic fast confidence on pull requests and pushes to `main` to run `npm test`, `npm run build`, `npm run validate:content`, `npm run validate:art-intake`, `npm run test:e2e:smoke:fast`, and `npm run smoke:preview` after `npm ci` and Playwright Chromium installation. v0.11.10 keeps heavy lanes manual and changes the release-matrix workflow input to run explicit hosted release groups plus simulator; optional visual QA and the full one-command release lane remain manual inputs. Heavy lanes remain manual so the local final gate stays authoritative and CI does not duplicate every expensive run on every push. v0.11.2 could not fetch remote Actions evidence from this environment because authenticated access was unavailable; use `docs/V112_MANUAL_GITHUB_ACTIONS_CHECKLIST.md` to inspect the workflow in GitHub UI and report hosted-run timing/artifacts before making CI-only tuning changes.
