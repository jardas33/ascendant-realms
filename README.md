...
Current v0.82 work implements only that smallest approved Lume runtime slice. It adds data validation, battle-local link resolution, HUD/selected-site/Results readability, focused runtime tests, one hosted proxy, and package metadata. It does not change save format, internal IDs, maps, factions, races, units, buildings, classes, runtime art/assets, desktop packaging, engine choice, runtime rebrand/display-copy migration, Jardas binding, Worker binding, hero binding, global resource production, enemy AI tuning, multiplayer, PvP, or co-op.

The package flow still packages the browser prototype for private human playtesting: playtest-safe production build output, ignored package folders under `artifacts/playtest/`, tester-facing README and feedback files, current retest materials, build metadata, local server launchers, and package verification. v0.82 package metadata includes the runtime Lume prototype docs, safety report, implementation report, and Emmanuel retest checklist.
