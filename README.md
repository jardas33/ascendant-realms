...
Current v0.13 work adds an automated playtest scenario lab and balance telemetry layer over the existing deterministic simulator: route/profile definitions, derived metrics, watchpoint classification, Markdown/JSON reports, and scripts for lab/profile/watchpoint output. No runtime gameplay, gameplay numbers, maps, factions, units, art/assets, save format, combat systems, campaign progression, or hosted release plumbing was changed.
...
`playtest:lab` regenerates `PLAYTEST_SCENARIO_LAB.md`, `PLAYTEST_SCENARIO_LAB.json`, and `PLAYTEST_WATCHPOINT_SUMMARY.md`. `playtest:profiles` regenerates `PLAYTEST_SCENARIO_PROFILES.md` and `PLAYTEST_SCENARIO_PROFILES.json`. These are automated deterministic evidence only; they do not include or replace human tester feedback.
