...
Current v0.14.3 work uses Emmanuel's retest of the first private playtest session and fixes the highest-friction remaining combat/control issues: marquee drag selection works again even when releasing over HUD, adjacent melee units can attack through contact reach, normal move-away orders get a short priority window before reacquiring combat, tutorial defeat shows no-save/no-reward results guidance, and hero class/origin choices explain actual stats, bonuses, and primary abilities. No gameplay data numbers, save format, maps, factions, units, runtime art/assets, broad combat AI/pathing rewrite, protected UI copying, or visual overhaul changed.
