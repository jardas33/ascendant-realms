...
Current v0.13.1 work extends the automated playtest scenario lab and balance telemetry layer over the existing deterministic simulator: repeated deterministic profile batches, profile comparison metrics, node-risk dashboarding, conservative regression thresholds, Markdown/JSON/CSV reports, and scripts for extended lab/profile/watchpoint output. No runtime gameplay, gameplay numbers, maps, factions, units, art/assets, save format, combat systems, campaign progression, or hosted release plumbing was changed.
