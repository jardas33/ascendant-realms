...
Current v0.14.1 work ingests Emmanuel's first real private playtest session and fixes narrow input/HUD/menu/command usability failures: focused text inputs accept movement-key letters, selection drag state clears over HUD, battle Menu pauses instead of immediately exiting, normal move orders can retreat from combat, and hero skill/order copy is clearer. No gameplay numbers, save format, maps, factions, units, runtime art/assets, broad combat AI, or balance tuning changed.
