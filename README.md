...
Latest v0.9.1 controlled Cinderfen style-frame intake checkpoint status, 2026-05-10: `npm run validate:content` passes, `npm run validate:art-intake` passes with the template-only empty intake, and `npm test` passes with 46 test files and 351 tests. v0.9.1 added only non-runtime review folders, docs, metadata templates, schema/types, and metadata validation for future candidates; it did not generate, import, or wire art assets.
