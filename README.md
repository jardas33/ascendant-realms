...
Current v0.103 work reduces stable battlefield visual clutter and adds private/dev-only performance profiling evidence. It adds contextual capture-site labels, quieter Lume Auto presentation, a private Performance Lab, local profiler reports, visual QA coverage, and package metadata. It does not change save format, internal IDs, maps, factions, races, units, buildings, classes, runtime art/assets, desktop packaging, engine choice, runtime rebrand/display-copy migration, Jardas binding, Worker binding, hero binding, global resource production, enemy AI tuning, multiplayer, PvP, or co-op.

The package flow still packages the browser prototype for private human playtesting: playtest-safe production build output, ignored package folders under `artifacts/playtest/`, tester-facing README and feedback files, current retest materials, build metadata, local server launchers, and package verification. v0.103 package metadata includes the battlefield clutter spec, profiler spec, Performance Lab manifest, baseline and optimization reports, implementation report, deferred findings, visual QA report, and Emmanuel retest checklist.
