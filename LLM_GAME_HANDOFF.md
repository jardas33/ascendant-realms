...
Status: v0.108 adds a private, no-save Representative Battle Benchmark group in the Playtest Hub plus browser benchmark scripts, merged benchmark reports, visual QA coverage, and provisional desktop acceptance docs. It uses existing current content only: Broken Ford, Aether Well Ruins, Aster, Worker, Militia, Ranger, Ashen enemies, existing Command Hall/Barracks/stronghold structures, existing capture-site infrastructure as mine/shrine equivalents, one resource-site focus, one Lume link, HUD/minimap/fog/notification/minimap interaction states, and a no-save private Lume Results transition. It does not create new mine or shrine building IDs.

Included work:

- Added `src/game/playtest/RepresentativeBattleBenchmark.ts` and focused tests.
- Added the private Playtest Hub group `REPRESENTATIVE BATTLE BENCHMARK` with ten isolated entries.
- Added benchmark-only private battle staging for Tier S smoke, Tier M representative, Tier L stress, Lume Hidden/Auto/Always, fog-heavy, notification-heavy, minimap interaction, and Results transition.
- Added `npm run benchmark:battle:smoke`, `npm run benchmark:battle:representative`, `npm run benchmark:battle:stress`, and `npm run benchmark:battle:report`.
- Added ignored benchmark output under `artifacts/benchmarks/v0108/`.
- Updated the private Results handoff so hub-launched forced-victory benchmark results keep the Playtest Hub return context.
- Updated visual QA to 213 screenshots and the visual review pack to 9 contact sheets, including a Representative Benchmark family.
- Updated private playtest package metadata/docs to v0.108.
- Added `docs/V0108_REPRESENTATIVE_BATTLE_PROFILE.md`, `docs/V0108_BENCHMARK_SCENARIO_MANIFEST.json`, `docs/V0108_BROWSER_BATTLE_BENCHMARK_REPORT.md`, `docs/V0108_DESKTOP_ACCEPTANCE_PROFILE.md`, `docs/V0108_PERFORMANCE_DELTA_REPORT.md`, `docs/V0108_VISUAL_QA_REPORT.md`, `docs/V0108_IMPLEMENTATION_REPORT.md`, and `docs/V0108_EMMANUEL_BENCHMARK_GUIDE.md`.

Runtime/save/art boundary:

- No save-version bump.
- No save fields, localStorage keys, stable IDs, serialized IDs, rewards, XP, Retinue state, relics, reputation, campaign progression, gameplay rules, combat balance, AI/pathing rules, maps, factions, generated/imported art, runtime asset paths, public benchmark controls, engine choice, desktop port, desktop save path, multiplayer, PvP, or co-op changed.
- Mine/shrine benchmark coverage is existing capture-site infrastructure only: `west_stone_cut` and `ford_toll`.
- Tier L stress is private/local-only regression-hunting evidence and is not a CI acceptance lane.
...
Browser plugin check - PASS, in-app Browser verified the Representative Battle Benchmark group plus smoke, representative, and Results-transition entries on the local dev server.
```
...
## Current v0.107 Salto Vertical Slice Composition Plan And Asset-Dimension Contracts - 2026-06-02

Status: v0.107 is a docs/tooling/validation checkpoint for the first polished Salto visual slice. It defines one representative battle slice, 15 planned asset contracts, one Results-frame companion, known v0.106 runtime slot mappings, fallback owners, QA scenario IDs, dependency order, and an ignored metadata-only packet generator. It does not generate or import images, add final art, approve runtime loading, alter gameplay, change saves, rename stable IDs, add maps/factions/races/units/buildings, choose an engine, create a desktop port, change package metadata, or start v0.108.

Included work:

- Added `docs/V0107_SALTO_VERTICAL_SLICE_COMPOSITION_SPEC.md`.
- Added `docs/V0107_ASSET_DIMENSION_CONTRACTS.md`.
- Added `docs/V0107_SALTO_VERTICAL_SLICE_MANIFEST.json`.
- Added `docs/V0107_GENERATION_DEPENDENCY_ORDER.md`.
- Added `docs/V0107_FIRST_SLICE_REVIEW_GATE.md`.
- Added `docs/V0107_IMPLEMENTATION_REPORT.md`.
- Added `docs/V0107_EMMANUEL_ART_GENERATION_CHECKLIST.md`.
- Added `tools/salto-slice/` manifest validation, deterministic packet generation, and tests.
- Added `npm run art:packet:salto-slice`.
- Added ignored output `artifacts/art-review/salto-slice-packet/`.

Runtime/save/art boundary:

- No save-version bump.
- No runtime save fields, localStorage keys, stable IDs, serialized IDs, rewards, XP, campaign progression, gameplay rules, balance, AI/pathing, maps, factions, generated/imported art, runtime asset paths, package metadata, desktop implementation, engine choice, or runtime title changed.
- v0.107 asset IDs are planning IDs that map to v0.105/v0.88 registry entries; every runtime slot remains fallback-owned.
- The optional private mock composition preview is deferred.

Verification:

```text
npx vitest run tools/salto-slice/saltoSlicePacket.test.ts --reporter=dot - PASS, 1 file / 9 tests.
npm run art:packet:salto-slice - PASS, generated ignored metadata packet.
...
## Current v0.106 Runtime Art Slot Adapter And Placeholder Fallback Harness - 2026-06-02

Status: v0.106 adds a typed runtime art slot contract and private fallback review harness. It defines 52 runtime art slots, keeps every slot fallback-safe by default, rejects unapproved/candidate/final-art paths, adds private-only diagnostics plus mock routing, adds Art Slot Fallbacks to the private Playtest Hub, updates visual QA to 203 screenshots, updates the visual review pack to 8 contact sheets, and updates package metadata to v0.106. It does not generate images, import candidate images, wire unapproved art into runtime, alter gameplay, change saves, rename stable IDs, change maps/factions/balance/rewards, choose an engine, start desktop work, or change the runtime title.

Included work:

- Added `src/game/art/RuntimeArtSlotTypes.ts`, `RuntimeArtSlots.ts`, and `RuntimeArtSlotAdapter.ts`.
- Added `npm run validate:runtime-art-slots`.
- Added private-only `Art Slots` diagnostics and mock routing in `PrivateRuntimeArtSlotDiagnostics.ts`.
- Added `art_slot_fallbacks` private Playtest Hub scenarios.
- Added v0.106 visual QA coverage for public hidden posture, diagnostics off/on, menu, campaign, battlefield, unit, building, Lume, HUD, Results, mock routing, and 1920/1600/1366 desktop viewports.
- Added an `Art Slot Fallbacks` visual review-pack family and contact sheet.
- Updated private playtest package metadata/validation to v0.106 and to include v0.105/v0.106 art docs.
- Added v0.106 contract, fallback matrix, validation, visual QA, implementation, and Emmanuel guide docs.

Runtime/save/art boundary:

- No save-version bump.
- No save fields, localStorage keys, stable IDs, serialized IDs, rewards, XP, campaign progression, gameplay rules, balance, AI/pathing, maps, factions, generated/imported art, unapproved runtime asset loading, desktop implementation, engine choice, or runtime title changed.
- `runtime-candidate-approved` remains non-loadable; only future `runtime-integrated` assets under `public/assets/runtime-art/` can load.
...
## Current v0.105 Visual Asset Registry, Candidate Review Workspace, and Art-Intake Tooling - 2026-06-02

Status: v0.105 is a tooling/schema/docs checkpoint for future art review. It adds a committed reference-only registry seeded from the v0.88 vertical-slice manifest, ignored candidate workspace tooling, art-review validation, SVG contact-sheet output, deterministic review reports, and Emmanuel's first art review packet. It does not generate images, import art, wire runtime assets, alter gameplay, change saves, rename stable IDs, change runtime title, alter package metadata, choose an engine, start desktop work, or start v0.106.

Included work:

- Added `src/game/art/visual-asset-registry.schema.json` and `src/game/art/visual-asset-registry.json`.
- Added `src/game/art/VisualAssetReviewRegistry.ts` plus focused validation tests.
- Added `npm run art:review:init`, `npm run art:review:validate`, `npm run art:review:contact-sheet`, and `npm run art:review:report`.
- Added ignored review artifact roots under `artifacts/art-review/candidates/`, `artifacts/art-review/contact-sheets/`, and `artifacts/art-review/reports/`.
- Added contact-sheet/report tooling tests that verify no runtime asset path is touched.
- Added v0.105 registry, workspace, state-machine, first-generation packet, implementation, and Emmanuel review docs.

Runtime/save/art boundary:

- No save-version bump.
- No save fields, localStorage keys, stable IDs, serialized IDs, rewards, XP, campaign progression, gameplay rules, balance, AI/pathing, maps, factions, generated/imported art, runtime asset paths, package metadata, desktop implementation, engine choice, or runtime title changed.
- `style-approved` remains reference-only; `runtime-integrated` is rejected without future integration proof.
...
Package generation and package verification are not required for v0.105 because package metadata and private playtest package contents are unchanged.

## Current v0.104 Profiler-Guided Rendering Optimization And Public Battle HUD Minimal Mode - 2026-06-02

Status: v0.104 is an evidence-led rendering and HUD presentation checkpoint. It reduces redundant HUD/minimap/fog/Lume rendering work using the v0.103 profiler baseline, makes public battles default to Minimal HUD density, keeps Standard/Debug HUD density controls private, expands the private profiler to 20 scenarios, and updates visual QA to 189 screenshots. It does not alter gameplay systems, balance, rewards, saves, stable IDs, campaign progression, Lume rules, art, maps, factions, desktop work, runtime title, or public private-control exposure.

Included work:

- Added `HudDensity` helpers, density-aware HUD panels, private Debug counters, and volatile HUD-region patching.
- Moved ordinary battle HUD snapshot work behind the existing 0.1s cadence while keeping forced refreshes immediate.
- Added minimap render-signature caching plus fog/Lume redraw skips when rendered signatures are unchanged.
- Added private HUD density profiler scenarios: `perf_hud_minimal`, `perf_hud_standard`, and `perf_hud_debug`.
- Added v0.104 profiler triage, optimization, performance delta, visual QA, implementation, and Emmanuel retest docs.
- Added 17 v0.104 visual QA screenshots and hardened visual QA launch waits without reducing assertions.

Runtime/save/art boundary:

- No save-version bump.
- No save fields, localStorage writes, stable IDs, serialized IDs, rewards, XP, Retinue, relics, reputation, campaign progression, Lume benefit multiplier, combat balance, AI/pathing, maps, factions, generated/imported art, desktop implementation, runtime title, or public Debug controls changed.
...
## Current v0.103 Battlefield Clutter Reduction And Private Performance Profiler - 2026-06-01

Status: v0.103 is a presentation and private QA-tooling checkpoint. It reduces stable battlefield visual noise, adds a private/dev-only performance profiler plus Performance Lab scenarios, updates visual QA to 172 screenshots, and updates private package metadata. It does not alter gameplay systems, balance, rewards, saves, stable IDs, campaign progression, Lume rules, art, maps, factions, desktop work, or public runtime posture.

Included work:

- Added `src/game/playtest/PrivatePerformanceProfiler.ts` and focused tests.
- Added private profiling scripts `npm run perf:profile:private` and `npm run perf:report:private`.
- Added Performance Lab entries to the private Playtest Hub using no-save fixtures.
- Added private scene counters for display objects, labels, Lume, fog, minimap, HUD, and notifications.
- Reduced quiet capture-site label/ring clutter and stable Lume Auto link clutter.
- Compacted Lume HUD optional-link/private-preview copy.
- Added 27 v0.103 visual QA screenshots and the v0.103 docs/retest packet.

Runtime/save/art boundary:

- No save-version bump.
- No save fields, localStorage writes, stable IDs, serialized IDs, rewards, XP, Retinue, relics, reputation, campaign progression, Lume benefit multiplier, combat balance, AI/pathing, maps, factions, generated/imported art, desktop implementation, or runtime title changed.
...
## Current v0.102 Browser Save Fixture Library And Desktop Translation Contract Proof - 2026-06-01

Status: v0.102 is a save testing, schema-contract, tooling, and documentation checkpoint. It creates deterministic fictional browser save fixtures and a pure translation-contract proof for future desktop experiments. It does not alter runtime save behavior, gameplay, balance, rewards, stable IDs, `CURRENT_SAVE_VERSION`, localStorage behavior, engine posture, package posture, or desktop implementation.

Baseline:

- Starting commit: `fbea00d`.
- Starting branch state: clean `main`, synced with `origin/main`.
- Baseline remote status: GitHub Actions run `26786777684` on `fbea00d` passed after rerunning the Fast confidence job; local exact `npm run test:e2e:smoke:fast` had also passed during baseline triage.
- v0.101 package generation was not required because v0.101 changed tooling/docs only.

Included work:

- Added pure save translation contract helper `src/game/save/SaveTranslationContract.ts`.
- Added focused tests in `src/game/save/SaveTranslationContract.test.ts`.
- Added CLI `tools/testSaveTranslationContract.ts`.
- Added `npm run test:save-translation-contract`.
- Added the v0.102 fixture library and manifest under `tests/fixtures/saves/v0102/`.
- Added ignored proof-output folder `artifacts/save-translation-contract/`.
- Added v0.102 save fixture, envelope, translation proof, unknown-ID/corruption policy, and implementation docs.

Runtime/save/art boundary:

- Tooling, tests, docs, fixtures, and ignored generated artifacts changed.
- No save-version bump.
- No runtime save fields, localStorage keys, real-save writes, stable IDs, serialized IDs, content definitions, gameplay rules, rewards, campaign progression, Retinue rules, settings behavior, desktop save path, profile UI, desktop port, engine choice, generated images, imported assets, or package metadata changed.
...
Status: v0.100 is a private-package QA convenience checkpoint. It adds a gated Playtest Hub, grouped Scenario Gallery, 8-minute visual tour, fixture launchers, save isolation, visual QA captures, and package validation docs. It does not expose shortcuts in production posture, alter normal progression, saves, persistent rewards, gameplay rules, balance, stable IDs, maps, factions, art, imported assets, desktop work, or start v0.101.
...
- Added a private-only `PlaytestHubScene` behind the existing private playtest tooling gate.
- Added grouped one-click scenarios for Campaign Shell, First Session, Battle Shell, Lume, and Meta review families.
- Added an 8-minute visual tour with Next / Back / Exit controls.
- Added deterministic no-save fixture helpers for campaign, battle, hero progression, Lume, ordinary Results, defeat Results, and private-demo Results previews.
- Added in-memory raw-save snapshot/restore helpers so hub previews return the existing save key to its prior value.
- Added hub return controls to Main Menu, Campaign Map, Hero Creation, Hero Progression, Battle HUD, and Results preview flows.
- Added `docs/V0100_SCENARIO_GALLERY_MANIFEST.json` with purpose, expected UI, absent UI, manual review question, screenshot ID, automated coverage, launch context, and save-isolation rule per scenario.
- Expanded visual QA from 136 to 145 screenshots with nine v0.100 hub/gallery captures.
- Generated the visual review pack from the full visual QA set: 145 screenshots and 7 contact sheets.
...
- Runtime presentation, private-tool launch routing, tests, visual QA, docs, and package metadata changed.
- No save-version bump.
...
Closeout note: commit as `Checkpoint v0.100 private playtest hub and scenario gallery`, package cleanly on the final commit, verify, push safely, and stop. Do not start v0.101 automatically.

## Current v0.99 Act 1 Mission Presentation Objective Clarity And Narrative Polish - 2026-06-01

Status: v0.99 is a campaign presentation and copy-only checkpoint. It makes Act 1 read like a journey from Salto through road control, support choices, Lume pressure, hillfort/church prerequisites, and Ashen Outpost finale. It does not add nodes, alter unlock rules, rewards, difficulty, AI, saves, stable IDs, broad branding, art, imported assets, desktop work, or start v0.100.
...
- Act 1 node descriptions now read as concise one-line premises instead of internal-feeling mission blurbs.
...
- Campaign selected-mission panels now show a compact lock reason or recommended next step without changing actions or progression.
- Results overview now uses campaign primary-objective copy and existing Act 1 next-step guidance when campaign completion data exists.
- Captain Malrec copy now frames him as disciplined, dangerous, and convinced controlled Lume prevents collapse.
- Visual QA expanded from 126 to 136 screenshots with every Act 1 presentation node plus Ashen Outpost Results summary.
- Package generation and validation now require/copy the v0.99 docs and report the v0.99 checkpoint in playtest build info.

Runtime/save/art boundary:

- Presentation copy, rendering helpers, tests, visual QA, docs, generated control evidence, and package metadata changed.
- No save-version bump.
...
Closeout note: commit as `Checkpoint v0.99 Act 1 mission presentation objective clarity and narrative polish`, package cleanly on the final commit, verify, push safely, and stop. Do not start v0.100 automatically.
...
Status: v0.97 is a controls-readability and tactical-feedback checkpoint. It improves selection focus, command destination confirmation, camera focus affordance, enemy inspection clarity, command-panel disclosure copy, visual QA, package metadata, and retest docs. It does not add gameplay systems, change unit stats, alter combat balance, change pathing rules, change saves, rename stable IDs, add maps/factions/art/assets, start desktop work, add multiplayer/PvP/co-op, or start v0.98.
...
- Added short-lived command feedback markers for existing move, attack-move, attack target, Patrol, rally, build, ability, invalid, and focus flows.
- Added reduced-motion-safe marker presentation and cleanup without new art assets.
- Added selection focus cards for hero, Worker, squad, unit, building, site, and read-only enemy inspection.
- Enemy inspection is explicit and read-only; player behavior controls remain limited to friendly player-controlled selections.
- Space focus now centers the current selection first, then falls back to Aster.
- Minimap and focus actions now show concise status copy and a brief focus marker.
- Camera center/scroll clamping now uses a shared pure helper.
- Command-panel secondary explanations now sit behind `More Details`.
...
- Presentation/input-feedback code, CSS, tests, docs, visual QA, and package metadata changed.
- No save-version bump.
- No save fields, localStorage keys, persistent settings, stable IDs, serialized IDs, mission IDs, map IDs, node IDs, site IDs, Lume IDs, unit IDs, building IDs, reward IDs, hero rules, campaign progression, replay rules, rewards, XP, balance values, pathing rules, combat stats, maps, factions, generated images, imported assets, desktop port, engine choice, runtime title, or public title changed.
...
Non-pass evidence: an attempted full local `npm run test:e2e:release` exceeded the 60-minute tool timeout with no pass/fail output. A follow-up local `npm run test:e2e:release:shard1of3` attempt hit three startup/timeout-style failures in older hosted deep-flow coverage; the three exact failed tests were rerun and passed. Existing hosted tests still log older verified actionability fallbacks; v0.97 did not add or broaden canvas/world force-click or DOM fallback behavior for world clicks.

Closeout note: commit as `Checkpoint v0.97 camera selection orders and tactical feedback polish`, package cleanly on the final commit, verify, push safely, and stop. Do not start v0.98 automatically.

## Current v0.96 First-Time Player Onboarding And Tutorial UX Rescue - 2026-06-01

Status: v0.96 is a first-session presentation checkpoint. It improves the playable Tutorial sequence, contextual help, campaign first-step guidance, visual QA, package metadata, and first-session retest docs. It does not add gameplay, change hero rules, alter rewards, change saves, rename stable IDs, change campaign progression, add maps/factions/races/art/assets, broaden Lume rules, start desktop work, or start v0.97.
...
- Reordered Proving Grounds so the first visible objective is selecting Aster, then selecting troops, moving, capturing Crown Shrine, selecting Command Hall, building Barracks, assigning a Worker to the Shrine, training Militia, setting rally, casting Rally Banner, surviving pressure, and finishing.
- Extended Tutorial steps with optional `reason`, collapsed `moreHelp`, and player-initiated `focusTarget` metadata.
- Added `selectTroops` and `assignWorker` Tutorial completion signals.
- Added Tutorial panel More Help, Focus Objective, Dismiss, and Reopen controls without blocking battle input.
- Added a reusable collapsed onboarding help surface for the battle HUD, pause menu, and campaign shell.
- Added compact Salto next-action guidance below the primary campaign action.
- Added v0.96 unit, smoke, layout, hosted layout, and visual-QA coverage; deterministic visual QA now captures 110 screenshots.
- Added all required v0.96 audit/spec/report/visual-QA/retest docs and package-validation metadata.

Runtime/save/art boundary:

- Tutorial copy/step metadata, Tutorial panel UI, HUD help UI, campaign help UI, CSS, tests, docs, and package metadata changed.
- No save-version bump.
- No save fields, localStorage keys, persistent onboarding preferences, stable IDs, serialized IDs, mission IDs, map IDs, node IDs, site IDs, Lume IDs, unit IDs, building IDs, reward IDs, hero rules, campaign progression, replay rules, rewards, XP, balance values, pathing, collision, fog-of-war simulation, Lume mechanics, maps, factions, races, units, buildings, generated images, imported assets, desktop port, engine choice, runtime title, or public title changed.
...
Non-pass evidence resolved during v0.96: initial fast smoke showed Tutorial auto-completion from preselected Aster, campaign primary action crowding, and Tutorial `More Help` click/drag conflict; all were fixed and rerun. An early 20-minute visual-QA run timed out before completion; the final long-timeout run passed. Hosted layout-core caught stale `dist`, then a one-pixel campaign action overflow plus hidden help-grid mobile overflow; final build/CSS fixes passed layout-core. Existing hosted tests still log older verified actionability fallbacks; v0.96 did not add or broaden canvas/world click fallbacks.

Closeout note: commit as `Checkpoint v0.96 first-time player onboarding and Tutorial UX rescue`, package cleanly on the final commit, verify, push safely, and stop. Do not start v0.97 automatically.
...
Status: v0.94 is a presentation-only usability checkpoint. It improves the first desktop menu impression, reorganizes existing hero creation into three staged panels, compacts the selected mission panel, strengthens campaign map readability, improves campaign-tab card hierarchy, and groups ordinary Results expanded details. It does not add gameplay, change hero rules, alter rewards, change saves, rename stable IDs, add races/maps/factions/art, rebrand the runtime title, or start desktop work.
...
- Updated `MainMenuScene` and main-menu CSS with a wider desktop composition and grouped Play, Practice, and Manage actions.
- Updated `HeroCreationScene` and form CSS with Step 1 Choose Class, Step 2 Choose Origin, and Step 3 Review Hero while preserving class/origin ids and hero rules.
- Updated `CampaignMapScene` and campaign CSS with compact selected mission facts, reward chips, clearer selected routes, dimmer future routes, larger labels, and card-based campaign tabs.
- Updated ordinary Results expanded details with accordion groups while preserving compact Results and the private-demo Results path.
- Added focused v0.94 layout coverage and expanded visual-QA coverage from 65 to 84 screenshots.
- Added all required v0.94 specs, reports, and Emmanuel retest checklist.

Runtime/save/art boundary:

- Presentation markup, CSS, and tests changed.
- No save-version bump.
- No save fields, localStorage keys, stable IDs, serialized IDs, hero rules, rewards, XP, campaign progression, replay rules, Tutorial rules, gameplay values, maps, factions, races, units, buildings, art assets, imported assets, desktop port, engine choice, runtime title, or public title changed.
...
Non-pass evidence resolved during v0.94: an initial fast-smoke run exposed campaign node overlap after larger node labels; node size/positions were tightened and fast smoke passed. An initial full-smoke run timed out before the Results objectives accordion default was opened; the exact failing path and full smoke then passed. Hosted layout-core later caught mobile-short main-menu overflow; responsive columns were collapsed and layout-core passed. Visual QA then caught the locked-mission primary action slightly below the 1366x768 fold; the mission panel pacing chip/spacing was compacted and final visual QA passed with the full 84-screenshot set.

Closeout note: commit as `Checkpoint v0.94 main menu ascendant creation and campaign-shell density rescue`, package cleanly on the final commit, verify, push safely, and stop. Do not start v0.95 automatically.
...
Status: v0.82 implements the smallest approved runtime Lume Network slice. It adds one battle-local Linked Control network to Aether Well Ruins on Broken Ford: capture West Stone Cut plus Ford Toll, or Ford Toll plus North Aether Spring, to activate Linked Ward. It does not rename runtime identifiers, migrate saves, generate/import assets, add maps/factions/races/units/buildings/classes, start a desktop port, choose an engine, add multiplayer/PvP/co-op, perform runtime rebrand/display-copy migration, add Jardas binding, Worker binding, hero binding, resource-production bonuses, global balance changes, enemy AI tuning, or start a broader Lume system.
...
- Added `src/game/types/LumeNetworkTypes.ts`, `src/game/data/lumeNetworks.ts`, and `src/game/data/validation/validateLumeNetworks.ts`.
- Added `src/game/battle/LumeNetworkDirector.ts` and focused director tests.
- Added battle-local Lume telemetry fields to battle stats/results only.
- Added Aether Well Ruins campaign briefing, battle HUD status, selected-site summary, and Results debrief.
- Added Linked Ward as a non-stacking 8% incoming-damage reduction near active linked sites.
- Added hosted proxy coverage for activation, enemy recapture/severing, and Results summary.
- Updated package metadata and validation to require the v0.82 docs in the private playtest package.
- Updated README, ROADMAP, CHANGELOG, DEVELOPMENT_CHECKPOINT, RELEASE_CHECKLIST, and this handoff.

Runtime/save/art boundary:

- Lume runtime is active only for eligible `campaign_node` launches on `aether_well_ruins` / `broken_ford`.
- Tutorial/no-reward launches are excluded.
- No save-version bump or save field change.
- No internal IDs were renamed.
- No art was generated, imported, downloaded, or wired.
- No desktop port, wrapper, engine choice, multiplayer, PvP, co-op, race, map, unit, building, class, campaign expansion, or broad UI work was started.
...
Next recommended action: after closeout, Emmanuel should retest with `docs/V082_EMMANUEL_RETEST_CHECKLIST.md`. Do not start broader Lume expansion, runtime copy migration, art generation, runtime rebrand, ID/save migration, v0.83, or desktop work without a new explicit goal.
...
Status: v0.66-v0.68 adds readable enemy tactical variety with small data-driven doctrines, capped battle-only elite squads, and short counterplay copy. It does not add maps, factions, runtime art/assets, save migration, global rebalance, broad pathing rewrite, enemy formation rewrite, shop/crafting, giant roster UI, or force-click/DOM fallback behavior for canvas/world clicks.
...
- Added Raider, Fortress, Hunter, and Warband doctrine definitions and validation.
- Added mission/modifier/enemy-hero doctrine selection with Tutorial/no-reward protection.
- Added conservative enemy AI hooks for Raider resource raids, Fortress reserves/tech, escorted Hunter hero/Retinue pressure, and late Warband push size.
- Added Ash Raider Vanguard and Cinder Iron Guard elite squad definitions with capped modest stat bonuses.
- Added doctrine and elite telemetry to battle stats and Results after-action copy.
- Added campaign briefing, battle HUD, selected-unit renderer, and Results counterplay copy.
- Updated package metadata and validation to require the v0.66-v0.68 docs.

Save format:

- No save-version bump.
- No new save fields.
- Doctrine and elite squad state is content-driven and battle-stat-only.
- Existing campaign, Retinue, replay, reward, relic, skill, control group, Patrol, and Tutorial save state remains valid.

Verification:

```text
Focused doctrine/AI/HUD/Results/campaign Vitest PASS, 6 files / 70 tests.
Focused hosted Raider/Fortress/elite proxy PASS, 2 tests, after rebuilding production dist.
npm test initial NON-PASS: elite damage multiplier needed a fallback for lightweight Unit/Hero test doubles.
npm test rerun PASS, 82 files / 620 tests.
npm run build PASS with the known Vite Phaser vendor chunk-size warning.
npm run validate:content PASS.
npm run validate:art-intake PASS.
npm run test:e2e:smoke:fast PASS, 8 tests.
npm run test:e2e:smoke NON-PASS first attempt, command wrapper timed out after 364 seconds with no Playwright summary; longer clean rerun PASS, 14 tests.
npm run playtest:controls PASS, 18 scenarios / 18 pass rows.
npm run playtest:controls:extended PASS, 18 scenarios / 90 pass rows.
npm run playtest:controls:verify PASS, 1658 checks.
npm run playtest:act1 PASS, 180 Act 1 runs summarized from 255 deterministic simulator runs.
...
Closeout note: commit as `Checkpoint v0.66-v0.68 enemy tactical doctrines and elite squad foundation`, regenerate and verify the clean package from the final commit, then push when safe. Use only a clean package whose `PLAYTEST_BUILD_INFO.md` commit matches the final checkpoint commit and whose dirty status says `no`.
...
Status: v0.51-v0.53 improves the current player-facing feel without adding a new major system. It focuses on cursor affordances, command disabled reasons, Worker build/repair/site intent, hero ability cooldown/Mana readability, combat status readability, and Act 1 confidence. It does not add maps, factions, gameplay systems, save fields, save-version bumps, runtime art/assets, broad UI redesigns, shop, crafting, broad AI/pathing rewrites, global rebalances, Patrol, formations, or force-click/DOM fallback behavior for canvas/world clicks.
...
- Added `CursorAffordance` rules for attack, build, repair, assign, invalid, and default cursor states.
- Exposed native cursor styles and readable canvas intent labels for attack/build/repair/assign/invalid hover states.
- Made Worker build/repair/resource-site intent visible in hosted browser coverage.
- Added stable command state and disabled-reason metadata to existing command buttons.
- Added stable ability reason metadata to hero ability buttons and standardized `Mana` copy.
- Made Burn status chips slightly larger, brighter, and more separated from health bars.
- Slightly increased battle hover tolerance for unit targets to improve enemy/target confidence without changing command semantics.
- Updated package metadata and validation to require the v0.51-v0.53 UX docs and Emmanuel retest checklist.

Save format:

- No save-version bump.
- No new save fields.
- Cursor, command, ability, and combat readability state is derived from existing scene/UI state only.
- Existing Act 1 campaign, replay, reward, relic, skill, inventory, equipment, and Tutorial no-reward saves remain compatible.
...
npm run validate:content PASS.
npm run validate:art-intake PASS, 1 candidate metadata JSON and 0 review manifest JSON files checked.
Focused hosted UX proxy PASS, 5 tests covering attack cursor, Worker build cursor, Worker repair cursor, Worker site assignment cursor, and hero ability cooldown reason metadata.
npm run test:e2e:smoke:fast PASS, 8 tests.
npm run test:e2e:smoke PASS, 14 tests.
npm run playtest:controls PASS, 18 scenarios / 18 pass rows.
npm run playtest:controls:extended PASS, 18 scenarios / 90 pass rows.
...
npm run test:e2e:release:hosted:deep-campaign-pressure PASS, 7 tests.
npm run visual:qa PASS, 5 tests / 18 screenshots / 0 browser console errors / 0 screenshot retries.
npm run test:e2e:release:shard1of3 NON-PASS, 43 passed / 1 failed in the local dev-server shard; failed `tests/e2e/deep-flow.spec.ts:6064` on Results inventory-button retry/fallback timing.
...
Closeout note: commit as `Checkpoint v0.51-v0.53 player-facing UX and command readability polish`, regenerate and verify the clean package from the final commit, then push when safe. Use only a clean package whose `PLAYTEST_BUILD_INFO.md` commit matches the final checkpoint commit and whose dirty status says `no`.
...
Status: v0.42-v0.44 adds the first mission-type, scenario modifier, and campaign briefing/pacing foundation on top of the v0.39-v0.41 campaign progression/replay layer. It uses existing maps, units, objectives, AI config, rewards, campaign UI, Results UI, and hero build identity copy. It does not add maps, factions, runtime art/assets, shop, crafting, a giant quest system, save-version bump, broad AI/pathing rewrite, global rebalance, Patrol, formations, or force-click/DOM fallback behavior for canvas/world clicks.

Baseline:

- Starting commit/package: `ac3d203`, `ascendant-realms-private-playtest-ac3d203`.
- Starting branch state: clean `main`, synced with `origin/main`.
- Baseline remote status: latest push run `26607386432` on `ac3d203` passed Fast confidence; heavier release-matrix lanes were skipped by expected push rules.
...
- Added mission type data for Assault, Control, Defense, and Skirmish / Training.
- Added battle-node briefing metadata for mission type, primary objective, reward preview, after-action summary, scenario modifiers, and recommended build hints.
- Added mission-local scenario modifiers: Rich Veins, Enemy Patrols, Fortified Enemy, and Aether Surge.
- Routed scenario modifiers through existing battle launch modifiers and existing hero/AI/resource-site systems.
- Added campaign node briefing/modifier/reward-preview copy and Results mission type/modifier/after-action copy.
- Preserved first-clear/replay reward rules, optional objective credit, relic choice, hero XP/skill reminders, and Tutorial no-save/no-reward protection.
...
- No save-version bump.
- No new save fields.
- Mission type, briefing, and scenario modifier metadata are content-driven.
- Existing campaign completion, reward claim, optional objective, hero XP, relic, and skill save fields remain compatible.
...
Closeout note: commit as `Checkpoint v0.42-v0.44 mission variety and scenario modifier foundation`, regenerate and verify the clean package from the final commit, then push when safe. Use only a clean package whose `PLAYTEST_BUILD_INFO.md` commit matches the final checkpoint commit and whose dirty status says `no`.
...
Status: v0.36-v0.38 adds a small Warrior / Seer / Commander hero skill-tree foundation, modest ability upgrades, and light equipped-relic build synergy. It uses existing hero XP, existing skill points, existing ability definitions, existing one-slot relic loadout, and existing `HeroSaveData.allocatedSkills`. It does not add maps, factions, runtime art/assets, shop, crafting, giant skill tree, broad inventory UI, enemy hero skill trees, save-version bump, broad AI/pathing rewrites, global rebalance, Patrol, formations, or force-click/DOM fallback behavior for canvas/world clicks.

Baseline:

- Starting commit/package: `b09ef96`, `ascendant-realms-private-playtest-b09ef96`.
- Starting branch state: clean `main`, synced with `origin/main`.
- Baseline remote status: latest push run `26551387988` on `b09ef96` passed Fast confidence; heavier release-matrix lanes were skipped by expected push rules.
...
- Reframed the visible skill-tree branches as Warrior, Seer, and Commander with 2-3 visible nodes per branch.
- Hid legacy compatibility nodes outside the current foundation and blocked new spending on hidden nodes.
- Added data-driven ability upgrade metadata and runtime derivation for effective ability cost, cooldown, amount, radius, and duration.
- Added modest Warrior Cleave, Seer Mana/cooldown, and Commander Rally Banner / War Cry support.
- Added equipped-relic synergy for Emberbrand Shard / Cinder-Seer Focus / Outpost Command Signet when the matching branch has an unlocked skill.
- Added build/synergy copy to the Hero Progression scene, Hero Inventory, Equipment rows, battle HUD, ability tooltips, and Results progression reminders.
...
- Existing `HeroSaveData.inventory` and `HeroSaveData.equipment.relic` carry relic loadout state.
- Old saves without skill-tree data load with empty allocated skills.
- Unknown skill ids and unknown relic/item ids remain loadable and are ignored by known stat, ability, and synergy helpers.
...
npm run validate:content PASS.
npm run validate:art-intake PASS, 1 candidate metadata JSON and 0 review manifest JSON files checked.
npx playwright test --config=playwright.hosted-release.config.ts tests/e2e/deep-flow.spec.ts --grep "inventory equipment, unequip, and skill spending|unlocked hero ability hotkeys" --reporter=line PASS after the exact skill/relic locator follow-up, 2 hosted tests.
npm run test:e2e:smoke:fast PASS, 8 tests.
npm run test:e2e:smoke PASS, 14 tests.
npm run playtest:controls PASS, 18 scenarios / 18 pass rows.
npm run playtest:controls:extended PASS, 18 scenarios / 90 pass rows.
...
Closeout note: commit as `Checkpoint v0.36-v0.38 hero skill tree and relic-build synergy foundation`, regenerate and verify the clean package from the final commit, then push when safe. Use only a clean package whose `PLAYTEST_BUILD_INFO.md` commit matches the final checkpoint commit and whose dirty status says `no`.
...
- This original run remains evidence of the old account/checkout block, not a code/test failure.
- v0.29.1 commit `765a99548a09575f84f42e661d224a4bf3e22789` was later pushed successfully to `main`.
- Push run `26478377719` completed successfully; Fast confidence passed and `actions/checkout@v4` succeeded.
- Manual `workflow_dispatch` run `26478600449` was started with `run_release_matrix=true`.
...
- UI changed: no.
- Tutorial changed: no.
- E2e tests changed: yes, hosted deep-battle world-move helper and behaviour gauntlet setup only.
...
Status: v0.21 adds the first safe Worker repair action and is locally verified through the requested unit, build, validation, smoke, control, hosted release, and dirty package gates. Final clean package generation should happen from the final v0.21 commit after this handoff update.
...
- Branch was clean and synced with `origin/main`.
- v0.18.3 construction pause/resume/pathing stability remains protected.

Docs added:

- `docs/V021_WORKER_REPAIR_FOUNDATION_SPEC.md`
- `docs/V021_IMPLEMENTATION_REPORT.md`

Runtime/UI summary:

- Worker units now have repair intent state separate from construction, move, and attack orders.
- New `RepairSystem` validates repair targets, routes Workers to footprint-adjacent approach points, restores HP over time, and pauses/resumes repair.
- Workers can repair damaged friendly completed Command Hall, Barracks, Mystic Lodge, and Watchtower buildings.
- Workers cannot repair enemy buildings, incomplete buildings, or full-health buildings.
- Full-health buildings report already repaired/full health instead of starting work.
- Explicit move and attack orders pause repair intent and do not pull the Worker back.
- Moving back into range or reissuing Repair resumes repair.
- Repair has no resource cost in v0.21; repair cost is deferred until the economy model exists.
- Selected Worker command UI shows repair targets, HP, no-cost status, and full-health disabled state.
- Selected Worker order summary shows `Repairing` or `Repair Paused`.
- Selected completed buildings show damaged/full-health repair status.
- No harvesting, resource dropoff, enemy repair AI, enemy construction AI, multiple-worker acceleration, save migration, new maps/factions/units/buildings, runtime art/assets, broad AI/pathing rewrite, global rebalance, Patrol, or formations are included.
...
Closeout note: commit as `Checkpoint v0.21 worker repair foundation`, then regenerate and verify the clean final package from that commit.

Emmanuel retest focus: Command Hall still trains Workers only; Workers still build Barracks/Mystic Lodge/Watchtower; construction pause/resume and base-cluster pathing still behave like v0.18.3; damaged friendly completed buildings can be repaired by a nearby Worker; explicit move/attack pauses repair without magneting; moving back or reissuing Repair resumes; enemy buildings cannot be repaired; full-health buildings show already repaired/full health; incomplete buildings remain construction sites; completed Barracks, Mystic Lodge, Watchtower, and tech roles remain stable.
...
Status: v0.19.1 production-role verification and readability polish is implemented and locally verified. Clean package generation should happen from the final v0.19.1 commit after this handoff update.

Baseline:

- Stable starting commit: `ec73568`, `Checkpoint v0.19 production architecture and building roles`.
- Stable starting package: `artifacts/playtest/ascendant-realms-private-playtest-ec73568`.
- Emmanuel result before this pass: v0.18.3 Worker construction assignment, pause/resume, and base-cluster pathing seemed resolved, and v0.19 production roles were package-ready for automated verification.
- GitHub Actions push run #112 on `ec73568` passed Fast confidence only; release-matrix lanes were skipped by push rules.
- GitHub Actions workflow-dispatch run #113 on `ec73568` passed Fast confidence, Release simulator, hosted deep-meta, hosted deep-battle, hosted deep-campaign-pressure, and hosted smoke. It failed only hosted layout-core/layout-cinderfen because the tests still expected removed Command Hall build/upgrade buttons.

Docs added:

- `docs/V0191_PRODUCTION_ROLE_VERIFICATION_PLAN.md`
- `docs/V0191_REMOTE_CI_STATUS.md`
- `docs/V0191_PRODUCTION_ROLE_POLISH_REPORT.md`

Runtime/UI summary:

- Command Hall role copy now explicitly says it trains Workers only.
- Completed building command panels now include the same role summary already used by selected-building UI, so completed Command Hall/Watchtower roles are readable from the command surface.
- Incomplete-building command panels now say actions are locked until construction finishes.
- Mystic Lodge copy consistently names Aether Study I as its existing research role.
- Watchtower copy says it is inactive while incomplete and attacks nearby enemies when complete.
- Defeat-tip copy now routes recovery through Command Hall Worker training followed by Worker-built Barracks, instead of implying direct Command Hall construction.
- No production stats, costs, build times, maps, factions, save format, runtime art/assets, broad pathing/AI rewrite, harvesting, repair, multiple-worker acceleration, enemy construction AI, Patrol, or formations changed.
...
Closeout note: commit as `Checkpoint v0.19.1 production architecture verification and role polish`, then regenerate/verify the clean package from the final commit.

Emmanuel retest focus: Command Hall Worker-only; Worker-built Barracks/Mystic Lodge/Watchtower; inactive incomplete structures; Barracks Militia/Ranger plus Infantry Weapons I, Reinforced Armor I, and Ranger Training I; Mystic Lodge Acolyte plus Aether Study I; Watchtower completed-only defense; Worker pause/resume and base-cluster pathing stability.
...
- Branch was clean and synced with `origin/main`.
- v0.17.5 package `artifacts/playtest/ascendant-realms-private-playtest-2fa6de3` existed from the previous closeout.
- Previous GitHub Actions baseline: CI Release Matrix Dry Run #102 green.
...
- Added one Free Marches Worker unit using existing unit asset conventions only.
- Command Hall can train Workers.
- Worker selection can place a Barracks construction site.
- Worker-placed Barracks stores assigned Worker id/name, status, and construction progress.
