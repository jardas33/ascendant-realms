...
draw.text((140, 140), "v0.286 PREFLIGHT - Field Barracks Reserve Ready Step", fill="#e8d99d")
draw.text((140, 215), "Base HEAD:", fill="#9ecfc5")
draw.text((140, 260), "049ffefc6303a81cce19ecdc91cac16098a0d78e", fill="#ffffff")
draw.text((140, 340), "Baseline exact-SHA Actions run:", fill="#9ecfc5")
draw.text((140, 385), "28691247678 - success", fill="#9fe2a3")
draw.text((140, 470), "Scope:", fill="#9ecfc5")
draw_wrapped(draw, (140, 520), "After accepted v0.285 Hold Line, selecting Field Barracks allows one manual Train click to prepare exactly one static RESERVE READY marker.", "#ffffff")
draw_wrapped(draw, (140, 635), "Forbidden: duplicate reserve, resources mutation, deployment, damage, HP loss, projectiles, death/despawn, pathing, movement, AI, waves, fog, economy, and true default runtime mutation.", "#d7c98f")
preflight.save(root / "01_v0286_preflight_head_ci_resolution.png")
...
draw.text((90, 92), "v0.286 RETAINED v0.282/v0.284 BASELINE LEDGER PROOF", fill="#e8d99d")
y = 155
...
draw.text((90, 775), "v0.286 only adds a static Field Barracks RESERVE READY marker after v0.285 Hold Line.", fill="#9ecfc5")
ledger_image.save(root / "03_v0286_retained_v0282_v0284_baseline_ledger_proof.png")
...
draw.text((100, 105), "v0.286 UI LAYOUT DIAGNOSTIC PROOF", fill="#e8d99d")
...
draw.text((100, 460), "Reserve label: RESERVE READY exactly once, no duplicate reserve after repeat Train.", fill="#d7c98f")
diagnostic.save(root / "25_v0286_ui_layout_diagnostic_proof_retained.png")

capture_paths = [root / name for name in capture_files]
missing = [path.name for path in capture_paths if not path.exists()]
if missing:
    raise SystemExit(f"Missing v0.286 capture files before pack assembly: {missing}")

all_stats = [stats_for(path) for path in capture_paths]
(root / "v0286-black-frame-stats.json").write_text(json.dumps({"stats": all_stats}, indent=2) + "\n", encoding="utf-8")
...
draw.rectangle((60, 60, 1540, 840), outline="#65e6d8", width=3)
draw.text((100, 105), "v0.286 BLACK-FRAME REJECTION REPORT", fill="#e8d99d")
...
draw.text((100, 520), "Rendered evidence includes true default, ledger, and 21 v0.286 opt-in reserve-ready states.", fill="#ffffff")
report.save(root / "27_v0286_black_frame_rejection_report.png")

sheet_paths = [root / "01_v0286_preflight_head_ci_resolution.png"] + capture_paths + [root / "25_v0286_ui_layout_diagnostic_proof_retained.png", root / "27_v0286_black_frame_rejection_report.png"]
tiles = []
...
sheet.save(root / "26_v0286_contact_sheet.png")
