...
if (Test-Path $ScreenshotRoot) {
  ...
  if (-not ($resolvedScreenshots.Path.StartsWith($resolvedArtifact.Path))) {
    throw "Refusing to remove screenshots outside v0.130 artifact root: $($resolvedScreenshots.Path)"
  }
  Remove-Item -LiteralPath $ScreenshotRoot -Recurse -Force
}
New-Item -ItemType Directory -Force -Path $ScreenshotRoot | Out-Null

function Wait-ForSaltoVerticalSliceCaptureArtifacts {
...
Wait-ForSaltoVerticalSliceCaptureArtifacts
node "desktop-spikes/godot-salto/tools/godotSpikeTool.mjs" player-slice-capture-v0130
