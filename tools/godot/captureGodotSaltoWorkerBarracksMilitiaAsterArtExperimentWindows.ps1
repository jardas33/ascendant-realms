...
function Invoke-GodotCaptureScenario {
  ...
  if ($GodotExitCode -ne 0) {
    throw "Godot Worker + Barracks + Militia + Aster capture scenario '$ScenarioId' exited with code $GodotExitCode."
  }
...
Invoke-GodotCaptureScenario -ScenarioId "worker-barracks-militia-aster" -ScenarioArgs ((Review-Args) + (Worker-ArtArgs) + (Barracks-ArtArgs) + (Militia-ArtArgs) + (Aster-ArtArgs))
