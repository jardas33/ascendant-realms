...
$CaptureRoot = Join-Path $RepoRoot 'artifacts\runtime\v0341'
...
if ($LASTEXITCODE -ne 0) { throw 'v0.341 Godot import scan failed' }
$env:V0331_ARTIFACT_ROOT = $CaptureRoot.Replace('\','/')
$args = '--path "{0}" --rendering-method gl_compatibility --rendering-driver opengl3 --scene res://scenes/review/V0341BarrosanAgriculturalBarnGoldAssetReview.tscn' -f $Project
$run = Start-Process -FilePath $Godot -ArgumentList $args -WorkingDirectory $Project -Wait -PassThru
if ($run.ExitCode -ne 0) { throw "v0.341 Godot capture failed with exit code $($run.ExitCode)" }
...
$media = Join-Path $CaptureRoot '08_CONTINUOUS_V0341_BARROSAN_BARN_GOLD_ASSET.mp4'
...
if ($LASTEXITCODE -ne 0) { throw 'v0.341 MP4 encoding failed' }
& 'C:\Users\barro\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' (Join-Path $RepoRoot 'tools\godot\buildV0341BarrosanAgriculturalBarnGoldAssetPack.py')
if ($LASTEXITCODE -ne 0) { throw 'v0.341 review pack build failed' }
Write-Output 'PASS_V0341_BARROSAN_AGRICULTURAL_BARN_GOLD_ASSET_CAPTURE_READY'
