...
function Assert-ExpectedFiles {
  ...
  foreach ($fileName in $missing) {
    throw "Missing expected artifact '$fileName' for v0.217 benchmark scenario '$ScenarioRoot'."
  }
}
...
Invoke-BenchmarkScenario -ScenarioId "selected-road-riverbank-water" -ExtraArgs @()
Invoke-BenchmarkScenario -ScenarioId "procedural-road-riverbank-water-fallback" -ExtraArgs @(
  "--salto-presentation-reboot-road-riverbank-water-missing-fallback"
)

node "tools/godot/saltoRoadRiverbankWaterTool.mjs" benchmark "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.217 road riverbank water benchmark failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0217_ROAD_RIVERBANK_WATER_BENCHMARK_READY"
