...
if (Test-Path -LiteralPath $ArtifactRoot) {
  ...
  $expectedRoot = Resolve-Path -LiteralPath (Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0152")
  if (-not ($resolvedArtifact.Path.StartsWith($expectedRoot.Path))) {
    throw "Refusing to remove Aster billboard repair evidence outside expected root: $($resolvedArtifact.Path)"
  }
...
node "tools/godot/asterBillboardSingleSlotTool.mjs" repair:validate "--artifact-root=$ArtifactArg"
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--aster-billboard-single-slot-repair", "--aster-billboard-repair-benchmark-sequence", "--artifact-root=$ArtifactArg")
...
$RuntimePath = Join-Path $ArtifactRoot "aster-billboard-repair-runtime.json"
$RuntimeReport = if (Test-Path -LiteralPath $RuntimePath) {
  Get-Content -Raw -LiteralPath $RuntimePath | ConvertFrom-Json
} else {
  $null
}
if ($GodotExitCode -ne 0 -or $RuntimeReport.status -ne "PASS_V0152_ASTER_BILLBOARD_REPAIR_RUNTIME_EVIDENCE") {
  throw "Godot Aster billboard repair benchmark exited with code $GodotExitCode."
}
node "tools/godot/asterBillboardSingleSlotTool.mjs" repair:report "--artifact-root=$ArtifactArg"
node "tools/godot/asterBillboardSingleSlotTool.mjs" repair:audit "--artifact-root=$ArtifactArg"
