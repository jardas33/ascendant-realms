...
$ArtifactRoot = Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0187"
...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath, $AsterSourcePath, $AsterMetadataPath, $AshenSourcePath, $AshenMetadataPath, $GroundSourcePath, $GroundMetadataPath, $RoadSourcePath, $RoadMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required v0.187 riverbank bridge approach validation path: $path"
  }
}
...
function Invoke-GodotScenario {
  ...
  if ($GodotExitCode -ne 0) {
    throw "Godot v0.187 scenario '$ScenarioRoot' exited with code $GodotExitCode."
  }
  foreach ($fileName in $ExpectedFiles) {
    if (-not (Test-Path -LiteralPath (Join-Path $ScenarioRoot $fileName))) {
      throw "Missing expected artifact '$fileName' for v0.187 scenario '$ScenarioRoot'."
    }
  }
}
...
function Structure-ShellBaselineArgs { return @("--player-slice", "--experimental-review-mode-label=Experimental opt-in: S1 structure baseline", "--salto-five-slot-review-framing", "--salto-environment-foundation-review", "--salto-environment-shell-live-qa", "--salto-structure-shell-hardening") }
function Riverbank-BridgeApproachArgs { return @("--player-slice", "--experimental-review-mode-label=Experimental opt-in: R1 riverbank bridge approach", "--salto-five-slot-review-framing", "--salto-environment-foundation-review", "--salto-environment-shell-live-qa", "--salto-structure-shell-hardening", "--salto-riverbank-bridge-approach-hardening") }

function Invoke-V0187Tool {
  param([Parameter(Mandatory = $true)][string]$Command)
  node "tools/godot/saltoRiverbankBridgeApproachTool.mjs" $Command "--artifact-root=$ArtifactRootArg"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.187 riverbank bridge approach tool '$Command' failed with exit code $LASTEXITCODE."
  }
}
...
Invoke-GodotScenario -ScenarioRoot (Join-Path $ValidationRoot "s1-structure-shell-hardening-baseline") -ScenarioArgs (@("--player-slice-validate") + (Structure-ShellBaselineArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("player-slice-validation-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $ValidationRoot "r1-riverbank-bridge-approach") -ScenarioArgs (@("--player-slice-validate") + (Riverbank-BridgeApproachArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("player-slice-validation-runtime.json")
Invoke-V0187Tool -Command "validation"

$CaptureRoot = Join-Path $ArtifactRoot "capture"
Reset-SafeDirectory -Path $CaptureRoot -Parent $ArtifactRoot
Invoke-GodotScenario -ScenarioRoot (Join-Path $CaptureRoot "s1-structure-shell-hardening-baseline") -ScenarioArgs (@("--player-slice-capture") + (Structure-ShellBaselineArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("screenshot-runtime-manifest.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $CaptureRoot "r1-riverbank-bridge-approach") -ScenarioArgs (@("--player-slice-capture") + (Riverbank-BridgeApproachArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("screenshot-runtime-manifest.json")
Invoke-V0187Tool -Command "capture"

$BenchmarkRoot = Join-Path $ArtifactRoot "benchmark"
Reset-SafeDirectory -Path $BenchmarkRoot -Parent $ArtifactRoot
Invoke-GodotScenario -ScenarioRoot (Join-Path $BenchmarkRoot "s1-structure-shell-hardening-baseline") -ScenarioArgs (@("--worker-art-opt-in-benchmark") + (Structure-ShellBaselineArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("worker-art-opt-in-benchmark-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $BenchmarkRoot "r1-riverbank-bridge-approach") -ScenarioArgs (@("--worker-art-opt-in-benchmark") + (Riverbank-BridgeApproachArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("worker-art-opt-in-benchmark-runtime.json")
Invoke-V0187Tool -Command "benchmark"

Invoke-V0187Tool -Command "boundary"

node "scripts/cleanupSaltoExperimentalArtifacts.mjs" "--output-root=$((Join-Path $ArtifactRoot 'cleanup-dry-run').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) {
  throw "v0.187 cleanup dry-run failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "v0.187 artifact retention validation failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0187_SALTO_RIVERBANK_BRIDGE_APPROACH_AUTOMATION_READY"
