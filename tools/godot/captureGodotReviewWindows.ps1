...
if (Test-Path $ScreenshotRoot) {
  ...
  if (-not ($resolvedScreenshots.Path.StartsWith($resolvedArtifact.Path))) {
    throw "Refusing to remove screenshots outside v0.118 artifact root: $($resolvedScreenshots.Path)"
  }
...
node "desktop-spikes/godot-salto/tools/godotSpikeTool.mjs" capture-review
