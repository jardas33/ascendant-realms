...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath, $AsterSourcePath, $AsterMetadataPath, $AshenSourcePath, $AshenMetadataPath, $GroundSourcePath, $GroundMetadataPath, $RoadSourcePath, $RoadMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required v0.186 structure shell hardening path: $path"
  }
}
...
$ArgumentList = @(
  "--player-slice",
  "--experimental-review-mode-label=Experimental opt-in: 5 slots + Barrosan ground + roads + structure shell hardening",
...
Write-Output "Launching v0.186 Salto structure shell hardening review."
Write-Output "Experimental opt-in: 5 slots + Barrosan ground + roads + structure shell hardening"
...
Write-Output "Structure-shell posture: visual-only procedural hierarchy hardening; default launcher remains procedural."

if ($Wait) {
  & $ExePath @ArgumentList
  $GodotExitCode = if ($null -eq $LASTEXITCODE) { 0 } else { $LASTEXITCODE }
  if ($GodotExitCode -ne 0) {
    throw "Packaged Godot structure shell hardening review exited with code $GodotExitCode."
  }
  return
}
