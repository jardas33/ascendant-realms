...
const evidenceRoot = process.env.P1R5_EVIDENCE_ROOT || "D:\\CodexData\\evidence\\ascendant-realms-p1-remediation-a\\p1r5";
const logsRoot = process.env.P1R5_LOG_ROOT || "D:\\CodexData\\logs\\ascendant-realms-p1-remediation-a";
...
function withAutoload(fn) {
  ...
  const marker = 'P1R5Capture="*res://tests/p1r5_gameplay_ui_legibility.gd"';
...
function run(width, height, output) {
  ...
  const log = path.join(logsRoot, `p1r5-${width}x${height}.log`);
  const r = spawnSync(godot, ["--path", project, "--resolution", `${width}x${height}`, "--windowed", "--position", "20,20", "--log-file", log], { cwd: repo, env: { ...process.env, TEMP: "D:\\CodexData\\temp", TMP: "D:\\CodexData\\temp", ASCENDANT_P1R5_SOURCE_SHA: sourceSha(), ASCENDANT_P1R5_WIDTH: String(width), ASCENDANT_P1R5_HEIGHT: String(height), ASCENDANT_P1R5_OUTPUT: output }, encoding: "utf8", timeout: 300000, windowsHide: false });
  const mf = path.join(output, "gameplay-ui-legibility-manifest.json");
  if (!existsSync(mf)) throw new Error(`missing ${mf}; exit=${r.status}`);
  return JSON.parse(readFileSync(mf, "utf8"));
}
const required = ["setup", "hero_profile", "settings", "worker", "hero", "building"];
function check(m, width) {
  const failures = [];
  if (m.schema !== "ascendant-realms-p1r5-gameplay-ui-legibility-v1") failures.push("schema");
...
function capture() {
  ...
  const summary = { schema: "ascendant-realms-p1r5-capture-v1", source_sha: sourceSha(), run_dir: runId, godot, captures, pass: failures.length === 0, failures };
  writeFileSync(path.join(evidenceRoot, "p1r5-capture-manifest.json"), JSON.stringify(summary, null, 2) + "\n");
  console.log(JSON.stringify(summary, null, 2)); if (failures.length) process.exitCode = 1;
}
function validate() {
  const failures = []; const file = path.join(evidenceRoot, "p1r5-capture-manifest.json");
  if (!existsSync(file)) failures.push("missing capture manifest");
  let summary = null;
  if (!failures.length) { summary = JSON.parse(readFileSync(file, "utf8")); if (summary.source_sha !== sourceSha()) failures.push("source sha mismatch"); if (!summary.pass) failures.push(...summary.failures); if (summary.captures.length !== 12) failures.push("capture count"); }
  const config = readFileSync(path.join(project, "project.godot"), "utf8"); if (config.includes("P1R5Capture")) failures.push("capture autoload persisted");
  const allowed = new Set(["package.json", "production/ascendant-realms-godot/scripts/ui/hud.gd", "production/ascendant-realms-godot/scripts/ui/skirmish_setup.gd", "production/ascendant-realms-godot/scripts/ui/hero_sheet.gd", "production/ascendant-realms-godot/scripts/ui/settings.gd", "production/ascendant-realms-godot/tests/p1r5_gameplay_ui_legibility.gd", "tools/godot/p1r5GameplayUiLegibilityTool.mjs"]);
  ...
  const report = { schema: "ascendant-realms-p1r5-validator-v1", source_sha: sourceSha(), summary, pass: failures.length === 0, failures };
  writeFileSync(path.join(evidenceRoot, "p1r5-validator-report.json"), JSON.stringify(report, null, 2) + "\n"); console.log(JSON.stringify(report, null, 2)); if (failures.length) process.exitCode = 1;
}
