...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--barrosan-ground-material-single-slot", "--ground-material-validate-only", "--artifact-root=$ArtifactArg")
...
$RuntimeValidationPath = Join-Path $ArtifactRoot "ground-material-validation-runtime.json"
...
if ($GodotExitCode -ne 0 -or $RuntimeValidation.status -ne "PASS_V0175_GROUND_MATERIAL_RUNTIME_VALIDATION") {
  throw "Godot Barrosan foothold ground material validation failed with code $GodotExitCode."
}
