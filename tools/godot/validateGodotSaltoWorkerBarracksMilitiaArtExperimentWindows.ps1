...
$ArtifactRoot = Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0164"
...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required Worker + Barracks + Militia validation path: $path"
  }
}
...
function Invoke-V0164Tool {
  param([Parameter(Mandatory = $true)][string]$Command)
  node "tools/godot/saltoWorkerBarracksMilitiaArtOptInTool.mjs" $Command "--artifact-root=$($ArtifactRoot.Replace('\', '/'))"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.164 Worker + Barracks + Militia opt-in tool '$Command' failed with exit code $LASTEXITCODE."
  }
}
...
Invoke-V0164Tool -Command "validation"
...
Invoke-V0164Tool -Command "capture"
...
Invoke-V0164Tool -Command "benchmark"
...
Invoke-GodotScenario -ScenarioRoot (Join-Path $RealInputRoot "worker-barracks-militia-restart-replay") -ScenarioArgs (@("--triple-natural-playthrough") + (Worker-ArtArgs) + (Barracks-ArtArgs) + (Militia-ArtArgs)) -ExpectedFiles @("triple-playthrough-report.json", "recovery-case-report.json", "restart-integrity-report.json", "no-softlock-proof.json", "no-shortcut-proof.json", "screenshot-manifest.json")
Invoke-V0164Tool -Command "real-input"

Invoke-V0164Tool -Command "boundary"

Write-Output "PASS_V0164_WORKER_BARRACKS_MILITIA_ART_OPT_IN_AUTOMATION_READY"
