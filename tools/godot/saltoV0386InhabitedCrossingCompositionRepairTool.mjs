...
const script = file('desktop-spikes/godot-salto/scripts/v0386_inhabited_crossing_composition_repair.gd');
const scene = file('desktop-spikes/godot-salto/scenes/v0386_inhabited_crossing_composition_repair.tscn');
const router = file('desktop-spikes/godot-salto/scripts/salto_spike_root.gd');
const packageJson = file('package.json');
const audit = file('artifacts/work/v0386-asset-selection-audit.md');
const iterationLog = file('artifacts/work/v0386-iteration-log.md');
const report = file('docs/V0386_INHABITED_CROSSING_COMPOSITION_ROLE_AND_CONNECTION_REPAIR_REPORT.md');
const priorReport = file('docs/V0385_CURATED_INHABITED_CROSSING_ASSET_REPLACEMENT_AND_HIERARCHY_REPAIR_REPORT.md');
const runtime = file('desktop-spikes/godot-salto/artifacts/runtime/v0386');
const pack = file('artifacts/manual-review/v0386-inhabited-crossing-composition-repair');
const images = [
...
function validate() {
  for (const [p, label] of [
    ...
    [script, 'v0.386 script'], [scene, 'v0.386 scene'], [router, 'opt-in router'],
    [packageJson, 'package commands'], [audit, 'asset audit'], [iterationLog, 'iteration log'],
    [report, 'v0.386 report'], [priorReport, 'preserved v0.385 report']
  ...
  for (const token of [
    'V0386_Inhabited_Crossing_Composition_Repair',
    'V0386_One_Primary_Barrosan_Homestead', 'V0386_One_Subordinate_Agricultural_Barn',
    'V0386_Five_Functional_Yard_Groups', 'V0386_Yard_Group_01_Cart',
    'V0386_Yard_Group_02_Stacked_Firewood', 'V0386_Yard_Group_03_Barrel_Crate_Storage',
    'V0386_Yard_Group_04_Trough_Workbench', 'V0386_Yard_Group_05_Fence_Gate',
    'V0386_Exactly_Three_Distinct_Character_Roles', 'V0386_Road_Yard_Entrance_Connection',
    'Farmer.gltf', 'Adventurer.gltf', 'Worker.gltf',
    'v0386-inhabited-crossing-composition-repair.json', 'gameplay": false', 'defaultRuntime": "unchanged"'
  ]) must(g.includes(token), `script contract ${token}`);
  for (const token of [
    '--v0386-inhabited-crossing', '--v0386-inhabited-crossing-smoke', '--v0386-inhabited-crossing-capture',
    'v0386_inhabited_crossing_composition_repair.tscn'
  ]) must(r.includes(token), `router contract ${token}`);
  for (const token of [
    'godot:play:v0386-inhabited-crossing', 'godot:smoke:v0386-inhabited-crossing',
    'godot:capture:v0386-inhabited-crossing', 'godot:validate:v0386-inhabited-crossing'
  ...
  if (!reportText.includes('v0.385 remains retained')) throw new Error('report does not preserve v0.385 ancestry');
  if ((g.match(/V0386_Exactly_Three_Distinct_Character_Roles/g) || []).length !== 1) throw new Error('character role container is not singular');
  if ((g.match(/_instantiate_asset\(V0386_QUATERNIUS_ROOT \+ "men\//g) || []).length !== 3) throw new Error('character asset count is not exactly three');
  ...
  if ((g.match(/V0386_Yard_Group_/g) || []).length < 5) throw new Error('five yard group names missing');

  for (const i of [1, 2, 3]) {
    const d = file(`desktop-spikes/godot-salto/artifacts/work/v0386-iteration-0${i}`);
  ...
  must(fs.existsSync(pack), 'v0.386 review pack');
  ...
  if (validation.status !== 'READY FOR HUMAN V0386 INHABITED CROSSING REVIEW') throw new Error('review pack status mismatch');
  ...
  console.log('PASS_V0386_INHABITED_CROSSING_COMPOSITION_REPAIR_VALIDATOR (7 real captures; separated primary/subordinate; 5 yard groups; 3 visible source-diverse roles; bridge/river preserved; opt-in visual only)');
}

if (process.argv[2] === 'validate') validate();
else throw new Error('usage: node tools/godot/saltoV0386InhabitedCrossingCompositionRepairTool.mjs validate');
