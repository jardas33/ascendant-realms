...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--worker-billboard-single-slot", "--capture-only", "--artifact-root=$ArtifactArg")
...
if ($GodotExitCode -ne 0) {
  throw "Godot Worker billboard capture exited with code $GodotExitCode."
}
