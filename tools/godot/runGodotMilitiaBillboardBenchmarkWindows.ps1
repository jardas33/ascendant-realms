...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--militia-billboard-single-slot", "--militia-billboard-single-slot-benchmark-sequence", "--artifact-root=$ArtifactArg")
...
if ($GodotExitCode -ne 0 -or $RuntimeReport.status -ne "PASS_V0154_MILITIA_BILLBOARD_RUNTIME_EVIDENCE") {
  throw "Godot Militia billboard benchmark exited with code $GodotExitCode."
}
