...
if (Test-Path -LiteralPath $ArtifactRoot) {
  ...
  $expectedRoot = Resolve-Path -LiteralPath (Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0153")
  if (-not ($resolvedArtifact.Path.StartsWith($expectedRoot.Path))) {
    throw "Refusing to remove hybrid three-slot composition evidence outside expected root: $($resolvedArtifact.Path)"
  }
...
node "tools/godot/asterBillboardSingleSlotTool.mjs" composition:validate "--artifact-root=$ArtifactArg"
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--hybrid-three-slot-composition-stress", "--hybrid-three-slot-composition-capture-only", "--artifact-root=$ArtifactArg")
...
$RuntimePath = Join-Path $ArtifactRoot "hybrid-three-slot-composition-runtime.json"
$RuntimeReport = if (Test-Path -LiteralPath $RuntimePath) {
  Get-Content -Raw -LiteralPath $RuntimePath | ConvertFrom-Json
} else {
  $null
}
if ($GodotExitCode -ne 0 -or $RuntimeReport.status -ne "PASS_V0153_HYBRID_THREE_SLOT_RUNTIME_EVIDENCE") {
  throw "Godot hybrid three-slot composition capture exited with code $GodotExitCode."
}
node "tools/godot/asterBillboardSingleSlotTool.mjs" composition:report "--artifact-root=$ArtifactArg"
