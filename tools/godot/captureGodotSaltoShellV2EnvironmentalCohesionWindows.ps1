...
function Wait-ForExecutable {
  ...
  throw "Missing exported Godot executable for v0.203 capture: $Path"
}
...
function Assert-ExpectedFiles {
  ...
  foreach ($fileName in $missing) {
    throw "Missing expected artifact '$fileName' for v0.203 capture scenario '$ScenarioRoot'."
  }
}
...
try {
  ...
  & (Join-Path $PSScriptRoot "launchGodotSaltoShellV2EnvironmentalCohesionWindows.ps1") -Wait "--player-slice-capture" "--artifact-root=$((Join-Path $CaptureRoot 'c1-shell-v2-environmental-cohesion').Replace('\', '/'))"
  Assert-ExpectedFiles -ScenarioRoot (Join-Path $CaptureRoot "c1-shell-v2-environmental-cohesion") -ExpectedFiles @("screenshot-runtime-manifest.json")
} finally {
...
node "tools/godot/saltoShellV2EnvironmentalCohesionTool.mjs" capture "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.203 environmental cohesion capture report failed with exit code $LASTEXITCODE."
}
