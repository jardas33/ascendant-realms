...
const root = resolve(arg("artifact-root") || join(repo, "artifacts", "desktop-spikes", "godot-salto", "v0278"));
const manual = join(repo, "artifacts", "manual-review", "v0278-barrosan-engage-armed-single-label-enforcement");
const docPath = join(repo, "docs", "V0278_BARROSAN_ENGAGE_ARMED_SINGLE_LABEL_ENFORCEMENT_REPORT.md");
const runtimePath = join(root, "engage-armed-single-label-enforcement-runtime", "screenshot-runtime-manifest.json");
const defaultPath = join(root, "default-runtime", "screenshot-runtime-manifest.json");
const statsPath = join(manual, "v0278-black-frame-stats.json");
...
const requiredImages = [
  "01_v0278_preflight_head_ci_resolution.png",
  "02_v0278_default_runtime_unchanged_visible.png",
  ...runtimeNames.map((name, index) => `${String(index + 3).padStart(2, "0")}_${name}.png`),
  "17_v0278_contact_sheet_visible.png",
  "18_v0278_black_frame_rejection_report_visible.png",
];
...
function evalEvidence() {
  ...
  const proof = skin.barrosanEngageArmedSingleLabelEnforcement ?? {};
  const snaps = proof.proofSnapshots ?? {};
  const errors = [];

  if (runtime.checkpoint !== "v0.278") errors.push("runtime checkpoint is not v0.278");
  ...
  if (proof.status !== "PASS") errors.push(`v0.278 single-label status is ${proof.status}`);
  if (!proof.readabilityOnly || !proof.screenshotTruthOnly || !proof.noCombatDamageProjectile) errors.push("v0.278 proof did not stay readability/screenshot-truth/no-combat");
  ...
  for (const name of [
    ...
    if (!snap.exactlyOneArmedWorldLabel || !snap.engagementContainedAbsentWhileArmed || !snap.bridgeHeldAbsentWhileArmed || !snap.duplicateEngageArmedLabelsAbsent) errors.push(`${name} failed single ENGAGE ARMED world-label enforcement`);
  }
  if (!snapshot(snaps, "v0278_engagement_contained_absent_while_armed_visible").engagementContainedAbsentWhileArmed) errors.push("Engagement contained world label remains visible while armed");
  if (!snapshot(snaps, "v0278_bridge_held_absent_while_armed_visible").bridgeHeldAbsentWhileArmed) errors.push("Bridge held world label remains visible while armed");
  if (!snapshot(snaps, "v0278_hud_full_state_single_world_label_visible").hudStillCarriesDetailedState) errors.push("HUD full-state proof failed");
  if (!snapshot(snaps, "v0278_repeat_engage_no_duplicate_label_visible").repeatEngageDoesNotStackLabels) errors.push("repeat Engage label stack proof failed");
  if (!snapshot(snaps, "v0278_repeat_engage_hud_already_armed_visible").repeatEngageHudAlreadyArmed) errors.push("repeat Engage HUD already-armed proof failed");
  if (!snapshot(snaps, "v0278_clear_guard_clean_cancel_label_visible").clearGuardCleanCancelLabel) errors.push("Clear Guard clean cancel proof failed");
  if (!snapshot(snaps, "v0278_reguard_available_again_visible").reguardAvailableClean) errors.push("Reguard available clean proof failed");
  if (!snapshot(snaps, "v0278_no_projectile_no_damage_visible").noCombatDamageProjectile) errors.push("no projectile/no damage proof failed");
  if (snapshot(snaps, "v0278_watchpost_no_engage_action_visible").engageActionOnWatchpost) errors.push("Watchpost leaked Engage action");
  if (snapshot(snaps, "v0278_barracks_no_engage_action_visible").engageActionOnBarracks) errors.push("Barracks leaked Engage action");
  if (snapshot(snaps, "v0278_default_runtime_unchanged_probe_visible").defaultRuntimeChanged) errors.push("default runtime changed probe failed");
...
function writeReport(e) {
  const lines = [
    "# v0.278 Barrosan Engage Armed Single-Label Enforcement Report", "",
    `- Verdict: \`${verdict}\`.`,
    "- Base commit: `9a506919bd8130bcd054f7b3c9ec6e82752db2a9`.",
    ...
    "- Review pack path: `artifacts/manual-review/v0278-barrosan-engage-armed-single-label-enforcement/`.", "",
    "## Boundary confirmation", "",
    "- v0.278 is screenshot-truth/readability only: yes.",
    ...
    "## Recommendation for v0.279", "",
    "- If separately authorized, v0.279 can begin a bounded explicit combat-resolution bridge from this armed state. Keep it narrow: one order, one target, one consequence, and preserve this single-label screenshot-truth arbitration.", "",
    "Stop before v0.279.", "",
  ];
...
try {
  ...
  const status = evidence.errors.length ? "FAIL_V0278_VALIDATION" : "PASS_V0278_BARROSAN_ENGAGE_ARMED_SINGLE_LABEL_ENFORCEMENT_VALIDATION";
  writeFileSync(join(root, "v0278-validation-report.json"), JSON.stringify({ status, errors: evidence.errors, pixelSummary: evidence.pixelSummary }, null, 2) + "\n");
