...
if (-not (Test-Path -LiteralPath (Join-Path $ArtifactRoot "capture\selected-integrated-reference-gap\screenshot-runtime-manifest.json"))) {
  & (Join-Path $PSScriptRoot "captureGodotSaltoIntegratedReferenceGapWindows.ps1")
}
...
Invoke-Reboot "selected-v0223-comparator" @("--salto-hud-visual-language")
Invoke-Reboot "selected-integrated-reference-gap" @("--salto-hud-visual-language", "--salto-integrated-reference-gap")
Invoke-Reboot "ground-missing-fallback" @("--salto-hud-visual-language", "--salto-integrated-reference-gap", "--salto-presentation-reboot-ground-missing-fallback")

node tools/godot/saltoIntegratedReferenceGapTool.mjs validation "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) { throw "v0.224 validation report failed." }
node scripts/cleanupSaltoExperimentalArtifacts.mjs "--output-root=$((Join-Path $ArtifactRoot 'cleanup-dry-run').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) { throw "v0.224 cleanup dry-run failed." }
...
if ($LASTEXITCODE -ne 0) { throw "v0.224 retention validation failed." }
Write-Output "PASS_V0224_INTEGRATED_REFERENCE_GAP_VALIDATION_READY"
