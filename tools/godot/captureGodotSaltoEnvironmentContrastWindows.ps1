...
function Invoke-CaptureScenario {
  ...
  if ($GodotExitCode -ne 0) {
    throw "Godot v0.179 capture scenario '$ScenarioId' exited with code $GodotExitCode."
  }
...
Invoke-CaptureScenario -ScenarioId "e1-prior-ground-material-opt-in" -ScenarioArgs ((Environment-PriorGroundArgs) + (Five-SlotArgs) + (Ground-MaterialArgs))
Invoke-CaptureScenario -ScenarioId "e2-environment-contrast-harmonized" -ScenarioArgs ((Environment-ContrastArgs) + (Five-SlotArgs) + (Ground-MaterialArgs))
Invoke-CaptureScenario -ScenarioId "ground-missing-art-contrast-fallback" -ScenarioArgs ((Environment-ContrastArgs) + (Five-SlotArgs) + (Ground-MissingArgs))
Invoke-CaptureScenario -ScenarioId "ground-hash-mismatch-contrast-fallback" -ScenarioArgs ((Environment-ContrastArgs) + (Five-SlotArgs) + (Ground-MismatchArgs))

node "tools/godot/saltoEnvironmentContrastHarmonizationTool.mjs" capture "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.179 environment contrast harmonization capture report failed with exit code $LASTEXITCODE."
}
