...
$CaptureRoot = Join-Path $RepoRoot 'artifacts\runtime\v0337'
...
if ($LASTEXITCODE -ne 0) { throw 'v0.337 Godot import scan failed' }
$env:V0331_ARTIFACT_ROOT = $CaptureRoot.Replace('\','/')
$args = '--path "{0}" --rendering-method gl_compatibility --rendering-driver opengl3 --scene res://scenes/review/V0337BarrosanHouse02SelectedGraniteReview.tscn' -f $Project
$run = Start-Process -FilePath $Godot -ArgumentList $args -WorkingDirectory $Project -Wait -PassThru
if ($run.ExitCode -ne 0) { throw "v0.337 Godot capture failed with exit code $($run.ExitCode)" }
...
if ($benchmark.ExitCode -ne 0) { throw "v0.337 benchmark failed with exit code $($benchmark.ExitCode)" }
$continuous = Join-Path $CaptureRoot 'continuous\frame_%04d.png'
$media = Join-Path $CaptureRoot '08_CONTINUOUS_V0337_HOUSE02_SELECTED_GRANITE.mp4'
...
if ($LASTEXITCODE -ne 0) { throw 'v0.337 MP4 encoding failed' }
& 'C:\Users\barro\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' (Join-Path $RepoRoot 'tools\godot\buildV0337House02SelectedGranitePack.py')
if ($LASTEXITCODE -ne 0) { throw 'v0.337 review pack build failed' }
Write-Output 'PASS_V0337_HOUSE02_SELECTED_GRANITE_CAPTURE_READY'
