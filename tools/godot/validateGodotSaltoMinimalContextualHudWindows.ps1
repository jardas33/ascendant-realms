...
$ArtifactRoot = Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0222"
...
if (-not (Test-Path -LiteralPath (Join-Path $ArtifactRoot "capture\selected-minimal-contextual-hud\screenshot-runtime-manifest.json"))) {
  & (Join-Path $PSScriptRoot "captureGodotSaltoMinimalContextualHudWindows.ps1")
}
if (-not (Test-Path -LiteralPath $ExePath)) {
  throw "Missing exported Godot executable for v0.222 validation: $ExePath"
}
...
Invoke-RebootValidationScenario -ScenarioId "v0221-before-minimal-contextual-hud" -ExtraArgs @(
  "--salto-minimal-contextual-hud-disabled"
)
Invoke-RebootValidationScenario -ScenarioId "selected-minimal-contextual-hud" -ExtraArgs @(
  "--salto-minimal-contextual-hud"
)

node "tools/godot/saltoMinimalContextualHudTool.mjs" validation "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.222 minimal contextual HUD validation report failed with exit code $LASTEXITCODE."
}

node "tools/godot/saltoMinimalContextualHudTool.mjs" boundary "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.222 minimal contextual HUD boundary scan failed with exit code $LASTEXITCODE."
}

node "scripts/cleanupSaltoExperimentalArtifacts.mjs" "--output-root=$((Join-Path $ArtifactRoot 'cleanup-dry-run').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) {
  throw "v0.222 cleanup dry-run failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "v0.222 artifact retention validation failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0222_MINIMAL_CONTEXTUAL_HUD_VALIDATION_READY"
