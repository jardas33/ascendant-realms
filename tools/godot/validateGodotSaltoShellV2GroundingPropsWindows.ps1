...
function Wait-ForExecutable {
  ...
  throw "Missing exported Godot executable for v0.205 validation: $Path"
}
...
function Assert-ExpectedFiles {
  ...
  foreach ($fileName in $missing) {
    throw "Missing expected artifact '$fileName' for v0.205 validation scenario '$ScenarioRoot'."
  }
}
...
try {
  ...
  & (Join-Path $PSScriptRoot "launchGodotSaltoShellV2StructureMaterialWindows.ps1") -Wait "--player-slice-validate" "--artifact-root=$((Join-Path $ValidationRoot 'b0-shell-v2-structure-material').Replace('\', '/'))"
  Assert-ExpectedFiles -ScenarioRoot (Join-Path $ValidationRoot "b0-shell-v2-structure-material") -ExpectedFiles @("player-slice-validation-runtime.json")

  & (Join-Path $PSScriptRoot "launchGodotSaltoShellV2GroundingPropsWindows.ps1") -Wait "--player-slice-validate" "--artifact-root=$((Join-Path $ValidationRoot 'p1-shell-v2-grounding-props').Replace('\', '/'))"
  Assert-ExpectedFiles -ScenarioRoot (Join-Path $ValidationRoot "p1-shell-v2-grounding-props") -ExpectedFiles @("player-slice-validation-runtime.json")
} finally {
  Remove-Item Env:\GODOT_SALTO_EXE_PATH -ErrorAction SilentlyContinue
}

node "tools/godot/saltoShellV2GroundingPropsTool.mjs" validation "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.205 grounding lighting props validation report failed with exit code $LASTEXITCODE."
}

node "tools/godot/saltoShellV2GroundingPropsTool.mjs" boundary "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.205 grounding lighting props boundary scan failed with exit code $LASTEXITCODE."
}

node "scripts/cleanupSaltoExperimentalArtifacts.mjs" "--output-root=$((Join-Path $ArtifactRoot 'cleanup-dry-run').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) {
  throw "v0.205 cleanup dry-run failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "v0.205 artifact retention validation failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0205_GROUNDING_LIGHTING_PROPS_VALIDATION_READY"
