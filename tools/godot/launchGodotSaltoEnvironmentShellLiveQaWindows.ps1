...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath, $AsterSourcePath, $AsterMetadataPath, $AshenSourcePath, $AshenMetadataPath, $GroundSourcePath, $GroundMetadataPath, $RoadSourcePath, $RoadMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required v0.185 environment shell live QA path: $path"
  }
}
...
$ArgumentList = @(
  "--player-slice",
  "--experimental-review-mode-label=Experimental opt-in: 5 slots + Barrosan ground + roads + shell live QA",
...
Write-Output "Launching v0.185 Salto environment shell live QA review."
Write-Output "Experimental opt-in: 5 slots + Barrosan ground + roads + shell live QA"
...
Write-Output "Environment-shell posture: visual-only shell live QA; default launcher remains procedural."

if ($Wait) {
  & $ExePath @ArgumentList
  $GodotExitCode = if ($null -eq $LASTEXITCODE) { 0 } else { $LASTEXITCODE }
  if ($GodotExitCode -ne 0) {
    throw "Packaged Godot environment shell live QA review exited with code $GodotExitCode."
  }
  return
}
