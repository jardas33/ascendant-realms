...
const root = resolve(arg("artifact-root") || join(repo, "artifacts", "desktop-spikes", "godot-salto", "v0272"));
const manual = join(repo, "artifacts", "manual-review", "v0272-barrosan-militia-clear-guard-command-lifecycle");
const docPath = join(repo, "docs", "V0272_BARROSAN_MILITIA_CLEAR_GUARD_COMMAND_LIFECYCLE_REPORT.md");
const runtimePath = join(root, "militia-clear-guard-command-lifecycle-runtime", "screenshot-runtime-manifest.json");
const defaultPath = join(root, "default-runtime", "screenshot-runtime-manifest.json");
const statsPath = join(manual, "v0272-black-frame-stats.json");
...
const requiredImages = [
  "01_v0272_preflight_head_ci_resolution.png",
  "02_v0272_default_runtime_unchanged_visible.png",
  ...runtimeNames.map((name, index) => `${String(index + 3).padStart(2, "0")}_${name}.png`),
  "48_v0272_contact_sheet_visible.png",
  "49_v0272_black_frame_rejection_report_visible.png",
];
...
function evalEvidence() {
  ...
  const bridge = skin.barrosanMilitiaClearGuardCommandLifecycle ?? {};
  const snaps = bridge.proofSnapshots ?? {};
  const errors = [];

  if (runtime.checkpoint !== "v0.272") errors.push("runtime checkpoint is not v0.272");
  ...
  if (runtime.captureCount !== 45) errors.push(`expected 45 runtime captures, saw ${runtime.captureCount}`);
  if (bridge.status !== "PASS") errors.push(`clear guard command lifecycle status is ${bridge.status}`);
  ...
  for (const [mode, snap] of Object.entries(snaps)) {
    ...
    if (!c.guardRequiredForContact) errors.push(`${mode} did not mark guard as required for v0.272 contact`);
  ...
  const ready = contactOf(snaps, "v0272_militia_ready_guard_available_visible");
  const readySnap = snapshot(snaps, "v0272_militia_ready_guard_available_visible");
  ...
  const pending = contactOf(snaps, "v0272_guard_order_pending_clear_guard_button_visible");
  ...
  const noGuard = contactOf(snaps, "v0272_current_detection_no_guard_no_contact_visible");
  ...
  const holding = contactOf(snaps, "v0272_guard_holding_contact_armed_visible");
  ...
  const pulse = contactOf(snaps, "v0272_first_contact_feedback_pulse_visible");
  const pulseSnap = snapshot(snaps, "v0272_first_contact_feedback_pulse_visible");
  ...
  const resolved = contactOf(snaps, "v0272_contact_resolved_cooldown_locked_visible");
  ...
  const repeat = contactOf(snaps, "v0272_no_repeated_damage_after_clear_and_reguard_visible");
  ...
  const watchpostHud = snapshot(snaps, "v0272_watchpost_hud_advisory_only_no_train_no_guard_no_clear_visible");
  ...
  const barracksHud = snapshot(snaps, "v0272_barracks_hud_train_militia_no_full_relay_visible");
  ...
  const memory = contactOf(snaps, "v0272_memory_only_no_new_contact_damage_visible");
  ...
  if (pixelStats.length !== 46) errors.push(`expected 46 captured PNG pixel stats, saw ${pixelStats.length}`);
...
try {
  ...
  const status = evidence.errors.length ? "FAIL_V0272_VALIDATION" : "PASS_V0272_BARROSAN_MILITIA_CLEAR_GUARD_COMMAND_LIFECYCLE_VALIDATION";
  writeFileSync(join(root, "v0272-validation-report.json"), JSON.stringify({ status, errors: evidence.errors, pixelSummary: evidence.pixelSummary }, null, 2) + "\n");
