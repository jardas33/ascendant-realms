...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--ashen-raider-visual-restraint-replacement", "--ashen-raider-visual-restraint-replacement-benchmark-sequence", "--artifact-root=$ArtifactArg")
...
if ($GodotExitCode -ne 0 -or $RuntimeReport.status -ne "PASS_V0157_ASHEN_RAIDER_RESTRAINT_REPLACEMENT_RUNTIME_EVIDENCE") {
  throw "Godot Ashen Raider visual-restraint replacement benchmark exited with code $GodotExitCode."
}
