...
$V0165ExeName = "AscendantRealmsGodotSalto-v0165.exe"
$ExePath = Join-Path $RepoRoot "desktop-spikes\godot-salto\builds\$V0165ExeName"
$ArtifactRoot = Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0165"
...
$env:GODOT_EXPORT_EXE_NAME = $V0165ExeName
...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required v0.165 three-slot visual-hardening path: $path"
  }
}
...
function Invoke-GodotScenario {
  ...
  if ($GodotExitCode -ne 0) {
    throw "Godot v0.165 scenario '$ScenarioRoot' exited with code $GodotExitCode."
  }
...
function Invoke-V0165Tool {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "v0.165 three-slot visual-hardening tool '$Command' failed with exit code $LASTEXITCODE."
  }
}
...
Write-Output "PASS_V0165_THREE_SLOT_VISUAL_HARDENING_AUTOMATION_READY"
