...
function Assert-ExpectedFiles {
  ...
  foreach ($fileName in $missing) {
    throw "Missing expected artifact '$fileName' for v0.197 benchmark scenario '$ScenarioRoot'."
  }
}
...
node "tools/godot/saltoShellV2MeshQaTool.mjs" benchmark "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.197 shell v2 mesh QA benchmark failed with exit code $LASTEXITCODE."
}
