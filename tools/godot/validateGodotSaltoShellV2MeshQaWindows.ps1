...
$ArtifactRoot = Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0197"
...
function Assert-ExpectedFiles {
  ...
  foreach ($fileName in $missing) {
    throw "Missing expected artifact '$fileName' for v0.197 scenario '$ScenarioRoot'."
  }
}
...
function Invoke-V0197Tool {
  param([Parameter(Mandatory = $true)][string]$Command)
  node "tools/godot/saltoShellV2MeshQaTool.mjs" $Command "--artifact-root=$ArtifactRootArg"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.197 shell v2 mesh QA tool '$Command' failed with exit code $LASTEXITCODE."
  }
}
...
Invoke-V0197Tool -Command "validation"
...
Invoke-V0197Tool -Command "benchmark"
Invoke-V0197Tool -Command "boundary"

node "scripts/cleanupSaltoExperimentalArtifacts.mjs" "--output-root=$((Join-Path $ArtifactRoot 'cleanup-dry-run').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) {
  throw "v0.197 cleanup dry-run failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "v0.197 safe-only cleanup failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "v0.197 artifact retention validation failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0197_SALTO_SHELL_V2_MESH_QA_VALIDATION_READY"
