...
Invoke-CaptureScenario -ScenarioId "v0222-before-hud-visual-language" -ExtraArgs @(
  "--salto-minimal-contextual-hud",
  "--salto-hud-visual-language-disabled"
)
Invoke-CaptureScenario -ScenarioId "selected-hud-visual-language" -ExtraArgs @(
...
try {
  node "tools/godot/saltoHudVisualLanguageTool.mjs" capture "--artifact-root=$ArtifactRootArg"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.223 HUD visual language capture report failed with exit code $LASTEXITCODE."
  }
} finally {
  Remove-Item Env:\SALTO_CONTACT_SHEET_PYTHON -ErrorAction SilentlyContinue
}

Write-Output "PASS_V0223_HUD_VISUAL_LANGUAGE_CAPTURE_READY"
