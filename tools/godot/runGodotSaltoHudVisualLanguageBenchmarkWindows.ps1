...
function Assert-ExpectedFiles {
  ...
  foreach ($fileName in $missing) {
    throw "Missing expected artifact '$fileName' for v0.223 benchmark scenario '$ScenarioRoot'."
  }
}
...
Invoke-BenchmarkScenario -ScenarioId "v0222-before-hud-visual-language" -ExtraArgs @(
  "--salto-minimal-contextual-hud",
  "--salto-hud-visual-language-disabled"
)
Invoke-BenchmarkScenario -ScenarioId "selected-hud-visual-language" -ExtraArgs @(
  "--salto-hud-visual-language"
)

node "tools/godot/saltoHudVisualLanguageTool.mjs" benchmark "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.223 HUD visual language benchmark failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0223_HUD_VISUAL_LANGUAGE_BENCHMARK_READY"
