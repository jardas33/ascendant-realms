...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--barrosan-road-material-single-slot", "--road-material-capture-only", "--artifact-root=$ArtifactArg")
...
if ($GodotExitCode -ne 0 -or $RuntimeReport.status -ne "PASS_V0180_ROAD_MATERIAL_RUNTIME_EVIDENCE") {
  throw "Godot Barrosan foothold road material capture failed with code $GodotExitCode."
}
