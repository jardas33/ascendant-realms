...
const root = resolve(arg("artifact-root") || join(repo, "artifacts", "desktop-spikes", "godot-salto", "v0279"));
const manual = join(repo, "artifacts", "manual-review", "v0279-barrosan-engage-armed-world-label-hard-fail-fix");
const docPath = join(repo, "docs", "V0279_BARROSAN_ENGAGE_ARMED_WORLD_LABEL_HARD_FAIL_FIX_REPORT.md");
const runtimePath = join(root, "engage-armed-world-label-hard-fail-fix-runtime", "screenshot-runtime-manifest.json");
const defaultPath = join(root, "default-runtime", "screenshot-runtime-manifest.json");
const statsPath = join(manual, "v0279-black-frame-stats.json");
...
const requiredImages = [
  "01_v0279_preflight_head_ci_resolution.png",
  "02_v0279_default_runtime_unchanged_visible.png",
  ...runtimeNames.map((name, index) => `${String(index + 3).padStart(2, "0")}_${name}.png`),
  "17_v0279_contact_sheet_visible.png",
  "18_v0279_black_frame_rejection_report_visible.png",
];
...
function evalEvidence() {
  ...
  const proof = skin.barrosanEngageArmedWorldLabelHardFailFix ?? {};
  const snaps = proof.proofSnapshots ?? {};
  const errors = [];

  if (runtime.checkpoint !== "v0.279") errors.push("runtime checkpoint is not v0.279");
  ...
  if (proof.status !== "PASS") errors.push(`v0.279 world-label hard-fail status is ${proof.status}`);
  if (!proof.readabilityOnly || !proof.screenshotTruthOnly || !proof.noCombatDamageProjectile) errors.push("v0.279 proof did not stay readability/screenshot-truth/no-combat");
  ...
  for (const name of [
    ...
    const snap = snapshot(snaps, name);
    const labels = snap.renderedTacticalWorldLabelTexts ?? [];
    if (!snap.exactlyOneArmedWorldLabel || !snap.engagementContainedAbsentWhileArmed || !snap.bridgeHeldAbsentWhileArmed || !snap.forbiddenRenderedWorldLabelsAbsentWhileArmed || !snap.staleFadingPooledLabelsHiddenWhileArmed || !snap.duplicateEngageArmedLabelsAbsent || !snap.stackedOverlappingTacticalLabelsAbsent) errors.push(`${name} failed rendered single ENGAGE ARMED world-label hard-fail enforcement: ${JSON.stringify(labels)}`);
  ...
  if (!snapshot(snaps, "v0279_engagement_contained_absent_close_visible").engagementContainedAbsentWhileArmed) errors.push("Engagement contained world label remains visible while armed");
  if (!snapshot(snaps, "v0279_bridge_held_absent_close_visible").bridgeHeldAbsentWhileArmed) errors.push("Bridge held world label remains visible while armed");
  if (!snapshot(snaps, "v0279_hud_full_state_single_world_label_visible").hudStillCarriesDetailedState) errors.push("HUD full-state proof failed");
  if (!snapshot(snaps, "v0279_repeat_engage_no_duplicate_label_visible").repeatEngageDoesNotStackLabels) errors.push("repeat Engage label stack proof failed");
  if (!snapshot(snaps, "v0279_repeat_engage_hud_already_armed_visible").repeatEngageHudAlreadyArmed) errors.push("repeat Engage HUD already-armed proof failed");
  if (!snapshot(snaps, "v0279_clear_guard_clean_cancel_label_visible").clearGuardCleanCancelLabel) errors.push("Clear Guard clean cancel proof failed");
  if (!snapshot(snaps, "v0279_reguard_available_again_visible").reguardAvailableClean) errors.push("Reguard available clean proof failed");
  if (!snapshot(snaps, "v0279_no_projectile_no_damage_visible").noCombatDamageProjectile) errors.push("no projectile/no damage proof failed");
  if (snapshot(snaps, "v0279_watchpost_no_engage_action_visible").engageActionOnWatchpost) errors.push("Watchpost leaked Engage action");
  if (snapshot(snaps, "v0279_barracks_no_engage_action_visible").engageActionOnBarracks) errors.push("Barracks leaked Engage action");
  if (snapshot(snaps, "v0279_default_runtime_unchanged_probe_visible").defaultRuntimeChanged) errors.push("default runtime changed probe failed");
...
try {
  ...
  const status = evidence.errors.length ? "FAIL_V0279_VALIDATION" : "PASS_V0279_BARROSAN_ENGAGE_ARMED_WORLD_LABEL_HARD_FAIL_FIX_VALIDATION";
  writeFileSync(join(root, "v0279-validation-report.json"), JSON.stringify({ status, errors: evidence.errors, pixelSummary: evidence.pixelSummary }, null, 2) + "\n");
