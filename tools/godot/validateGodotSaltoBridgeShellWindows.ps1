...
if (-not (Test-Path -LiteralPath (Join-Path $ArtifactRoot "capture\selected-bridge-shell\screenshot-runtime-manifest.json"))) {
  & (Join-Path $PSScriptRoot "captureGodotSaltoBridgeShellWindows.ps1")
}
if (-not (Test-Path -LiteralPath $ExePath)) {
  throw "Missing exported Godot executable for v0.218 validation: $ExePath"
}
...
Invoke-RebootValidationScenario -ScenarioId "selected-bridge-shell" -ExtraArgs @()
Invoke-RebootValidationScenario -ScenarioId "legacy-bridge-comparator" -ExtraArgs @(
  "--salto-presentation-reboot-legacy-bridge"
)

node "tools/godot/saltoBridgeShellTool.mjs" validation "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.218 bridge shell validation report failed with exit code $LASTEXITCODE."
}

node "tools/godot/saltoBridgeShellTool.mjs" boundary "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.218 bridge shell boundary scan failed with exit code $LASTEXITCODE."
}

node "scripts/cleanupSaltoExperimentalArtifacts.mjs" "--output-root=$((Join-Path $ArtifactRoot 'cleanup-dry-run').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) {
  throw "v0.218 cleanup dry-run failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "v0.218 artifact retention validation failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0218_BRIDGE_SHELL_VALIDATION_READY"
