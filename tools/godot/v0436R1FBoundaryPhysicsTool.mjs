...
const expectedBase = '6b14ba305afc4b3090d57f400da885550d512183';
...
async function capture() {
  ...
  await fs.writeFile(path.join(pack, 'preflight.json'), JSON.stringify({ schema: 'v0436-r1f-preflight-v1', branch: git(['branch', '--show-current']), head: git(['rev-parse', 'HEAD']), base: expectedBase, godot: godot(), project, no_r1c_or_conquest_capture: true }, null, 2) + '\n');
...
async function validate() {
  ...
  if (head !== expectedBase) failures.push(`expected base HEAD ${expectedBase}, got ${head}`);
  ...
  for (let index = 1; index <= 3; index += 1) {
    ...
    if (result.final_inside_bounds !== true || result.no_duplicate_movement !== true || result.contamination_free !== true) failures.push(`run ${index} integrity failed`);
  ...
  const out = { schema: 'v0436-r1f-boundary-physics-validator-v1', branch, head, expectedBase, failures, passed: failures.length === 0, runs: runs.map(run => run.run_index), pack: 'artifacts/manual-review/v0436-r1f-boundary-recovery-physics-truth/' };
