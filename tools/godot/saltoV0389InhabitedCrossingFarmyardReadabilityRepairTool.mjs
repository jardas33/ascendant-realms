...
const script = file('desktop-spikes/godot-salto/scripts/v0389_inhabited_crossing_farmyard_readability_repair.gd');
const scene = file('desktop-spikes/godot-salto/scenes/v0389_inhabited_crossing_farmyard_readability_repair.tscn');
const router = file('desktop-spikes/godot-salto/scripts/salto_spike_root.gd');
const packageJson = file('package.json');
const audit = file('artifacts/work/v0389-asset-selection-audit.md');
const iterationLog = file('artifacts/work/v0389-iteration-log.md');
const report = file('docs/V0389_INHABITED_CROSSING_FARMYARD_READABILITY_REPAIR_REPORT.md');
...
const runtime = file('desktop-spikes/godot-salto/artifacts/runtime/v0389');
const pack = file('artifacts/manual-review/v0389-inhabited-crossing-farmyard-readability-repair');
const images = [
...
function validate() {
  for (const [p, label] of [
    ...
    [script, 'v0.389 script'], [scene, 'v0.389 scene'], [router, 'opt-in router'],
    [packageJson, 'package commands'], [audit, 'asset audit'], [iterationLog, 'iteration log'],
    [report, 'v0.389 report'], [priorReport, 'preserved v0.386 report'],
  ...
  for (const token of [
    'V0389_Inhabited_Crossing_Farmyard_Readability_Repair',
    'V0389_One_Primary_Barrosan_Homestead', 'V0389_One_Subordinate_Agricultural_Barn',
    'V0389_Five_Functional_Yard_Groups', 'V0389_Yard_Group_01_Cart',
    'V0389_Yard_Group_02_Stacked_Firewood', 'V0389_Yard_Group_03_Barrel_Crate_Storage',
    'V0389_Yard_Group_04_Trough_Workbench', 'V0389_Yard_Group_05_Fence_Gate',
    'V0388_Exactly_Three_Distinct_Character_Roles', 'V0388_Road_Yard_Entrance_Connection',
    'V0389_Continuous_Bridge_Yard_Door_Route', 'V0389_Worn_Earth_Route_Inlay', 'continuous_route_ribbon',
    'Farmer.gltf', 'Adventurer.gltf', 'Worker.gltf',
    'v0389-inhabited-crossing-farmyard-readability-repair.json', 'gameplay": false', 'defaultRuntime": "unchanged"'
  ]) must(g.includes(token), `script contract ${token}`);
  for (const token of [
    '--v0389-inhabited-crossing', '--v0389-inhabited-crossing-smoke', '--v0389-inhabited-crossing-capture',
    'v0389_inhabited_crossing_farmyard_readability_repair.tscn'
  ]) must(r.includes(token), `router contract ${token}`);
  for (const token of [
    'godot:play:v0389-inhabited-crossing', 'godot:smoke:v0389-inhabited-crossing',
    'godot:capture:v0389-inhabited-crossing', 'godot:validate:v0389-inhabited-crossing'
  ...
  if (!reportText.includes('v0.388 remains retained')) throw new Error('report does not preserve v0.388 ancestry');
  ...
  if ((g.match(/V0389_Yard_Group_/g) || []).length < 5) throw new Error('five yard group names missing');

  for (const i of [1, 2, 3]) {
    const d = file(`desktop-spikes/godot-salto/artifacts/work/v0389-iteration-0${i}`);
  ...
  must(fs.existsSync(pack), 'v0.389 review pack');
  ...
  if (validation.status !== 'REJECTED BY INDEPENDENT V0389 VISUAL REVIEW') throw new Error('review pack status mismatch');
  if (validation.independentVerdict !== 'REJECT') throw new Error('independent visual verdict missing');
  ...
  console.log('PASS_V0389_INHABITED_CROSSING_FARMYARD_READABILITY_REPAIR_VALIDATOR (7 real captures; structural contracts green; independent rendered verdict recorded as REJECT; opt-in visual only)');
}

if (process.argv[2] === 'validate') validate();
else throw new Error('usage: node tools/godot/saltoV0389InhabitedCrossingFarmyardReadabilityRepairTool.mjs validate');
