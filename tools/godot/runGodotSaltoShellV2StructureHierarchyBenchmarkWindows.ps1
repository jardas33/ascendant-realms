...
function Assert-ExpectedFiles {
  ...
  foreach ($fileName in $missing) {
    throw "Missing expected artifact '$fileName' for v0.199 benchmark scenario '$ScenarioRoot'."
  }
}
...
node "tools/godot/saltoShellV2StructureHierarchyTool.mjs" benchmark "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.199 structure hierarchy benchmark failed with exit code $LASTEXITCODE."
}
