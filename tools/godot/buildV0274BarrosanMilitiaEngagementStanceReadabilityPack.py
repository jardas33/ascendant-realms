...
draw.text((140, 145), "v0.274 PREFLIGHT - BARROSAN Militia Engagement Stance Readability", fill="#e8d99d")
draw.text((140, 225), "Base HEAD / origin/main:", fill="#9ecfc5")
draw.text((140, 275), "95f8f22a146e19ca171755f88a8a88cdb5e62456", fill="#ffffff")
draw.text((140, 365), "Baseline exact-SHA Actions run:", fill="#9ecfc5")
draw.text((140, 415), "28511807437 - success", fill="#9fe2a3")
draw.text((140, 510), "Scope:", fill="#9ecfc5")
draw.text((140, 560), "Command/readability/player-agency only. Engagement Stance Readability is readability-only after resolved contact. It never auto-moves, auto-attacks, or adds damage.", fill="#ffffff")
draw.text((140, 620), "Contact eligibility is unchanged from v0.272; Bridge Held appears only after resolved Militia guard contact.", fill="#ffffff")
draw.text((140, 700), "Preserve one-time pressure integrity 100/100 -> 90/100. No repeat damage, projectiles, tower behavior, death/despawn, slow/stop/redirect, AI/pathing/wave/economy/default-runtime changes.", fill="#d7c98f")
preflight.save(root / "01_v0274_preflight_head_ci_resolution.png")

capture_paths = [root / "02_v0274_default_runtime_unchanged_visible.png"] + [
...
(root / "v0274-black-frame-stats.json").write_text(json.dumps({"stats": all_stats}, indent=2) + "\n", encoding="utf-8")
...
draw.text((100, 105), "v0.274 BLACK-FRAME REJECTION REPORT", fill="#e8d99d")
...
draw.text((100, 590), "Default runtime and opt-in engagement-stance runtime were both inspected.", fill="#d7c98f")
report.save(root / "53_v0274_black_frame_rejection_report_visible.png")

sheet_paths = [root / "01_v0274_preflight_head_ci_resolution.png"] + capture_paths + [root / "53_v0274_black_frame_rejection_report_visible.png"]
tiles = []
...
sheet.save(root / "52_v0274_contact_sheet_visible.png")
