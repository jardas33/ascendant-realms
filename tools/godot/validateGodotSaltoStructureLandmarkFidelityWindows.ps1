...
if (-not (Test-Path (Join-Path $ArtifactRoot "capture\selected-structure-landmark-fidelity\screenshot-runtime-manifest.json"))) {
  & (Join-Path $PSScriptRoot "captureGodotSaltoStructureLandmarkFidelityWindows.ps1")
}
...
Invoke-Reboot "ground-missing-fallback" @("--salto-hud-visual-language", "--salto-structure-landmark-fidelity", "--salto-presentation-reboot-ground-missing-fallback")
node tools/godot/saltoStructureLandmarkFidelityTool.mjs validation "--artifact-root=$($ArtifactRoot.Replace('\','/'))"
if ($LASTEXITCODE -ne 0) { throw "v0.229 validation report failed." }
node scripts/cleanupSaltoExperimentalArtifacts.mjs "--output-root=$((Join-Path $ArtifactRoot 'cleanup-dry-run').Replace('\','/'))"
if ($LASTEXITCODE -ne 0) { throw "v0.229 cleanup dry run failed." }
Write-Output "PASS_V0229_STRUCTURE_LANDMARK_FIDELITY_VALIDATION_READY"
