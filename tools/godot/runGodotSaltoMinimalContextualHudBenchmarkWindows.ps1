...
function Assert-ExpectedFiles {
  ...
  foreach ($fileName in $missing) {
    throw "Missing expected artifact '$fileName' for v0.222 benchmark scenario '$ScenarioRoot'."
  }
}
...
Invoke-BenchmarkScenario -ScenarioId "v0221-before-minimal-contextual-hud" -ExtraArgs @(
  "--salto-minimal-contextual-hud-disabled"
)
Invoke-BenchmarkScenario -ScenarioId "selected-minimal-contextual-hud" -ExtraArgs @(
  "--salto-minimal-contextual-hud"
)

node "tools/godot/saltoMinimalContextualHudTool.mjs" benchmark "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.222 minimal contextual HUD benchmark failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0222_MINIMAL_CONTEXTUAL_HUD_BENCHMARK_READY"
