...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--aster-billboard-single-slot", "--aster-billboard-single-slot-validate-only", "--artifact-root=$ArtifactArg")
...
$RuntimeValidationPath = Join-Path $ArtifactRoot "aster-billboard-single-slot-validation-runtime.json"
...
if ($GodotExitCode -ne 0 -or $RuntimeValidation.status -ne "PASS_V0151_ASTER_BILLBOARD_RUNTIME_VALIDATION") {
  throw "Godot Aster billboard validation exited with code $GodotExitCode."
}
