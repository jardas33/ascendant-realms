...
const script = file('desktop-spikes/godot-salto/scripts/v0387_inhabited_crossing_route_hierarchy_repair.gd');
const scene = file('desktop-spikes/godot-salto/scenes/v0387_inhabited_crossing_route_hierarchy_repair.tscn');
const router = file('desktop-spikes/godot-salto/scripts/salto_spike_root.gd');
const packageJson = file('package.json');
const audit = file('artifacts/work/v0387-asset-selection-audit.md');
const iterationLog = file('artifacts/work/v0387-iteration-log.md');
const report = file('docs/V0387_INHABITED_CROSSING_ROUTE_HIERARCHY_REPAIR_REPORT.md');
const priorReport = file('docs/V0386_INHABITED_CROSSING_COMPOSITION_ROLE_AND_CONNECTION_REPAIR_REPORT.md');
const runtime = file('desktop-spikes/godot-salto/artifacts/runtime/v0387');
const pack = file('artifacts/manual-review/v0387-inhabited-crossing-route-hierarchy-repair');
const images = [
...
function validate() {
  for (const [p, label] of [
    ...
    [script, 'v0.387 script'], [scene, 'v0.387 scene'], [router, 'opt-in router'],
    [packageJson, 'package commands'], [audit, 'asset audit'], [iterationLog, 'iteration log'],
    [report, 'v0.387 report'], [priorReport, 'preserved v0.386 report']
  ...
  for (const token of [
    'V0387_Inhabited_Crossing_Route_Hierarchy_Repair',
    'V0387_One_Primary_Barrosan_Homestead', 'V0387_One_Subordinate_Agricultural_Barn',
    'V0387_Five_Functional_Yard_Groups', 'V0387_Yard_Group_01_Cart',
    'V0387_Yard_Group_02_Stacked_Firewood', 'V0387_Yard_Group_03_Barrel_Crate_Storage',
    'V0387_Yard_Group_04_Trough_Workbench', 'V0387_Yard_Group_05_Fence_Gate',
    'V0387_Exactly_Three_Distinct_Character_Roles', 'V0387_Road_Yard_Entrance_Connection',
    'Farmer.gltf', 'Adventurer.gltf', 'Worker.gltf',
    'v0387-inhabited-crossing-route-hierarchy-repair.json', 'gameplay": false', 'defaultRuntime": "unchanged"'
  ]) must(g.includes(token), `script contract ${token}`);
  for (const token of [
    '--v0387-inhabited-crossing', '--v0387-inhabited-crossing-smoke', '--v0387-inhabited-crossing-capture',
    'v0387_inhabited_crossing_route_hierarchy_repair.tscn'
  ]) must(r.includes(token), `router contract ${token}`);
  for (const token of [
    'godot:play:v0387-inhabited-crossing', 'godot:smoke:v0387-inhabited-crossing',
    'godot:capture:v0387-inhabited-crossing', 'godot:validate:v0387-inhabited-crossing'
  ...
  if (!reportText.includes('v0.386 remains retained')) throw new Error('report does not preserve v0.386 ancestry');
  if ((g.match(/V0387_Exactly_Three_Distinct_Character_Roles/g) || []).length !== 1) throw new Error('character role container is not singular');
  if ((g.match(/_instantiate_asset\(V0387_QUATERNIUS_ROOT \+ "men\//g) || []).length !== 3) throw new Error('character asset count is not exactly three');
  ...
  if ((g.match(/V0387_Yard_Group_/g) || []).length < 5) throw new Error('five yard group names missing');

  for (const i of [1, 2, 3]) {
    const d = file(`desktop-spikes/godot-salto/artifacts/work/v0387-iteration-0${i}`);
  ...
  must(fs.existsSync(pack), 'v0.387 review pack');
  ...
  if (validation.status !== 'READY FOR HUMAN V0387 INHABITED CROSSING REVIEW') throw new Error('review pack status mismatch');
  ...
  console.log('PASS_V0387_INHABITED_CROSSING_COMPOSITION_REPAIR_VALIDATOR (7 real captures; separated primary/subordinate; 5 yard groups; 3 visible source-diverse roles; bridge/river preserved; opt-in visual only)');
}

if (process.argv[2] === 'validate') validate();
else throw new Error('usage: node tools/godot/saltoV0387InhabitedCrossingRouteHierarchyRepairTool.mjs validate');
