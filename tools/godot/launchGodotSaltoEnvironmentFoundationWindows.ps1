...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath, $AsterSourcePath, $AsterMetadataPath, $AshenSourcePath, $AshenMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required environment foundation review path: $path"
  }
}
...
$ArgumentList = @(
  "--player-slice",
  "--experimental-review-mode-label=Experimental opt-in art: Worker + Barracks + Militia + Aster + Ashen + environment",
...
Write-Output "Launching v0.173 Salto environment foundation opt-in review."
Write-Output "Posture: five selected Worker + Barracks + Militia + Aster + Ashen slots, plus zero-slot procedural environment foundation."
Write-Output "Environment flag: --salto-environment-foundation-review"

if ($Wait) {
  & $ExePath @ArgumentList
  $GodotExitCode = if ($null -eq $LASTEXITCODE) { 0 } else { $LASTEXITCODE }
  if ($GodotExitCode -ne 0) {
    throw "Packaged Godot environment foundation review exited with code $GodotExitCode."
  }
  return
}
