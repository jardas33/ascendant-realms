...
$ArtifactRoot = Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0164\capture"
...
function Invoke-GodotCaptureScenario {
  ...
  if ($GodotExitCode -ne 0) {
    throw "Godot Worker + Barracks + Militia capture scenario '$ScenarioId' exited with code $GodotExitCode."
  }
...
function Worker-ArtArgs {
  return @("--worker-art-opt-in", "--worker-art-source=$($WorkerSourcePath.Replace('\', '/'))", "--worker-art-metadata=$($WorkerMetadataPath.Replace('\', '/'))", "--worker-art-expected-sha256=$WorkerExpectedSha", "--worker-art-scale=1.00")
}

function Barracks-ArtArgs {
  return @("--barracks-material-opt-in", "--barracks-material-source=$($BarracksSourcePath.Replace('\', '/'))", "--barracks-material-metadata=$($BarracksMetadataPath.Replace('\', '/'))", "--barracks-material-expected-sha256=$BarracksExpectedSha")
}

function Militia-ArtArgs {
  param([string]$Source = $MilitiaSourcePath, [string]$Expected = $MilitiaExpectedSha, [string]$FallbackMode = "none")
  return @("--militia-art-opt-in", "--militia-art-source=$($Source.Replace('\', '/'))", "--militia-art-metadata=$($MilitiaMetadataPath.Replace('\', '/'))", "--militia-art-expected-sha256=$Expected", "--militia-art-scale=1.00", "--militia-art-fallback-mode=$FallbackMode")
}

Reset-SafeDirectory -Path $ArtifactRoot -Parent (Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0164")
...
Invoke-GodotCaptureScenario -ScenarioId "worker-barracks-militia" -ScenarioArgs ((Worker-ArtArgs) + (Barracks-ArtArgs) + (Militia-ArtArgs))
Invoke-GodotCaptureScenario -ScenarioId "militia-missing-art-fallback" -ScenarioArgs ((Worker-ArtArgs) + (Barracks-ArtArgs) + (Militia-ArtArgs -Source $MissingMilitiaSourcePath -FallbackMode "missing"))
Invoke-GodotCaptureScenario -ScenarioId "militia-hash-mismatch-fallback" -ScenarioArgs ((Worker-ArtArgs) + (Barracks-ArtArgs) + (Militia-ArtArgs -Expected $MismatchSha -FallbackMode "hash-mismatch"))

node "tools/godot/saltoWorkerBarracksMilitiaArtOptInTool.mjs" capture "--artifact-root=$((Join-Path $RepoRoot 'artifacts\desktop-spikes\godot-salto\v0164').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) {
  throw "v0.164 Worker + Barracks + Militia capture report failed with exit code $LASTEXITCODE."
}
