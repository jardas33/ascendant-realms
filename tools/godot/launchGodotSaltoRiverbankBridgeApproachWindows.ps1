...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath, $AsterSourcePath, $AsterMetadataPath, $AshenSourcePath, $AshenMetadataPath, $GroundSourcePath, $GroundMetadataPath, $RoadSourcePath, $RoadMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required v0.187 riverbank bridge approach path: $path"
  }
}
...
$ArgumentList = @(
  "--player-slice",
  "--experimental-review-mode-label=Experimental opt-in: R1 riverbank bridge approach",
...
Write-Output "Launching v0.187 Salto riverbank bridge approach review."
Write-Output "Experimental opt-in: 5 slots + Barrosan ground + roads + riverbank bridge approach"
...
Write-Output "Riverbank/bridge posture: visual-only procedural crossing hardening; default launcher remains procedural."

if ($Wait) {
  & $ExePath @ArgumentList
  $GodotExitCode = if ($null -eq $LASTEXITCODE) { 0 } else { $LASTEXITCODE }
  if ($GodotExitCode -ne 0) {
    throw "Packaged Godot riverbank bridge approach review exited with code $GodotExitCode."
  }
  return
}
