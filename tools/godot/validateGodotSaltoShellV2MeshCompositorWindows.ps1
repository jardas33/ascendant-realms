...
function Assert-ExpectedFiles {
  ...
  foreach ($fileName in $missing) {
    throw "Missing expected artifact '$fileName' for v0.196 scenario '$ScenarioRoot'."
  }
}
...
function Invoke-V0196Tool {
  param([Parameter(Mandatory = $true)][string]$Command)
  node "tools/godot/saltoShellV2MeshCompositorTool.mjs" $Command "--artifact-root=$ArtifactRootArg"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.196 shell v2 mesh compositor tool '$Command' failed with exit code $LASTEXITCODE."
  }
}
...
try {
  ...
  & (Join-Path $PSScriptRoot "launchGodotSaltoShellV2ScopedMaterialRecoveryWindows.ps1") -Wait "--player-slice-validate" "--artifact-root=$((Join-Path $ValidationRoot 'p1-shell-v2-scoped-material').Replace('\', '/'))"
  Assert-ExpectedFiles -ScenarioRoot (Join-Path $ValidationRoot "p1-shell-v2-scoped-material") -ExpectedFiles @("player-slice-validation-runtime.json")
  & (Join-Path $PSScriptRoot "launchGodotSaltoShellV2MeshCompositorWindows.ps1") -Wait "--player-slice-validate" "--artifact-root=$((Join-Path $ValidationRoot 'm1-shell-v2-mesh-compositor').Replace('\', '/'))"
  Assert-ExpectedFiles -ScenarioRoot (Join-Path $ValidationRoot "m1-shell-v2-mesh-compositor") -ExpectedFiles @("player-slice-validation-runtime.json")
} finally {
  Remove-Item Env:\GODOT_SALTO_EXE_PATH -ErrorAction SilentlyContinue
}
Invoke-V0196Tool -Command "validation"
...
Invoke-V0196Tool -Command "benchmark"
Invoke-V0196Tool -Command "boundary"

node "scripts/cleanupSaltoExperimentalArtifacts.mjs" "--output-root=$((Join-Path $ArtifactRoot 'cleanup-dry-run').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) {
  throw "v0.196 cleanup dry-run failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "v0.196 artifact retention validation failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0196_SALTO_SHELL_V2_MESH_COMPOSITOR_VALIDATION_READY"
