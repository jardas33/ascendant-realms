...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath, $AsterSourcePath, $AsterMetadataPath, $AshenSourcePath, $AshenMetadataPath, $GroundSourcePath, $GroundMetadataPath, $RoadSourcePath, $RoadMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required v0.185 environment shell live QA validation path: $path"
  }
}
...
function Invoke-GodotScenario {
  ...
  if ($GodotExitCode -ne 0) {
    throw "Godot v0.185 scenario '$ScenarioRoot' exited with code $GodotExitCode."
  }
  foreach ($fileName in $ExpectedFiles) {
    if (-not (Test-Path -LiteralPath (Join-Path $ScenarioRoot $fileName))) {
      throw "Missing expected artifact '$fileName' for v0.185 scenario '$ScenarioRoot'."
    }
  }
}
...
function Invoke-V0185Tool {
  param([Parameter(Mandatory = $true)][string]$Command)
  node "tools/godot/saltoEnvironmentShellLiveQaTool.mjs" $Command "--artifact-root=$ArtifactRootArg"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.185 environment shell live QA tool '$Command' failed with exit code $LASTEXITCODE."
  }
}
...
Invoke-GodotScenario -ScenarioRoot (Join-Path $ValidationRoot "e3-geometry-convergence-baseline") -ScenarioArgs (@("--player-slice-validate") + (Environment-GeometryBaselineArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("player-slice-validation-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $ValidationRoot "e4-refined-shell-live-qa") -ScenarioArgs (@("--player-slice-validate") + (Environment-ShellLiveQaArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("player-slice-validation-runtime.json")
Invoke-V0185Tool -Command "validation"

$CaptureRoot = Join-Path $ArtifactRoot "capture"
Reset-SafeDirectory -Path $CaptureRoot -Parent $ArtifactRoot
Invoke-GodotScenario -ScenarioRoot (Join-Path $CaptureRoot "e3-geometry-convergence-baseline") -ScenarioArgs (@("--player-slice-capture") + (Environment-GeometryBaselineArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("screenshot-runtime-manifest.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $CaptureRoot "e4-refined-shell-live-qa") -ScenarioArgs (@("--player-slice-capture") + (Environment-ShellLiveQaArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("screenshot-runtime-manifest.json")
Invoke-V0185Tool -Command "capture"

$BenchmarkRoot = Join-Path $ArtifactRoot "benchmark"
Reset-SafeDirectory -Path $BenchmarkRoot -Parent $ArtifactRoot
Invoke-GodotScenario -ScenarioRoot (Join-Path $BenchmarkRoot "e3-geometry-convergence-baseline") -ScenarioArgs (@("--worker-art-opt-in-benchmark") + (Environment-GeometryBaselineArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("worker-art-opt-in-benchmark-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $BenchmarkRoot "e4-refined-shell-live-qa") -ScenarioArgs (@("--worker-art-opt-in-benchmark") + (Environment-ShellLiveQaArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("worker-art-opt-in-benchmark-runtime.json")
Invoke-V0185Tool -Command "benchmark"

Invoke-V0185Tool -Command "boundary"

node "scripts/cleanupSaltoExperimentalArtifacts.mjs" "--output-root=$((Join-Path $ArtifactRoot 'cleanup-dry-run').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) {
  throw "v0.185 cleanup dry-run failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "v0.185 artifact retention validation failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0185_SALTO_ENVIRONMENT_SHELL_LIVE_QA_AUTOMATION_READY"
