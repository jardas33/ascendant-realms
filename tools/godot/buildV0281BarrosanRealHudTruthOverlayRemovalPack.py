...
draw.text((140, 145), "v0.281 PREFLIGHT - BARROSAN Real HUD Truth Overlay Removal", fill="#e8d99d")
draw.text((140, 225), "Base HEAD:", fill="#9ecfc5")
draw.text((140, 275), "e971ed1948f7efdf3674beaa5911e7b449343c51", fill="#ffffff")
draw.text((140, 365), "Baseline exact-SHA Actions run:", fill="#9ecfc5")
draw.text((140, 415), "28668874720 - success", fill="#9fe2a3")
draw.text((140, 510), "Scope:", fill="#9ecfc5")
draw.text((140, 560), "Real selected-unit HUD/card truth only. v0.280 review card overlay removed from v0.281 review path.", fill="#ffffff")
draw.text((140, 650), "One stateful consequence retained: pressure checked 90/100 -> 80/100 once. Forbidden: projectile, unit/enemy damage, death/despawn, pathing, AI, waves, economy, fog, default runtime mutation.", fill="#d7c98f")
preflight.save(root / "01_v0281_preflight_head_ci_resolution.png")

capture_paths = [root / "02_v0281_default_runtime_unchanged_visible.png"] + [
...
(root / "v0281-black-frame-stats.json").write_text(json.dumps({"stats": all_stats}, indent=2) + "\n", encoding="utf-8")
...
draw.text((100, 105), "v0.281 BLACK-FRAME REJECTION REPORT", fill="#e8d99d")
...
draw.text((100, 590), "Default runtime and opt-in v0.281 readability runtime were both inspected.", fill="#d7c98f")
report.save(root / "18_v0281_black_frame_rejection_report_visible.png")

sheet_paths = [root / "01_v0281_preflight_head_ci_resolution.png"] + capture_paths + [root / "18_v0281_black_frame_rejection_report_visible.png"]
tiles = []
...
sheet.save(root / "17_v0281_contact_sheet_visible.png")
