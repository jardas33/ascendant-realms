...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--ashen-raider-visual-restraint-replacement", "--ashen-raider-visual-restraint-replacement-validate-only", "--artifact-root=$ArtifactArg")
...
$RuntimeValidationPath = Join-Path $ArtifactRoot "ashen-raider-visual-restraint-replacement-validation-runtime.json"
...
if ($GodotExitCode -ne 0 -or $RuntimeValidation.status -ne "PASS_V0157_ASHEN_RAIDER_RESTRAINT_REPLACEMENT_RUNTIME_VALIDATION") {
  throw "Godot Ashen Raider visual-restraint replacement validation exited with code $GodotExitCode."
}
