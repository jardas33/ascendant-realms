...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--aster-billboard-single-slot", "--aster-billboard-single-slot-benchmark-sequence", "--artifact-root=$ArtifactArg")
...
if ($GodotExitCode -ne 0 -or $RuntimeReport.status -ne "PASS_V0151_ASTER_BILLBOARD_RUNTIME_EVIDENCE") {
  throw "Godot Aster billboard benchmark exited with code $GodotExitCode."
}
