...
function Assert-ExpectedFiles {
  ...
  foreach ($fileName in $missing) {
    throw "Missing expected artifact '$fileName' for v0.197 capture scenario '$ScenarioRoot'."
  }
}
...
node "tools/godot/saltoShellV2MeshQaTool.mjs" capture "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.197 shell v2 mesh QA capture report failed with exit code $LASTEXITCODE."
}
