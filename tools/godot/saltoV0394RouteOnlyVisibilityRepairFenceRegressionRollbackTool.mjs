...
const script = file('desktop-spikes/godot-salto/scripts/v0394_route_only_visibility_repair_fence_regression_rollback.gd');
const scene = file('desktop-spikes/godot-salto/scenes/v0394_route_only_visibility_repair_fence_regression_rollback.tscn');
const router = file('desktop-spikes/godot-salto/scripts/salto_spike_root.gd');
const packageJson = file('package.json');
const audit = file('artifacts/work/v0394-asset-selection-audit.md');
const iterationLog = file('artifacts/work/v0394-iteration-log.md');
const report = file('docs/V0394_ROUTE_ONLY_VISIBILITY_REPAIR_FENCE_REGRESSION_ROLLBACK_REPORT.md');
...
const runtime = file('desktop-spikes/godot-salto/artifacts/runtime/v0394');
const pack = file('artifacts/manual-review/v0394-route-only-visibility-repair-fence-regression-rollback');
const images = [
...
function validate() {
  for (const [p, label] of [
    ...
    [script, 'v0.394 script'], [scene, 'v0.394 scene'], [router, 'opt-in router'],
    [packageJson, 'package commands'], [audit, 'asset audit'], [iterationLog, 'iteration log'],
    [report, 'v0.394 report'], [priorReport, 'preserved v0.386 report'],
  ...
  for (const token of [
    'V0394_Route_Only_Visibility_Repair_Fence_Regression_Rollback',
    'V0394_One_Primary_Barrosan_Homestead', 'V0394_One_Subordinate_Agricultural_Barn_Separate',
    'V0394_Five_Functional_Yard_Groups', 'V0394_Yard_Group_01_Cart_Direct_Ground',
    'V0394_Yard_Group_02_Stacked_Firewood_Direct_Ground', 'V0394_Yard_Group_03_Barrel_Crate_Storage_Direct_Ground',
    'V0394_Yard_Group_04_Trough_Workbench_Direct_Ground', 'V0394_Yard_Group_05_Fence_Regression_Rolled_Back',
    'V0388_Exactly_Three_Distinct_Character_Roles', 'V0394_One_Flush_Continuous_Worn_Earth_Route_Only',
    'V0394_One_Flush_Contrasting_Bridge_Yard_Door_Route', 'V0394_Short_Flush_Contrasting_Barn_Entrance_Branch', 'continuous_route_ribbon',
    'V0394_Bridge_Desaturated_Value', 'V0394_Bridge_Value_Control', 'Workbench_Narrow_Top', 'v0393_fence_geometry_removed', 'BaseMaterial3D.CULL_DISABLED', 'BaseMaterial3D.SHADING_MODE_UNSHADED',
    'Farmer.gltf', 'Adventurer.gltf', 'Worker.gltf',
    'v0394-route-only-visibility-repair-fence-regression-rollback.json', 'gameplay": false', 'defaultRuntime": "unchanged"'
  ]) must(g.includes(token), `script contract ${token}`);
  for (const token of [
    '--v0394-route-only', '--v0394-route-only-smoke', '--v0394-route-only-capture',
    'v0394_route_only_visibility_repair_fence_regression_rollback.gd'
  ]) must(r.includes(token), `router contract ${token}`);
  for (const token of [
    'godot:play:v0394-route-only', 'godot:smoke:v0394-route-only',
    'godot:capture:v0394-route-only', 'godot:validate:v0394-route-only'
  ...
  if ((g.match(/V0394_Yard_Group_/g) || []).length < 4) throw new Error('four visible yard group names missing');
  if (!g.includes('V0394_Yard_Group_05_Fence_Regression_Rolled_Back')) throw new Error('yard group 05 rollback marker missing');
  if (g.includes('Front_Fence_Run_') || g.includes('Open_Gate_Post_')) throw new Error('v0.393 fence geometry retained');
  if (!g.includes('BaseMaterial3D.CULL_DISABLED')) throw new Error('route material is not cull-safe');

  for (const i of [1, 2, 3]) {
    const d = file(`desktop-spikes/godot-salto/artifacts/work/v0394-iteration-0${i}`);
  ...
  must(fs.existsSync(pack), 'v0.394 review pack');
  ...
  if (validation.status !== 'READY FOR INDEPENDENT V0394 VISUAL REVIEW' && validation.status !== 'ACCEPTED BY INDEPENDENT V0394 VISUAL REVIEW' && !String(validation.status).startsWith('REJECTED BY INDEPENDENT V0394')) throw new Error('review pack status mismatch');
  ...
  console.log('PASS_V0394_ROUTE_ONLY_VISIBILITY_REPAIR_FENCE_REGRESSION_ROLLBACK_VALIDATOR (7 real captures; cull-safe route; v0.393 fence rollback; opt-in visual only)');
}

if (process.argv[2] === 'validate') validate();
else throw new Error('usage: node tools/godot/saltoV0394RouteOnlyVisibilityRepairFenceRegressionRollbackTool.mjs validate');
