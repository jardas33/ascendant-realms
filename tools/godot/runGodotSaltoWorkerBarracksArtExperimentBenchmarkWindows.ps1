...
$ArtifactRoot = Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0162\benchmark"
...
function Invoke-GodotBenchmarkScenario {
  ...
  if ($GodotExitCode -ne 0) {
    throw "Godot Worker + Barracks benchmark scenario '$ScenarioId' exited with code $GodotExitCode."
  }
}
...
Invoke-GodotBenchmarkScenario -ScenarioId "worker-only" -ScenarioArgs (Worker-ArtArgs)
Invoke-GodotBenchmarkScenario -ScenarioId "worker-barracks" -ScenarioArgs ((Worker-ArtArgs) + (Barracks-ArtArgs))
Invoke-GodotBenchmarkScenario -ScenarioId "barracks-missing-art-fallback" -ScenarioArgs ((Worker-ArtArgs) + (Barracks-ArtArgs -Source $MissingBarracksSourcePath -FallbackMode "missing"))
Invoke-GodotBenchmarkScenario -ScenarioId "barracks-hash-mismatch-fallback" -ScenarioArgs ((Worker-ArtArgs) + (Barracks-ArtArgs -Expected $MismatchSha -FallbackMode "hash-mismatch"))

node "tools/godot/saltoWorkerBarracksArtOptInTool.mjs" benchmark "--artifact-root=$((Join-Path $RepoRoot 'artifacts\desktop-spikes\godot-salto\v0162').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) {
  throw "v0.162 Worker + Barracks benchmark report failed with exit code $LASTEXITCODE."
}
