...
$ArtifactRoot = Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0179"
...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath, $AsterSourcePath, $AsterMetadataPath, $AshenSourcePath, $AshenMetadataPath, $GroundSourcePath, $GroundMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required v0.179 environment contrast validation path: $path"
  }
}
...
function Invoke-GodotScenario {
  ...
  if ($GodotExitCode -ne 0) {
    throw "Godot v0.179 scenario '$ScenarioRoot' exited with code $GodotExitCode."
  }
...
function Environment-PriorGroundArgs { return @("--player-slice", "--experimental-review-mode-label=Experimental opt-in art: 5 slots + prior textured foothold", "--salto-five-slot-review-framing", "--salto-environment-foundation-review") }
function Environment-ContrastArgs { return @("--player-slice", "--experimental-review-mode-label=Experimental opt-in art: 5 slots + textured foothold contrast", "--salto-five-slot-review-framing", "--salto-environment-foundation-review", "--salto-environment-contrast-harmonization") }
function Environment-ContrastMissingArgs { return @("--player-slice", "--experimental-review-mode-label=Experimental opt-in art: 5 slots + contrast ground fallback missing", "--salto-five-slot-review-framing", "--salto-environment-foundation-review", "--salto-environment-contrast-harmonization") }
function Environment-ContrastMismatchArgs { return @("--player-slice", "--experimental-review-mode-label=Experimental opt-in art: 5 slots + contrast ground fallback hash mismatch", "--salto-five-slot-review-framing", "--salto-environment-foundation-review", "--salto-environment-contrast-harmonization") }
...
function Invoke-V0179Tool {
  param([Parameter(Mandatory = $true)][string]$Command)
  node "tools/godot/saltoEnvironmentContrastHarmonizationTool.mjs" $Command "--artifact-root=$ArtifactRootArg"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.179 environment contrast harmonization tool '$Command' failed with exit code $LASTEXITCODE."
  }
}
...
Invoke-GodotScenario -ScenarioRoot (Join-Path $ValidationRoot "e1-prior-ground-material-opt-in") -ScenarioArgs (@("--player-slice-validate") + (Environment-PriorGroundArgs) + (Five-SlotArgs) + (Ground-MaterialArgs)) -ExpectedFiles @("player-slice-validation-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $ValidationRoot "e2-environment-contrast-harmonized") -ScenarioArgs (@("--player-slice-validate") + (Environment-ContrastArgs) + (Five-SlotArgs) + (Ground-MaterialArgs)) -ExpectedFiles @("player-slice-validation-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $ValidationRoot "ground-missing-art-contrast-fallback") -ScenarioArgs (@("--player-slice-validate") + (Environment-ContrastMissingArgs) + (Five-SlotArgs) + (Ground-MissingArgs)) -ExpectedFiles @("player-slice-validation-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $ValidationRoot "ground-hash-mismatch-contrast-fallback") -ScenarioArgs (@("--player-slice-validate") + (Environment-ContrastMismatchArgs) + (Five-SlotArgs) + (Ground-MismatchArgs)) -ExpectedFiles @("player-slice-validation-runtime.json")
Invoke-V0179Tool -Command "validation"

$CaptureRoot = Join-Path $ArtifactRoot "capture"
Reset-SafeDirectory -Path $CaptureRoot -Parent $ArtifactRoot
Invoke-GodotScenario -ScenarioRoot (Join-Path $CaptureRoot "e1-prior-ground-material-opt-in") -ScenarioArgs (@("--player-slice-capture") + (Environment-PriorGroundArgs) + (Five-SlotArgs) + (Ground-MaterialArgs)) -ExpectedFiles @("screenshot-runtime-manifest.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $CaptureRoot "e2-environment-contrast-harmonized") -ScenarioArgs (@("--player-slice-capture") + (Environment-ContrastArgs) + (Five-SlotArgs) + (Ground-MaterialArgs)) -ExpectedFiles @("screenshot-runtime-manifest.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $CaptureRoot "ground-missing-art-contrast-fallback") -ScenarioArgs (@("--player-slice-capture") + (Environment-ContrastMissingArgs) + (Five-SlotArgs) + (Ground-MissingArgs)) -ExpectedFiles @("screenshot-runtime-manifest.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $CaptureRoot "ground-hash-mismatch-contrast-fallback") -ScenarioArgs (@("--player-slice-capture") + (Environment-ContrastMismatchArgs) + (Five-SlotArgs) + (Ground-MismatchArgs)) -ExpectedFiles @("screenshot-runtime-manifest.json")
Invoke-V0179Tool -Command "capture"

$BenchmarkRoot = Join-Path $ArtifactRoot "benchmark"
Reset-SafeDirectory -Path $BenchmarkRoot -Parent $ArtifactRoot
Invoke-GodotScenario -ScenarioRoot (Join-Path $BenchmarkRoot "e1-prior-ground-material-opt-in") -ScenarioArgs (@("--worker-art-opt-in-benchmark") + (Environment-PriorGroundArgs) + (Five-SlotArgs) + (Ground-MaterialArgs)) -ExpectedFiles @("worker-art-opt-in-benchmark-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $BenchmarkRoot "e2-environment-contrast-harmonized") -ScenarioArgs (@("--worker-art-opt-in-benchmark") + (Environment-ContrastArgs) + (Five-SlotArgs) + (Ground-MaterialArgs)) -ExpectedFiles @("worker-art-opt-in-benchmark-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $BenchmarkRoot "ground-missing-art-contrast-fallback") -ScenarioArgs (@("--worker-art-opt-in-benchmark") + (Environment-ContrastMissingArgs) + (Five-SlotArgs) + (Ground-MissingArgs)) -ExpectedFiles @("worker-art-opt-in-benchmark-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $BenchmarkRoot "ground-hash-mismatch-contrast-fallback") -ScenarioArgs (@("--worker-art-opt-in-benchmark") + (Environment-ContrastMismatchArgs) + (Five-SlotArgs) + (Ground-MismatchArgs)) -ExpectedFiles @("worker-art-opt-in-benchmark-runtime.json")
Invoke-V0179Tool -Command "benchmark"

Invoke-V0179Tool -Command "boundary"

node "scripts/cleanupSaltoExperimentalArtifacts.mjs" "--output-root=$((Join-Path $ArtifactRoot 'cleanup-dry-run').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) {
  throw "v0.179 cleanup dry-run failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "v0.179 artifact retention validation failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0179_SALTO_ENVIRONMENT_CONTRAST_HARMONIZATION_AUTOMATION_READY"
