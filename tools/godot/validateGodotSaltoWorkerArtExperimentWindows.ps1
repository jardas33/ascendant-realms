...
$ArtifactRoot = Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0160\validation"
...
function Invoke-GodotValidationScenario {
  param(
  ...
  foreach ($fileName in @(
    ...
    Remove-Item -LiteralPath (Join-Path $ScenarioRoot $fileName) -Force -ErrorAction SilentlyContinue
  }
  $ArtifactArg = $ScenarioRoot.Replace("\", "/")
  $ArgumentList = @("--player-slice-validate", "--artifact-root=$ArtifactArg") + $ScenarioArgs
  ...
  if ($GodotExitCode -ne 0) {
    throw "Godot Worker art validation scenario '$ScenarioId' exited with code $GodotExitCode."
  }
  if (-not (Test-Path (Join-Path $ScenarioRoot "player-slice-validation-runtime.json"))) {
    throw "Missing validation runtime artifact for scenario '$ScenarioId'."
  }
}

Invoke-GodotValidationScenario -ScenarioId "default-procedural" -ScenarioArgs @()
Invoke-GodotValidationScenario -ScenarioId "worker-opt-in" -ScenarioArgs @(
  "--worker-art-opt-in",
...
Invoke-GodotValidationScenario -ScenarioId "missing-art-fallback" -ScenarioArgs @(
  "--worker-art-opt-in",
...
Invoke-GodotValidationScenario -ScenarioId "hash-mismatch-fallback" -ScenarioArgs @(
  "--worker-art-opt-in",
  "--worker-art-source=$($SourcePath.Replace('\', '/'))",
  "--worker-art-metadata=$($MetadataPath.Replace('\', '/'))",
...
node "tools/godot/saltoWorkerArtOptInTool.mjs" validate "--artifact-root=$($ArtifactRoot.Replace('\', '/'))"
node "tools/godot/saltoWorkerArtOptInTool.mjs" boundary "--artifact-root=$($ArtifactRoot.Replace('\', '/'))"
