...
Invoke-CaptureScenario -ScenarioId "selected-road-riverbank-water" -ExtraArgs @()
Invoke-CaptureScenario -ScenarioId "procedural-road-riverbank-water-fallback" -ExtraArgs @(
...
try {
  node "tools/godot/saltoRoadRiverbankWaterTool.mjs" capture "--artifact-root=$ArtifactRootArg"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.217 road riverbank water capture report failed with exit code $LASTEXITCODE."
  }
} finally {
  Remove-Item Env:\SALTO_CONTACT_SHEET_PYTHON -ErrorAction SilentlyContinue
}

Write-Output "PASS_V0217_ROAD_RIVERBANK_WATER_CAPTURE_READY"
