...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath, $AsterSourcePath, $AsterMetadataPath, $AshenSourcePath, $AshenMetadataPath, $GroundSourcePath, $GroundMetadataPath, $RoadSourcePath, $RoadMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required v0.181 ground+road material validation path: $path"
  }
}
...
function Invoke-GodotScenario {
  ...
  if ($GodotExitCode -ne 0) {
    throw "Godot v0.181 scenario '$ScenarioRoot' exited with code $GodotExitCode."
  }
...
function Invoke-V0181Tool {
  param([Parameter(Mandatory = $true)][string]$Command)
  node "tools/godot/saltoGroundRoadMaterialOptInTool.mjs" $Command "--artifact-root=$ArtifactRootArg"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.181 ground+road material opt-in tool '$Command' failed with exit code $LASTEXITCODE."
  }
}
...
Invoke-GodotScenario -ScenarioRoot (Join-Path $ValidationRoot "e2-ground-road-material-opt-in") -ScenarioArgs (@("--player-slice-validate") + (Environment-GroundRoadMaterialArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("player-slice-validation-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $ValidationRoot "road-missing-art-fallback") -ScenarioArgs (@("--player-slice-validate") + (Environment-RoadMissingArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MissingArgs)) -ExpectedFiles @("player-slice-validation-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $ValidationRoot "road-hash-mismatch-fallback") -ScenarioArgs (@("--player-slice-validate") + (Environment-RoadMismatchArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MismatchArgs)) -ExpectedFiles @("player-slice-validation-runtime.json")
Invoke-V0181Tool -Command "validation"
...
Invoke-GodotScenario -ScenarioRoot (Join-Path $CaptureRoot "e2-ground-road-material-opt-in") -ScenarioArgs (@("--player-slice-capture") + (Environment-GroundRoadMaterialArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("screenshot-runtime-manifest.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $CaptureRoot "road-missing-art-fallback") -ScenarioArgs (@("--player-slice-capture") + (Environment-RoadMissingArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MissingArgs)) -ExpectedFiles @("screenshot-runtime-manifest.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $CaptureRoot "road-hash-mismatch-fallback") -ScenarioArgs (@("--player-slice-capture") + (Environment-RoadMismatchArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MismatchArgs)) -ExpectedFiles @("screenshot-runtime-manifest.json")
Invoke-V0181Tool -Command "capture"
...
Invoke-GodotScenario -ScenarioRoot (Join-Path $BenchmarkRoot "e2-ground-road-material-opt-in") -ScenarioArgs (@("--worker-art-opt-in-benchmark") + (Environment-GroundRoadMaterialArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("worker-art-opt-in-benchmark-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $BenchmarkRoot "road-missing-art-fallback") -ScenarioArgs (@("--worker-art-opt-in-benchmark") + (Environment-RoadMissingArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MissingArgs)) -ExpectedFiles @("worker-art-opt-in-benchmark-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $BenchmarkRoot "road-hash-mismatch-fallback") -ScenarioArgs (@("--worker-art-opt-in-benchmark") + (Environment-RoadMismatchArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MismatchArgs)) -ExpectedFiles @("worker-art-opt-in-benchmark-runtime.json")
Invoke-V0181Tool -Command "benchmark"

Invoke-V0181Tool -Command "boundary"

node "scripts/cleanupSaltoExperimentalArtifacts.mjs" "--output-root=$((Join-Path $ArtifactRoot 'cleanup-dry-run').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) {
  throw "v0.181 cleanup dry-run failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "v0.181 artifact retention validation failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0181_SALTO_GROUND_ROAD_MATERIAL_OPT_IN_AUTOMATION_READY"
