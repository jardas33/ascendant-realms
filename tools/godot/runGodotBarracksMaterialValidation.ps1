...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--barrosan-barracks-material-single-slot", "--barracks-material-validate-only", "--artifact-root=$ArtifactArg")
...
$RuntimeValidationPath = Join-Path $ArtifactRoot "barracks-material-validation-runtime.json"
...
if ($GodotExitCode -ne 0 -and $RuntimeValidation.status -ne "PASS_V0149_BARRACKS_MATERIAL_RUNTIME_VALIDATION") {
  throw "Godot Barrosan Barracks material validation exited with code $GodotExitCode."
}
