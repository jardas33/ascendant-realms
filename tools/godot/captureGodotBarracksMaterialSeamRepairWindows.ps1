...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--barrosan-barracks-material-seam-repair", "--barracks-material-seam-repair-capture-only", "--artifact-root=$ArtifactArg")
...
if ($GodotExitCode -ne 0 -and $RuntimeReport.status -ne "PASS_V0150_BARRACKS_MATERIAL_SEAM_REPAIR_RUNTIME_EVIDENCE") {
  throw "Godot Barrosan Barracks material seam repair capture exited with code $GodotExitCode."
}
