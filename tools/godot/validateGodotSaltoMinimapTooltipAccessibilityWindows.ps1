...
if (-not (Test-Path -LiteralPath (Join-Path $ArtifactRoot "capture\opt-in\screenshot-runtime-manifest.json"))) {
  & (Join-Path $PSScriptRoot "captureGodotSaltoMinimapTooltipAccessibilityWindows.ps1")
}

$OptInValidationRoot = Join-Path $ArtifactRoot "validation\opt-in"
$PriorValidationRoot = Join-Path $ArtifactRoot "validation\prior-production-objectives-log"
$DefaultValidationRoot = Join-Path $ArtifactRoot "validation\default-procedural"

Invoke-GodotPass -ScriptPath (Join-Path $PSScriptRoot "launchGodotSaltoUiShellExperimentWindows.ps1") -Args @("--player-slice-validate", "--salto-selection-command-panel", "--salto-production-objectives-log", "--salto-minimap-tooltip-accessibility", "--artifact-root=$($OptInValidationRoot.Replace('\', '/'))") -ManifestPath (Join-Path $OptInValidationRoot "player-slice-validation-runtime.json") -ExpectedStatus "PASS_PLAYER_SLICE_VALIDATION"
Invoke-GodotPass -ScriptPath (Join-Path $PSScriptRoot "launchGodotSaltoUiShellExperimentWindows.ps1") -Args @("--player-slice-validate", "--salto-selection-command-panel", "--salto-production-objectives-log", "--artifact-root=$($PriorValidationRoot.Replace('\', '/'))") -ManifestPath (Join-Path $PriorValidationRoot "player-slice-validation-runtime.json") -ExpectedStatus "PASS_PLAYER_SLICE_VALIDATION"
...
node "tools/godot/saltoMinimapTooltipAccessibilityTool.mjs" validation "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.212 minimap tooltip accessibility validation failed with exit code $LASTEXITCODE."
}

node "tools/godot/saltoMinimapTooltipAccessibilityTool.mjs" boundary "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.212 minimap tooltip accessibility boundary scan failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "v0.212 artifact retention validation failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0212_MINIMAP_TOOLTIP_ACCESSIBILITY_VALIDATION_READY"
