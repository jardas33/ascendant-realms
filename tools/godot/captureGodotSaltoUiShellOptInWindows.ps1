...
function Invoke-UiShellExperimentPass {
  param(
...
$FallbackRoot = Join-Path $CaptureRoot "fallback"
$OptInArg = $OptInRoot.Replace("\", "/")
$FallbackArg = $FallbackRoot.Replace("\", "/")

Invoke-UiShellExperimentPass -Args @("--player-slice-capture", "--artifact-root=$OptInArg") -ManifestPath (Join-Path $OptInRoot "screenshot-runtime-manifest.json") -ExpectedStatus "PASS_PLAYER_SLICE_CAPTURE"
Invoke-UiShellExperimentPass -Args @("--player-slice-capture", "--artifact-root=$FallbackArg", "--salto-ui-shell-force-fallback") -ManifestPath (Join-Path $FallbackRoot "screenshot-runtime-manifest.json") -ExpectedStatus "PASS_PLAYER_SLICE_CAPTURE"
...
try {
  node "tools/godot/saltoUiShellOptInTool.mjs" capture "--artifact-root=$ArtifactRootArg"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.209 UI shell opt-in review pack creation failed with exit code $LASTEXITCODE."
  }
} finally {
