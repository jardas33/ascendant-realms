...
if (Test-Path -LiteralPath $ArtifactRoot) {
  ...
  $expectedRoot = Resolve-Path -LiteralPath (Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0180")
  if (-not ($resolvedArtifact.Path.StartsWith($expectedRoot.Path))) {
    throw "Refusing to remove road material evidence outside expected root: $($resolvedArtifact.Path)"
  }
...
node "tools/godot/roadMaterialSingleSlotTool.mjs" validate "--artifact-root=$ArtifactArg"
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--barrosan-road-material-single-slot", "--road-material-benchmark-sequence", "--artifact-root=$ArtifactArg")
...
$RuntimePath = Join-Path $ArtifactRoot "road-material-runtime.json"
$RuntimeReport = if (Test-Path -LiteralPath $RuntimePath) {
  Get-Content -Raw -LiteralPath $RuntimePath | ConvertFrom-Json
} else {
  $null
}
if ($GodotExitCode -ne 0 -or $RuntimeReport.status -ne "PASS_V0180_ROAD_MATERIAL_RUNTIME_EVIDENCE") {
  throw "Godot Barrosan foothold road material benchmark failed with code $GodotExitCode."
}
node "tools/godot/roadMaterialSingleSlotTool.mjs" report "--artifact-root=$ArtifactArg"
