...
draw.text((140, 145), "v0.280 PREFLIGHT - BARROSAN Engage Commit Resolution Bridge", fill="#e8d99d")
draw.text((140, 225), "Base HEAD:", fill="#9ecfc5")
draw.text((140, 275), "12e21632e8af6f2fb24d830972fad58f059bd07d", fill="#ffffff")
draw.text((140, 365), "Baseline exact-SHA Actions run:", fill="#9ecfc5")
draw.text((140, 415), "28636151686 - success", fill="#9fe2a3")
draw.text((140, 510), "Scope:", fill="#9ecfc5")
draw.text((140, 560), "Manual Commit Engage bridge. Armed state keeps exactly ENGAGE ARMED; committed state keeps exactly PRESSURE CHECKED.", fill="#ffffff")
draw.text((140, 650), "One stateful consequence only: pressure checked 90/100 -> 80/100 once. Forbidden: projectile, unit/enemy damage, death/despawn, pathing, AI, waves, economy, fog, default runtime mutation.", fill="#d7c98f")
preflight.save(root / "01_v0280_preflight_head_ci_resolution.png")

capture_paths = [root / "02_v0280_default_runtime_unchanged_visible.png"] + [
...
(root / "v0280-black-frame-stats.json").write_text(json.dumps({"stats": all_stats}, indent=2) + "\n", encoding="utf-8")
...
draw.text((100, 105), "v0.280 BLACK-FRAME REJECTION REPORT", fill="#e8d99d")
...
draw.text((100, 590), "Default runtime and opt-in v0.280 readability runtime were both inspected.", fill="#d7c98f")
report.save(root / "17_v0280_black_frame_rejection_report_visible.png")

sheet_paths = [root / "01_v0280_preflight_head_ci_resolution.png"] + capture_paths + [root / "17_v0280_black_frame_rejection_report_visible.png"]
tiles = []
...
sheet.save(root / "16_v0280_contact_sheet_visible.png")
