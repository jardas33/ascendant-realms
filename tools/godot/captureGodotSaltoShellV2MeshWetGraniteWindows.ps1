...
function Assert-ExpectedFiles {
  ...
  foreach ($fileName in $missing) {
    throw "Missing expected artifact '$fileName' for v0.198 capture scenario '$ScenarioRoot'."
  }
}
...
try {
  & (Join-Path $PSScriptRoot "launchGodotSaltoShellV2MeshQaWindows.ps1") -Wait "--player-slice-capture" "--artifact-root=$((Join-Path $CaptureRoot 'm2-shell-v2-mesh-qa').Replace('\', '/'))"
  Assert-ExpectedFiles -ScenarioRoot (Join-Path $CaptureRoot "m2-shell-v2-mesh-qa") -ExpectedFiles @("screenshot-runtime-manifest.json")
  & (Join-Path $PSScriptRoot "launchGodotSaltoShellV2MeshWetGraniteWindows.ps1") -Wait "--player-slice-capture" "--artifact-root=$((Join-Path $CaptureRoot 'w1-shell-v2-mesh-wet-granite').Replace('\', '/'))"
  Assert-ExpectedFiles -ScenarioRoot (Join-Path $CaptureRoot "w1-shell-v2-mesh-wet-granite") -ExpectedFiles @("screenshot-runtime-manifest.json")
} finally {
...
node "tools/godot/saltoShellV2MeshWetGraniteTool.mjs" capture "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.198 wet-granite mesh opt-in capture report failed with exit code $LASTEXITCODE."
}
