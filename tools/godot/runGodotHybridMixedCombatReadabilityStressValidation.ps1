...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--hybrid-mixed-combat-readability-stress", "--hybrid-mixed-combat-readability-stress-validate-only", "--artifact-root=$ArtifactArg")
...
$RuntimeValidationPath = Join-Path $ArtifactRoot "hybrid-mixed-combat-validation-runtime.json"
...
if ($GodotExitCode -ne 0 -or $RuntimeValidation.status -ne "PASS_V0158_HYBRID_MIXED_COMBAT_RUNTIME_VALIDATION") {
  throw "Godot hybrid mixed-combat validation exited with code $GodotExitCode."
}
node "tools/godot/hybridMixedCombatReadabilityStressTool.mjs" validate "--artifact-root=$ArtifactArg"
