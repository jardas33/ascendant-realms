...
const checkpoint = "v0.200";
const artifactRootDefault = join(repoRoot, "artifacts", "desktop-spikes", "godot-salto", "v0200");
const manualReviewRoot = join(repoRoot, "artifacts", "manual-review", "v0200-grounding-lighting");
...
const scenarios = {
  ...
  hardened: "g1-shell-v2-grounding-lighting"
};
...
function checkGroundingLighting(report, id, errors) {
  checkStructureHierarchy(report, id, errors);
  const s = shell(report);
  const m = mesh(report);
  const h = hierarchy(report);
  const gl = groundingLighting(report);
  if (report?.environmentShellV2GroundingLightingEnabled === false || s.groundingLightingEnabled !== true || m.groundingLightingEnabled !== true || gl.enabled !== true) {
    errors.push(`${id} did not enable the v0.200 grounding lighting flag.`);
  }
  if (h.groundingLightingEnabled !== true) errors.push(`${id} hierarchy status did not acknowledge v0.200 grounding lighting.`);
  if (gl.aiImageGenerated !== false || Number(gl.newArtSlotsAdded ?? 99) !== 0 || Number(gl.newImportedTextures ?? 99) !== 0) {
    errors.push(`${id} reported new images, slots, or textures.`);
  }
  for (const key of [
    "overcastValueBalanceRestrained",
    "contactShadowsUsed",
    "sparseRockClustersUsed",
    "sparseMossGrassAccentsUsed",
    "timberStoneEdgePropsUsed",
    "bankEdgeAccentsUsed",
    "restrainedWarmHearthCuesUsed",
    "sceneLessEmpty",
    "noClutterNoise",
    "tacticalRoutesClear",
    "structuresUnitsLegible",
    "structureHierarchyPreserved",
    "routeRiverBridgeReadabilityPreserved",
    "wetGraniteMeshComparatorPreserved"
  ]) {
    if (gl[key] !== true) errors.push(`${id} grounding lighting gate ${key} was not true.`);
  }
  if (Number(gl.visualNodeCount ?? 0) < 22) errors.push(`${id} grounding lighting node count too low: ${gl.visualNodeCount ?? "missing"}.`);
  if (Number(gl.visualNodeCount ?? 99) > 42) errors.push(`${id} grounding lighting node count too high/noisy: ${gl.visualNodeCount}.`);
}
...
function scenarioSummary(id, loaded) {
  ...
  return {
    ...
    groundingLightingEnabled: report?.environmentShellV2GroundingLightingEnabled ?? false,
    meshCompositor: mesh(report),
    structureHierarchy: hierarchy(report),
    groundingLighting: groundingLighting(report),
...
function validationCommand(root) {
  ...
  const configs = [
    ...
    { id: scenarios.hardened, check: checkGroundingLighting }
  ];
  ...
  const report = {
    schemaVersion: 1,
    checkpoint,
    status: errors.length === 0 ? "PASS_V0200_GROUNDING_LIGHTING_VALIDATION" : "FAIL_V0200_GROUNDING_LIGHTING_VALIDATION",
    scenarios: summaries,
    errors
  };
  writeJson(join(root, "validation", "grounding-lighting-validation-report.json"), report);
  if (errors.length) {
...
function captureCommand(root) {
  ...
  if (hardened.report) checkGroundingLighting(hardened.report, scenarios.hardened, errors);
  const hardenedIds = new Set((hardened.report?.captures ?? []).map((capture) => capture.id));
  for (const id of requiredCaptureIds) {
    if (!hardenedIds.has(id)) errors.push(`G1 capture is missing ${id}.`);
  }
  const manual = [
    [captureById(hardened.report, "overview"), "01_overview.png"],
    [captureById(hardened.report, "ground_roads"), "02_ground_roads.png"],
    [captureById(hardened.report, "river_bridge"), "03_river_bridge.png"],
    [captureById(hardened.report, "structures"), "04_structures.png"],
    [captureById(hardened.report, "units"), "05_units.png"],
    [captureById(hardened.report, "combat"), "06_combat.png"],
    [captureById(hardened.report, "pan_zoom"), "07_pan_zoom.png"]
  ];
  ...
  if (!existsSync(contactSheetPath)) errors.push("Missing v0.200 contact sheet PNG.");
  else manualPaths.push(contactSheetPath);
  const contactLines = [
    "# v0.200 Grounding Lighting Manual Review Pack",
    "",
  ...
  const report = {
    schemaVersion: 1,
    checkpoint,
    status: errors.length === 0 ? "PASS_V0200_GROUNDING_LIGHTING_CAPTURE_PACKET" : "FAIL_V0200_GROUNDING_LIGHTING_CAPTURE_PACKET",
  ...
  writeJson(join(root, "capture", "grounding-lighting-capture-report.json"), report);
  if (errors.length) {
    console.error(errors.join("\n"));
    process.exit(1);
  }
...
function benchmarkCommand(root) {
  ...
  if (hardened.report) checkGroundingLighting(hardened.report, `benchmark-${scenarios.hardened}`, errors);
  ...
  if (fpsRatio < 0.90) errors.push(`G1/S1 FPS ratio below 0.90: ${fpsRatio.toFixed(4)}.`);
  if (p95WorseningRatio > 0.15) errors.push(`G1 p95 worsening above 15%: ${(p95WorseningRatio * 100).toFixed(2)}%.`);
  const report = {
    schemaVersion: 1,
    checkpoint,
    status: errors.length === 0 ? "PASS_V0200_GROUNDING_LIGHTING_BENCHMARK" : "FAIL_V0200_GROUNDING_LIGHTING_BENCHMARK",
  ...
  writeJson(join(root, "benchmark", "grounding-lighting-benchmark-report.json"), report);
  writeText(join(root, "benchmark", "grounding-lighting-scorecard.md"), [
    "# v0.200 Grounding Lighting Benchmark",
    "",
    `Status: \`${report.status}\``,
    "",
    `S1 baseline FPS: \`${baselineFps}\``,
    `G1 hardened FPS: \`${hardenedFps}\``,
    `FPS ratio: \`${fpsRatio.toFixed(4)}\``,
    `S1 baseline p95 ms: \`${baselineP95}\``,
    `G1 hardened p95 ms: \`${hardenedP95}\``,
    `p95 worsening: \`${(p95WorseningRatio * 100).toFixed(2)}%\``,
    "",
    "Thresholds: G1/S1 FPS ratio >= 0.90; G1 p95 worsening <= 15%."
  ].join("\n") + "\n");
  if (errors.length) {
    console.error(errors.join("\n"));
    process.exit(1);
...
function boundaryCommand(root) {
  ...
  const allowedEvidenceImagePatterns = [
    /^artifacts\/manual-review\/v0200-grounding-lighting\/\d{2}_[^/]+\.png$/u
  ];
  ...
  const launchPath = join(repoRoot, "tools/godot/launchGodotSaltoShellV2GroundingLightingWindows.ps1");
  const launch = existsSync(launchPath) ? readFileSync(launchPath, "utf8") : "";
  if (!launch.includes("--salto-shell-v2-grounding-lighting")) errors.push("Grounding lighting launcher lacks --salto-shell-v2-grounding-lighting.");
  if (!launch.includes("--salto-shell-v2-structure-hierarchy")) errors.push("Grounding lighting launcher lacks --salto-shell-v2-structure-hierarchy.");
  if (!launch.includes("--salto-shell-v2-mesh-compositor")) errors.push("Grounding lighting launcher lacks mesh compositor comparator flag.");
  if (!launch.includes("--bridge-riverbank-material-opt-in")) errors.push("Grounding lighting launcher did not preserve wet-granite context.");
  if (!launch.includes(bridgeRiverbankSha)) errors.push("Grounding lighting launcher lacks exact selected v0.189 SHA.");
  const report = {
    schemaVersion: 1,
    checkpoint,
    status: errors.length === 0 ? "PASS_V0200_GROUNDING_LIGHTING_BOUNDARY_SCAN" : "FAIL_V0200_GROUNDING_LIGHTING_BOUNDARY_SCAN",
    changedFiles,
    assertions: {
      ...
      groundingLightingOptInOnly: true,
      structureHierarchyPreserved: true,
      wetGraniteMeshContextPreserved: true
    },
    errors
  };
  writeJson(join(root, "boundary", "grounding-lighting-boundary-scan.json"), report);
  if (errors.length) {
    console.error(errors.join("\n"));
    process.exit(1);
  }
...
if (command === "validation") validationCommand(root);
  ...
  console.error("Usage: node tools/godot/saltoShellV2GroundingLightingTool.mjs <validation|capture|benchmark|boundary> [--artifact-root=...]");
  process.exit(1);
}
