...
$WetGraniteArgs = @(
  "--experimental-review-mode-label=Experimental opt-in: Salto shell v2 mesh wet granite",
...
if ($RemainingArgs) {
  $WetGraniteArgs += $RemainingArgs
}

Write-Output "Launching v0.198 Salto shell v2 mesh-compositor wet-granite bridge-riverbank review."
Write-Output "Wet granite SHA: $BridgeRiverbankExpectedSha"
Write-Output "QA scope: scoped mesh bridge abutments and riverbank retaining edges only."
Write-Output "Legacy shell remains available as comparator/fallback; default launcher remains procedural."

& (Join-Path $PSScriptRoot "launchGodotSaltoPresentationShellV2Windows.ps1") -Wait:$Wait @WetGraniteArgs
