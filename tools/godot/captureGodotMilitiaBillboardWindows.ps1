...
if (Test-Path -LiteralPath $ArtifactRoot) {
  ...
  $expectedRoot = Resolve-Path -LiteralPath (Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0154")
  if (-not ($resolvedArtifact.Path.StartsWith($expectedRoot.Path))) {
    throw "Refusing to remove Militia billboard evidence outside expected root: $($resolvedArtifact.Path)"
  }
...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--militia-billboard-single-slot", "--militia-billboard-single-slot-capture-only", "--artifact-root=$ArtifactArg")
...
$RuntimePath = Join-Path $ArtifactRoot "militia-billboard-single-slot-runtime.json"
$RuntimeReport = if (Test-Path -LiteralPath $RuntimePath) {
  Get-Content -Raw -LiteralPath $RuntimePath | ConvertFrom-Json
} else {
  $null
}
if ($GodotExitCode -ne 0 -or $RuntimeReport.status -ne "PASS_V0154_MILITIA_BILLBOARD_RUNTIME_EVIDENCE") {
  throw "Godot Militia billboard capture exited with code $GodotExitCode."
}
node "tools/godot/militiaBillboardSingleSlotTool.mjs" report "--artifact-root=$ArtifactArg"
node "tools/godot/militiaBillboardSingleSlotTool.mjs" audit "--artifact-root=$ArtifactArg"
