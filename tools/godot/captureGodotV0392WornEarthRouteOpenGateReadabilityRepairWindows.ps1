...
Set-Location $repo
$iteration = if ($env:V0392_ITERATION) { $env:V0392_ITERATION } else { '3' }
$outputRoot = if ($env:V0392_OUTPUT_ROOT) { $env:V0392_OUTPUT_ROOT } else { 'artifacts/runtime/v0392' }
...
$process = Start-Process -FilePath $godot -ArgumentList @('--path','desktop-spikes/godot-salto','--v0392-worn-earth-capture',"--artifact-root=$outputRoot", "--v0392-iteration=$iteration") -Wait -PassThru -NoNewWindow
if ($process.ExitCode -ne 0) { exit $process.ExitCode }
foreach ($name in @('01_PRIMARY_RTS_VIEW.png','02_SETTLEMENT_AND_CROSSING_CONTEXT.png','03_ROAD_YARD_ENTRANCE_CONNECTION.png','04_FIVE_FUNCTIONAL_YARD_GROUPS.png','05_THREE_CHARACTER_ROLE_AUDIT.png','06_BUILDING_SEPARATION_AND_ENTRANCES.png','07_GRAYSCALE_PRIMARY.png','v0392-worn-earth-route-open-gate-readability-repair.json')) {
...
Write-Output 'PASS_V0392_RENDERED_WORN_EARTH_ROUTE_OPEN_GATE_READABILITY_REPAIR'
