...
Set-Location $repo
$iteration = if ($env:V0393_ITERATION) { $env:V0393_ITERATION } else { '3' }
$outputRoot = if ($env:V0393_OUTPUT_ROOT) { $env:V0393_OUTPUT_ROOT } else { 'artifacts/runtime/v0393' }
...
$process = Start-Process -FilePath $godot -ArgumentList @('--path','desktop-spikes/godot-salto','--v0393-explicit-route-capture',"--artifact-root=$outputRoot", "--v0393-iteration=$iteration") -Wait -PassThru -NoNewWindow
if ($process.ExitCode -ne 0) { exit $process.ExitCode }
foreach ($name in @('01_PRIMARY_RTS_VIEW.png','02_SETTLEMENT_AND_CROSSING_CONTEXT.png','03_ROAD_YARD_ENTRANCE_CONNECTION.png','04_FIVE_FUNCTIONAL_YARD_GROUPS.png','05_THREE_CHARACTER_ROLE_AUDIT.png','06_BUILDING_SEPARATION_AND_ENTRANCES.png','07_GRAYSCALE_PRIMARY.png','v0393-explicit-route-silhouette-front-yard-gate-repair.json')) {
...
Write-Output 'PASS_V0393_RENDERED_EXPLICIT_ROUTE_SILHOUETTE_FRONT_YARD_GATE_REPAIR'
