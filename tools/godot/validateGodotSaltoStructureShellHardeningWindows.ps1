...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath, $AsterSourcePath, $AsterMetadataPath, $AshenSourcePath, $AshenMetadataPath, $GroundSourcePath, $GroundMetadataPath, $RoadSourcePath, $RoadMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required v0.186 structure shell hardening validation path: $path"
  }
}
...
function Invoke-GodotScenario {
  ...
  if ($GodotExitCode -ne 0) {
    throw "Godot v0.186 scenario '$ScenarioRoot' exited with code $GodotExitCode."
  }
  foreach ($fileName in $ExpectedFiles) {
    if (-not (Test-Path -LiteralPath (Join-Path $ScenarioRoot $fileName))) {
      throw "Missing expected artifact '$fileName' for v0.186 scenario '$ScenarioRoot'."
    }
  }
}
...
function Environment-GeometryBaselineArgs { return @("--player-slice", "--experimental-review-mode-label=Experimental opt-in: 5 slots + Barrosan ground + roads + geometry convergence baseline", "--salto-five-slot-review-framing", "--salto-environment-foundation-review", "--salto-environment-geometry-convergence") }
function Environment-ShellLiveQaArgs { return @("--player-slice", "--experimental-review-mode-label=Experimental opt-in: 5 slots + Barrosan ground + roads + refined shell live QA", "--salto-five-slot-review-framing", "--salto-environment-foundation-review", "--salto-environment-shell-live-qa") }
function Structure-ShellHardeningArgs { return @("--player-slice", "--experimental-review-mode-label=Experimental opt-in: 5 slots + Barrosan ground + roads + structure shell hardening", "--salto-five-slot-review-framing", "--salto-environment-foundation-review", "--salto-environment-shell-live-qa", "--salto-structure-shell-hardening") }

function Invoke-V0186Tool {
  param([Parameter(Mandatory = $true)][string]$Command)
  node "tools/godot/saltoStructureShellHardeningTool.mjs" $Command "--artifact-root=$ArtifactRootArg"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.186 structure shell hardening tool '$Command' failed with exit code $LASTEXITCODE."
  }
}
...
Invoke-GodotScenario -ScenarioRoot (Join-Path $ValidationRoot "e4-refined-shell-live-qa-baseline") -ScenarioArgs (@("--player-slice-validate") + (Environment-ShellLiveQaArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("player-slice-validation-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $ValidationRoot "s1-structure-shell-hardening") -ScenarioArgs (@("--player-slice-validate") + (Structure-ShellHardeningArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("player-slice-validation-runtime.json")
Invoke-V0186Tool -Command "validation"

$CaptureRoot = Join-Path $ArtifactRoot "capture"
Reset-SafeDirectory -Path $CaptureRoot -Parent $ArtifactRoot
Invoke-GodotScenario -ScenarioRoot (Join-Path $CaptureRoot "e4-refined-shell-live-qa-baseline") -ScenarioArgs (@("--player-slice-capture") + (Environment-ShellLiveQaArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("screenshot-runtime-manifest.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $CaptureRoot "s1-structure-shell-hardening") -ScenarioArgs (@("--player-slice-capture") + (Structure-ShellHardeningArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("screenshot-runtime-manifest.json")
Invoke-V0186Tool -Command "capture"

$BenchmarkRoot = Join-Path $ArtifactRoot "benchmark"
Reset-SafeDirectory -Path $BenchmarkRoot -Parent $ArtifactRoot
Invoke-GodotScenario -ScenarioRoot (Join-Path $BenchmarkRoot "e4-refined-shell-live-qa-baseline") -ScenarioArgs (@("--worker-art-opt-in-benchmark") + (Environment-ShellLiveQaArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("worker-art-opt-in-benchmark-runtime.json")
Invoke-GodotScenario -ScenarioRoot (Join-Path $BenchmarkRoot "s1-structure-shell-hardening") -ScenarioArgs (@("--worker-art-opt-in-benchmark") + (Structure-ShellHardeningArgs) + (Five-SlotArgs) + (Ground-MaterialArgs) + (Road-MaterialArgs)) -ExpectedFiles @("worker-art-opt-in-benchmark-runtime.json")
Invoke-V0186Tool -Command "benchmark"

Invoke-V0186Tool -Command "boundary"

node "scripts/cleanupSaltoExperimentalArtifacts.mjs" "--output-root=$((Join-Path $ArtifactRoot 'cleanup-dry-run').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) {
  throw "v0.186 cleanup dry-run failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "v0.186 artifact retention validation failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0186_SALTO_STRUCTURE_SHELL_HARDENING_AUTOMATION_READY"
