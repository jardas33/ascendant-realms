...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath, $AsterSourcePath, $AsterMetadataPath, $AshenSourcePath, $AshenMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required Worker + Barracks + Militia + Aster + Ashen opt-in path: $path"
  }
}
...
$ArgumentList = @(
  "--player-slice",
  "--experimental-review-mode-label=Experimental opt-in art: Worker + Barracks + Militia + Aster + Ashen",
...
Write-Output "Launching v0.170 Worker + Barracks + Militia + Aster + Ashen opt-in player slice."
...
Write-Output "Ashen: ashen_raider_billboard_static_v0156 / HYBRID_ASHEN_RAIDER_V0157_TRIMMED_1024 / $AshenExpectedSha"
Write-Output "Mode label: Experimental opt-in art: Worker + Barracks + Militia + Aster + Ashen"

if ($Wait) {
  & $ExePath @ArgumentList
  $GodotExitCode = if ($null -eq $LASTEXITCODE) { 0 } else { $LASTEXITCODE }
  if ($GodotExitCode -ne 0) {
    throw "Packaged Godot five-slot art opt-in player slice exited with code $GodotExitCode."
  }
  return
}
