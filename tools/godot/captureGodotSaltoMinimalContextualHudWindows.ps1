...
Invoke-CaptureScenario -ScenarioId "v0221-before-minimal-contextual-hud" -ExtraArgs @(
  "--salto-minimal-contextual-hud-disabled"
)
Invoke-CaptureScenario -ScenarioId "selected-minimal-contextual-hud" -ExtraArgs @(
...
try {
  node "tools/godot/saltoMinimalContextualHudTool.mjs" capture "--artifact-root=$ArtifactRootArg"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.222 minimal contextual HUD capture report failed with exit code $LASTEXITCODE."
  }
} finally {
  Remove-Item Env:\SALTO_CONTACT_SHEET_PYTHON -ErrorAction SilentlyContinue
}

Write-Output "PASS_V0222_MINIMAL_CONTEXTUAL_HUD_CAPTURE_READY"
