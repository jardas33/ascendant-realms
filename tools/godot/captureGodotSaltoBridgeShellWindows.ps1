...
Invoke-CaptureScenario -ScenarioId "selected-bridge-shell" -ExtraArgs @()
Invoke-CaptureScenario -ScenarioId "legacy-bridge-comparator" -ExtraArgs @(
...
try {
  node "tools/godot/saltoBridgeShellTool.mjs" capture "--artifact-root=$ArtifactRootArg"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.218 bridge shell capture report failed with exit code $LASTEXITCODE."
  }
} finally {
  Remove-Item Env:\SALTO_CONTACT_SHEET_PYTHON -ErrorAction SilentlyContinue
}

Write-Output "PASS_V0218_BRIDGE_SHELL_CAPTURE_READY"
