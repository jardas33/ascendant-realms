...
$ArtifactRoot = Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0162"
...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath)) {
  if (-not (Test-Path $path)) {
    throw "Missing required Worker + Barracks validation path: $path"
  }
}
...
function Invoke-V0162Tool {
  param([Parameter(Mandatory = $true)][string]$Command)
  node "tools/godot/saltoWorkerBarracksArtOptInTool.mjs" $Command "--artifact-root=$($ArtifactRoot.Replace('\', '/'))"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.162 Worker + Barracks opt-in tool '$Command' failed with exit code $LASTEXITCODE."
  }
}
...
Invoke-V0162Tool -Command "validation"
...
Invoke-V0162Tool -Command "capture"
...
Invoke-V0162Tool -Command "benchmark"
...
Invoke-V0162Tool -Command "real-input"

Invoke-V0162Tool -Command "boundary"
Invoke-V0162Tool -Command "summary"

Write-Output "PASS_V0162_WORKER_BARRACKS_ART_OPT_IN_AUTOMATION_READY"
