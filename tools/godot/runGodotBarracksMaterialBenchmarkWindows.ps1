...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--barrosan-barracks-material-single-slot", "--barracks-material-benchmark-sequence", "--artifact-root=$ArtifactArg")
...
if ($GodotExitCode -ne 0 -and $RuntimeReport.status -ne "PASS_V0149_BARRACKS_MATERIAL_RUNTIME_EVIDENCE") {
  throw "Godot Barrosan Barracks material benchmark exited with code $GodotExitCode."
}
