...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--barrosan-road-material-single-slot", "--road-material-validate-only", "--artifact-root=$ArtifactArg")
...
$RuntimeValidationPath = Join-Path $ArtifactRoot "road-material-validation-runtime.json"
...
if ($GodotExitCode -ne 0 -or $RuntimeValidation.status -ne "PASS_V0180_ROAD_MATERIAL_RUNTIME_VALIDATION") {
  throw "Godot Barrosan foothold road material validation failed with code $GodotExitCode."
}
