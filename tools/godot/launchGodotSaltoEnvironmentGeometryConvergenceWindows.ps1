...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath, $AsterSourcePath, $AsterMetadataPath, $AshenSourcePath, $AshenMetadataPath, $GroundSourcePath, $GroundMetadataPath, $RoadSourcePath, $RoadMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required v0.184 environment geometry convergence path: $path"
  }
}
...
$ArgumentList = @(
  "--player-slice",
  "--experimental-review-mode-label=Experimental opt-in: 5 slots + Barrosan ground + roads + geometry convergence",
...
Write-Output "Launching v0.184 Salto environment geometry convergence review."
Write-Output "Experimental opt-in: 5 slots + Barrosan ground + roads + geometry convergence"
...
Write-Output "Environment-shell posture: visual-only geometry convergence; default launcher remains procedural."

if ($Wait) {
  & $ExePath @ArgumentList
  $GodotExitCode = if ($null -eq $LASTEXITCODE) { 0 } else { $LASTEXITCODE }
  if ($GodotExitCode -ne 0) {
    throw "Packaged Godot environment geometry convergence review exited with code $GodotExitCode."
  }
  return
}
