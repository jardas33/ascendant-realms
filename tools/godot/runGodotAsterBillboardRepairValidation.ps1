...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--aster-billboard-single-slot-repair", "--aster-billboard-repair-validate-only", "--artifact-root=$ArtifactArg")
...
$RuntimeValidationPath = Join-Path $ArtifactRoot "aster-billboard-repair-validation-runtime.json"
...
if ($GodotExitCode -ne 0 -or $RuntimeValidation.status -ne "PASS_V0152_ASTER_BILLBOARD_REPAIR_RUNTIME_VALIDATION") {
  throw "Godot Aster billboard repair validation exited with code $GodotExitCode."
}
