...
draw.text((140, 140), "v0.288 PREFLIGHT - bridge signal reserve acknowledged step", fill="#e8d99d")
draw.text((140, 215), "Base HEAD:", fill="#9ecfc5")
draw.text((140, 260), "04da01acfa5ee5fa8e6bdaab258750b81c957354", fill="#ffffff")
draw.text((140, 340), "Baseline exact-SHA Actions run:", fill="#9ecfc5")
draw.text((140, 385), "28709658954 - success", fill="#9fe2a3")
draw.text((140, 470), "Scope:", fill="#9ecfc5")
draw_wrapped(draw, (140, 520), "After accepted v0.287 Reserve Assigned, selecting the East bridge defender exposes one Signal option that creates BRIDGE SIGNAL SENT and one static RESERVE ACK.", "#ffffff")
draw_wrapped(draw, (140, 635), "Forbidden: duplicate signal/ack, second marker, resources mutation, deployment, damage, HP loss, projectiles, death/despawn, pathing, movement, AI, waves, fog, economy, and true default runtime mutation.", "#d7c98f")
preflight.save(root / "01_v0288_preflight_head_ci_resolution.png")
...
draw.text((90, 92), "v0.288 RETAINED v0.282/v0.284/v0.285/v0.286/v0.287 LEDGER PROOF", fill="#e8d99d")
y = 155
...
draw.text((90, 775), "v0.288 only adds static bridge Signal and Reserve ACK after v0.287 Reserve Assigned.", fill="#9ecfc5")
ledger_image.save(root / "03_v0288_retained_v0282_v0284_baseline_ledger_proof.png")
...
draw.text((100, 105), "v0.288 UI LAYOUT DIAGNOSTIC PROOF", fill="#e8d99d")
...
draw.text((100, 320), "Top strip vocabulary includes BRIDGE SIGNAL SENT and still caps at 36 chars.", fill="#ffffff")
...
draw.text((100, 460), "Signal labels: SIGNAL SENT exactly once near defender; RESERVE ACK exactly once near reserve.", fill="#d7c98f")
diagnostic.save(root / "35_v0288_ui_layout_diagnostic_proof_retained.png")

capture_paths = [root / name for name in capture_files]
missing = [path.name for path in capture_paths if not path.exists()]
if missing:
    raise SystemExit(f"Missing v0.288 capture files before pack assembly: {missing}")

all_stats = [stats_for(path) for path in capture_paths]
(root / "v0288-black-frame-stats.json").write_text(json.dumps({"stats": all_stats}, indent=2) + "\n", encoding="utf-8")
...
draw.rectangle((60, 60, 1540, 840), outline="#65e6d8", width=3)
draw.text((100, 105), "v0.288 BLACK-FRAME REJECTION REPORT", fill="#e8d99d")
...
draw.text((100, 520), "Rendered evidence includes true default, ledger, and 31 v0.288 opt-in bridge-signal states.", fill="#ffffff")
report.save(root / "37_v0288_black_frame_rejection_report.png")

sheet_paths = [root / "01_v0288_preflight_head_ci_resolution.png"] + capture_paths + [root / "35_v0288_ui_layout_diagnostic_proof_retained.png", root / "37_v0288_black_frame_rejection_report.png"]
tiles = []
...
sheet.save(root / "36_v0288_contact_sheet.png")
