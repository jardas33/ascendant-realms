...
$ArtifactRoot = Join-Path $RepoRoot "artifacts\desktop-spikes\godot-salto\v0223"
...
if (-not (Test-Path -LiteralPath (Join-Path $ArtifactRoot "capture\selected-hud-visual-language\screenshot-runtime-manifest.json"))) {
  & (Join-Path $PSScriptRoot "captureGodotSaltoHudVisualLanguageWindows.ps1")
}
if (-not (Test-Path -LiteralPath $ExePath)) {
  throw "Missing exported Godot executable for v0.223 validation: $ExePath"
}
...
Invoke-RebootValidationScenario -ScenarioId "v0222-before-hud-visual-language" -ExtraArgs @(
  "--salto-minimal-contextual-hud",
  "--salto-hud-visual-language-disabled"
)
Invoke-RebootValidationScenario -ScenarioId "selected-hud-visual-language" -ExtraArgs @(
  "--salto-hud-visual-language"
)

node "tools/godot/saltoHudVisualLanguageTool.mjs" validation "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.223 HUD visual language validation report failed with exit code $LASTEXITCODE."
}

node "tools/godot/saltoHudVisualLanguageTool.mjs" boundary "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.223 HUD visual language boundary scan failed with exit code $LASTEXITCODE."
}

node "scripts/cleanupSaltoExperimentalArtifacts.mjs" "--output-root=$((Join-Path $ArtifactRoot 'cleanup-dry-run').Replace('\', '/'))"
if ($LASTEXITCODE -ne 0) {
  throw "v0.223 cleanup dry-run failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "v0.223 artifact retention validation failed with exit code $LASTEXITCODE."
}

Write-Output "PASS_V0223_HUD_VISUAL_LANGUAGE_VALIDATION_READY"
