...
draw.text((140, 140), "v0.290 PREFLIGHT - reserve deployment approval gate step", fill="#e8d99d")
...
draw_wrapped(draw, (140, 520), "After accepted v0.289 SUPPORT ORDER READY, selecting the Field Barracks exposes one Approve option that creates DEPLOYMENT APPROVED and one static APPROVED marker.", "#ffffff")
...
preflight.save(root / "01_v0290_preflight_head_ci_resolution.png")
...
draw.text((90, 92), "v0.290 RETAINED v0.282/v0.284/v0.285/v0.286/v0.287/v0.288/v0.289 LEDGER PROOF", fill="#e8d99d")
y = 155
...
draw.text((90, 775), "v0.290 only adds a static approval gate after v0.289 support order readiness.", fill="#9ecfc5")
ledger_image.save(root / "03_v0290_retained_v0282_v0284_baseline_ledger_proof.png")
...
draw.text((100, 105), "v0.290 UI LAYOUT DIAGNOSTIC PROOF", fill="#e8d99d")
...
draw.text((100, 320), "Top strip vocabulary includes DEPLOYMENT APPROVED and still caps at 36 chars.", fill="#ffffff")
...
draw.text((100, 460), "Approve labels: APPROVED exactly once near reserve; ORDER READY and RESERVE ACK retained exactly once.", fill="#d7c98f")
diagnostic.save(root / "46_v0290_ui_layout_diagnostic_proof_retained.png")

capture_paths = [root / name for name in capture_files]
missing = [path.name for path in capture_paths if not path.exists()]
if missing:
    raise SystemExit(f"Missing v0.290 capture files before pack assembly: {missing}")
all_stats = [stats_for(path) for path in capture_paths]
(root / "v0290-black-frame-stats.json").write_text(json.dumps({"stats": all_stats}, indent=2) + "\n", encoding="utf-8")
...
draw.rectangle((60, 60, 1540, 840), outline="#65e6d8", width=3)
draw.text((100, 105), "v0.290 BLACK-FRAME REJECTION REPORT", fill="#e8d99d")
...
draw.text((100, 520), "Rendered evidence includes true default, ledger, and 42 v0.290 opt-in approval-gate states.", fill="#ffffff")
report.save(root / "48_v0290_black_frame_rejection_report.png")

sheet_paths = [root / "01_v0290_preflight_head_ci_resolution.png"] + capture_paths + [root / "46_v0290_ui_layout_diagnostic_proof_retained.png", root / "48_v0290_black_frame_rejection_report.png"]
tiles = []
...
sheet.save(root / "47_v0290_contact_sheet.png")