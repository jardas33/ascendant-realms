...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--hybrid-three-slot-composition-stress", "--hybrid-three-slot-composition-benchmark-sequence", "--artifact-root=$ArtifactArg")
...
if ($GodotExitCode -ne 0 -or $RuntimeReport.status -ne "PASS_V0153_HYBRID_THREE_SLOT_RUNTIME_EVIDENCE") {
  throw "Godot hybrid three-slot composition benchmark exited with code $GodotExitCode."
}
