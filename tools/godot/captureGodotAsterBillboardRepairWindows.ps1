...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--aster-billboard-single-slot-repair", "--aster-billboard-repair-capture-only", "--artifact-root=$ArtifactArg")
...
if ($GodotExitCode -ne 0 -or $RuntimeReport.status -ne "PASS_V0152_ASTER_BILLBOARD_REPAIR_RUNTIME_EVIDENCE") {
  throw "Godot Aster billboard repair capture exited with code $GodotExitCode."
}
