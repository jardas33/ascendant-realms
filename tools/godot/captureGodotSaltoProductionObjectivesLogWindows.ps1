...
function Invoke-ProductionObjectivesPass {
  param(
...
Invoke-ProductionObjectivesPass -Args @("--player-slice-capture", "--salto-selection-command-panel", "--salto-production-objectives-log", "--artifact-root=$OptInArg") -ManifestPath (Join-Path $OptInRoot "screenshot-runtime-manifest.json") -ExpectedStatus "PASS_PLAYER_SLICE_CAPTURE"
...
try {
  node "tools/godot/saltoProductionObjectivesLogTool.mjs" capture "--artifact-root=$ArtifactRootArg"
  if ($LASTEXITCODE -ne 0) {
    throw "v0.211 production objectives log review pack creation failed with exit code $LASTEXITCODE."
  }
} finally {
