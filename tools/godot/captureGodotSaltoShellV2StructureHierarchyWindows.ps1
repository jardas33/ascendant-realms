...
function Assert-ExpectedFiles {
  ...
  foreach ($fileName in $missing) {
    throw "Missing expected artifact '$fileName' for v0.199 capture scenario '$ScenarioRoot'."
  }
}
...
node "tools/godot/saltoShellV2StructureHierarchyTool.mjs" capture "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.199 structure hierarchy capture report failed with exit code $LASTEXITCODE."
}
