...
const script = file('desktop-spikes/godot-salto/scripts/v0390_inhabited_crossing_natural_yard_grounding_value_repair.gd');
const scene = file('desktop-spikes/godot-salto/scenes/v0390_inhabited_crossing_natural_yard_grounding_value_repair.tscn');
const router = file('desktop-spikes/godot-salto/scripts/salto_spike_root.gd');
const packageJson = file('package.json');
const audit = file('artifacts/work/v0390-asset-selection-audit.md');
const iterationLog = file('artifacts/work/v0390-iteration-log.md');
const report = file('docs/V0390_INHABITED_CROSSING_NATURAL_YARD_GROUNDING_VALUE_REPAIR_REPORT.md');
...
const runtime = file('desktop-spikes/godot-salto/artifacts/runtime/v0390');
const pack = file('artifacts/manual-review/v0390-inhabited-crossing-natural-yard-grounding-value-repair');
const images = [
...
function validate() {
  ...
  for (const token of [
    'V0390_Inhabited_Crossing_Natural_Yard_Grounding_Value_Repair',
    'V0390_One_Primary_Barrosan_Homestead', 'V0390_One_Subordinate_Agricultural_Barn',
    'V0390_Five_Functional_Yard_Groups', 'V0390_Yard_Group_01_Cart_Direct_Ground',
    'V0390_Yard_Group_02_Stacked_Firewood_Direct_Ground', 'V0390_Yard_Group_03_Barrel_Crate_Storage_Direct_Ground',
    'V0390_Yard_Group_04_Trough_Workbench_Direct_Ground', 'V0390_Yard_Group_05_Fence_Gate_Open',
    'V0388_Exactly_Three_Distinct_Character_Roles', 'V0388_Road_Yard_Entrance_Connection',
    'V0390_One_Flush_Bridge_Yard_Door_Route', 'V0390_Flush_Irregular_Worn_Earth_Yard', 'continuous_route_ribbon',
    'Farmer.gltf', 'Adventurer.gltf', 'Worker.gltf',
    'v0390-inhabited-crossing-natural-yard-grounding-value-repair.json', 'gameplay": false', 'defaultRuntime": "unchanged"'
  ]) must(g.includes(token), `script contract ${token}`);
  for (const token of [
    '--v0390-inhabited-crossing', '--v0390-inhabited-crossing-smoke', '--v0390-inhabited-crossing-capture',
    'v0390_inhabited_crossing_natural_yard_grounding_value_repair.tscn'
  ]) must(r.includes(token), `router contract ${token}`);
  for (const token of [
    'godot:play:v0390-inhabited-crossing', 'godot:smoke:v0390-inhabited-crossing',
    'godot:capture:v0390-inhabited-crossing', 'godot:validate:v0390-inhabited-crossing'
  ...
  if (!reportText.includes('v0.389 remains retained')) throw new Error('report does not preserve v0.389 ancestry');
  ...
  if ((g.match(/V0390_Yard_Group_/g) || []).length < 5) throw new Error('five yard group names missing');

  for (const i of [1, 2, 3]) {
    const d = file(`desktop-spikes/godot-salto/artifacts/work/v0390-iteration-0${i}`);
  ...
  must(fs.existsSync(pack), 'v0.390 review pack');
  ...
  if (validation.status !== 'REJECTED BY INDEPENDENT V0390 VISUAL REVIEW') throw new Error('review pack status mismatch');
  if (validation.independentVerdict !== 'REJECT') throw new Error('independent v0.390 verdict missing');
  ...
  console.log('PASS_V0390_INHABITED_CROSSING_NATURAL_YARD_GROUNDING_VALUE_REPAIR_VALIDATOR (7 real captures; independent visual verdict recorded as REJECT; opt-in visual only)');
}

if (process.argv[2] === 'validate') validate();
else throw new Error('usage: node tools/godot/saltoV0390InhabitedCrossingNaturalYardGroundingValueRepairTool.mjs validate');
