...
$Arguments = @("--path", $GodotProjectPath, "--disable-vsync", "--max-fps", "0", "--resolution", "1600x900", "--", "--barrosan-structure-finish-material-single-slot", "--structure-finish-material-validate-only", "--artifact-root=$ArtifactArg")
...
$RuntimeValidationPath = Join-Path $ArtifactRoot "structure-finish-material-validation-runtime.json"
...
if ($GodotExitCode -ne 0 -or $RuntimeValidation.status -ne "PASS_V0202_STRUCTURE_FINISH_MATERIAL_RUNTIME_VALIDATION") {
  throw "Godot Barrosan foothold structure-finish material validation failed with code $GodotExitCode."
}
