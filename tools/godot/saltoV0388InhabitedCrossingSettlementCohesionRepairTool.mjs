...
const script = file('desktop-spikes/godot-salto/scripts/v0388_inhabited_crossing_settlement_cohesion_repair.gd');
const scene = file('desktop-spikes/godot-salto/scenes/v0388_inhabited_crossing_settlement_cohesion_repair.tscn');
const router = file('desktop-spikes/godot-salto/scripts/salto_spike_root.gd');
const packageJson = file('package.json');
const audit = file('artifacts/work/v0388-asset-selection-audit.md');
const iterationLog = file('artifacts/work/v0388-iteration-log.md');
const report = file('docs/V0388_INHABITED_CROSSING_SETTLEMENT_COHESION_REPAIR_REPORT.md');
...
const runtime = file('desktop-spikes/godot-salto/artifacts/runtime/v0388');
const pack = file('artifacts/manual-review/v0388-inhabited-crossing-settlement-cohesion-repair');
const images = [
...
function validate() {
  for (const [p, label] of [
    ...
    [script, 'v0.388 script'], [scene, 'v0.388 scene'], [router, 'opt-in router'],
    [packageJson, 'package commands'], [audit, 'asset audit'], [iterationLog, 'iteration log'],
    [report, 'v0.388 report'], [priorReport, 'preserved v0.386 report'],
  ...
  for (const token of [
    'V0388_Inhabited_Crossing_Settlement_Cohesion_Repair',
    'V0388_One_Primary_Barrosan_Homestead', 'V0388_One_Subordinate_Agricultural_Barn',
    'V0388_Five_Functional_Yard_Groups', 'V0388_Yard_Group_01_Cart',
    'V0388_Yard_Group_02_Stacked_Firewood', 'V0388_Yard_Group_03_Barrel_Crate_Storage',
    'V0388_Yard_Group_04_Trough_Workbench', 'V0388_Yard_Group_05_Fence_Gate',
    'V0388_Exactly_Three_Distinct_Character_Roles', 'V0388_Road_Yard_Entrance_Connection',
    'V0388_Continuous_Bridge_Yard_Door_Route', 'continuous_route_ribbon',
    'Farmer.gltf', 'Adventurer.gltf', 'Worker.gltf',
    'v0388-inhabited-crossing-settlement-cohesion-repair.json', 'gameplay": false', 'defaultRuntime": "unchanged"'
  ]) must(g.includes(token), `script contract ${token}`);
  for (const token of [
    '--v0388-inhabited-crossing', '--v0388-inhabited-crossing-smoke', '--v0388-inhabited-crossing-capture',
    'v0388_inhabited_crossing_settlement_cohesion_repair.tscn'
  ]) must(r.includes(token), `router contract ${token}`);
  for (const token of [
    'godot:play:v0388-inhabited-crossing', 'godot:smoke:v0388-inhabited-crossing',
    'godot:capture:v0388-inhabited-crossing', 'godot:validate:v0388-inhabited-crossing'
  ...
  if (!reportText.includes('v0.387 remains retained')) throw new Error('report does not preserve v0.387 ancestry');
  ...
  if ((g.match(/V0388_Yard_Group_/g) || []).length < 5) throw new Error('five yard group names missing');

  for (const i of [1, 2, 3]) {
    const d = file(`desktop-spikes/godot-salto/artifacts/work/v0388-iteration-0${i}`);
  ...
  must(fs.existsSync(pack), 'v0.388 review pack');
  ...
  if (validation.status !== 'READY FOR HUMAN V0388 INHABITED CROSSING REVIEW') throw new Error('review pack status mismatch');
  ...
  console.log('PASS_V0388_INHABITED_CROSSING_SETTLEMENT_COHESION_REPAIR_VALIDATOR (7 real captures; separated primary/subordinate; 5 yard groups; 3 visible source-diverse roles; bridge/river preserved; opt-in visual only)');
}

if (process.argv[2] === 'validate') validate();
else throw new Error('usage: node tools/godot/saltoV0388InhabitedCrossingSettlementCohesionRepairTool.mjs validate');
