...
foreach ($path in @($ExePath, $WorkerSourcePath, $WorkerMetadataPath, $BarracksSourcePath, $BarracksMetadataPath, $MilitiaSourcePath, $MilitiaMetadataPath, $AsterSourcePath, $AsterMetadataPath, $AshenSourcePath, $AshenMetadataPath)) {
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required v0.174 environment readability capture path: $path"
  }
}
...
function Invoke-CaptureScenario {
  ...
  if ($GodotExitCode -ne 0) {
    throw "Godot v0.174 capture scenario '$ScenarioId' exited with code $GodotExitCode."
  }
...
Invoke-CaptureScenario -ScenarioId "e1-environment-foundation-baseline" -ScenarioArgs ((Environment-FoundationArgs) + (Five-SlotArgs))
Invoke-CaptureScenario -ScenarioId "e2-road-river-bridge-site-marker-hardening" -ScenarioArgs ((Environment-ReadabilityArgs) + (Five-SlotArgs))

node "tools/godot/saltoEnvironmentReadabilityTool.mjs" capture "--artifact-root=$ArtifactRootArg"
if ($LASTEXITCODE -ne 0) {
  throw "v0.174 environment readability capture report failed with exit code $LASTEXITCODE."
}
