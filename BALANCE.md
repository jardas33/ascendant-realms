...
## Current First-Campaign Baseline - 2026-05-09 v0.7.3 Real-Input Pressure Playtest Gate
...
- Latest v0.7.3 pressure playtest: no numeric tuning is applied. Controlled browser-input review found Cinderfen Crossing and Cinderfen Watch pressure warnings readable enough to keep unchanged; strategy telemetry keeps Greedy Economy as a timeout/closure read, Fast Army as valid quick-clear expression, and Retinue + Training Yard II as a saved-progress power watchpoint rather than a pressure bug. Emmanuel's manual checklist remains the missing direct human evidence before any stronger pressure experiment.
- Latest Enemy Strategic Pressure V1 balance gate: v0.7.1 applies no numeric tuning. Pressure is explicitly scoped to `cinderfen_crossing` and `cinderfen_watch`, produced 75 pressure-enabled Cinderfen runs, triggered in 63 of them, showed 149 warnings, applied 0 simulated reinforcements, and produced no enemy-pressure analyzer warnings. Cinderfen Crossing remains 26 wins / 0 defeats / 13 timeouts, Cinderfen Watch remains 25 wins / 0 defeats / 11 timeouts, and Ashen Outpost remains untouched by pressure at 22 wins / 0 defeats / 14 timeouts.
- Current pressure interpretation: V1 is visible enough for telemetry, subtle enough to avoid structural difficulty flags, and still not full enemy construction. v0.7.1 improves warning copy, gives pressure warnings a longer prioritized status read, and keeps objective/capture feedback higher priority. `reinforce_next_wave`, `contest_capture_site`, and `defensive_hold` remain warning/telemetry-only; the only live/sim effect is the existing next-wave timing nudge.
- Latest pressure-feel reports: `docs/V071_ENEMY_PRESSURE_FEEL_REPORT.md` records the warning polish/status-priority gate, `docs/V072_PRESSURE_PLAY_REVIEW_REPORT.md` records the no-change Cinderfen play-review decision, and `docs/V073_REAL_INPUT_PRESSURE_PLAYTEST_REPORT.md` records the controlled browser-input pressure playtest. Future pressure changes should start from Emmanuel's manual checklist evidence, not from automatic escalation.
...
- Current human-review focus: Aether Well Ruins, Bandit Hillfort, Ashen Outpost, Cinderfen Crossing, and Cinderfen Watch with Retinue + Training Yard II need feel/readability review before more numeric tuning. Enemy Strategic Pressure V1 has passed controlled browser-input readability review, but still needs Emmanuel's manual checklist results before any reinforcement, contest, or defensive-hold action becomes even a simulator-only next step.
